...
- **v0.9.x** — retro level grants, location-sorted people, clickable quests with GM guidance, trivial actions (no dice/energy for no-stakes acts), energy efficiency scaling, map details panel with explicit travel.
- **v1.0.0 (this)** — **Deep attributes**: sub-attribute cap 20 with diminishing returns past the soft cap of 4 (×20/point through 4, +5/point beyond) — mastery buys power against hard rolls without trivializing easy ones. **GM-generated abilities**: `newAbility` contract field for capabilities earned in fiction (training, quest rewards); engine-sanitized (cost 4–15, axes clamped) and capped at floor(level/2)+1 learned abilities; stored per-character in `customAbilities`, merged into the catalog everywhere. **Shared world**: with GitHub sync on, `syncSharedWorld` runs at every tick — event stages merge to the furthest anyone reached, spectrum drift to the strongest pressure, other characters' ledger deeds arrive as "word reaches you" news, and consolidated region state pushes back to `world/regions/valley.json` (SHA-retry). One valley, true for everyone.
- **v1.1** — same-scene party play via `world/scenes/` polling; world-level time mode; NPC relationship arcs; known roads + crit mastery.
