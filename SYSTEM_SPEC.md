...
| **HEAD verified** | BATCH-11 at **v1.8.105** · confirmed against origin: 74 engine modules · 43 core rules files · 96 locations / 25 regions · 285 abilities / 24 traditions (+3 folk) · 44 combinations · 41 NPCs · 9 companions · 58 random-encounter entries. **Count freshness is now machine-gated** — `tests/wiring_audit.mjs` fails the build when this line drifts from HEAD (the 38/137-era drift, found by BATCH-11 §0, must not recur silently). |
...
the ones that can silently stop firing, and the audit prints them as the set worth checking by hand.

<!-- generated by tests/world_drive_audit.mjs — do not hand-edit -->
| Path | What the world does | Trigger | Builder | Spec |
|---|---|---|---|---|
| `worldPressureDetail` | the world's own pressure — what is building whether or not you engage it | always (paced) | `app pendingPressure (SNG-080)` | §19 |
| `worldArcsDetail` | world arcs advancing on their own clock | always | `worldtick.worldArcsForGM` | SNG-203 §3 |
| `latentArcsDetail` | arcs not yet surfaced, ripening | the generation turn surfacing an arc that fomented on the world count | `latentarcs.arcsForGM (SNG-191 §7)` | §7 |
| `livingWorldDetail` | the living world — what other people's play has made true here | always | `generate.livingWorldForGM` | §19 |
| `newsDetail` | news travelling between communities | always | `worldtick.newsForGM` | §19 |
| `npcErrandsDetail` | NPCs who want something from you, unprompted | always (a known errand-giver) | `quests.npcQuestsForGM` | SNG-203 §5 |
| `wakesDetail` | the wake of what you did — consequences arriving later | always (a significant outcome recently resolved) | `wake.wakesForGM (SNG-204)` | SNG-204 §OQ1 |
| `legendDetail` | a great figure surfacing at an apt beat (CCODE-90) | qualifying beat | `app.maybeLegendDetail (SNG-042)` | §16 |
| `legendsPursuableDetail` | great figures you could reach | always (a practiced tradition or a legend at hand) | `legends.legendsForGM (SNG-208)` | SNG-208 wiring |
| `seasonalDetail` | the season acting on the world | always (the character clock always has a season) | `latentarcs.seasonalDetailForGM (SNG-191 §7.4)` | §7.4 |
| `anomalyDetail` | the world noticing its own inconsistency | Repair panel | `corrections.detectAnomalies→anomaliesForGM (SNG-137)` | §11 |
| `encounterOfferDetail` | the world offering an encounter | the narrative-time roll picking a STRUCTURED (duel/challenge) entry — the engine decides, the model no longer judges 'when the fiction invites it' (the Silas fix) | `app pendingEncounterOffer (SNG-236 fix A)` | SNG-236 |
| `emergenceDetail` | practice ripening into new power | practice | `practice.emergenceNoticeForGM` | §7 |
| `perilNote` | peril the world has put in front of you | precursor use | `character.precursorAxes band` | §6 |
| `assignmentsDetail` | standing obligations coming due | the player putting a known person in charge of ongoing work (delegateOps) | `assignments.assignmentsForGM (SNG-191 §4)` | §4 |
| `teacherOfferDetail` | a teacher offering, unasked | the ENGINE finding a present teacher with a reachable next step + room this beat (not the same beat as the general offer) — the model no longer judges 'when the moment fits' | `pacing.roomForATeacherOffer + company.teacherOfferReady (SNG-195 G2)` | SNG-195 G2 |
| `offerDetail` | an offer the world makes | the ENGINE finding room this beat (a lull or arrival, no encounter/gambit/intent grip, off cooldown) — the model never judges it | `pacing.roomForAnOffer + npcs.npcFearsForGM (SNG-194 §4b)` | SNG-194 |
