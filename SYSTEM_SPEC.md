...
| `presence.js` | WHO IS AROUND TODAY (SPEC_npc_presence_cadence) — the half of the world the GM was never shown. **API** `TIER_RATE, TIER_REACH, presentToday, presenceForGM`. ⛔ **Measured: 117 authored figures and 110 Silas had never met**, because `npcsPresent` comes back IN the turn, KNOWN PEOPLE is only who was already met (so a STRANGER can never arrive from it), and `generateRequest` is reactive by rule. ⚑ **A per-person daily CHANCE** = the tier's rate × how near they are, with **reach scaling by standing** — a storekeeper is in their village, a heroic has a circuit, a legend is wherever the story is. Measured to Erik's cadence: heroic 1/8d, epic 1/15d, legendary 1/42d, a notable most days. **NEVER a quota** — the count falls out of who actually turned up, so a remote post offers one or none and that difference is information. **NEVER** offers a mythic on a timer: *a mythic appearing IS the event*. |
