...
| `quests.js` | Quest state (freeform ops) + **structured quests (§14)** with effects[] application. **API** `resolveQuest, applyQuestUpdates, dedupeQuests, questsFor, questsForGMView, questPublic, hydrateQuest, questsForGM, slugify, isRealQuest, startStructuredQuest, completeQuestStage, resolveStructuredQuest, availableStructuredQuests, routesForCharacter, structuredQuestsForGM`. **`truth` IS THE GM-EYES DRAWER** (CCODE-458) — authored on a quest, a stage or an outcome, fetched from the def for the GM under gm.js rule 4, never written into a save, and DELETED by `questsFor`, which is the call every player surface makes. **THE DEF OWNS THE WORDS AND PLAY OWNS THE STATE**: every read AND every write resolves prose, stages and outcomes through the def, so a content fix reaches a started quest and an ending authored later is a live button. **NEVER** silently drops an unresolvable op — surfaces a note. |
