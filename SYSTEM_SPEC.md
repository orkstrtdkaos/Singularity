...
| `armory.js` | The armory (CCODE-445): what a hold's forge and smithy make, what its armory keeps, and gear handed to a body of hands. **API** `armoryTable, armoryOf, makersAt, setForgeOrder, tickArmory, outfitContingent, gearPrice, sellGear, armoryLine, armorySaid, ARMORY_DEFAULTS`. **NEVER** makes gear without the store's material, hands out more than the armory holds or more than one of a kind a head, or outfits a named person — and every number is a stand-in until `economy.armory` is authored. |
| `orderofbattle.js` | The order of battle (CCODE-448): a stance's weights, a band's turn by assignment, an ally's craft by stance and preference, the party's round the GM is told. **API** `stanceTable, stanceOf, slotsFor, hungarian, allocateTurn, allyChoice, rollOdds, partyRound, partyDirective, landsOf, STANCE_DEFAULTS`. **NEVER** rolls its own dice (every odds is `jobCraftsOf`'s, the Jobs tab's) and never changes a fight's numbers — it decides who does what and tells it. |
| `holdwork.js` | Standing work at a hold (CCODE-450): who is put to what the hold needs, what their good days bank, and what the work does while it is done. **API** `WORK_KINDS, workTable, workOf, workersAt, workAt, workMods, assignWork, unassignWork, tickWork, workSaid`. **NEVER** imports (a leaf: `holdings.js` and `jobs.js` both read it), never rolls its own odds (`goodDayOf` is handed in from the Jobs tab's dice), and never offers a kind whose effect is not wired. |
| `field.js` | THE POWER FIELD, evaluated AT A POINT and never interpolated between markers (CCODE-457, extracted from the `exesa_field` prototype before it is deleted). The region vote, the AUTHORED 44 sources as Gaussians where a NEGATIVE strength is a sink, ordered and wild as two accumulators that overlap, patchiness on wild only, meaning from places and roads, and **arc stages that shift the whole field** — "the ground is not where it was authored". **API** `MEMBERSHIP, TERRAIN_FEATURE_FLOOR_DEG, FIELD_KINDS, KIND_LABEL, ARC_EFFECTS, ARC_STEP, PROBE_REACH, loadSources, makeField, fieldDataFrom, arcShift, voteWeight, noise2, lonDelta` — and a field answers `densityAt, naniteAt, meaningAt, strengthAt, sampleWindow, texture, coverage, probe, withArcStages`. **`sampleWindow` IS THE EXTRACTION**: the same evaluator at the world, region and location extents, with the antimeridian unwrapped inside it once. **`withArcStages` returns a NEW field** — the story moving the ground must not move it under a caller still reading the authored world. **NEVER** fetches, bakes or draws, and reads no raster: below `TERRAIN_FEATURE_FLOOR_DEG` there is FIELD detail and no landform at all. **ONE evaluator for all three map tiers**, which is the point. |
| `market.js` | A hold is a local market (CCODE-440): sell from the pack at the place's price, change money at the place's rates. **API** `sellQuote, sellFromPack, exchangeRatesHere, exchangeAt, ITEM_WORTH_BY_KIND, EXCHANGE_DEFAULTS`. **NEVER** invents a price (`economy.priceOf` is the number) and never sells what the story carries. |
| `money.js` | Money by place (CCODE-437): what a place pays in, what it takes and at what rate — Erik's "local first, else a worse rate". **API** `moneyClassOf, moneyHere, payAt, earnAt, priceHere, incomeHere, moneyLine, saidPaid, saidEarned`. **NEVER** moves money except through `credit`/`debit`, and never pays part of a cost — a cost the purse cannot meet moves nothing. |
| `spectrum.js` | The twelve axes and the one door into a character's fingerprint (CCODE-436). **API** `spectrumIdsOf, cleanAxes, driftAlignment, retireOffAtlasAxes, PLACEHOLDER_AXES`. **NEVER** writes a key that is not one of the content's twelve, or a value that is not a number — and never erases an off-atlas value: it is set aside in `_retiredAxes`. |
...
| `champion.js` | SNG-587 SOMEBODY ELSE TAKES THE FIGHT. Erik: "if it's too easy for the main PC, he could have someone else deal with it and get the experience. If it's too hard for the PC, having a legendary companion take care of it would make sense." Both cases are ONE reading — the band is relative (CCODE-52), so the same foe is `beneath notice` to one body and `a real fight` to another. **API** `championsFor, resolveChampion, creditChampion, championLine, sendingIsGrim`. **NEVER** builds a sheet or scores a power itself (both injected), and **NEVER** runs the round panel — a player who sends somebody is choosing not to fight. A win credits `completions`, which is R37a’s existing growth term, not a second ladder. |
...
| `battle_turn.js` | THE ONE skill-battle turn for play and the harness (2026-09-05). **API** `battleSkillsForCharacter, declFromSelection, resolveDeclRank, openGuards, playTurn, applyRoundToCharacter, collapseIfFinished, personOpponentFor, duelFromTarget, endBattle`. **NEVER** touches the DOM or narrates; app.js delegates, `tests/lib/realgame.mjs` drives. |
...
| `playerprofile.js` | Player identity + earned play-style + the content-rating ceiling. **API** `defaultRating, ratingLevel, canSetRating, setRating, setMinorFlag, newProfile, updateProfile, deriveAptitudes, aptitudeMods, profileInsight, WISHES_MAX, playerWishesForGM`. CCODE-355: `wishes` is the player's session-zero note ("what I want from this game"), on the PROFILE so it follows them to every character and device, and read by the GM every turn. **NEVER** raises the ceiling to R/R+ without the adult gate (§17), and never lets `wishes` move the rating either way. |
...
| `worldtick.js` | Offscreen advancement (event stages, news spread, NPC evolution, shared-canon sync). **API** `initWorldState, buildRegionView, effectiveLocation, runWorldTick, syncSharedWorld, syncSharedCanon, syncTravelers, TRAVELERS_PATH, advanceGeneratedOffscreen, takeUnseenNews, newsForGM`. **NEVER** runs unless in-game days have passed since the last tick. |
...
| `worldevents.js` | CCODE-354 A CRISIS ANOTHER TRAVELER ANSWERED READS AS ANSWERED, FOR EVERYONE. Erik: "the Intent is to have the world living and breathing and changing for everyone… her particular quest needs to morph into discovering that the water coming down the watershed IS clean now." Aevi (SNG-595): "a state is overtaken by the most recent actor — never by the worst one", and "the merge must carry who". ⚑ Measured: the shared merge adopted a stage only when HIGHER (escalation crossed, easing was thrown away), nothing could mark a crisis answered, and the region file was an overwrite of a file every client writes — so Silas's day-26 answer never reached a single world. ⛑ A stage carries `rev`/`atWorldDay`/`by`; the latest wins, an ANSWER beats any question, and two never-stamped states keep the old higher-stage rule. A world-tier quest ending lands on the shared `questOutcomes` record, and the next traveler to take the quest up gets Aevi's `priorOutcomeBoards` for the world as it now IS. **API** `WORLD_MOVED_ON_BEATS, actorOf, questKey, stampEventChange, latestEventState, mergeEventStages, resolveEvent, recordQuestOutcome, mergeQuestOutcomes, aftermathOf, answeredEventLine, worldMovedOnForGM, noteWorldMovedOnShown, priorBoardFor`. **NEVER** lets an unanswered state overwrite an answer, hands the GM an event's GM-eyes `truth` beside "answered", invents an aftermath the content did not author, or repeats "the world moved on" past its beats. |
| `travelers.js` | SNG-595 A FELLOW TRAVELER IS A PERSON THE WORLD HAS A RECORD OF. Erik: "Courtney's PC Adelheid asked Edvar Crane about Silas and the gm doesn't seem to be able to follow the details of that story. How much crosses the worlds?" ⚑ Measured: crisis stages, one frozen 90-character canon card per person, and news headlines crossed; RELATIONSHIPS never did (every person is a per-save copy), the ledger reader opened only the current month's file (15 of Silas's 23 deeds unreadable since August 1st), and nothing told the GM that "Silas Weir" was a player — so it built him from the news lines in front of it and wrote a Harmonic suspect into her codex. ⛑ The reader keyed by PERSON: a minimal public card per character (`world/travelers.json`, one key each, merged), the whole ledger, and the canon provenance that says who made a person real. **API** `travelerCard, cardChanged, mergeTravelerCard, ledgerMonthOf, ledgerMonthsSince, namedTravelers, travelerDeeds, placeWords, peopleInTheirStory, travelersForGM`. ⛔ CCODE-359 (Erik: "it would be great to have Adelheid and he meet"): the card also carries `where` — community, town, place, the world-day they came, refreshed while they play — so `travelersHere` can say another traveler is in this same town and an open scene is findable from anywhere in it. **API** also `whereOf, meetKey, travelersHere, travelerHereLine, travelersHereForGM, agoWords, PRESENCE_FRESH_MS, PRESENCE_REFRESH_MS`. **NEVER** carries anything from another player's save beyond the card (name, level, origin, stated pronouns, and which town and place), never attributes a `hidden` or `unseen` deed to anyone, never lets a GM voice or move another player's character, and never invents a traveler the index does not hold. |
| `invitations.js` | CCODE-360 AN INVITATION CARRIED BY SOMEONE YOU BOTH KNOW. Erik: "I would like the opportunity to invite her to my Band of the Fell Pell - probably through mutual connections when I recruit." ⚑ Measured: a band's members are contingents keyed by `npcId` in the leader's own save; nothing let one player's character invite another's or carried an answer back; and only the receiver's own save can say whether she knows a given person — on the real saves, six people stand in the Fellowship of the Fell Pell and Mara Wells is the one Adelheid's save knows. ⛑ The sender names a carrier from his band; `world/invitations.json` holds one merged record per sender, addressee and band; it ARRIVES only where the receiver's registry knows the carrier (by id, or by name under another id); the addressee writes only her answer, once; the leader's tick applies it once, as word from elsewhere. **API** `INVITES_PATH, bandPhrase, invitationId, makeInvitation, mergeInvitation, answerInto, knowsCarrier, incomingInvitations, sentInvitations, joinBandLocally, applyAnswers, invitationsForGM, bandsJoinedForGM`; IO in `worldtick.syncInvitations / sendInvitation / answerInvitation`. **NEVER** adds a traveler to a band's contingents (no strength, threat or clash counts another player's character), never un-answers a yes, never answers for anyone — silence changes nothing — and never shows an invitation whose carrier the receiver has not met. |
| `home.js` | CCODE-369 A HOME IS A PLACE THAT IS YOURS. Erik: "Yes, a home is a different type than a hold... but it could BECOME a hold. to start with it's a location." ⚑ Measured: nothing let a character call a place home; the nearest thing was a holding, whose condition drops every ~30 days without a keeper and which can be raided — the opposite of the cabin Courtney asked for and Aevi authored (The Painter's Shelf). ⛑ A home is a location the character chose, and since when: a ⌂ door in the place header, a Home line on the sheet with "Go home" by the map's own road test, and a GM row that says it is a place, not a holding. **API** `homeOf, isHome, makeHome, homeForGM`. **NEVER** read by the holdings machinery — no condition, keeper, decay or raid — and never set by the engine: only the player makes a place home. |
| `companionlives.js` | SNG-597 §3 (CCODE-372) A COMPANION WHO IS ALSO A FIGURE OF THE WORLD. Erik: "she didn't mention it to Silas — now that she's in his party" → "Go with Aevi's suggestion on Marrow" (Aevi: "Do not take the arc away from her. Take the silence away."). ⚑ Measured: Marrow travels with Silas and the legend `maren_ossitide` acts in the world, and no content field links them; Silas's save knows her as "Maren (Marrow) Ossitide". ⛑ `boundFigures` answers who in the company is also a figure — an authored `legendId`/`companionId` first, else a name the save knows them by that IS the figure's own name, epithet aside. While they travel, `worldtick` keeps their cares leaning on the world and records what they want, and takes nothing physical elsewhere (no melee, strike, guard, challenge or retrieval attempt); `companionWishNews` tells the player a new wish once, `companionLivesForGM` lets them bring it up, and the world tab marks them "(travelling with you, as Marrow)". **API** `boundFigures`. **NEVER** guesses a link by fuzzy name. |
| `fates.js` | CCODE-381 A LEGEND'S FATE IS THE WORLD'S (shared lives, second stage). Erik: "The world changes for everyone." ⚑ Measured: 33 legends lived in two or more saves' worlds and the saves disagreed about 29. One fate per authored legend in `world/people/valley.json`, folded by one symmetric rule — the earliest death; a death over anything but a later return; otherwise the latest event, the heavier on a tie — with `fateOf`/`mergeFate`/`foldFates`/`adoptFates`/`fateNews`; `worldtick.syncSharedFates` reads before a save's pass and publishes after. A save's first read is silent. Authored legends only. ⛔ CCODE-384 (Erik: "Yes people could be shared"): the PEOPLE the world makes join it — an id derived from the event that made them (`personIdFor`/`mintKey`: one survivor and one successor per death, one taker per empty arc per fourteen days, whichever worlds saw it), `people` in the same file (the first world to publish sets who they are; `foldPeople`/`adoptPeople`/`birthNews`), their fates folded with the legends'; the five legacy `minted-N` people move onto ids of their own world at load (`scopeLegacyMintedIds`). ⛔ CCODE-385: WHAT IS TRUE OF A PERSON is the world's and what a player KNOWS stays theirs — `applyNpcUpdates` stamps a changed role or status with the world-day; `lives` in the same file fold by the later day (a death stands against anything but a return); a death is written onto every registry that knows them, a new role is held beside what the character knows (`worldLife`) and shown to the GM as IN THE WORLD NOW; only between the same full name (Usnea's "Pell" is not Silas's "Pell Ran Marsh"); nothing backfilled. |
| `sharedholds.js` | CCODE-383 A HOLD NEARBY IS KNOWN — what it is, and who runs it. Erik: "if there is a hold nearby PCs should hear about what it is and who's running it. they can and should interact with it.." ⚑ Measured: 6 holdings across the saves, none visible outside their owner's game, while Adelheid stood in Millbrook beside the Fell Pell. The owner's game publishes a card per holding to `world/holds/valley.json` (one set per owner, replaced whole; never the store, debts or obligations) with the position, so a world that never made the place can measure the walk; `holdsNear`/`holdsNearForGM`/`holdNearLine` read them within two walking days; `worldtick.syncHolds` is the tick's door. |
| `holdtrade.js` | CCODE-388 TRADING WITH ANOTHER PLAYER'S HOLD, on the rules Erik said yes to: the owner opens a hold (`trade`); goods sell at its own Reach's price (`tradeOffer`, on the hold's card); the visitor pays and carries them away (`buyFromHold`, recorded by the GM's `holdTrades` op when the keeper sells — not a shop, per economy.json's tradersNotShops); the owner's tick fills the order from the store (`settleOrders`) and the buyer's pays back what could not be filled (`refundOrders`) — each written on the character before it is published; `mergeOrders` only moves a status forward; the money is moved, never made. |
...
| `npcsheet.js` | CCODE-248: AN NPC WITH A CHARACTER SHEET, AND A WAY TO GROW ONE. **API** `sheetFor, derivedLevel, leansOf, craftsOf, growthFor`. ⛔ **Erik's Pell case is an EMERGENT NPC** — she is in `npc_interiority` and the save registry, not the catalogue, and `ironsense` is not an authored craft. ⚠️ **WHY IRONSENSE GETS OVERDONE:** `skillsObserved` is a capped bag of FREE TEXT and nothing turned it into a kit, so the narrator has one remembered thing to reach for. The repetition is a sheet problem, not a narration one. This DERIVES a sheet from what the world recorded (meetings, time known, standing, role lean) and MATCHES observations to real crafts. **NEVER mints a craft** — an unmatched observation returns as `wantsAuthoring`, which is a prompt to author rather than licence to hallucinate. **An AUTHORED sheet wins outright.** |
| `library.js` | ✅ SNG-538 §4 (Aevi 2026-09-11; Erik GO): THE LIBRARY SERVES THE PLAYER, NOT MY NOTES. **API** `LIBRARY_INDEX, loreToHtml, libMdToHtml, libInline, libSkipKey, libPretty, stripTitle, LIB_SKIP`. The Library's index and renderers, out of app.js so §181 runs the production path over every document: every string they emit passes `playerText` (788 authoring glyphs were reaching the page), and a `_`-prefixed key is private (57 build notes were served as lore under headings). Pure string builders — no DOM; the fetch, its cache and the great circle stay in app.js. |
| `carriage.js` | ✅ SPEC_mobile_holdings + ERIK 2026-09-12 ("I want mobile holdings prioritized fairly high"): THE HOLDING THAT MOVES. **API** `CARRIAGE_KINDS, carriageOf, isMoored, voyageOf, voyagePosition, whereaboutsOf, voyageLine, bearerWill, canSail, sailHolding, voyageTick, featureRuling, canBuildOn`. Five kinds of carriage (crewed · powered · drifting · living · willed), the garrison as the engine, a bearer whose standing decides and who may leave with the place, the terrain gate on `tags`, a voyage that arrives on world time — and Erik's ruling over the spec: a hull under way is raidable WHERE SHE IS, derived from the day. |
| `fellowship.js` | ✅ SPEC_SNG-541 (Aevi) + ERIK 2026-09-12 ("don't make everything about a single band… we'll need to be able to form them into a legion later"): WHO HAS THROWN IN WITH YOU, WHICH BAND THEY STAND IN, AND WHERE THEY ARE. **API** `unitsOf, unitLine, poolRows, atSideRows, wherePerson, canBringForward, rosterLine, rosterForGM`. ⛔ PLURAL BY CONSTRUCTION — nothing reads `bands[0]`, every row names its unit, and a character with no bands answers with nothing rather than a fabricated one. The headcount is DERIVED from the contingents (the stored `count` is never read and never shown). ⛔ THE POOL IS A VIEW, not a third container: everyone sworn who is not in `company` today, so membership and posture cannot disagree. Whereabouts name their own basis — at your side, last seen (with its date), an authored home, the band's seat, where you met — and an id that resolves to no place degrades rather than printing a slug; a place is an authored location, one the fiction minted, or a holding of yours. ⚠️ A LEGION NEEDS NO NEW CONTAINER: `group.js` already holds "a legion is deep; a band is broad and thin" and `legionClash` resolves one, so a legion is a unit whose contingents are several bands' with `formedFrom` naming them — deliberately not built until a caller exists. |
| `version.js` | ✅ SNG-545: ONE SOURCE OF TRUTH FOR THE BUILD STAMP. **API** `APP_VERSION`. ⛔ It is its own module because `state.js` loads the whole content pack and cannot reach a const declared in app.js — so every authored file was fetched unversioned and a browser could answer a NEW app.js from last release's cache. Measured, not theorised: verifying v1.9.470 the page died on a cached `quests.js` missing an export it had just gained, and then served a cached `tier_signals.json` that made a council member read as one more irrigator. Four things are stamped from here — index.html's stylesheet and app.js queries, the import map that pins all 112 engine modules, and the query on every content fetch. ⚠️ app.js keeps a GATED COPY because six readers take the version from its const; `scripts/bump_version.mjs` writes all three and `scripts/module_map.mjs --check` fails the ship when any two disagree. |
| `repair_note.js` | ✅ ERIK 2026-09-11 (§171): THE ASK CHANNEL'S REPAIR NOTE MEASURES THE STATE, NOT FOUR COUNTS. **API** `repairFingerprint, repairNote, REPAIR_FIELDS`. ⛔ The note under a GM repair compared four counts (holdings, quests, NPCs, facts) before and after `applyTurn`, so a repair that changed an EXISTING record — Logana's charge corrected, same number of NPCs — read NOTHING MOVED under a narration that said done. `repairFingerprint` serialises every collection the sixteen op families write (underscore keys and timestamps dropped); `repairNote` says one of three true things: changed; an op family did not take (the isolated failure `applyStep` records, with its message); or nothing was different afterwards, it may already have been so. Consumed by app.js's ask handler. |
...
| `powers.js` | SNG-634: **WHO HOLDS THE GROUND, AND WHAT THAT COSTS YOU FOR STANDING ON IT.** **API** `powersFrom, powerById, powerStateOf, isStanding, headsOf, contingentsOf, powersReaching, dangerLiftAt, dangerLiftReceipt, raiderPowerAt, raidersFrom, notePowerLoss, breakPower`. ⛔ **Aevi's finding is the reason it exists**: the hold raid drew its attackers as `{ n: dangerLevel, what: "raiders" }`, so the band and legion machinery built in CCODE-404–407 had NOBODY WITH A NAME on the other side of the field. **C1** a raid on a hold inside a power's `reach` is drawn from that power's own strength, is named, and its losses PERSIST; **C2** effective danger = authored + Σ `dangerLift` of the STANDING powers reaching the place — Erik's 2026-07-19 ruling, so clearing them lowers it. ⚠️ **The lift is SIGNED**: the Keelmouth Slip carries −1 and the Firstsight Barony +1 over the same coast, and they cancel. ⛑ **`powerState` lives on the SAVE, never on the record** — two players meet the same authored Tollmen and only one of them has broken the chain-post. **NEVER** invents a second casualty rule: the rate is `bloodBand`'s, read from the raiders' side of the tide. |
| `nemesis.js` | WHO THIS CHARACTER'S NEMESIS IS (SNG-648). **API** `eligibleNemeses, scoreNemesis, nemesisCandidates, buildNemesisPrompt, applyNemesisChoice, protectsOffscreen, nemesisIdOf, inheritNemesis, nemesisForGM, nemesisOverdue`. ⛔ **SIX AUTHORED SIGNALS, AND EVERY ONE SAYS WHETHER IT COULD BE READ AT ALL** — `unreadable[]` comes back beside `hits[]`, because a signal scoring zero for want of data looks exactly like one that looked and found nothing. ⛔ **A BOUND NEMESIS IS NOT KILLED OFFSCREEN** — one predicate, both doors (`worldtick.applyEpicClashOutcome` and `fates.adoptFates`): wounded, never slain, except by the character or in a scene they are present for. ⚠️ The CHOICE is a model call (it judges whether the figure and the arc answer each other, which no score can); reconcile stakes the shortlist and `app.maybeChooseNemesis` binds it off the play loop. |
...
| `quests.js` | Quest state (freeform ops) + **structured quests (§14)** with effects[] application. **API** `resolveQuest, applyQuestUpdates, dedupeQuests, questsFor, questsForGMView, questPublic, hydrateQuest, questsForGM, slugify, isRealQuest, startStructuredQuest, completeQuestStage, resolveStructuredQuest, availableStructuredQuests, routesForCharacter, structuredQuestsForGM`. **A QUEST CAN BE LOST** (CCODE-459, Erik's ruling): a stage may carry `choices` — a decision with its own `effects[]`, paid through the same applier the endings use, which may `forecloses` endings or `opens` one; an outcome may declare `failure: true` and the quest ends `failed`, paying its effects, deed, chronicle and wake like any other ending; and `deadlineDays`/`deadlineOutcome` let the days simply RUN OUT, in absolute world days, derived from `startedWorldDay` so an unstamped save can never lapse. A structured quest ends exactly one way — through an outcome — and the flat `fail` op no longer bypasses that. **`truth` IS THE GM-EYES DRAWER** (CCODE-458) — authored on a quest, a stage or an outcome, fetched from the def for the GM under gm.js rule 4, never written into a save, and DELETED by `questsFor`, which is the call every player surface makes. **THE DEF OWNS THE WORDS AND PLAY OWNS THE STATE**: every read AND every write resolves prose, stages and outcomes through the def, so a content fix reaches a started quest and an ending authored later is a live button. **NEVER** silently drops an unresolvable op — surfaces a note. |

**Memory / knowledge**
| Module | Owns · API · Never |
|---|---|
| `codex.js` | The character's knowledge graph (typed topics + facts) via clamped ops. **API** `ensureCodex, resolveTopic, applyCodexUpdates, seedReferenceTopic, summaryPressure, topicsNeedingSummary, applySummaries, markNotSame, mergeInto, mergeCodexTopics, suggestMerges, codexForGM, searchCodex` (re-exports `namesMatch` for back-compat — it belongs to `namematch.js`). **NEVER** writes topics outside clamped ops, with ONE stated exception: `seedReferenceTopic` (CCODE-483) mints the world's own furniture — the powers a character has heard of — and refuses to touch a topic play has already written. ⛔ **NO TOPIC CAP** (CCODE-484, Erik): a learned subject is never refused; `summaryPressure` condenses harder as the codex fills, which is what replaced the old stop at sixty. |
...
| `firetests.js` | SNG-560 / Erik: *"go after the 15 never-emitted ops"* and *"fire testing every part of the game. There are still parts that have never been used."* **API** `FIRE_TESTS, diffKeys`. ⛑ **ONE REPRESENTATIVE OP PER NEVER-FIRED CLASS**, built against the character in hand and handed to the SAME `applyTurn` a beat uses — no stub, no seam (Erik 2026-09-05: harnesses simulate the real game). ⚡ **MEASURED FIRST, AND THE FIFTEEN SPLIT IN TWO:** 11 UNTRIED (never fired AND no state — unambiguous) and 4 UNKNOWN (`holdingOps`, `bandOps`, `adoptSchool`, `newAbility` — state exists, but `_opEmitted` began 2026-07-19 and the save is older, so a zero beside it proves nothing until `countsCoverWholeLife`). ⛔ **`no-op` — applied cleanly and wrote nothing — is the `pendingCompanyOffers` shape and the finding this exists to surface;** `threw` is a live defect; `not-applicable` is correct behaviour, never a failure. **NEVER** runs outside dev mode, **NEVER** touches the live character (the runner swaps the binding to a deep copy and restores it in a `finally`), and applies for real only when explicitly asked. |
| `devreport.js` | SNG-559 / Erik: *"make them a robust Play/Dev that generates the data you would need to find and fix anything that is misfiring or not wired along the way"* — and *"I don't want to have to copy and paste anything... so make it so the outputs go back to the repo for you to read as needed."* **API** `buildDevReport, unknownOpsIn, fieldCoverage`. ⛔ **THE DESIGN IS ONE DAY'S BUGS, AND THEY ALL HAVE ONE SHAPE: a wiring fact the running game already knew and nobody collected** — `moveTo 20 ✓0` (a badge wired on one branch, so its number could never rise), an invented `companionOps` discarded in silence, ops trapped in the narration, "Done." with no op, `pendingCompanyOffers` written and never read, `partyOps` at 3 emissions in 369 turns, `age` 0 of 53 and `sex` 6 of 54. ⚠️ **NOT ONE needed a new measurement to be detectable — they needed someone to look.** Pure builders; the writer lives in app.js and pushes to `data/dev/report-<playerKey>-<charId>.json`. **NEVER** writes onto the character save (it is 1.38MB and every write uploads all of it — SNG-550), **NEVER** runs outside dev mode, and **NEVER** lets a failed send interrupt a beat. |
...
| Module | Owns · API · Never |
|---|---|
| `build.js` | CCODE-394 A TAB RUNNING AN OLD BUILD DOES NOT WRITE TO THE WORLD. Erik: "any browser running the game would have an auto reload on next action… would that minimize the risk of having an old version written?" ⚑ The hazard has bitten twice — a stale tab overwriting the shared region file on its next tick, and SNG-549's three pushes carrying lower revs onto a good copy. `version.json` (written by `bump_version.mjs` and nowhere else) is what a running tab can ask; `staleBuild` compares it with its own baked `APP_VERSION`, cache-proof and never a verdict when it cannot ask. Read by `sync.js`'s one write chokepoint, which REFUSES the write (`GH_STALE_BUILD`), and by app.js, which reloads through a fresh URL once the turn and any encounter have cleared. NEVER on a rollback — only a strictly newer build counts. ✅ §277 |
| `canon.js` | Shared-canon promotion, contradiction→ranked opposed roll, rating-lens. **API** `ensureCanonStore, canonRecords, promotionCandidates, buildCanonRecord, weightOf, findCanonCollision, resolveContradiction, promoteInto, mergeCanonStores, lensDecision, adaptView, canonForViewer, SEXUAL_MARKERS, HARD_INTENSITY_MARKERS`. **NEVER** overwrites authored core canon (weight floor 100) or needs a human curator. |
| `sync.js` | GitHub transport for the shared world (owned-file writes + append-only ledger, SHA-retry). **API** `getSyncConfig, setSyncConfig, syncEnabled, ghList, fetchRepoJSON, fetchLedger, fetchLedgerMonths, fetchLedgerAll, pushOwnedFile, pushMergedFile, appendLedger, fetchRemoteCharacter, resolveSaveConflict, pushCharacterGuarded, backupSaves`. **NEVER** edits shared/region files in place — region state is written only by the world-tick. |
...
| `state.js` | Content load (manifest whitelist) + localStorage save/load + player/character registries + import/export. **API** `loadContent, getPlayerKey, setPlayerKey, lastPlayerKey, choosePlayer, charactersForPlayer, repairOwnership, dedupePlayers, listCharacters, saveCharacter, adoptRemoteCharacter, loadCharacter, saveProfile, loadProfile, exportSave, importSave`. ⛔ CCODE-355: who is playing is CHOSEN on every load (Erik: "a profile selection on the load screen - instead of defaulting to the assumed profile and characters") — `choosePlayer` clears a redirect a name-match wrote, `charactersForPlayer` follows a redirect only for a key with no profile of its own, and the repo folder a save lives in is its ownership record (`repairOwnership`). `dedupePlayers` no longer runs at boot: it folded Courtney's tablet into Erik because both profiles said "Erik". **NEVER** loads content from anywhere but the served repo files, and never merges two players by display name behind their backs. |
...
| `legionplan.js` | CCODE-407 (ERIK): *"I want this to be easy on the PC so we can have a delegate (GM through a chosen npc) come up with the legion plan."* **SOMEBODY ELSE DRAWS UP THE ORDER OF BATTLE.** **API** `planDraftersFor, draftLegionPlan`. ⛔ THE ARITHMETIC IS KNOWABLE, SO IT IS NOT GUESSED — which holds have spare capacity, which people the WORK bar reaches, which gaps a person would close, who is the highest level standing in a band: the engine knows each exactly, so the plan is DETERMINISTIC (no rng, no model call) and the same save always drafts the same one. What a delegate adds is a voice and the player's decision to trust somebody with it. ⚠️ IT PROPOSES AND NEVER WRITES: every step names the existing writer that performs it (`addContingent`, `setUnitLeader`, `formLegion`), so there is one way to put somebody in a unit and a plan cannot drift from what a player does by hand. ⛑ And drafting is FREE on Erik's ruling — it reports what calling the result would cost and spends nothing. PURE. |
...
| `art.js` | The image pipeline (assemble → rating/minor floors → URL → persist-once → gallery). **API** `getArtMode, setArtMode, locationImage, sceneImage, itemImage, npcImage, sanitizeImagePrompt, assembleImagePrompt, imageURLFor, artworkStyle, ensureImage, ensureGallery, addGalleryImage` (CCODE-357: a picture the CHARACTER made — named by the GM's `imageMedium` — is drawn in its own medium and kept as `artwork`; every other picture keeps the house medium). **NEVER** bypasses the floors, or re-assembles a record born with an image. |
| `recovery.js` | MERGE A RECOVERY SNAPSHOT BACK, LOSING NOTHING FROM EITHER. **API** `mergeRecovery, mergeReceiptLine`. ⛔ **The sync kept the losing copy of every both-advanced conflict and offered no door to it** — Erik lost the rider runner's encrypted slate exactly this way: a branch never pushed, replaced when the repo copy won, saved in localStorage and unreachable. ⚑ **MERGE, NOT RESTORE:** what the snapshot HAS that the save LACKS is added — items by id then name, people by id, codex topics and facts by text, quests, chronicle, deeds by stamp, holdings, bands — **nothing is removed, level and xp are floors, twice changes nothing.** **NEVER** merges a different character's snapshot. The door is Settings → Recovery copies. |
| `partybond.js` | WHAT ONE PLAYER'S CHARACTER FEELS ABOUT ANOTHER'S, AND IT ONLY GOES UP. **API** `noteHeard, partyBondOf, unheardOf, unheardBeat, ensurePartyBonds`. ⛔ **SPEC_intent_heard_and_unheard §3** — Aevi withdrew the penalty half of her own §3d: *"a player who is charged for being overruled stops stating intents, and the leader loses the briefing that makes the leader model work at all."* ⚑ **HEARD:** the leader took your suggestion — a small nudge between the two characters, **once per scene per pair**, clamped, positive only. ⚑ **UNHEARD:** a BEAT, automatic, attributed, shared — **it moves no number**; it is a thread the GM may pick up a scene later, not a debt. ⚠️ **NOT `growBond`:** companion bonds drive stage-taught craft RANKS, and a player must never grant another player ranks. |
| `sovereign.js` | R41 — A SOVEREIGN ARRIVES TWICE, AND THE SHAPE IS ONE RECORD WITH TWO FORMS. **API** `sovereignFormFor, sovereignFormLine`. ⛔ **One record, never two** — two ids for one being would have to be taught to the codex, the registry, `hingeNpcs`, `rivals` and the anti-Sovereigns who KNOW (R41c); identity is the thing R41 is about. A record carries `forms.diminished` (`atStage`, `level`, optional `abilities`) and `forms.final`; the form in force is chosen by the arc's LIVE stage, threaded in as `stageOf` from `worldtick.arcStageNow` the way `arceffects` already takes it — the engine never reads the tick. Before either stage: not arrived, the plain record; no `forms`: exactly as authored. `personOpponentFor` honours the form's level (`levelOverride`) and kit, and `duelFromTarget` writes the arrival on the def's `setup` — R41a: *the arrival IS the event*. ⚑ Reader before field: nobody authors `forms` yet; Aevi authors, Erik owns which stage. §147. |
| `recovery_snapshots.js` | DIFFS OF OVERWRITTEN SAVE BRANCHES, CARRIED IN CODE. **API** `SNAPSHOTS`. ⛔ **A committed save cannot win against an open tab** — on 2026-09-06 the repo copy with the slate (4a50f0cf, rev 2500) was overwritten three times by pushes with a LOWER rev (1853, 1858, 1794). **Reconcile step 41 merges each entry into its one character at the next load** through `mergeRecovery` — adds what the copy lacks, removes nothing, idempotent by version and by record. ⚠️ Computed on TIDIED copies: the load-time tidy re-keys topics to entity ids and a raw diff plants duplicates. |
| `presence.js` | WHO IS AROUND TODAY (SPEC_npc_presence_cadence) — the half of the world the GM was never shown. **API** `TIER_RATE, TIER_CADENCE, TIER_REACH, presenceScale, presentToday, presenceForGM`. ⛔ **TWO DIALS, TWO UNITS** (CCODE-485): `TIER_RATE` is a per-person daily rate for the LOCAL rungs, whose count should fall out of who lives nearby; `TIER_CADENCE` is Erik's ruling for the great rungs in DAYS BETWEEN OFFERS, and `presenceScale` derives the per-person chance from it and the rung's population — so tiering more people cannot move a ruled cadence, which is how a weekly heroic had become every second day. ⛔ **Measured: 117 authored figures and 110 Silas had never met**, because `npcsPresent` comes back IN the turn, KNOWN PEOPLE is only who was already met (so a STRANGER can never arrive from it), and `generateRequest` is reactive by rule. ⚑ **A per-person daily CHANCE** = the tier's rate × how near they are, with **reach scaling by standing** — a storekeeper is in their village, a heroic has a circuit, a legend is wherever the story is. Measured to Erik's cadence: heroic 1/8d, epic 1/15d, legendary 1/42d, a notable most days. **NEVER a quota** — the count falls out of who actually turned up, so a remote post offers one or none and that difference is information. **NEVER** offers a mythic on a timer: *a mythic appearing IS the event*. |
| `caravan.js` | R49 TRADE — **a road to prices that already exist, not a new economy**. **API** `caravansOf, roadDanger, sendCaravan, standingCarriers, resolveRoadHazard, tickCaravans, arriveCaravan, caravansForGM`. ⛔ **The load leaves the store at departure** — on the road or at the hold, never both, or it could be sold twice. ⛔ **What a robbed caravan costs (Erik 2026-09-06):** a normal raid takes a **share**; the escort **wiped in a fight you LOST** takes it **all** — *"especially if all your people get killed"* — and the total loss follows the fiction rather than a second die. ⛔ **Winning takes nothing, whatever it cost in people**: `personalRisk` has a floor, so a won fight can still kill a carrier, and a model where that lost the load would punish victory. **NEVER** lets the GM decide what became of a load — the engine resolves the road and the news reports it. |
| `jobs.js` | CCODE-420 JOBS — a job is sent, takes time, and is decided by the dice the people sent would roll. Erik: "Each task needs to come in with a level reference and a difficulty… feed into a roll… the outcome should have concrete effects." **API** `jobOpposition, craftOdds, jobCraftsOf, coverOf, jobDistribution, workOf, tripOf, planJob, suggestTeam, rollJob, jobCost, jobEffects, sayEffects, jobDirective, jobPersonFor, jobPoolOf, jobRouteOf, jobsForGM, applyJobEffects, settleDueJobs`. ⛔ **One roll, the fight's**: a need is covered by whoever is best at it, at the job's opposition passed to `successChance` as a numeric difficulty; the job's five outcomes are enumerated exactly. ⛔ **One scale**: a level-L job opposes as a level-L person does (`personThreatForLevel`). Work credits the crafts (`tierRate` per tier); the road is each person's own (`routeBetween`, their wits, march crafts). Effects go through the game's doors. Dials: `JOB_DEFAULTS`, over by `rules.jobs`. |
| `jobstate.js` | CCODE-420 where a job lives on the character — `character.jobs = { board, out, back }` — and who is away on one. ⚑ **A leaf, no imports**, so the fight's ally list can ask `awayOnJob` without pulling in the dice. **API** `ensureJobs, awayOnJob, jobAwayIds, normalizeJob, postJob, dropJob, sendOnJob, dueJobs, landJob, untoldJobs, markJobsTold`. ⛔ A posted job is clamped to what its level can honestly pay and cost; a result is marked told only after the GM's call returns. |
| `journey.js` | Routes over the MIXED graph — roads weighted by real `walkingDays`, plus the gate network — as a **decision, not an answer**. **API** `roadDistances, pathFrom, roadRoute, gatesUsableBy, routeBetween, routeLine`. ⛔ **Two NAMED options** (SNG-331 §1) — but only when the second is a real one: a 653%-worse detour is a trap, and `soleOption` says the world offers one way here rather than manufacturing a choice. ⛔ **The route is the TRAVELLER'S OWN** — only gates they have found (measured: 2 of 26 on a real save), which is what keeps walking a real option. **NEVER** weights a road by hop count, and **NEVER** offers a gate that saves nothing — it costs energy and a journey. |
| `journeyplan.js` | CCODE-387 A JOURNEY IS AGREED, THEN READIED, THEN WALKED. Erik: "you can agree to go on a journey and it logs it as a task... like a one step quest... that gives you time to stock up, collect your people, organize the trip, then begin the journey" — SNG-331/333, with Aevi's staged `journey_skills`. ⚑ Measured: every trip cost three hours whatever the distance. `planJourney` (a day's walk or more: the measured ways, days, rations, nights, worst stretch, company), `journeyQuest` (the one-step quest), `journeyOutcome` (the road's hours, rations eaten, forage, hunger's cost to energy and health — capped, never fatal), `journeyCraftsOf` (greenlore/lifesense/beastfriend forage, long_road marches, staunch and second_wind bear the hunger, wildcraft makes camp — dials in `rules.journey`), `journeyForGM`, `journeyArrivalPrompt`. |
| `journeyroad.js` | CCODE-390 THE ROAD, WALKED LEG BY LEG. Erik's "Proceed" on the stages CCODE-387 left open — Aevi's SNG-333 ("you either play out of a failure or pass the way you intended") and her theQuestShape ("an encounter mid-leg does not cancel it"). Setting out walks the chosen way's legs (`legsOfWay`, on every way): a day at a time on the one hunger model the whole road is priced by (`walkRoadDays`), the authored `onTravel` roll after each leg, an encounter stopping the journey where it happened (`stopRoadOn`). A leg into perilous country (danger 3+, a dial — ⚑ measured: at 2 it would be 66% of legs) is a plan of three steps (`legGambitFor`): the way, the danger, the camp, each taken with the best craft the character holds that claims the step's `challengeTypes` (their first reader) or that the step's authored `crafts` list names, rolled as the craft rolls and at its rank, with fallbacks it can actually take and the plain way last; assessed and run on the gambit's own resolver; a broken step played out where it happened (`legFailurePrompt`). Go around (`aroundLeg`), take up again from where they stand (`logJourneyOn` keeps what was walked), or end it with the record (`endJourneyOn`). Read by `app.js` (`walkRoad`, `takeDangerousLeg`, `faceLegPlan`, the card) and `gm_registry.js` (`journeyUnderwayDetail`). NEVER moves a traveller the player has not sent on, and never walks a stopped road for them. ✅ §274 |
...
<!-- verified by tests/verification_ledger.mjs v1.0.0 on 2026-08-04 — 781 gates across 114 requirements, all green at HEAD -->
...
| `SNG-273` | *"stage 2 of the Bleed is in effect, so what?"* | engine/arceffects.js — a stage's effects reach the cost path, the roads, the encounter pool, the GM's NPC block, and (SNG-581) what a carried thing FETCHES here: `priceShift` reached nobody until `env.arcEffects` was set | **28 gates** in `tests/smoke.mjs` | THE 2.0.0 BLOCKER. A stage carried publicFace and pressureOnAdvance, both narration, so 66 figures of attention, contests and casualties resolved into a number that changed a SENTENCE. Aevi authored 54 effects across 18 stages; 4 of her 5 kinds had a real consumer, and the testOnlyExports ratchet caught me shipping `encounterBias` unwired before it reached HEAD. ⚠️ `priceShift` has NONE — no module in this engine computes a price — so its 11 effects are inert, declared in `EFFECT_CONSUMERS` rather than left looking live. ⛔ ERIK REOPENED IT: "I want regional and local arcs as well." ⚠️ AND `scale` WAS A LABEL — authored on all six arcs, three world, two REGIONAL, one cosmic, and read by NOBODY on the effects path. `activeArcEffects` iterated every arc and applied every stage effect everywhere: measured at stage 3, `arc_block_bleed` — scale `regional`, "the Blocklands / the Gearlands" — put craftCost ×1.9 on lattice, figurist and hourkeeper IN THE DEEPWOOD. An effect carried no place at all; `match` supports `traditions` and nothing else. So a GENERATED arc, which the stub borns at `scale: "local"`, would have taxed the whole world — the born-whole contract broken one field along. ⚠️ AND THE PROSE COULD NOT BE THE REACH: `crossesRegions` is authored as sentences the GM says aloud ("all Reaches", "every deep site", "the manifest domains (which the substrate runs)") and 14 of its 17 entries resolve to no region id — reading it as ids would have stranded two-thirds of the world's arcs in nowhere. ⛑ SO THE MACHINE-READABLE REACH IS A SEPARATE `regions` FIELD OF REGION IDS, and the prose stays prose. Absent `regions` means EVERYWHERE, which is exactly what the engine did before, so Aevi's six arcs do not move on the day the capability lands and she rules the reach when she is ready; `world` and `cosmic` are everywhere regardless, so listing a few example regions on a world arc cannot shrink it. ⛔ AND IT IS LIVE EXACTLY WHERE ERIK ASKED: a generated arc is born carrying the region it was minted in, so a local thread stops charging the Deepwood for something that happened outside Cairnhold. ⚠️ MY FIRST VERSION OF THESE GATES CRASHED THE SUITE — `AE` and `content` are block-scoped to the 273 block above and I reached for them from a new one, and the whole file aborted at that line rather than failing a check. ⛔ AND I ALMOST SHIPPED `regions` WITH NO READER, WHICH IS THE EXACT DEFECT THIS SESSION KEPT FINDING: `activeArcEffects` iterated `content.greaterArcs` — the authored file and nothing else — and `arcStageNow` searches the same list, so a GENERATED arc was a record and an offscreen rumour and never a force in the world. ⛑ `character` had been a parameter of that reader since it was written and never read once; this is what it was for. Generated arcs now sit on the same path as authored ones: same stage lookup, same place filter, same consumer register. ⚠️ INERT TODAY AND MEASURED — a stub borns no `stages`, so 23 effects with the stub and 23 without — and the gate beside it proves the inertness is the STUB's doing rather than the reader never looking: a generated arc carrying one stage binds in the region it was born in and contributes nothing in another. |
...
**113 of 114 requirements carry a machine-proved gate** (781 checks total).
...
- **Journeys and routes (SNG-331 §1 / SNG-386 §4.4, v1.9.379).** A route is a **decision, not an answer**: `routeBetween` returns **two NAMED options** — Aevi's rule, *"four days through the Wend, or seven around it"* — over the **mixed graph of roads AND gates**. Roads are `connections` weighted by **`walkingDays`, never hop count** (measured: one 150-day leg would otherwise read as the shortest road in the world); the gate leg composes `waygate.js`'s `gateHopCost`, which turns a 236-day walk to the Gearlands Verge into 3.9 days. ⛔ **A second option is offered only when it is a real one** — banning the Axis Gate turns a 53-day walk into a 402-day one, and a 653%-worse detour is a trap dressed as a choice; `soleOption` says plainly that the world offers one way here rather than manufacturing a decision. ⛔ **The route is the TRAVELLER'S OWN**: gates are restricted to ones they have discovered and can aim at (measured, Silas can use 2 of 26), which is what keeps walking a real choice — unrestricted, the network wins 80-98% of every journey. Direction comes from `bearingBetween` in **Aevi's four words** (hubward/outward · spinward/widdershins, SNG-386) and never from a compass this world does not have. Chain per §23: engine `journey.js` (+ `worldmap.js` for geometry, `waygate.js` for who may pass) · consumer `buildTravelDirective` in app.js · registered `bearingsDetail` for the standing compass · reachable on any travel intent · contracted here.
...
#### ✅ v2.0.0 CUT — 2026-09-14 (SNG-581)

Erik: *"I think it's about time to roll to 2.0.0."*

⛑ **AND THE ONE ROW THIS SECTION NAMED AS THE HONEST REASON TO WAIT IS CLOSED.** The v1.9.0 record above
refused 2.0.0 on a measurement rather than a feeling: §25.3 asks that *"every player should be able to see
the arcs moving"*, and Aevi's SNG-273 finding was that **a stage had no mechanical field at all** — the whole
chain of 66 figures, attention budgets, contests, casualties and vacancies resolved into a number that
changed a sentence. ⚠️ That is the paragraph directly above this one, and it is the reason this record has
to answer it with numbers.

⚑ **MEASURED TODAY, AT ALL FOUR DOORS** — authored → loaded → read → surfaced, because passing three of
four is what "the arcs move and nobody feels them" looked like:

| door | at the v1.9.0 cut | now |
|---|---|---|
| **authored** | 0 stages carry a mechanical field | **18 of 22**, on **5 of 6** greater arcs |
| **loaded** | — | the same `greater_arcs.json` the prose already came from |
| **read** | nobody | `engine/arceffects.js` — pure, reads `effects`, `stageOf` injected so it stays free of the world-tick |
| **surfaced** | `publicFace` only, a sentence | **six consumers**, all wired in `app.js` |

The six are what turn a stage into something felt: `craftCostNote` (what the craft costs you *here*),
`travelCostFactor` (what the road costs), `encounterBias` (what you meet on it), `npcMoodLines` (how people
carry themselves), `effectsInPlainWords` (the world's behaviour, stated), and `arcReachesRegion` — the
`regions` gate, without which `arc_block_bleed` put a ×1.9 craft cost in the Deepwood.

⚠️ **AND THE DESIGN RULE HELD WHILE IT CLOSED**: a stage changes the WORLD's behaviour, never taxes the
player's sheet. A flat −1 to rolls is a punishment and reads as one; grammar-work costing double in the
Bleed is a world you can route around, pay deliberately, or leave. And not every arc is decay —
`what_wakes_beneath` *lowers* precursor cost. An arc model that only ever made things worse would be a
misery meter, and a misery meter is not a world.

⛔ **WHAT IS STILL BARE, NAMED RATHER THAN ROUNDED UP.** `arc_the_disagreement` carries **0 of 4** stages
with `effects` — The Pause Holds, Attention, The Question Reopens, Resumption. Every other greater arc is
...
and the freshness gate in `wiring_audit` is the reason this line can never quietly sit still for 180 point
releases again.
...
### 26.7 ⛔ THE WORLD IS FROZEN — ruled 2026-09-14 (SNG-582)

**Erik:** *"i've already ruled that I don't intend to ever regenerate the world again… so the new normal are
authored locations and mods to what we have."*

⚑ **WHAT THAT MAKES `terrain.json`.** It stops being a derived artifact and becomes **ground truth** — the
same promotion §26.1 gave the scale constants. `scripts/world/generate_world.mjs` is kept as the documented
derivation *behind* the world, not as a step anyone runs, and **its write path refuses** without an explicit
override. A ruling that lives only in a transcript gets re-broken by whoever did not read it.

⚠️ **AND IT RETIRED A GATE, WHICH IS THE PART WORTH RECORDING.** `world --check` asserted DETERMINISM — does
regenerating reproduce the committed asset byte for byte. Under this ruling that question can only ever
answer **no**, and it can never be worth fixing, because fixing it means running the forbidden command. It
had already been baselined as an accepted failure, which is how a gate dies quietly. ⛑ Measured the day of
the ruling: **not one part still reproduces** — not the layers, not the points, not the seats, not the
hydrology.

⬜ Aevi reached the same conclusion from the other side under SNG-565 and her sentence is the rule:
**"AN AUTHORISATION IS NOT A MEASUREMENT."** Resolving place names against a rebuild had six *working* river
names reading as broken, and she was one command from retiring them "to satisfy a gate about a world nobody
will ever load."

⛔ **THE CONSEQUENCE HE DID NOT STATE, AND IT WAS ALREADY LIVE.** The asset was baked 2026-08-10 with **135**
place rows while canon had **138** placed locations. Firstsight, Keelmouth and The Mountain Pass existed in
the fiction, carried real coordinates, and had **no row in the file the world map reads** — no pin, no
metadata. Four more rows, **the Crossing among them**, named a region canon had since reassigned. Under the
old rule the next rebuild folded all seven in; under this one there is no next rebuild.

⛑ **SO THE PLACE LIST GETS ITS OWN DOOR.** `scripts/world/place_rows.mjs` splices location rows into the
frozen file and touches nothing else — not a layer byte, not a seed, not a biome, not the hydrology, not the
seats. ⚠️ It shares **one derivation** with the retired build (`locationRows`), because two copies of the row
shape would drift the day a field is added and the map would then disagree with itself about places nobody
touched. A row canon no longer knows is **named, never removed**: deleting is a different decision from
adding, and it is the destructive one.

**The new gate asks what the ruling makes real** — is the frozen asset INTACT (its layers the size its own
encoding declares, 118 seeds, biomes present), and does it still know **every placed location**? It no longer
builds anything to answer, so it runs in under a second instead of nine. §233 holds the refusal, the single
derivation, and the splice-touches-only-the-list rule.
...
| `mechanic.crit` / `crit` | `critFor` → a per-craft **chance bias** and a **sentence on the receipt**. ⚠️ **Nothing else** — see §39.4 | `craftmechanics.js` → `resolve.js` | **93** |
| `damageType` | `answers(l)` — which soak layers apply | `skill_battle.js` | 26 |
| `wardTypes` | `wardAnswer` → **a ward answers PART of a mix**, by type and by rank. ⚠️ **THIS ROW SAID "NOTHING — the string does not appear in `skill_battle.js`" UNTIL 2026-08-30, AND IT WAS FALSE:** CCODE-281 wired it and the string appears **10 times**, mapped to `soakTypes` at the guard. ⛔ **A spec that declares a load-bearing field dead is worse than silence** — it tells an author not to write the one field that decides whether a blow lands. Found by CCODE-326's tournament, where carrying a typed ward correlated with winning at r = +0.58. | `damagetypes.js` → `skill_battle.js` | **55** |
