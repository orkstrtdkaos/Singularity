...
| `field.js` | THE POWER FIELD, evaluated per texel and never interpolated between markers (CCODE-457, extracted from the `exesa_field` prototype before it is deleted). The region vote, sources as Gaussians where a NEGATIVE strength is a sink, ordered and wild as two accumulators that overlap, patchiness on wild only, meaning from places and roads, and **arc stages that shift the whole field** — "the ground is not where it was authored". **API** `MEMBERSHIP, FIELD_KINDS, ARC_EFFECTS, ARC_STEP, makeGrids, strengthAt, paint, coverage, probeAt, arcShift, indexAt, texelCentre, voteWeight, noise2, lonDelta`. **NEVER** fetches, bakes or draws — the caller hands it the world's numbers, so "the authored 44 or the baked 43" stays one decision at one call site. **ONE evaluator for all three map tiers**, which is the point. |
