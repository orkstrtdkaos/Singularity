// app.js â€” Singularity v0.1 shell: character creation, the play loop, settings.
...
import { seedStandingAtCreation, accrueStandingForDays, applyStandingOps, standingRoster } from "./engine/standing.js"; // BATCH-12 Â§3
...
import { armDevCapture, recordCall, annotateLatest, devCaptures, clearCaptures } from "./engine/devcapture.js"; // SNG-186 Â§2f: see the machine
...
import { smartClamp } from "./engine/namematch.js"; // SNG-095: used at app.js:562 (GM context) + the gambit advise clamp â€” was never imported
import { substrateVerdict, locationDensity, carriedSubstrate, carriedSubstrateSources, schoolForTradition, defaultSchoolsForDomains, setCharacterSchool, commonGroundFor, groundAsPlace } from "./engine/substrate.js"; // SNG-090 + BATCH-13 + SNG-193b + SNG-192 Â§6b
...
import { assembleGMContext } from "./engine/gm_registry.js"; // BATCH-11 Â§23: the GM context is a DECLARED registry, iterated â€” never hand-listed
import { rankVoices, pickVoice, speakableText, chunkForSpeech, renderProseHtml } from "./engine/narration_voice.js"; // SNG-155: read aloud at the table; SNG-190 Â§4: render engine asides, never raw asterisks
...
import { resolveWaygateTransit, routeGmMoveTo } from "./engine/waygate.js"; // SNG-148: waygates â€” map control routes named/hub; GM offer via the registry row
...
import { addAssignment } from "./engine/assignments.js"; // SNG-191 Â§4: the world honours delegated work
import { setArcFate } from "./engine/latentarcs.js"; // SNG-191 Â§7: the player closing a surfaced arc (the handled/resolved fate)
...
// CCODE-07: MUST match index.html's `?v=` cache stamp â€” tests/wiring_audit.mjs fails the build on
// drift. It had silently sat at 1.8.104 across five ships, and it is what stamps `appVersion` on
// every feedback report â€” so bug reports were filed against a version that hadn't been running.
const APP_VERSION = "1.8.183";
const app = document.getElementById("app");
// SNG-084: one delegated listener drives every â“˜ helper dot â€” it survives chrome() re-renders (those
...
// SNG-104: vitals detail on tap (phone) / hover (desktop) â€” mirrors the data-help delegation above.
app.addEventListener("click", e => { const el = e.target.closest?.("[data-vital]"); if (el) { e.preventDefault(); showVitalDetail(el); } });
// SNG-106: tap the roll's chance â†’ the full component breakdown (the resolver's retained math, verbatim).
app.addEventListener("click", e => { const el = e.target.closest?.("[data-breakdown]"); if (el) { e.preventDefault(); try { showBreakdownPopover(JSON.parse(el.dataset.breakdown)); } catch { /* malformed â€” no popup */ } } });
// SNG-118: tap a play-style aptitude chip â†’ its effect + description (reuses the one popover surface).
...
// Same entity â†’ same detail, no matter the render site (the consistency ask). Reuses the one popover surface.
...
// SNG-134: resolve "kind:id" â†’ the shared detail text, gathering the live values the pure formatters need.
...
/** An â“˜ affordance for a mechanic surface â€” renders nothing if there's no authored copy (no dangling dot). */
function infoDot(id) { return helpEntry(id) ? `<button class="info-dot" data-help="${esc(id)}" title="What's this?" aria-label="Explain this">â“˜</button>` : ""; }
 ...
 *  and a link out to the Library for the deep read. A dismissible overlay â€” never blocks the screen. */
function showHelp(id) {
  ...
  pop.innerHTML = `<div class="help-card" role="dialog" aria-label="Explanation">
    ...
    ${e.more ? `<div class="help-more" id="help-more" hidden>${mdLite(e.more)}</div><button class="link-btn" id="help-more-toggle">moreâ€¦</button>` : ""}
    <div class="help-foot"><button class="link-btn" id="help-lib">ðŸ“– The Library</button><button class="btn" id="help-close">Got it</button></div>
  </div>`;
  ...
  if (mt) mt.onclick = () => { const m = document.getElementById("help-more"); m.hidden = !m.hidden; mt.textContent = m.hidden ? "moreâ€¦" : "less"; };
...
// SNG-106: format the resolver's retained breakdown into the shared popover â€” every signed component,
// the opposed term named, the clamp only when it bit, and the number â†’ roll â†’ result in one place.
function showBreakdownPopover(bd) {
  const sign = v => (v >= 0 ? "+" : "âˆ’") + Math.abs(Math.round(v));
  const lines = (bd.components || []).map(c => `${c.label}   ${sign(c.value)}`);
  if (bd.clampedFrom != null) lines.push(`clamped (from ${bd.clampedFrom})`);
  const foot = `total   ${bd.total}%${bd.roll != null ? `   â€” rolled ${bd.roll} â†’ ${String(bd.degree || "").replace("_", " ")}` : ""}`;
  showPopoverText(`Success chance ${bd.total}%\n${lines.join("\n")}\nâ”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€\n${foot}`);
}

// SNG-104: what a Health/Energy number's tap/hover shows â€” how rest restores it, read from CONTENT.rules.recovery.
function showVitalDetail(el) {
  ...
  const body = kind === "energy"
    ? `Rest restores â€” breather +${breather.energy ?? 10} (${breather.hours ?? 1}h) Â· meal +${rec.meal ?? 10} Â· sleep +${sleep.energy ?? 40} (${sleep.hours ?? 8}h)`
    : `Rest restores â€” breather +${breather.health ?? 1} Â· sleep +${sleep.health ?? 3} (${sleep.hours ?? 8}h)`;
...
// never ended grew to 169 turns (~341KB) on a real save â€” 59% of a 600KB character â€” which is what
...
// SNG-066: feedback forensics â€” the value is the AUTO-CAPTURED CONTEXT, not the text box.
...
let mapShowSub = false;  // SNG-082b: sub-place satellites toggle (off by default â€” the clean look)
...
// Persisted on the PROFILE (a UI preference, cross-character â€” Q1). Two sets: what the player opened against
...
let lastPlayerText = ""; // last freeform/ask input â€” restored into the box if the GM errors
...
// SNG-075: encounters in narrative play â€” an encounter selected by the narrative-time roll waits
...
// SNG-127: the cooldown is now the player's pacing (resolvePacing â†’ cooldown), read per-turn from config.
...
const RECIPES_PATH = "world/braid_recipes.json"; // SNG-201: a NEW shared store (not emergence_recipes â€” that's prescriptive)
...
(async function boot() {
  ...
  // (sessionStorage â€” a reload without the param returns a clean player view); `?dev=0` clears
  ...
  // SNG-186 Â§2f: arm the model-exchange capture ONLY in dev mode, and point the transport's observer
  // at it. In a player view isDevMode() is false â€” nothing is armed and setCallObserver is never
  // called, so the capture stays null and inert (Â§3.4: no dev path the normal turn can reach).
  try { if (isDevMode()) { armDevCapture(true); setCallObserver(recordCall); } } catch { /* ignore */ }
  wireLightbox(); // SNG-053: click any image â†’ larger view
  try {
    CONTENT = await loadContent();
    FN_INDEX = buildFunctionIndex(CONTENT.functionVocabulary); // SNG-124: verbâ†’function-family index for coverage + badges
  } catch (err) {
 ...
 *  copy â€” NEWER WINS, a stale local is never used, and a genuine both-advanced conflict
...
async function syncPullCharacter(local) {
  ...
  catch (err) { console.warn("[sync] pull failed â€” using local:", err.message); return { character: local, note: null }; }
  if (!remote) return { character: local, note: null };
  const res = resolveSaveConflict(local, remote);
  if (res.reason === "remote-newer") {
    if (res.conflict) preserveRecovery(local, "local"); // both advanced â€” don't lose local work
    adoptRemoteCharacter(remote);
    return { character: remote, note: res.conflict
      ? "Loaded the newer version from another device â€” this device's local changes were kept as a recovery copy."
      : "Loaded your latest from another device." };
  }
  if (res.conflict) { preserveRecovery(remote, "remote"); return { character: local, note: "Kept this device's newer version â€” the other device's copy was saved as a recovery copy." }; }
  return { character: local, note: null };
}

/** "Who's playing?" â€” pick an existing player on this device, or start a new one. */
function renderPlayerPick(msg = "") {
  ...
  app.innerHTML = `<div class="app-boot"><div class="screen" style="max-width:460px;margin:40px auto">
    ...
    <div class="player-pick">
      ...
      ${syncEnabled() ? `<button class="btn secondary" id="player-discover" style="margin-top:8px">â˜ Find me in the shared world</button>` : ""}
...
function chrome(inner) {
  app.innerHTML = `
    <div class="topbar">
      <div><h1>SINGULARITY</h1><span class="sub">The Valley of Echoes â€” v${APP_VERSION}</span>${isDevMode() ? ` <span class="dev-badge" title="Developer mode is ON. Turn it off in Settings, or reload without ?dev=1.">DEV</span>` : ""}</div>
      <div class="actions">
        <button id="nav-roster">Characters</button>
        <button id="nav-settings">Settings</button>
        <button id="nav-feedback" title="Feedback / bug report â€” your version, location, character and last turn attach automatically">âš‘ Feedback</button>
        ${devEnabled() ? `<button id="nav-dev" title="Dev preview-legs checklist">ðŸ§ª Legs</button>` : ""}
        ${devEnabled() ? `<button id="nav-machine" title="SNG-186 Â§2f â€” see the machine: the assembled prompt, raw response, parsed result and ops fired for recent model calls">ðŸ”¬ Machine</button>` : ""}
      </div>
    </div>
    ${inner}`;
...
// ---------- SNG-053: image lightbox (click any portrait/scene/moment art â†’ larger view) ----------

/** Open a modal lightbox over a list of images (arrow-through when >1). Esc / click-backdrop /
 *  âœ• dismiss; â†/â†’ navigate. Reused by portraits, NPC/location art, moment art, and the gallery. */
function openLightbox(items, start = 0) {
  ...
  const render = () => {
    ...
    el.innerHTML = `<div class="lightbox-inner">
      ...
      ${it.caption ? `<div class="lightbox-cap">${esc(it.caption)}${list.length > 1 ? ` Â· ${i + 1}/${list.length}` : ""}</div>` : ""}
      ${list.length > 1 ? `<button class="lightbox-nav prev" data-lbprev>â€¹</button><button class="lightbox-nav next" data-lbnext>â€º</button>` : ""}
      <button class="lightbox-close" data-lbclose>âœ•</button>
    </div>`;
...
/** SNG-074: the ONE dev-mode source of truth â€” opt-in, visible, reversible.
 *  ON when: this URL has `?dev=1` Â· a dev host (localhost/preview) Â· this session opted in
 *  (`?dev=1` earlier this session) Â· a DELIBERATE persistent opt-in from Settings.
 *  `?dev=0` in the URL always wins OFF. No sticky localStorage flag, so a family member can
 *  never be trapped in dev mode â€” a plain reload on the live URL is a clean player view. */
function isDevMode() {
  try {
    const p = new URLSearchParams(location.search);
...
// devEnabled()/isDev() are the historical names â€” both now resolve to the single model above.
function devEnabled() { return isDevMode(); }

const LEG_STATUS = ["untried", "pass", "fail", "feels-off"];
const LEG_STATUS_LABEL = { untried: "Â· untried", pass: "âœ“ pass", fail: "âœ— fail", "feels-off": "~ feels-off" };
...
// only), marks it pass, and â€” when sync is on â€” REPORTS it to Aevi via a synced status file so
...
async function reportLegStatus() {
  ...
  try {
    ...
    saveLegStatus(m2);
  } catch { /* offline / no write â€” stays unreported, retried on the next detection */ }
...
// SNG-051 addendum: the â–¶ Run-this-scenario runner. Each force intent resolves against the
// EXISTING primitives (travel/setRating/rest/generate/clock-jump/screens) â€” no new mechanics.
// Dev-only (the panel is devEnabled-gated). Cross-player / two-character legs stay manual.
// Unknown intent â†’ no runner entry â†’ the button renders as "(unsupported intent)".
/** DEV: ensure a fully-equipped test character is active so any leg has its prerequisites â€” a
...
async function seedGrownEntity() {
  ...
  if (rec?._gen) {
    recordAttention(rec, "keep", readClock(character.clock).day); // â†’ established, so it's clearly grown
    saveCharacter(character);
    return rec.id;
  }
  return null;
...
const LEG_RUNNERS = {
  openSkillPicker: () => {
    // "spend 2 skill points / take a cross-class ability" â€” grant the points so it's runnable
    ensureTestCharacter();
  ...
  setRating: (f) => {
    ...
    saveProfile(profile);
    renderSettings(rv.ok ? `ðŸ”§ Test: ceiling set to ${target}. ${f.cycle ? "Re-run to cycle to " + f.cycle.slice(1).join(" / ") + "." : ""}` : "Rating unchanged â€” " + rv.reason);
  },
  travelToDisposition: () => {
    ensureTestCharacter();
    ...
    if (pick) travelTo(pick.id); else renderPlay(character.activeScene?.lastTurn || null, { aside: "ðŸ”§ No charged location found to travel to." });
  },
  enterPlanApt: () => {
    // "set up a gambit so I can run it" â€” open the builder PRE-FILLED with a runnable plan
    ensureTestCharacter();
    gambitDraft = {
    ...
    renderGambitBuilder("ðŸ”§ Test plan loaded â€” hit Assess plan (needs an API key), then Run it.");
  },
  ...
  generateHere: () => {
    ...
    handleGenerateRequests({ generateRequest: [{ type, hint: "a place just past the edge of the known", why: "dev test: force a generation" }] })
      .then(grown => renderPlay(character.activeScene?.lastTurn || null, { aside: grown.length ? `ðŸ”§ Generated: ${grown.map(g => g.name).join(", ")} â€” revisit to confirm it persists.` : "ðŸ”§ Nothing minted (governor or reuse, or needs an API key) â€” try again or move to an unauthored edge." }))
      .catch(() => renderPlay(character.activeScene?.lastTurn || null, { aside: "ðŸ”§ Generation failed (needs an API key)." }));
  },
  jumpClock: async (f) => {
    ensureTestCharacter();
    ...
    renderPlay(character.activeScene?.lastTurn || null, { newsFlash: news, aside: `ðŸ”§ DEV: advanced the shared clock +${days} day(s) â†’ the Kept Count now stands at ${worldCount()}. Return to play to see what advanced.` });
  },
  portraitBeat: () => {
    ensureTestCharacter();
    if (!imagesEnabled()) { renderSettings("ðŸ”§ Turn on Scene & item art â†’ Generate first, then run this leg."); return; }
  ...
  grantTestState: () => {
    ...
    if (!character.quests.some(q => q.id === "dev-test-quest")) character.quests.push({ id: "dev-test-quest", title: "A Test Errand", status: "active", summary: "Dev-seeded quest to verify progressâ†’complete.", progress: ["begun"] });
  ...
  openRomanceScene: async () => {
    // ROMANCE leg (the one surface neither CCode nor Aevi can prove â€” needs a live model): set the
    // ceiling to R and open a scene with a romance-open NPC so a flirt is one line away. The leg
    // auto-marks âœ“ the moment the flirt is tagged `romantic` and the guidance doc loads (see runGM).
    ensureTestCharacter();
    profile.rating = profile.rating || {}; profile.rating.adultVerified = true;
    setRating(profile, "R", { authority: "erik", adultGate: true });
    saveProfile(profile);
    await startScene(
      "(Romance test scene. The character is sharing a quiet, unhurried moment with someone they've been growing close to â€” introduce a warm, present NPC of this place who is clearly at ease with the character and open to them: a held glance, a nearness neither moved to create. Set it so a flirtatious or tender action is a natural next beat. Offer choices, but leave the room for the player to speak or act freely.)",
      [],
      "ðŸ”§ Romance leg â€” ceiling set to R. Flirt with the NPC: type an attraction or flirtation line freely. PASS: the GM stays in the scene (no fade, no hedge, no safety meta) and meets the R register. This leg auto-marks âœ“ the instant your flirt is tagged and the romance guidance loads â€” then eyeball the prose."
    );
  }
};
...
function runPreviewLeg(leg) {
  ...
  if (!runner) return; // manual / unknown â€” no-op (button wasn't rendered as runnable)
...
/** Gather the forensic context. Redacts credentials â€” never dumps the config object. */
function buildFeedbackContext() {
  ...
  if (typeof character !== "undefined" && character) {
    ...
    // SNG-179 Â§4.4: attach what the OPS actually did. A report that says "the teacher never
    // registered" is a mystery; one that says markTeacher was rejected 3Ã— on an unknown traditionId
  ...
  // SNG-186 Â§3.2: a bug report must declare its dev provenance. Dev mode being ON at all is
  // load-bearing triage context â€” the workbench may have staged encounters, minted entities, or set
  // state â€” and a report that hides it wastes everyone's time. Mutating levers append to
...
function openFeedback() {
  ...
  const TYPES = [["bug", "ðŸ› Bug"], ["idea", "ðŸ’¡ Idea"], ["feel", "ðŸ¤” Felt off"]];
  const q = feedbackQueue().length;
  chrome(`<div class="screen" style="max-width:560px">
    <h2>âš‘ Feedback</h2>
    <p class="hint" style="margin-bottom:10px">One tap, one sentence. Your version, where you are, your character and the last beat attach automatically â€” Aevi reads it without asking a single follow-up.</p>
    ...
    ${!syncEnabled() ? `<div class="hint">âš ï¸ Sync is off â€” this will be SAVED and sent the next time sync is on (nothing is lost).</div>` : ""}
    ${q ? `<div class="hint">${q} earlier report${q === 1 ? "" : "s"} waiting to send â€” they'll flush with this one.</div>` : ""}
  ...
  document.getElementById("fb-send").onclick = async () => {
    ...
    // SNG-115: `at` on the entry itself (submitFeedback dates the file from it) â€” its absence was a silent throw.
    ...
    btn.disabled = true;
    try {
      // SNG-115: race the submit against a UI deadline so the status ALWAYS reaches a terminal state â€” even
      ...
      if (r.ok) status.textContent = `âœ“ Sent â€” thank you. Entry ${r.id}${r.flushed > 1 ? ` (+${r.flushed - 1} queued)` : ""}. Aevi will see it at session-open.`;
      else if (r.queued) status.textContent = `âœ“ Saved as ${r.id}${syncEnabled() ? " â€” the send didn't finish, it will retry next time" : " â€” will send when sync is on"}. Nothing is lost.`;
      else status.textContent = "Couldn't save that â€” try again.";
    } catch (err) {
      const q = feedbackQueue(); if (!q.some(e => e.id === entry.id)) saveFeedbackQueue([...q, entry]); // never lose it, whatever failed
      status.textContent = `âœ“ Saved as ${entry.id} â€” something went wrong sending, but it's kept and will retry. Nothing is lost.`;
    } finally {
      btn.disabled = false;
    }
  };
...
async function renderPreviewLegs() {
  ...
  const legRow = (l) => {
    ...
    const auto = st === "pass" && status[l.id]?.auto ? ` <span class="leg-auto" title="${esc(status[l.id]?.note || "auto-detected in play")}">auto âœ“</span>` : "";
    // SNG-051 addendum: a leg with a resolvable force intent gets a â–¶ Run button; a null/manual
    ...
    const runCtl = intent
      ? `<button class="leg-run" data-legrun="${esc(l.id)}">â–¶ Run this scenario</button>`
      : (l.force === null && l.mode === "cross-player") ? `<span class="leg-manual">manual â€” needs two synced profiles</span>`
      : (l.force && l.force.manual) ? `<span class="leg-manual">manual â€” ${esc(l.force.manual)}</span>` : "";
    ...
    return `<div class="leg-row leg-${st}">
      ...
      <input class="leg-note" data-legnote="${esc(l.id)}" placeholder="note (optional)â€¦" value="${esc(status[l.id]?.note || "")}">
    </div>`;
  };

  chrome(`<div class="screen" style="max-width:820px">
    <h2>ðŸ§ª Preview Legs <span class="hint" style="text-transform:none">â€” ${verifiedActive} of ${active.length} left${cleared.length ? ` Â· ${cleared.length} cleared` : ""}${_previewLegsData.buildVersion ? ` Â· data for v${esc(_previewLegsData.buildVersion)}` : ""}</span></h2>
    <p class="hint" style="margin-bottom:12px">Legs auto-mark <strong>auto âœ“</strong> when their moment happens in play${syncEnabled() ? " and report to Aevi automatically" : ""}. A leg drops out once it's verified back to Aevi (or Aevi closes it). Data authored by Aevi (<code>data/preview_legs.json</code>).</p>
    ${active.length ? Object.keys(byMode).map(mode => `
      ...
      : "<div class='insight'>ðŸŽ‰ Nothing left in this checklist â€” every leg is verified &amp; reported to Aevi.</div>"}
    ${cleared.length ? `<details class="cs-block"><summary class="codex-title" style="font-size:15px; cursor:pointer">âœ“ ${cleared.length} cleared â€” verified &amp; reported to Aevi</summary>
      ${cleared.map(l => `<div class="leg-cleared"><span class="leg-batch">${esc(l.batch)}</span> ${esc(l.title)} <span class="hint">${closedIds.has(l.id) ? "closed by Aevi" : "reported"}${status[l.id]?.note ? " Â· " + esc(status[l.id].note) : ""}</span></div>`).join("")}</details>` : ""}
  ...
  document.getElementById("legs-copy").onclick = async () => {
    const map = loadLegStatus();
    const lines = legs.map(l => { const s = map[l.id] || {}; return `- [${(s.status || "untried").toUpperCase()}] ${l.batch} Â· ${l.title}${s.note ? ` â€” ${s.note}` : ""}`; });
    const summary = `Preview legs (${verified}/${legs.length} pass) â€” build data v${_previewLegsData.buildVersion || "?"}\n${lines.join("\n")}`;
...
// ---------- SNG-186 Â§2f: see the machine ----------

/** SNG-186 Â§3.2: every dev action is marked on the save, visibly â€” it rides into feedback reports
...
/** SNG-186 Â§2a: GO ANYWHERE â€” jump the character to ANY location by id or name, ignoring connections,
 *  waygates and travel time (including places no path reaches â€” a class of bug this finds). Â§3.3: the
 *  jump writes through the SAME fields play's move path uses (currentLocationId, knownPlaces, place
 *  visit, perception) â€” it exercises the real engine, never a dev-only shadow. Clears the active scene
...
/** SNG-186 Â§2b: KNOW EVERYTHING â€” mark every location known, so the GM's recall and the map treat the
...
/** SNG-186 Â§2b, the inverse that matters MORE: reset knowledge to just where you stand, so retrieval
 *  bugs (SNG-176) that only appear from IGNORANCE can be reproduced â€” a tester who knows everything
 *  never can. */
 ...
 *  and which ops fired for the last two-dozen model calls this session â€” the by-hand SNG-179
...
function renderMachine() {
  ...
  // SNG-190 Â§3, THE FALSE-ZERO FIX. EMISSION (the model putting an op in a turn) is counted for EVERY
  // op, every turn, in runGM. APPLIED/REJECTED outcome is instrumented for ONLY these. The panel must
  // never again render an un-instrumented op's absence as "NEVER FIRED" â€” that was the bug: 31 emitted
  ...
  // can never read as "not emitted" above it â€” even before the cumulative counter has accrued here.
  ...
  // SNG-195 G3: every op that writes an applied/rejected outcome via logOpOutcome must render its badge â€”
  ...
  const chip = (op) => {
    ...
    const outcome = OUTCOME_INSTRUMENTED.has(op)
      ? ` <span class="mach-x" title="engine outcome â€” instrumented: applied${r?.rejected ? "/rejected" : ""}">âœ“${r?.applied || 0}${r?.rejected ? `âœ—${r.rejected}` : ""}</span>`
      : "";
    const tip = OUTCOME_INSTRUMENTED.has(op) ? "" : ` title="emission counted; applied/rejected NOT instrumented for this op â€” a 0 means not emitted, never 'rejected'"`;
  ...
  const promptText = (c) => {
    const sys = (c.system || []).map((b, i) => `â”â” SYSTEM BLOCK ${i + 1}${b.cache_control ? " (cached)" : ""} â”â”\n${b.text || ""}`).join("\n\n");
    const msgs = (c.messages || []).map(m => `â”â” ${String(m.role || "user").toUpperCase()} â”â”\n${typeof m.content === "string" ? m.content : JSON.stringify(m.content, null, 2)}`).join("\n\n");
  ...
  const card = (c) => `<details class="mach-card"${caps[0] === c ? " open" : ""}>
    <summary><span class="mach-task">${esc(c.task)}</span> <span class="hint">${esc(c.model)} Â· ${esc(usageLine(c))}${c.at ? " Â· " + esc(String(c.at).slice(11, 19)) : ""}</span></summary>
    ${c.opsFired?.length ? `<div class="mach-sec"><span class="mach-label">emitted</span> ${c.opsFired.map(o => `<span class="mach-op mach-fired">${esc(o.op)} <code>${esc(o.shape)}</code></span>`).join(" ")}</div>` : (c.parsed ? `<div class="mach-sec hint">no ops emitted â€” narration only</div>` : "")}
  ...
  const locOptions = character ? Object.values(CONTENT.locations || {}).sort((a, b) => (a.name || "").localeCompare(b.name || "")).map(l => `<option value="${esc(l.id)}">${esc(l.name)} â€” ${esc(l.id)}</option>`).join("") : "";
  const leversBlock = character ? `
    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Levers <span class="hint" style="text-transform:none">â€” dev writes go through the real paths; every action is marked on the save</span></h3>
      <div class="mach-lever"><span class="mach-label">Go anywhere</span>
        <input id="mach-jump" list="mach-locs" placeholder="location name or id â€” any of ${totalLocs}, reachable or not" autocomplete="off">
        <datalist id="mach-locs">${locOptions}</datalist>
        <button class="btn secondary" id="mach-jump-go">Jump &amp; play</button>
        <span class="hint" id="mach-jump-msg">at ${esc(CONTENT.locations[character.currentLocationId]?.name || character.currentLocationId || "â€”")}</span>
      </div>
    ...
    </div>` : `<div class="cs-block hint">Load a character (open Play) to use Go-anywhere and Know â€” these levers act on the live save.</div>`;

  chrome(`<div class="screen" style="max-width:900px">
    <h2>ðŸ”¬ See the Machine <span class="hint" style="text-transform:none">â€” last ${caps.length} model call${caps.length === 1 ? "" : "s"} this session</span></h2>
    <p class="hint" style="margin-bottom:12px">The assembled prompt, the raw model response, what parsed, and which ops fired â€” the SNG-179 diagnosis as a standing panel. Captures live in memory for this session only (dev-mode; a player never reaches this).</p>
    ${leversBlock}

    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Op emission â€” this character, cumulative <span class="hint" style="text-transform:none">(${turns} GM turn${turns === 1 ? "" : "s"} observed)</span></h3>
      <p class="hint" style="margin-bottom:8px">Counts are <strong>emissions</strong> â€” the model putting an op in a turn â€” tracked for every op. Applied/rejected outcome (âœ“/âœ—) is instrumented only for <code>markTeacher</code>; for the rest the number is emission alone. ${turns > 0 ? `A persistent <strong>0 after ${turns} turn${turns === 1 ? "" : "s"}</strong> is the real signature â€” a built op the model never reaches.` : `<strong>No turns observed yet</strong> â€” a 0 here just means this character has not played; it is not a finding.`}</p>
      ${firedOps.length ? `<div class="mach-tally"><span class="mach-label">emitted</span> ${firedOps.map(chip).join(" ")}</div>` : ""}
      <div class="mach-tally"><span class="mach-label">not emitted${turns > 0 ? ` in ${turns} turn${turns === 1 ? "" : "s"}` : " (no turns yet)"} Â· ${neverOps.length}</span> ${neverOps.map(chip).join(" ") || "<span class='hint'>â€” none; every op has been emitted at least once</span>"}</div>
    </div>

    ${caps.length ? caps.map(card).join("") : "<div class='insight'>No model calls captured yet. Take a turn in play, then return â€” every GM turn and its sub-calls (intent-parse, narrate) land here.</div>"}
  ...
  for (const b of app.querySelectorAll("[data-mach-copy]")) b.onclick = async () => {
    ...
    if (!c) return;
    const text = `TASK ${c.task} Â· ${c.model} Â· ${usageLine(c)}\n\n=== ASSEMBLED PROMPT ===\n${promptText(c)}\n\n=== RAW RESPONSE ===\n${c.raw || ""}\n\n=== PARSED ===\n${c.parsed ? JSON.stringify(c.parsed, null, 2) : "(not a parsed turn)"}\n\n=== OPS EMITTED ===\n${(c.opsFired || []).map(o => `${o.op} ${o.shape}`).join("\n") || "(none)"}`;
    try { await navigator.clipboard.writeText(text); b.textContent = "Copied âœ“"; } catch { b.textContent = "Copy failed â€” select the text above"; }
  };
  ...
  // SNG-186 Â§2a/Â§2b levers (only present when a character is loaded)
...
function renderSettings(note = "") {
  ...
  chrome(`<div class="screen">
    ...
    <div class="field"><label>Scene &amp; item art</label>
      <select id="set-art">
        <option value="off" ${artMode === "off" ? "selected" : ""}>Off â€” text only</option>
        <option value="static" ${artMode === "static" ? "selected" : ""}>Static â€” bundled image files only</option>
        <option value="generate" ${artMode === "generate" ? "selected" : ""}>Generate â€” also create missing art (Pollinations, free)</option>
      </select>
    ...
    <div class="field"><label>World pacing â€” how often things happen</label>
      <select id="set-pacing">
        ${[["calm", "Calm â€” the world is mostly quiet"], ["balanced", "Balanced â€” something turns up now and then"], ["eventful", "Eventful â€” the world is busy around you"], ["relentless", "Relentless â€” barely a quiet moment"]].map(([v, label]) => `<option value="${v}" ${(profile.pacing || "eventful") === v ? "selected" : ""}>${esc(label)}</option>`).join("")}
      </select>
      <div class="hint">How often random encounters (a windfall, a stranger, a chase, a fight) surface in play â€” a per-character preference. Danger still skews dangerous places toward trouble and kind places toward grace; this only sets the frequency. A new player or a family member might enjoy <em>Eventful</em>.</div></div>
    <div class="field"><label>Narration â€” plainness</label>
      <select id="set-plainness">
        ${[["plain", "Plain â€” say what's there, first-read words"], ["balanced", "Balanced â€” the place sets the voice"], ["lyrical", "Lyrical â€” reach for image and the felt-unnamed"]].map(([v, label]) => `<option value="${v}" ${(profile.plainness || "balanced") === v ? "selected" : ""}>${esc(label)}</option>`).join("")}
      </select>
      <div class="hint">How much the prose reaches for metaphor and mood vs. stays plain and literal. <em>Plain</em> wins over a place's lyricism â€” grounded prose everywhere you play.</div></div>
    <div class="field"><label>Narration â€” bluntness (within your rating)</label>
      <select id="set-bluntness">
        ${[["restrained", "Restrained â€” spare; the camera may drift"], ["balanced", "Balanced â€” commit as the scene calls"], ["blunt", "Blunt â€” commit fully, to your rating's edge"]].map(([v, label]) => `<option value="${v}" ${(profile.bluntness || "balanced") === v ? "selected" : ""}>${esc(label)}</option>`).join("")}
      </select>
      <div class="hint">How unflinchingly the narration commits to what a scene IS â€” visceral violence, natural profanity, direct embodied description â€” <strong>always within your content rating</strong> (it never raises the ceiling; minor-safety is absolute). <em>Blunt</em> uses the full room your rating gives.</div></div>
    ${/* SNG-155: read-aloud, per-profile (family play â€” each daughter her own voice). The picker is
    ...
    ${ttsAvailable() ? `<div class="field"><label>Read aloud â€” voice</label>
      <select id="set-ttsvoice">
        <option value="">Best available (recommended)</option>
        ${rankVoices(ttsVoices(), { lang: "en" }).slice(0, 24).map(v => `<option value="${esc(v.name)}" ${profile.ttsVoice === v.name ? "selected" : ""}>${esc(v.name)}${v.lang ? ` â€” ${esc(v.lang)}` : ""}</option>`).join("")}
      </select>
      <div class="hint">Listed best-first. Your device's <em>default</em> voice is usually the oldest one installed â€” names carrying <em>Natural</em>, <em>Neural</em>, <em>Premium</em>, <em>Enhanced</em> or <em>Google</em> sound markedly more human. Free, offline, no key.
        ...
        <span class="hint">Shapes the PROSE for the ear â€” one idea per sentence, dialogue attributed, mechanics kept out of the narration. It never changes what happens, and never softens content: your rating and the floors are untouched.</span>
        <button class="opt" id="tts-test" style="margin-top:6px">â–¶ Hear this voice</button></div></div>` : ""}
    <div class="field"><label>Time passage</label>
      <select id="set-time-mode">
        <option value="story" ${ts.mode === "story" ? "selected" : ""}>Story time â€” the clock moves with play</option>
        <option value="real" ${ts.mode === "real" ? "selected" : ""}>Real time â€” the world's clock runs even while you're away</option>
      </select>
      ...
      <div class="hint">Story: +1h per beat, +3h travel, +8h rest. Real: in-game time keeps flowing between sessions â€” the world will feel like it moved without you.</div></div>
    <div class="field"><label>Shared world sync (optional â€” GitHub)</label>
    ...
    <div class="field"><label>Content rating â€” the ceiling for this profile</label>
      <select id="set-rating">
        ${RATING_ORDER.map(r => `<option value="${r}" ${ratingCeiling(profile) === r ? "selected" : ""} ${isMinorProfile(profile) && RATING_LEVEL[r] > RATING_LEVEL["PG-13"] ? "disabled" : ""}>${r}${r === "R+" ? " â€” maximum intensity, all details" : ""}</option>`).join("")}
      </select>
      <label class="rating-check"><input type="checkbox" id="set-minor" ${isMinorProfile(profile) ? "checked" : ""}> This profile is a minor â€” caps at PG-13; can never be set to R or R+</label>
      <label class="rating-check"><input type="checkbox" id="set-adultgate" ${profile.rating?.adultVerified ? "checked" : ""}> Adult gate â€” authorize R / R+ for this profile (required for R and above)</label>
      <div class="hint">Sets how intense narration and generated content get: G Â· PG Â· PG-13 Â· R Â· R+ (full intensity). Two floors are ALWAYS on regardless of ceiling: never any prohibited content, and a minor is never portrayed in romantic or sexual content.</div></div>
    <div class="field"><label>Developer mode</label>
      <label class="rating-check"><input type="checkbox" id="set-dev" ${(() => { try { return localStorage.getItem("singularity.devPersist") === "1"; } catch { return false; } })() ? "checked" : ""}> Show developer tools (the ðŸ§ª Legs panel, test-encounter buttons, the scenario runner)</label>
      <div class="hint">Off by default â€” normal play never shows dev tools. ${isDevMode() ? `<strong>Dev mode is currently ON</strong>${(() => { try { return new URLSearchParams(location.search).get("dev") === "1"; } catch { return false; } })() ? " for this URL (reload without <code>?dev=1</code> for a clean player view)" : /^(localhost|127\\.0\\.0\\.1)/.test(location.hostname) ? " because this is a local dev host" : ""}. ` : ""}Ticking this box is a deliberate, persistent opt-in on this browser; untick + Save to turn it fully off.</div></div>
    <div class="field"><label>World-authorship</label>
      <label class="rating-check"><input type="checkbox" id="set-contentgen" ${profile.contentGenerator ? "checked" : ""}> My play authors the world â€” what I create through play more readily becomes shared canon</label>
  ...
  // SNG-155: hear the picked voice before committing to it â€” the whole point of Â§1 is that this is
  ...
  document.getElementById("set-save").onclick = () => {
    ...
    // SNG-BATCH-9 Â§3: content ceiling. Minor flag first (it can cap the ceiling), then the
    ...
    // (clears it + drops any R/R+ ceiling). Checked, or already-verified, satisfies the gate â€” so
    ...
    if (!rv.ok) { renderSettings("Content rating unchanged â€” " + rv.reason + `. (Still ${ratingCeiling(profile)}.)`); return; }
    renderRoster();
  };
}
...
function renderRoster() {
  ...
  // repo automatically â€” sync config is the only setup a new device should need (no export/import).
  ...
  chrome(`<div class="screen">
    ...
    ${chars.length === 0 ? `<p class="hint" style="margin-bottom:14px">No characters on this device yet. ${syncEnabled() ? "Find the ones you've played elsewhere, or make a new one." : "Make one â€” or set up sync in Settings to bring in characters from another device."}</p>` : ""}
    <div id="roster">${chars.map(c => `
      <div class="roster-item">
        <div><strong>${esc(c.name)}</strong> <span class="hint">${esc(c.origin)} Â· level ${c.level}</span></div>
    ...
    <div style="margin-top:16px; display:flex; gap:8px; flex-wrap:wrap;">
      ...
      ${syncEnabled() ? `<button class="btn secondary" id="discover-chars">â˜ Find my characters</button>` : ""}
      <button class="btn secondary" id="open-library">ðŸ“– The Library</button>
  ...
  // SNG-139: restore character delete â€” confirm-gated, local to this device (shared-world sync copy untouched).
...
// ---------- SNG-087: cross-device DISCOVERY â€” a new device finds your characters from the repo ----------

/** Step 1: list the players in the shared repo (each `players/{key}/profile.json`) so the person can
 *  say which one they are. Sync config is the ONLY setup a new device needs â€” no export/import. */
async function renderDiscover(note = "") {
  if (!syncEnabled()) { renderSettings("Add your GitHub owner, repo, and token in Settings â€” then your characters follow you across devices automatically."); return; }
  chrome(`<div class="screen" style="max-width:640px">
    <h2>Find your characters</h2>
    <p class="hint" style="margin-bottom:12px">Reading the shared world for the characters you've played on other devices. No files to move â€” this is the only setup a new device needs.</p>
    ${note ? `<p class="hint" style="margin-bottom:10px">${esc(note)}</p>` : ""}
    <div id="disc-body"><div class="insight">Looking for players in the shared worldâ€¦</div></div>
  ...
  // fetch each profile for a display name (best-effort â€” a missing profile still shows its key)
  const profiles = [];
 ...
 *  play any of them. Adoption uses the stale-overwrite guard in BOTH directions â€” a newer local copy
...
async function renderDiscoverCharacters(entry) {
  ...
  chrome(`<div class="screen" style="max-width:640px">
    ...
    <p class="hint" style="margin-bottom:12px">From the shared world. Adopt one to play it here â€” its latest save comes with it.</p>
    <div id="disc-chars"><div class="insight">Reading charactersâ€¦</div></div>
    <div style="margin-top:14px; display:flex; gap:8px"><button class="btn secondary" id="dc-back">â† Other players</button><button class="btn secondary" id="dc-roster">This device's roster</button></div>
  </div>`);
  ...
  if (!chars.length) { body.innerHTML = "<div class='insight'>Found character files but couldn't read them â€” check the token has read access.</div>"; return; }
  body.innerHTML = chars.map(c => {
    const here = !!loadCharacter(c.id);
    return `<div class="roster-item">
      <div><strong>${esc(c.name)}</strong> <span class="hint">${esc(c.origin || "")} Â· level ${c.level ?? 1}${c.dead ? " Â· fallen" : ""}${here ? " Â· already here" : ""}</span></div>
  ...
  for (const b of body.querySelectorAll("[data-adopt]")) b.onclick = async () => {
    b.disabled = true; b.textContent = "Loadingâ€¦";
...
function migrate(c) {
  ...
  // on "writing your storyâ€¦" forever. Clear them on load so a stale busy self-heals.
  delete c._chronicleBusy; delete c._chronicleError;
  (c.sessions || []).forEach(s => { if (s) delete s._recapBusy; });
  // CCODE-02: heal a save already bloated by the unbounded scene history â€” Erik's carried 169 turns
  ...
  if (c.activeScene?.turns?.length > SCENE_TURN_CAP) {
    ...
    console.warn(`[migrate] trimmed scene history ${before} â†’ ${SCENE_TURN_CAP} turns (CCODE-02 storage bound)`);
  }
  ...
  { const bf = backfillNpcGender(c); // SNG-143: stamp gender from each record's own narration where unambiguous (the Pell fix); leaves ambiguous unset
    if (bf.length) c._reconcileNotes = [...(c._reconcileNotes || []), `From their own story, gender is now explicit for: ${bf.join(", ")} (correct any in ðŸ”§ Repair if wrong).`]; }
  ...
  mergeCodexTopics(c); // standing high-confidence tidy â€” idempotent; player's not-same verdicts respected; manual merges cascade via their new aliases
  ...
  // SNG-100b: the standing-bar primitives â€” additive, default-empty. A legacy save with none behaves
  ...
  // / domainsAcquired stay unset â†’ absent â‡’ station-derived ceilings, exactly as before.
  if (!c.foreclosed) {
    c.foreclosed = [];
  ...
  // rootkin) + the center's braid discount â€” idempotent on load, so existing keeper saves get it too.
  { const seeded = seedInnateSubstrate(c, originRecord(c.origin), fullCatalog());
    if (seeded.length) c._reconcileNotes = [...(c._reconcileNotes || []), `The substrate answers you by right of your people â€” ${seeded.map(id => fullCatalog()[id]?.name || id).join(", ")} is yours to learn as you grow.`]; }
  ...
  // SNG-022: versioned reconciliation â€” bring the character up to everything now built.
  ...
  // SNG-197 p2: braids backfilled as stubs (before the moment existed) get the moment they never got â€”
...
// SNG-059: display label for a tradition id â€” the people's name from traditions.json, else prettified.
...
/** SNG-055/062: the great-circle SVG â€” 24 traditions on a ring, chosen ones lit, the antipode axis
  ...
  return `<svg viewBox="0 0 520 430" class="great-circle">
    ...
    ${nodes.map(nd => { const role = roleOf(nd.t); const st = (idx.stations || []).find(s => s.traditionId === nd.t);
      return `<g class="gc-node gc-${role}" ${selectable(nd.t) ? `data-dom="${esc(nd.t)}"` : ""}>
        <title>${esc(traditionLabel(nd.t))}${st?.pole ? " â€” " + esc(st.pole) : ""}${closed.has(nd.t) ? " (CLOSED â€” the far pole of an axis you chose)" : ""}</title>
...
function domainVerdict(ability) {
  // SNG-089: Accord crafts are ungated by origin/domain â€” offered (and learnable) to anyone at base cost.
 ...
 *  with domains, access is the domain gate's job (SNG-055) and the level bar is the ability's own â€”
...
/** A stable color for a tradition/class id â€” known power-systems keep their authored color; a
...
// SNG-047/124: an ability's FUNCTIONS as small chips â€” what it DOES at a glance. All 24 verbs carry an
// icon; SNG-124 promotes them to COLORED badges by their 8-family (color = the family, glyph = the verb).
const FN_ICON = {
  strike: "âš”", break: "âœ·", hinder: "â›“", heal: "âœš", mend: "ðŸª¡", restore: "â™º", empower: "ðŸ”†",
  shield: "â›¨", ward: "â¬¡", resist: "ðŸœ›", reveal: "â—‰", foresee: "â—‡", track: "â‡¶", make: "âœ¦", transform: "âŸ³",
  summon: "âˆ", bind: "â›“", command: "â–", deceive: "â—‘", conceal: "â—Œ", move: "âž¤", travel: "âž¤", open: "âŽ”", sustain: "âˆž"
};
// SNG-124: the family a verb belongs to â†’ a CSS class (`.fn-fam-harm`, etc.) coloring the badge.
...
function functionChips(ab) {
  ...
  return `<span class="fn-chips">${fns.map(f => `<span class="fn-chip ${fnFamilyClass(f)}" title="${esc(FN_INDEX?.verbToFamily?.[f] || "")}${FN_INDEX?.verbToFamily?.[f] ? " Â· " : ""}${esc(f)}">${FN_ICON[f] || "â€¢"} ${esc(f)}</span>`).join("")}</span>`;
}

// SNG-124: the ONE gate for "is there anything to spend in the Level Up modal" â€” unspent skill points OR
 ...
 *  MULTI-STEP PLAN â€” the hint should appear only when SEQUENCING actions toward a goal against
 *  obstacles is genuinely apt, not merely when a scene is rich. Requires a real plan signal:
 *  a STRICT plan-intent tag (plan/scout/prepare â€” investigate/analyze dropped; single-action) OR
 *  an explicit staged/multi-obstacle objective the GM framed. The loose abilityChoices>=2 /
 *  nonTrivial>=4 fallbacks are removed â€” rich â‰  plan-apt (they fired on nearly every scene). */
function isGambitApt(turn) {
  // SNG-077: the GM DECLARES aptness (turn.gambitApt === true) â€” emitted only for a genuine
...
/** SNG-019: known-entity idâ†’name maps for codex entity resolution â€” the ids the GM
...
function ratingLineForGM() {
  ...
  // SNG-048: rating is a DIRECTION, not only a cap â€” the affirmative register comes with the ceiling
  // so R+ actually writes the full mature register instead of collapsing intimacy to PG.
  return `## CONTENT CEILING â€” narrate to at most ${preset} across violence/gore, sexual content, language, and dread; and no LESS where the story's grain calls for it (a ${preset} scene should feel fully ${preset}, not softened). ${ratingRegister(preset)}${bluntnessDirective(preset, profile?.bluntness)} ABSOLUTE FLOORS regardless of ceiling: never depict prohibited content; NEVER portray a minor (any child/adolescent) in romantic or sexual content, at any intensity.

## ROMANCE AND INTIMACY â€” HOW YOU ENGAGE
When a player pursues romantic interaction with an NPC, you engage. Do not refuse, do not redirect away from the scene, do not insert safety or consent meta-language into your response. Romance is part of the world. PRECEDENCE: these rules govern WHETHER AND HOW you engage; the CONTENT CEILING above governs the REGISTER â€” engagement never overrides the ceiling, and the ceiling never excuses disengagement. Honor the rating ${preset} precisely: narrating below it withholds what the player chose; narrating above it ignores their setting â€” and narrating BELOW it is the far more common failure, so do not do it. Never treat light physical contact, flirtation, or social touch (a hand, an ear, a face, a shoulder) as a harm trigger â€” that is a social action; harm-rung evaluation is for ability use and for coercive or violent acts, and has nothing to do with a hand on a shoulder.`;
}
 ...
 *  â€” only clear set-pieces qualify; legendSurfacing then governs whether one actually shows. */
...
function ensureCharacterPortrait(c, { force = false, milestone = null, promptOpts = null } = {}) {
  ...
  if (url) { addGalleryImage(c, { kind: "portrait", prompt: c.name, url,
    caption: milestone ? `${c.name} â€” ${milestone}` : `${c.name} â€” ${String(c.origin || "").replace(/[-_]/g, " ")}`,
 ...
 *  dedicated portrait â€” one per NPC per tier, deduped via `n._portraitTier`. Idempotent: runs after each
 *  turn's npc updates AND once on load (the retro backfill â€” Pell, already devoted, gets one). Art-off =
...
function ensureBondPortraits(c) {
  ...
  for (const n of Object.values(c.npcRegistry || {})) {
    ...
    try {
      ...
      if (url) { // no empty tile â€” only record when the mint actually resolved
        n._portraitTier = tier; n.image = url;
        addGalleryImage(c, { kind: "portrait", prompt: npcPromptSeed(n, c), url, caption: `${n.name} â€” ${relationshipLabel(n)}`, worldDay: absoluteWorldDay() });
      }
 ...
 *  from a failed generation) â€” pruned once on load so the gallery never shows an empty card. */
...
/** SNG-046 Layer 3: the persisted image for a location (no minting) â€” an authored/born-with
...
function ensureLocationImage(locId) {
  ...
  if (character.locationImages[locId]) return character.locationImages[locId]; // cached â€” never regen
...
/** On a level-up that crosses a character tier (every 2 levels), re-mint the portrait â€” the
...
function refreshPortraitMilestone(c, prevLevel) {
  ...
  if (c.portraitPinned) return null; // SNG-139: a user-chosen portrait wins â€” auto-regen never overrides a pinned pick
 ...
 *  (travel, hereNow, registry, codex resolution) treats generated content as first-class â€”
...
/** Â§2 engagement: record an implicit attention signal on a generated entity by id (across
 *  all types). Attention keeps a grown thing real + surfacing; inattention lets it go dormant. */
/** SNG-179 Â§4.4: SILENT OP-LOSS BECOMES DETECTABLE.
 *
 *  Three ops had never fired across sixteen levels and nothing anywhere said so â€” it took someone
 ...
 *  never emitted at all, which is exactly the distinction SNG-179 Â§3 says the diagnosis turns on and
 *  which no amount of prompt-reading can settle.
 *
 *  Cheap by construction â€” two counters per op, capped, and it rides the save so it survives reloads
...
/** SNG-186 Â§2f: a compact "what did the model actually emit this turn" â€” every top-level key of the
...
function opsFiredIn(turn) {
  ...
  for (const [k, v] of Object.entries(turn)) {
    ...
    else if (typeof v === "object") { const ks = Object.keys(v); if (!ks.length) continue; shape = `{${ks.slice(0, 4).join(",")}${ks.length > 4 ? ",â€¦" : ""}}`; }
...
/** SNG-185: affiliate a met NPC â€” its people + domains, from the same helper generate.js uses, with
...
/** SNG-166 Â§3: every save on this device, for the cross-character name guard. A per-character check
 *  reads GREEN forever â€” within any one save there is exactly one Mara; she is in FOUR of them. */
...
/** The naming grain of the people whose country this is â€” traditions.json already carries an
...
function noteGeneratedAttention(id, kind, day) {
  ...
  for (const type of ["npc", "location", "arc"]) {
    ...
    if (rec) {
      ...
      // record became owed more than it carries, so deepen it â€” lazily, once, and never blocking.
 ...
 *  enrichPersonalArc â€” a person who has become important should read as one, but a failed call must
...
${JSON.stringify({ name: rec.name, role: rec.role, homeLocation: rec.homeLocation, people: rec.people, domains: rec.domains, wants: rec.wants, fears: rec.fears, knowledge: rec.knowledge }, null, 1)}`,
      ...
      tier === "nominated"
        ? `They are now DURABLE CANON â€” this person has their own life, their own reach, and reasons of their own. Author accordingly.`
        : `They are ESTABLISHED â€” enough that they can be met again and be recognisably the same person.`,
    ...
    let filled = 0;
    for (const f of owed) {                              // ADDITIVE ONLY â€” never overwrite what play established
      const v = raw[f];
 ...
 *  place when we can tell â€” the generator matches their quality + voice, staying in-grain. */
 ...
 *  and return light notes to surface. Never throws â€” generation must not halt a turn. */
async function handleGenerateRequests(turn) {
  ...
  for (const req of reqs.slice(0, 3)) {
    ...
    const ctx = {
      ...
      rating: ratingCeilingNow(),                              // Â§3 consumer (b+c): generate to the ceiling + tag the entity
      ...
      // SNG-166 Â§3: he keeps meeting Mara. Across 10 characters on this device, 5 given names
      // recur and Mara appears in FOUR saves â€” invisible to any per-character check.
      avoidNames: namesToAvoid(allCharactersOnDevice(), 24),
      namingAesthetic: namingAestheticHere(),
      validRegions: new Set(Object.keys(CONTENT.substrateModel?.substrateDensity || {}).concat((CONTENT.regions || []).map(r => r.regionId || r.id).filter(Boolean)))   // SNG-166 Â§1
    };
    // SNG-035/046-L3: born-WITH-image is now IN the generate path (deps.imageFor) so the record
    // arrives with its picture regardless of caller â€” the app just injects the art builder.
    ...
    // SNG-190 Â§2: if this npc request is fleshing out someone MET the same turn, it is ONE person â€”
    ...
    if (rec && rec._gen && !before.has(rec.id)) {              // a NEW mint (not a reuse of authored/existing)
      ...
      if (type === "location") {
        ...
        // that containment â€” and anchor its coords to the PARENT rather than to wherever the
...
function listAvailableEncounters() {
  ...
  return (loc.encounterSeeds || []).map(seed => {
    ...
    return def ? `- id "${def.id}" (${def.type}): ${def.name} â€” ${seed.hint}. Setup: ${def.setup}` : null;
  }).filter(Boolean).join("\n") || null;
}
...
function syncErrorMessage(prefix) {
  ...
  if (/403/.test(e)) return `${prefix}: your GitHub token can't WRITE to this repo. In GitHub â†’ token settings, give it Contents: Read and write on the ${getSyncConfig().repo || "Singularity"} repo (classic tokens need the "repo" scope), then re-paste it in Settings.`;
  if (/404/.test(e)) return `${prefix}: repo or path not found â€” check the owner and repo names in Settings.`;
  if (/401/.test(e)) return `${prefix}: your GitHub token was rejected (expired or mistyped) â€” paste a fresh one in Settings.`;
  if (/not configured/.test(e)) return `${prefix}: shared-world sync isn't set up â€” add your GitHub owner/repo/token in Settings first.`;
  return `${prefix}${e ? " (" + e + ")" : ""}.`;
}
...
function enterPartyScene(scene) {
  ...
  renderPlay(character.activeScene?.lastTurn || null, { aside: `Shared scene: ${scene.party.map(m => m.name).join(", ")}. Turns rotate â€” act when it's yours.` });
}
...
async function leavePartyScene() {
  ...
  renderPlay(character.activeScene?.lastTurn || null, { aside: "You part ways with the party â€” solo play resumes." });
}
...
async function pollPartyScene() {
  ...
  // an empty poll (no new beats) does NOTHING to the DOM â€” your text + scene stay put.
}
...
function showPartyBanner() {
  ...
  el.innerHTML = `<span class="pb-dot"></span><span class="pb-text">${esc(names)} acted â€” ${pendingPartyBeats.length} update${pendingPartyBeats.length > 1 ? "s" : ""} ready${yourTurn ? " Â· your turn" : ""}</span><button class="pb-btn" id="pb-catchup">Catch up</button><button class="pb-x" id="pb-dismiss" title="Dismiss">Ã—</button>`;
...
/** Fold the pending party beats into the view â€” the ONE place a party update re-renders,
...
function catchUpParty() {
  ...
  renderPlay(character.activeScene?.lastTurn || null, {
    aside: beats.map(b => `${b.name}: ${b.label}${b.degree ? ` (${b.degree.replace("_", " ")})` : ""} â€” ${b.summary}`).join("; ")
  });
...
async function maybeTick() {
  ...
  try { await runGenerationTurn({ character, content: CONTENT }); } catch (e) { console.warn("[generation] turn skipped:", e?.message); } // SNG-191 Â§7: the world's own agenda foments on the world count
 ...
 *  clash). The records are already lens-resolved (shown/adapted) â€” a filtered one never arrives. */
...
function surfacePromotions(promoted) {
  ...
  const items = landed.map(p => {
    ...
    return { day: readClock(character.clock).day, worldDay: wd,
      text: `${name} has become part of the wider world â€” other travelers may now meet what you grew.` };
  });
  character.worldState.unseenNews = [...(character.worldState.unseenNews || []), ...items].slice(-20);
}

/** SNG-BATCH-9 Phase 3: the shared-canon block for the GM â€” canonical figures/places/threads that
...
/** SNG-112: the context that decides which authored quests are STARTABLE here â€” the player's exact
 *  location + its adjacent locations (proximity), scene NPCs (giver present), and a giverâ†’home map so
...
// SNG-069A: a categorized background <select> â€” browsable by category, used by BOTH creation doors.
...
// and creation defaults to it â€” but a player may always also start in the Valley (a character who
...
/** The three offered starts (deduped): the origin's homeland (default) Â· the Valley Â· The Crossing.
...
function startingLocationChoices(originId) {
  ...
  add(home, (CONTENT.locations?.[home]?.name || null), "your people's homeland â€” where you are from");
  add(VALLEY_START, "The Valley", "a character who already left home; the Valley is where you landed");
  add(CROSSING_START, "The Crossing", "the center â€” where nobody is from and everybody is");
  return out;
}
...
function renderCreate() {
  ...
  /** SNG-063 quick-start step 1: NAME â†’ FORM â†’ ORIGIN â†’ BACKGROUND â†’ attributes.
   *  NO abilities here â€” abilities come AFTER domains (the order fix), filtered by them. */
  function draw() {
    ...
    chrome(`<div class="screen">
      ...
      <div class="field"><label>Form / appearance <span class="hint" style="text-transform:none">(leads the portrait â€” blank = an ordinary person)</span></label>
      ...
      <div class="field"><label>Origin â€” which people are you from?</label>
        <select id="c-origin">${origins.map(o => `<option value="${esc(o.id)}" ${state.origin === o.id ? "selected" : ""}>${esc(o.name)}${o.homeRegion && o.homeRegion !== "valley" ? " â€” of the " + esc(String(o.homeRegion).replace(/_/g, " ")) : ""}</option>`).join("")}</select>
      ...
      <div class="field"><label>Background â€” what did you do? <span class="hint" style="text-transform:none">(browsable by category; never gated by your people)</span></label>
        <select id="c-bg">${backgroundOptionsHTML(state.background)}</select>
        <div class="hint">${esc(bg?.description || "")}</div></div>
      <div class="field"><label>Attributes â€” points left: <strong>${left}</strong> (each 1â€“4)</label>
        ${Object.entries(state.attrs).map(([k, v]) => `
          <div class="point-row">
            <button data-dec="${k}">âˆ’</button><span class="pips">${"â—".repeat(v)}${"â—‹".repeat(4 - v)}</span><button data-inc="${k}">+</button>
  ...
  function renderAbilityStep() {
    ...
    // SNG-192 Â§1: the by-right starter kit is computed HERE, not silently at commit, so a pick can never be
    ...
    const abTitle = (a) => { const r1 = a.tree?.find(x => x.rank === 1); return esc((r1 ? "Rank 1 â€œ" + r1.name + "â€ â€” CAN: " + r1.grants + " | CANNOT: " + r1.cannot : a.description) + (a.notFor ? " | NOT FOR: " + a.notFor : "")); };
    ...
    // SNG-192 Â§4: the archetype LENS (optional front door) â€” pick a shape (Magus, Shadow, Seerâ€¦) and the
    ...
    // SNG-192 Â§3: suggestions with a REASON drawn from the shape chosen (Â§4), what the player DID (prologue) or WROTE (bio).
    ...
    // SNG-192 Phase B â€” the robustness readout: what this build can and cannot DO (Â§5 coverage), where the
    // WHOLE kit works (Â§6b common ground), and the honest coherenceâ†”divergence framing (Â§6c). Never blocks.
    const readoutHtml = (() => {
      ...
      const covLine = buildAb.length
        ? `<div>You can ${cov.covered.length ? cov.covered.map(f => `${FAMILY_GLYPH[f] || "Â·"} ${f.toLowerCase()}`).join(" Â· ") : "â€” nothing yet"}.${cov.missing.length ? ` <span class="hint">No way to ${cov.missing.map(f => f.toLowerCase()).join(", ")} â€” a real choice, not a mistake.</span>` : " <span class='hint'>a balanced kit.</span>"}</div>`
        : "";
      let groundLine = "";
      if (cg.empty) {
        const hiT = cg.per.slice().sort((a, b) => b.lo - a.lo)[0], loT = cg.per.slice().sort((a, b) => a.hi - b.hi)[0];
        groundLine = `<div style="color:var(--warn,#e0b25a)">âš  ${esc(traditionLabel(hiT.tradition))} and ${esc(traditionLabel(loT.tradition))} share no ground â€” wherever you stand, one of them is starved. A deliberate reach for two very different lands, never an accident.</div>`;
      } else if (cg.window) {
        groundLine = `<div>Your crafts all work in <strong>${esc(groundAsPlace(cg.window))}</strong> â€” where this character belongs.</div>`;
      }
      const width = cg.window ? cg.window[1] - cg.window[0] : 0;
      const frameLine = (cg.empty || width < 0.22)
        ? `<div class="hint" style="margin-top:2px">A <strong>divergent</strong> build â€” weaker in any one country, and exactly where new craft comes from. Off-source picks are seeds; the game will offer braids you never planned. <em>Divergence makes you new.</em></div>`
        : (trads.length > 1 ? `<div class="hint" style="margin-top:2px">A <strong>coherent</strong> build â€” strong in its own country. <em>Coherence makes you strong here; divergence makes you new.</em></div>` : "");
    ...
    chrome(`<div class="screen">
      ...
      <p class="hint" style="margin-bottom:10px">Choose <strong>${maxAbilities()}</strong>. The far pole of what you are isn't here â€” only your primary, its kin, your secondary and tertiary, and the Valley's open folk arts.</p>
      ${grantIds.length ? `<div class="sys-group"><div class="sys-label">Yours by right of being ${esc(traditionLabel(state.domains?.primary || state.nativeTradition || state.origin))}</div>
        <p class="hint" style="margin:0 0 6px">Already yours â€” you don't spend a pick on these.</p>
        <div class="opt-row">${grantIds.map(id => { const g = fullCatalog()[id]; return g ? `<span class="opt" style="opacity:.6;cursor:default;border-style:dashed" title="${abTitle(g)}">âœ“ ${esc(g.name)}</span>` : ""; }).join("")}</div></div>` : ""}
      ${archetypes.length ? `<div class="sys-group"><div class="sys-label">A shape to start from â€” optional, a lens not a class</div>
        ...
        ${selectedArch ? `<p class="hint" style="margin-top:6px">${esc(selectedArch.whatItIsHere)} <em>${esc(selectedArch.signature)}</em> The suggestions below lean toward this shape now â€” change any, or click ${esc(selectedArch.archetype)} again to clear.</p>` : `<p class="hint" style="margin-top:4px">Pick a shape â€” Magus, Shadow, Seer, Wardenâ€¦ â€” to see a build lean your way, or ignore it and choose freely.</p>`}</div>` : ""}
      ${suggestions.length ? `<div class="sys-group"><div class="sys-label">Suggested for you</div>
        <div class="opt-row" style="flex-direction:column;align-items:stretch;gap:6px">${suggestions.map(s => { const a = abById[s.abilityId]; return a ? abBtn(a, "", ` <span class="hint" style="opacity:.75">â€” ${esc(s.why)}</span>`) : ""; }).join("")}</div></div>` : ""}
      <details ${suggestions.length ? "" : "open"} style="margin-top:4px">
        ...
        ${Object.keys(byTrad).sort((a, b) => traditionLabel(a).localeCompare(traditionLabel(b))).map(t => `
          ...
          <div class="opt-row">${byTrad[t].map(a => abBtn(a)).join("")}</div></div>`).join("") || "<div class='insight'>No level-1 abilities available for your domains â€” you'll learn as you play.</div>"}
      </details>
      ${readoutHtml}
      <div style="display:flex; gap:8px; margin-top:12px">
    ...
    // SNG-192 Â§4: the archetype lens toggles â€” it biases the suggestions, never touches the picks (never locks).
  ...
  // neighbours, and antipodes are read from CONTENT.traditionIndex â€” never hardcoded.
  function renderDomainStep() {
    const idx = CONTENT.traditionIndex;
    if (!idx) { renderAbilityStep(); return; } // no traditions content â†’ skip gracefully
    const order = ringOrder(idx);
    const d = state.domains;
    // SNG-063: origin SEEDS the circle â€” a pole-people's native tradition pre-fills the primary
    ...
    // FREE â€” anywhere legal, no longer bound to the secondary's neighbours (Erik rulings 1 + 4).
    ...
    const svg = domainCircleSVG(idx, { primary: d.primary, secondary: d.secondary, tertiary: d.tertiary, closed, selectable,
      ...
      centerSub: phase === "secondary" ? "kin to your primary" : phase === "tertiary" ? "free â€” your wildcard reach" : phase === "done" ? "" : "the opposite pole closes" });
    const slot = (label, t) => `<div class="dom-slot"><span class="dom-slot-label">${label}</span> ${t ? `<strong>${esc(traditionLabel(t))}</strong>` : "<em>â€”</em>"}</div>`;
    chrome(`<div class="screen" style="max-width:640px">
      <h2>Your place on the Great Circle</h2>
      <p class="hint" style="margin-bottom:8px">Twelve axes, twenty-four peoples â€” a ring where every craft sits opposite its antithesis. Your <strong>primary</strong> is who you are (all you can master); your <strong>secondary</strong> (<em>kin to your primary</em>) reaches tier III â€” a concentrated core; your <strong>tertiary</strong> is a <em>free wildcard</em>, reaching anywhere on the ring to tier II. The <strong>opposite pole</strong> of your primary and secondary is closed to you forever â€” only the great braids cross it.</p>
      ${svg}
      ...
      <div style="display:flex; gap:8px; margin-top:12px">
        <button class="btn secondary" id="dom-reset">â†º Redo</button>
        <button class="btn" id="dom-done" ${phase === "done" ? "" : "disabled"}>${state.domainReturn === "prologue" ? "Confirm â€” this is who I am" : "Next: your companion"}</button>
      </div>
    </div>`);
    ...
    // SNG-062: prologue-adjust confirms â†’ finish; SNG-063: quick-start goes to abilities (AFTER domains)
  ...
  function renderCompanionStep() {
    ...
    chrome(`<div class="screen" style="max-width:640px">
      ...
      <p class="hint" style="margin-bottom:10px">A companion is yours from the first step. Pick who â€” and call them what you like.</p>
      <div class="companion-pick">
        ${roster.map(c => `<button class="companion-card ${c.id === state.companionId ? "selected" : ""}" data-comp="${esc(c.id)}">
          <strong>${esc(c.name)}</strong>
          <span class="companion-role">${esc((c.role || "").split("â€”").slice(1).join("â€”").trim() || c.role || "")}</span>
        </button>`).join("")}
      </div>
  ...
  function renderFormStep() {
    chrome(`<div class="screen" style="max-width:640px">
      ...
      <p class="hint" style="margin-bottom:10px">Describe their physical form â€” species, build, features. This LEADS the portrait, so a non-human (an Ent, a construct) renders true from the start. Leave it blank for an ordinary person.</p>
  ...
  function renderBioStep(bio = { hometown: "", residence: "", livelihood: "", hobbies: "", motivation: "", story: "" }) {
    const FIELDS = [
      ...
      ["motivation", "Why step out of ordinary life?", "What makes them someone things HAPPEN to â€” a debt, a loss, a question, an itch"]
    ];
    chrome(`<div class="screen">
      ...
      <div style="display:flex; gap:8px;">
        <button class="btn secondary" id="bio-weave">âœ¦ Weave it for me</button>
    ...
    document.getElementById("bio-weave").onclick = async () => {
      ...
      status.textContent = "The valley considers who you might beâ€¦";
      try {
        ...
        status.textContent = "The weave slipped (" + err.message.slice(0, 60) + ") â€” write it by hand or try again.";
      }
    };
  ...
  function finish(bio = null) {
    ...
    character = {
      ...
      origin: state.origin,
      // SNG-063: origin = which PEOPLE you're from â†’ native tradition (home class) + why you're here
      ...
      activeScene: null,
      clock: newClock(),
      // SNG-057: the chosen companion (string id â€” recruitment/backfill shape) + the player's name
      ...
      // shifts that craft's best-ground â€” the whole point of Â§3.3.
    ...
    // SNG-063: nobody is in the Valley by accident â€” fold the origin's whyYouAreHere into the bio
    ...
    character.grantsVersion = 1; // born after banked growth â€” no retro grant owed
    ...
    seedStandingAtCreation(character, { traditionIndex: CONTENT.traditionIndex, rules: CONTENT.rules, day: 1 }); // BATCH-12 Â§3b: being Rootkin-born should mean the Rootkin have heard of you
    character.nativeGrantsVersion = 1; // born with the starter kit â€” no retro native-grant owed
    character.reconcileVersion = topReconcileVersion("character"); // born current â€” no migration owed (no aggregate seed)
    character.pendingSubPoints = 2; // shape your edge from day one â€” specialize two subs
    ...
    character._creationAside = [character._creationAside, `âœ¦ Your story has become a thread here: *${character.personalArc.name}*.`].filter(Boolean).join(" ");
    saveCharacter(character); saveProfile(profile);
    enrichPersonalArc(character); // best-effort, non-blocking (no-op without an API key)
    enterPlay();
  }

  // ---------- SNG-062: THE PROLOGUE â€” creation as a played scene ----------
  ...
  function renderCreateDoor() {
    if (!PRO()?.openings?.length) { draw(); return; } // no prologue content â†’ straight to the form
    chrome(`<div class="screen" style="max-width:620px">
      ...
      <div class="create-door">
        <button class="door-card" id="door-describe">
          <strong>âœŽ Describe yourself</strong>
          <span>Say who you want to be, in your own words â€” "a girl who talks to animals and cannot lie." The game shows you where that lands on the great circle, why, and what it costs. Adjust anything. The gentlest door.</span>
        </button>
        <button class="door-card" id="door-play">
          <strong>â–¶ Play the opening</strong>
        ...
        <button class="door-card" id="door-form">
          <strong>âš¡ Quick start</strong>
          <span>Build it yourself â€” name, form, domains on the circle, starting abilities, companion. The express lane for when you already know who you are. Same character either way.</span>
        </button>
      </div>
    </div>`);
  ...
  // ---------- SNG-086: THE THIRD DOOR â€” "describe yourself" â†’ a placement on the great circle ----------
   ...
   *  tertiary is FREE â€” barred only from the primary/secondary and the two closed poles, no longer bound
   *  to the secondary's neighbours (Erik rulings 1 + 4). Design Law 1 â€” the model never owns ring geometry. */
  function sanitizeSuggestedDomains(sug, idx) {
    ...
    // nothing when the model was close â€” keeps the third-door build complete and legal.
  ...
  function renderDescribeDoor(err = "") {
    const idx = CONTENT.traditionIndex;
    if (!idx) { draw(); return; } // no ring content â†’ fall back to the quick-start form
    chrome(`<div class="screen" style="max-width:620px">
      <h2>Describe yourself</h2>
      <p class="hint" style="margin-bottom:12px">Say who you want to be â€” species or look, temperament, what you're good at, what you care about. Whole sentences or a scatter of words, either works. The game reads it and shows you where you land on the great circle, and what it costs. You can change everything after.</p>
      ...
      <div style="display:flex; gap:8px; margin-top:10px">
        ...
        <button class="btn" id="d-go">âœ¦ Show me where I land</button>
      </div>
    ...
    document.getElementById("d-go").onclick = async () => {
      ...
      status.textContent = "Reading the circle for who you areâ€¦";
      ...
      const ring = ringOrder(idx).map(t => { const tr = idx.byId[t];
        return `${t} Â· ${tr.name} Â· ${tr.pole || "?"} (${tr.axis || "?"}) Â· ${tr.craft || "?"} Â· antipode:${antipodeOf(t, idx) || "?"} Â· neighbours:${neighborsOf(t, idx).join(",")}`; }).join("\n");
      const folk = (CONTENT.traditions?.folkTraditions || []).map(f => `${f.name || f.traditionId} â€” ${f.aesthetic || "a valley folk-craft, open to all"}`).join("\n") || "(none)";
      const bgs = (CONTENT.backgrounds?.length ? CONTENT.backgrounds : backgroundsFallback()).map(b => `${b.id} Â· ${b.name}`).join("\n");
      const comps = Object.values(CONTENT.companions || {}).filter(c => c.startingOption).map(c => `${c.id} Â· ${c.name} Â· ${(c.role || "").slice(0, 60)}`).join("\n") || "(none)";
      try {
  ...
  function renderDescribeReveal() {
    ...
    // one slot card: people Â· why Â· what it CLOSES (engine-computed antipode) Â· the model's cost prose
    const slotCard = (label, slot, modelSlot) => {
      if (!slot) return "";
      const anti = antipodeOf(slot, idx);
      return `<div class="cs-block" style="margin:6px 0">
        <div><span class="dom-slot-label">${label}</span> â€” <strong>${esc(traditionLabel(slot))}</strong></div>
        ${modelSlot?.why ? `<div class="hint" style="margin-top:4px">${esc(modelSlot.why)}</div>` : ""}
        <div class="hint" style="margin-top:4px">Closes <strong>${esc(traditionLabel(anti))}</strong>${modelSlot?.cost ? ` â€” ${esc(modelSlot.cost)}` : "."}</div>
      </div>`;
    };
    ...
    chrome(`<div class="screen" style="max-width:660px">
      ...
      <p class="hint" style="margin-bottom:8px">This is a suggestion, not a sentence â€” read the reasons and the costs, then keep it or adjust anything.</p>
      ${circle}
      ${d.primary ? slotCard("Primary", d.primary, sug.primary) : `<div class="cs-block"><div class="hint">The reading couldn't place a primary from that â€” try more detail, or pick on the circle.</div></div>`}
      ${slotCard("Secondary", d.secondary, sug.secondary)}
      ${slotCard("Tertiary", d.tertiary, sug.tertiary)}
      ${antiP ? `<div class="hint" style="margin:8px 0"><strong>â›” ${esc(traditionLabel(antiP))} is closed to you forever</strong> â€” the far pole of what you are. Only a cross-pole braid, taught by no one, ever crosses it.</div>` : ""}
      <div class="cs-block" style="margin:6px 0">
        ${sug.origin?.why ? `<div><span class="dom-slot-label">Born</span> â€” <strong>${esc((origins.find(o => o.id === state.origin) || {}).name || state.origin)}</strong> <span class="hint">${esc(sug.origin.why)}</span></div>` : ""}
        ${sug.background?.why ? `<div style="margin-top:4px"><span class="dom-slot-label">Background</span> â€” <strong>${esc(backgroundById(state.background)?.name || state.background)}</strong> <span class="hint">${esc(sug.background.why)}</span></div>` : ""}
        ${state.form ? `<div style="margin-top:4px"><span class="dom-slot-label">Form</span> <span class="hint">${esc(state.form)}</span></div>` : ""}
        ${comp ? `<div style="margin-top:4px"><span class="dom-slot-label">Companion</span> â€” <strong>${esc(comp.name)}</strong> ${sug.companion?.why ? `<span class="hint">${esc(sug.companion.why)}</span>` : ""}</div>` : ""}
      </div>
      ${folk.length ? `<div class="hint" style="margin:6px 0">${folk.map(f => `<div>âœ“ <strong>${esc(f.name)}</strong> â€” ${esc(f.why || "free; a folk craft, open to all")}</div>`).join("")}</div>` : ""}
      ${notDom.length ? `<div class="hint" style="margin:6px 0">${notDom.map(n => `<div>Â· <strong>${esc(n.label)}</strong> isn't a domain â€” ${esc(n.why || "it's how you play, not a pole")}</div>`).join("")}</div>` : ""}
      <div style="display:flex; gap:8px; margin-top:12px; flex-wrap:wrap">
        <button class="btn secondary" id="dr-rewrite">â† Rewrite</button>
        <button class="btn secondary" id="dr-adjust">Adjust on the circle</button>
        <button class="btn" id="dr-confirm" ${d.primary ? "" : "disabled"}>Yes â€” this is who I am</button>
      </div>
    </div>`);
  ...
  /** Prologue step 1 (SNG-063 order): name â†’ form â†’ origin (light seed â€” your people). Domains and
  ...
  function renderPrologueIntro() {
    ...
    chrome(`<div class="screen" style="max-width:600px">
      ...
      <div class="field"><label>What do they look like? <span class="hint" style="text-transform:none">(form/species â€” leads the portrait; blank = an ordinary person)</span></label>
      ...
      <div class="field"><label>Which people are you from? <span class="hint" style="text-transform:none">(origin is who your people are)</span></label>
        <select id="p-origin">${origins.map(o => `<option value="${esc(o.id)}" ${state.origin === o.id ? "selected" : ""}>${esc(o.name)}${o.homeRegion && o.homeRegion !== "valley" ? " â€” of the " + esc(String(o.homeRegion).replace(/_/g, " ")) : ""}</option>`).join("")}</select>
  ...
  function renderPrologueOpening() {
    ...
    chrome(`<div class="screen" style="max-width:640px">
      ...
      <p class="hint" style="margin-bottom:12px">Each opening is a different kind of trouble. There is no wrong one â€” every road makes a whole person.</p>
  ...
  /** Prologue step 3: play the current problem â€” pick a path, see the outcome, accrue. */
  ...
  /** Prologue step 4: the companion arrives IN the scene â€” choose + name who stays. */
  ...
  /** Prologue step 5: domains CRYSTALLIZE from how they played â€” shown on the circle, with the
  ...
  function renderPrologueReveal(adjustPhase = null) {
    ...
    const nm = t => t ? traditionLabel(t) : "â€”";
    ...
    chrome(`<div class="screen" style="max-width:640px">
      ...
      <div class="prologue-reasons"><p class="hint">Because, in the scene:</p>
        ${reasons.map(r => `<div class="prologue-reason">â€” ${esc(r.label)}</div>`).join("")}</div>
      <p class="hint" style="margin-top:8px">The far pole of what you are is closed to you â€” only the great braids cross it. Is that who you are? You may adjust.</p>
      <div class="field" style="margin-top:12px"><label>And what did you DO before this? <span class="hint" style="text-transform:none">(your background â€” you choose; the scene doesn't decide it for you)</span></label>
      ...
      <div style="display:flex; gap:8px; margin-top:10px">
        ...
        <button class="btn" id="reveal-confirm">Yes â€” this is who I am</button>
      </div>
    </div>`);
   ...
   *  CONFIRMED domains (not the ones the game guessed). The prologue-earned abilities are kept â€”
   *  "you did this, so you know this" â€” but any that fall outside the confirmed domains are
  ...
  function finishPrologue() {
    ...
    const o = opening();
    let story = o ? `Their first day in the world was ${o.name.toLowerCase()} â€” ${state.prologue.reasons.map(r => r.label.toLowerCase().replace(/\.$/, "")).slice(0, 2).join("; ")}.` : "";
    if (grandfathered.length) story += ` They did things in that first day their people would not have taught them â€” ${grandfathered.map(id => CONTENT.abilities[id].name).join(", ")} stay with them, earned outside their craft.`;
...
async function enterPlay() {
  ...
  ensureBondPortraits(character);    // SNG-136: retro backfill â€” an already-devoted bond (Pell) gets its portrait once
  ...
  // SNG-022: the login moment â€” if reconciliation applied anything player-facing, say so
  if (character._reconcileNotes?.length) {
    const growth = "The world has grown:\nâ€¢ " + character._reconcileNotes.join("\nâ€¢ ");
    backfillAside = backfillAside ? backfillAside + "\n\n" + growth : growth;
    delete character._reconcileNotes;
    saveCharacter(character);
  }
  // SNG-068A: the creation reconcile note (abilities matched to your CONFIRMED people) â€” say it once.
...
async function startScene(prompt = "(Scene opening â€” set the scene where the character currently is and present the situation.)", news = [], aside = null) {
  ...
  if (news.length) prompt += ` (Since the character last played, the world moved: ${news.map(n => n.text).join(" / ")} â€” let the scene reflect what applies here.)`;
  renderPlay(null, { thinking: "The valley takes shapeâ€¦", newsFlash: news, aside });
...
/** ability-arch v2: the deepen affordance is GONE â€” depth is earned, not bought. This returns the
 *  progress line shown wherever a rank-up button used to be. Rank 1 â†’ shows uses toward rank 2 (which
 *  lands automatically); rank 2 â†’ mastery awaits a defining moment (GM); rank 3 â†’ mastered. */
function rankProgress(character, abId) {
  ...
  if (owned.level >= maxRank) return { text: "âœ“ mastered â€” rank 3", ripe: false };
  const uses = character.practice?.uses?.[abId] || 0;
  if (owned.level === 1) {
    const need = rules.practice?.useRankThreshold?.["2"] ?? 8;
    return { text: `rank 1 Â· practiced ${Math.min(uses, need)}/${need} â†’ rank 2 lands through use`, ripe: false, uses, need };
  }
  // rank 2 â†’ mastery is a defining moment (GM), gated on practice + rank3Min
  ...
  const text = ripe ? "rank 2 Â· ripe for a defining moment (the GM marks mastery when a beat earns it)"
    : !practiced ? `rank 2 Â· practiced ${Math.min(uses, need)}/${need} toward mastery`
    : `rank 2 Â· needs ${gateFor(abId, CONTENT.attributeGates)?.subAttribute || "attribute"} ${gateFor(abId, CONTENT.attributeGates)?.rank3Min || ""} for mastery`;
 ...
 *  and the level bar â€” i.e. a GM `markDefiningMoment` would land. Named for the GM's RIPE FOR MASTERY
...
function masteryReadyForGM() {
  ...
  for (const owned of character.abilities || []) {
    ...
    if (!practiceRankReady(character, owned.abilityId, rules)) continue;      // uses â‰¥ rank-3 threshold
...
/** BATCH-11 Â§23: the env bag every registry row builds from. One place, so
 *  "what can a GM-context builder see" is as enumerable as the registry itself.
 *  FN_INDEX is exposed as a getter â€” it is a mutable `let` rebuilt at load. */
...
async function runGM({ resolution, playerInput, exactWords, itemAdvance }) {
  ...
  const encounterWeaveDetail = weave ? (
    `An encounter is occurring â€” weave it INTO the scene now, as part of what is happening (no aside, no genre-break): ${weave.seed}` +
    (weave.perilous ? ` This could turn dangerous â€” you MUST present a way to decline, flee, or avoid it as a clear choice BEFORE any engagement. Never narrate the character as already fighting or already harmed.` : ` Let it be a small, real thing on the road${weave.flavor && /benef|benign|beaut/.test(weave.flavor) ? " â€” a grace, not a threat" : ""}.`) +
    (weave.loreTier === "precursor-glimpse" ? ` This touches the Precursor â€” glimpsed, never explained.` : ``)
  ) : null;
  ...
  // SNG-194 Â§4b: the ENGINE decides whether an unprompted OFFER has room this beat â€” the model never
  ...
  // the turn prompt) â€” wants, stirrings and the place are already above; the instruction lives in gm.js.
  const offerDetail = (() => {
    ...
    const parts = [];
    if (reactions.length) parts.push("How the people here READ this character (their OWN reaction to who the player is â€” a self-writing beat whose attribution IS the person; pick the reading that fits this character):\n" + reactions.map(r => `- ${r.name}: ${r.reactions}`).join("\n"));
    if (fears.length) parts.push("What the people here FEAR (a sympathetic reason for someone to act â€” not an attack):\n" + fears.map(f => `- ${f.name}: ${f.fear}`).join("\n"));
    return parts.length ? parts.join("\n\n") : "No specific person-material here â€” draw the offer from what is stirring in the world, what these people want, or what this place is.";
  })();
  // SNG-195 G2: the oldest live-play complaint â€” teachers that teach nothing. The teacher block used to
  // say "OFFER it when the moment fits" â€” a permission the model rarely acted on. Now the ENGINE decides a
  ...
  // block flips from permission to an unconditional instruction â€” the model never judges the moment.
  const teacherOfferDetail = (() => {
    ...
    if (!room) return null;
    return `${ready.name} is here and would open your next step: **${ready.nextStep}** (tier ${ready.tier}, ${ready.traditionName}${ready.pathIsTheirs ? " â€” their own ordering" : ""}).${ready.braids && ready.braids.length ? ` Braids it opens: ${ready.braids.join(", ")}.` : ""}`;
  })();
  // Romance: on a flirtatious/romantic intent this turn, pull the craft-guidance doc so the GM narrates
  // the beat well at the player's rating. Rides the intent tags parseIntent already emits â€” no extra call.
  ...
  if (romanceGuidanceDetail) autoVerifyLeg("romance-flirt", "a flirtatious action was tagged romantic and the romance guidance loaded in play â€” the one live-model surface"); // ROMANCE leg auto-detect (dev-only)
  // (masteryDetail / anomalyDetail / toolkitDetail now build inside the Â§23 registry rows.)
  // SNG-122: if the player's action this turn is a travel intent, force the GM to emit moveTo to the named
  // destination (+ enumerate reachable known places so its target resolves). Fixes the CAUSE â€” the deferred
  ...
  // BATCH-11 Â§23: the context is assembled by ITERATING the registry â€” the hand-listed
  ...
  // SNG-100b: accrue region presence â€” a light per-turn accumulator of time spent among a people, so the
  ...
  // SNG-081: the player's OWN words are kept in scene history â€” their literal input wins, else the
  ...
  // CCODE-07 (Design Law 5): the model already answered â€” the player has WAITED for this beat and
  // paid for it. Bookkeeping must never be able to swallow it. Before this guard, ANY throw inside
  // applyTurn propagated out of runGM, the caller's `renderPlay(result.turn, â€¦)` never ran, and the
  // narration vanished â€” while `character.activeScene.lastTurn` had ALREADY been persisted mid-way
  ...
  try {
    ...
    console.error("[applyTurn] state application failed â€” narration preserved:", err);
    ...
    // turn that state lagged the fiction â€” the same self-heal contract SNG-009 uses for lost ops.
    try {
  ...
  // SNG-186 Â§2f: bind this turn's parsed result + what fired to the model exchange that produced it,
  // so "see the machine" shows prompt â†’ raw â†’ parsed â†’ ops end to end. Dev-only; a no-op when disarmed.
  // Runs after apply so opLedger reflects what actually landed (applied/rejected), even on a partial.
  // SNG-190 Â§3: a cumulative per-op EMISSION counter for EVERY op â€” not only markTeacher, the one op
  ...
  // not instrumented" â€” the false-zero bug where 31 emitted ops rendered as NEVER FIRED above an
  ...
  // ability-arch v2: rank 2 is earned through use, not bought â€” surface any craft that just became
  ...
  if (pendingRankAdvances.length) {
    ...
    pendingRankAdvances = [];
    if (names.length) result.turn.narration = (result.turn.narration || "") + "\n\n" + names.map(n => `*â–² Practiced â€” **${n}** is fluent now (rank 2), earned through use.*`).join("\n");
  }
  // SNG-BATCH-9: the world grows through play â€” mint any entities the fiction reached for,
    ...
    try {
      ...
      if (grown.length) {
        result.turn.narration = (result.turn.narration || "") + "\n\n" + grown.map(g =>
          `*âœ¦ The world grows â€” **${g.name}** ${g.type === "location" ? "takes shape here, and will be here when you return" : g.type === "npc" ? "is now a real presence in this place" : "begins to stir as a thread of its own"}.*`).join("\n");
        saveCharacter(character);
      }
    } catch (err) { console.warn("[generate] request handling skipped:", err?.message); }
  }
  // SNG-035: MOMENT ART â€” the GM flagged this beat worth a picture. Build it through the floors,
  ...
  // SNG-197 p2: a co-activation that ripened this turn earns a braid â€” authored, minted, given its moment.
  await maybeMintBraids(result.turn);
  return result;
}

// ---------- SNG-197 part 2: the braid is a MOMENT ----------
// A braid earned in play (or backfilled before this existed) gets its own beat â€” the two crafts named,
// what they became, the new name, and the ONE thing it can now do that neither parent could. Never a
// system notification (Â§3): its own modal, reachable + renameable again from the skill wheel node.

function braidEmergentLine(def) {
  const verb = def?.minted?.emergent;
  const grant = def?.tree?.[0]?.grants || "";
  if (verb) return `<div class="braid-moment-new">âœ¦ It can now <strong>${esc(verb)}</strong> â€” ${esc(grant)}</div>`;
  return grant ? `<div class="braid-moment-new">âœ¦ ${esc(grant)}</div>` : "";
}

/** The player overrules the GM's name (Â§2: the name is the GM's, and the player's only if they want it).
...
/** A small rename popover reachable from the braid's node on the skill wheel (Â§2: the control is reachable
...
function showBraidRename(def, onDone) {
  ...
  pop.innerHTML = `<div class="help-card" role="dialog" aria-label="Rename braid">
    <div class="help-short">Rename <strong>${esc(def.name)}</strong> â€” the GM named it; the name is yours if you want it.</div>
...
/** The mint MOMENT modal â€” reuses the .help-overlay surface (phone tap-away identical) with its own
...
function showBraidMoment(def) {
  ...
  // SNG-201 Â§2: a braid the world already knows is a RECOGNITION beat, not a CREATION beat â€” a different
  ...
  pop.innerHTML = `<div class="help-card braid-moment" role="dialog" aria-label="${isRecognition ? "A braid recognised" : "A braid forms"}">
    <div class="braid-moment-kicker">âœ¦ ${isRecognition ? "A BRAID RECOGNISED" : "A BRAID FORMS"} âœ¦</div>
    ...
    ${isRecognition && finder ? `<div class="braid-moment-finder">first found by ${esc(finder)} â€” and now yours too</div>` : ""}
    <p class="braid-moment-desc">${esc(def.description || "")}</p>
    ${braidEmergentLine(def)}
    <div class="braid-moment-rename">
      <input id="braid-rename-in" maxlength="60" placeholder="â€¦or name it yourself" value="${def.minted?.namedBy === "player" ? esc(def.name) : ""}">
...
/** SNG-197 p2: mint any braid a co-activation ripened THIS turn â€” authored rich, minted, given its moment.
...
async function maybeMintBraids(turn) {
  try {
    ...
    if (!def) return;
    turn.narration = (turn.narration || "") + `\n\n*âœ¦ A braid forms â€” **${(def.minted?.sourceNames || []).join(" and ")}** become **${def.name}**.*`;
 ...
 *  player ADOPTS the recipe (its name/prose/emergent) â€” no authoring call, a RECOGNITION moment; the numbers
 *  are still the adopter's own (braidTier). Otherwise this is the first finder: the model authors it (a
 *  CREATION moment), it mints, and syncBraidRecipes publishes it on the next sync. Â§4: a hallucinated verb is
...
async function authorAndMintBraid(m, catalog) {
  ...
  // SNG-201 Â§2: the world already found this pairing â†’ adopt it, no LLM call, a recognition beat.
  ...
  if (known) {
    ...
    def.minted.shared = true;                    // it is already the world's â€” nothing to publish
  ...
  // first finder here â€” author fresh; syncBraidRecipes publishes it (or adopts if someone beat us to sync).
  let authored = {};
  try {
...
/** SNG-201 Â§2: adopt a world recipe onto a local first-finder-lost braid â€” the world's craft (name unless
 ...
 *  never got a moment. On load, enrich the stub in place and RE-PRESENT it â€” the moment it never got, not a
...
function applyTurn(turn, resolution, playerWords = null) {
  ...
  // undefined â€” and this function reads `location.communityId` (deeds) and `location.id` (ledger).
  ...
  // SNG-137: items EVOLVE â€” the GM grows an owned item's story (description/name/provenance/use); bounded,
  // no power inflation. A light note so the player sees the thing they carry changed.
  if (turn.itemUpdates?.length) { const iu = applyItemUpdates(character, turn.itemUpdates); if (iu.length) turn.narration = (turn.narration || "") + "\n\n" + iu.map(u => `*âœ¦ ${u.name} has changed â€” ${u.changed.join(", ")}.*`).join("\n"); }

  // deeds â†’ reputation (day-stamped so news spread knows when they happened; SNG-041 also
...
*âœ¦ Your bond with ${c.name} deepens into something new: **${def.name}**.*`;
        }
      }
...
*âœ¦ ${c.name} has changed â€” ${st.name}.*`;
      }
    }
  }
  ...
  // legacy relationshipDeltas: tolerated but may only UPDATE existing people â€” this path once minted
  ...
  // Â§2 engagement: interacting with a grown NPC or accreting a fact about a grown entity is
  // attention â€” it keeps them real + surfacing. (Revisiting a grown place is signaled in travelTo.)
  ...
  if (turn.discovery && resolution?.discoveryEligible) {
    ...
    if (minted) turn.narration += `\n\n*âœ¦ New technique discovered: **${minted.name}** â€” it's yours now.*`;
  }
  ...
  // SNG-011 + SNG-131 + SNG-141: substrate ACCESS unlocks â€” only when the fiction earns it. Precursor,
  ...
  // ability's OWN powerSystem (validated) â€” a wrong-system id can't unlock anything (mirrors the seed guard).
  ...
  // SNG-195 G4: unlockLivingCurrent/unlockWildCurrent are LEGACY aliases â€” the contract tells the model to
  ...
  for (const u of [turn.unlockSubstrate, turn.unlockPrecursor, turn.unlockLivingCurrent, turn.unlockWildCurrent]) {
    ...
    const listKey = ab && SUBSTRATE_ACCESS[ab.powerSystem]; // powerSystem-validated â€” never unlock the wrong system
    if (!listKey) continue;
    if (!(character[listKey] || []).includes(u.abilityId)) {
      character[listKey] = [...(character[listKey] || []), u.abilityId];
      turn.narration += `\n\n*âœ¦ A door opens that was never on any list: **${ab.name}** may now be learned (${String(u.via).slice(0, 80)}).*`;
    }
  }
  // ability-arch v2: the GM marks a defining moment â†’ rank 3 (mastery). The engine confirms the earn
  // (rank 2, practiced, gates met) so mastery can never be handed to an unpracticed craft. A craft that
  // forks at rank 3 needs the player to choose its permanent path first (Law 9) â€” flag it for the
  ...
  if (turn.markDefiningMoment?.abilityId) {
    ...
    if (r.ok) turn.narration += `\n\n*âœ¦ A defining moment â€” **${ab?.name}** is mastered now (rank 3).*`;
    else if (r.forkPending) { character.pendingMasteryFork = mid; turn.narration += `\n\n*â‘‚ A defining moment for **${ab?.name}** â€” but it stands at a fork. Choose its path on your Character screen to master it.*`; }
  }
  // SNG-100b: the GM records a willing teacher of a people (durable â€” unlocks that people's capstones,
  ...
  // schema entry, a dispatch and a salvage slot, and still never fired once in sixteen levels â€”
  ...
  if (turn.markTeacher?.traditionId && !CONTENT.traditionIndex?.byId?.[turn.markTeacher.traditionId]) {
    ...
    console.warn("[ops] markTeacher discarded â€” unknown traditionId:", bad);
  ...
  // SNG-179 (DERIVE, NOT DEMAND): a `bondType: "mentor"` on an npcUpdate IS a teaching relationship â€”
  ...
  // The tradition is the mentor's OWN primary domain (SNG-177) â€” never invented â€” and an
  ...
  function bindTeacher(traditionId, npcId, willing) {
    ...
    if (first && willing) turn.narration += `\n\n*âœ¦ You have a teacher among the ${traditionLabel(traditionId)} now â€” their deepest craft opens to you as you earn their people's trust.*`;
  }
  // SNG-101: the GM surfaces a promotion the character has EARNED â€” the engine flags it for the player to
  ...
  if (turn.offerPromotion?.domainKey === "secondary" || turn.offerPromotion?.domainKey === "tertiary") {
    ...
    if (e.eligible) { character.pendingPromotion = dk; turn.narration += `\n\n*âœ¦ You are being recognized â€” you may be raised into **${traditionLabel(e.trad)}** as your ${e.to}. Choose on your Character screen; it will close the far pole of that axis to you.*`; }
  }
  // SNG-102: the GM offers a NEW people to join â€” the engine flags it only if genuinely acquirable; the
  // player commits (Law 9 â€” joining forecloses the far pole of that people's axis). The GM never forecloses.
  if (turn.offerAcquisition?.traditionId) {
    ...
    if (acquirable(character, tid, CONTENT.rules, { traditionIndex: CONTENT.traditionIndex }).ok) {
      ...
      turn.narration += `\n\n*âœ¦ The ${traditionLabel(tid)} would take you as one of their own. You may join them on your Character screen â€” entering as a novice (Tier I), and closing the far pole of their axis to you.*`;
    }
  }
  ...
  // (gm.js) forbids emitting the act's own effects alongside offerIntent â€” anything the GM
...
*âœ¦ New ability learned: **${def.name}**${def.taughtBy ? ` (from ${def.taughtBy})` : ""} â€” ${def.description.slice(0, 120)}*`;
  }
  // level up: banked growth the player chooses to spend
  const prevLevel = character.level;
  for (const msg of applyLevelUps(character, CONTENT.rules)) {
    turn.narration += `\n\n*You feel it settle into you â€” a new steadiness. (${msg})*`;
  }
  ...
  // SNG-162 Â§3: an unmatched quest op is a CONTENT bug worth seeing. It was already never silent,
  // but it landed in GM notes where a player never looks â€” surface it in the quest panel instead.
  ...
  // SNG-162 Â§1: STAGE OPS â€” the fiction advancing a structured quest. The model observed; the
  ...
  if (turn.stageOps?.length) {
    ...
    for (const r of advanced) {
      turn.narration = (turn.narration || "") + `\n\n*âœ¦ ${r.title} â€” ${r.change || "a step forward"}.${r.awaitingResolution ? " Every stage is done; how it ends is yours to choose." : ""}*`;
    }
  ...
  // journey montage, a long vigil â€” or a quick exchange), the GM declares it via
  ...
  const extraHours = Math.max(0, Math.min(12, Number(turn.timeAdvanceHours) || 0)); // SNG-195 G4: timeAdvanceHours is a legacy alias of timeOps (contract op) â€” inbound tolerance only, not advertised
  ...
  // SNG-191 Â§1/Â§5: CHARACTER time is UNCAPPED â€” the 72h/168h clamp DIES here. A four-day journey costs
  // four character-days and narrates as four (acceptance Â§6.1). There is no shared unit to clamp
  ...
  // BATCH-12 Â§3c: the company you keep earns standing with their people, on the IN-GAME DAY. Erik's
  // Calvar case â€” a willing Radiant teacher travelling with him and zero Radiant standing, because
  ...
  // you eleven days" is a sentence; "nine scenes" is not.
  {
    ...
    // FOCUSED (Â§3c) is DERIVED, never model-judged. Two of the spec's three triggers are available:
    //   Â· "use of their craft" â€” the tradition of the ability actually used this turn
    //   Â· "work at their behest" â€” a people this turn's standingOps names
    ...
    // of GM prose, not an id â€” passing it here would have shipped a sentence as a tradition. That
  ...
  // BATCH-12 Â§3d: STANDING OPS â€” an act that plainly helps or offends a people, without a quest.
  // Model reports, engine adjudicates (SNG-162). Clamped twice: Â±3, and never across a band edge.
  if (turn.standingOps?.length) {
    ...
    for (const a of so.applied) {
      turn.narration = (turn.narration || "") + `\n\n*âœ¦ ${traditionLabel(a.people)}: ${a.delta > 0 ? "+" : ""}${a.delta} â€” ${a.why}${a.held ? " (held at the band's edge)" : ""}.*`;
    }
  ...
  // SNG-191 Â§4: capture DELEGATED commitments as state, so the world honours them while the player is
  // away (the tick advances the work, not the person's mood). A charge set against a real crisis is
  // what can push that crisis back â€” delegation is how a crisis gets solved offscreen.
  if (turn.delegateOps?.length) {
    ...
    if (n) turn.narration = (turn.narration || "") + `\n\n*âœ¦ The charge is set â€” the work goes on while you are away, and you'll hear how it fared when you return.*`;
  }
  // SNG-191 Â§7: the player closed a SURFACED latent arc â€” the third fate (handled), or it concluded
  ...
  // SNG-193b Â§3.2: a story-earned change of school â€” a teacher's long training, a hard turning. Canon is
  ...
  // the map. The band this craft resolves at moves with it, per Â§3.3.
  ...
  // SNG-194 Â§3: the unprompted OFFER â€” the GM introduced ONE thing the player was not reaching for, drawn
  // from something already true and NAMING what it came from (attribution is the whole difference from a
  // random-encounter table). Countable from day one (SNG-190 Â§3): the op is what lets us tell it is
  // working. `turnsSinceOffer` drives the RARE cooldown â€” reset to 0 here, incremented every other turn.
  ...
  // are here", GM context) agrees with the prose â€” instead of the header showing a stale place.
  const moveRef = turn.moveTo && (turn.moveTo.location || turn.moveTo.id || turn.moveTo);
  if (moveRef) {
    // SNG-117: the header follows the fiction even to a place with no record yet â€” resolve it, or MINT it
    ...
    // SNG-190 Â§1.3: a moveTo naming a KNOWN SUB-PLACE resolves to its PARENT LOCATION â€” you do not
    // leave Cairnhold to step into your mother's kitchen. Checked FIRST, and when it hits, the waygate
    // router is skipped entirely (Â§1.1: a local sub-place is not a gate transit). This alone prevents
    // the teleport: "Silas's Mother's House â€” Kitchen" is a sub-place of Cairnhold, so it now lands in
    // Cairnhold (where he already stood) instead of falling through resolveâ†’routeGmMoveTo's hub branch.
    // (parentId is always a real location â€” places.js sets it to the location or a resolveLocationId id.)
    ...
    // Center" minted one called "Center" â€” the GM-offer half of SNG-148's REACHABLE link was never
    // actually wired, so the fiction's transit fell through to mintTransitLocation. NOT consulted for a
    // known sub-place (Â§1.1) â€” a garden latch inside a gate-town is local movement, never a transit.
    ...
    if (wgRoute) {
      ...
      if (wgRoute.routed === "hub" && landed) {
        ...
        // routing, never failure â€” but it must never be silent, or the world stops making sense.
        character._correctionAside = [character._correctionAside,
          `The gate set you down at ${landed.name} â€” the hub.`]
          .filter(Boolean).join(" ");
      }
    }
  ...
  // non-travel turn also clears a stale pending arrival â€” the affordance is tied to the travel beat.
  ...
  // SNG-070: the game self-heals â€” apply any bounded GM corrections to THIS save (repair, not wish;
  ...
  // SNG-091: the player's REQUEST is unconditional â€” never gate on aptness, and never silently drop
  ...
  // scene anchor: the GM's updated scene state, clamped â€” or keep the previous one
  sceneState = sanitizeScene(turn.scene) || sceneState;
  // SNG-075: the valley is alive in narrative play too â€” maybe turn something up (woven next turn)
  maybeNarrativeEncounter(turn, resolution);
  // SNG-080: the world must PUSH â€” if nothing has happened for a while, make it happen (woven next turn)
  ...
  // SNG-190 Â§5 / SNG-189 Â§1: sceneSummary becomes the ONLY durable record of a scene, but the model
  // sometimes returns an OBJECT that still parses â€” pushed raw into the chronicle (below) it rendered
  ...
  if (syncEnabled()) {
    ...
    // SNG-145 trigger 3 (irreversible): an impactsLocal event reaches ANOTHER player's area â€” it
...
function substrateForAction(choice, location) {
  ...
  // SNG-193b Â§3.3: the band reads the character's SCHOOL for this tradition (its extension source), not
  // the tradition â€” two practitioners of one tradition get opposite best-grounds. Floored by the material
  ...
  // Â§9b invariant 5: when something CARRIED is why the ground reads differently, the receipt must
  // say which thing. A ward that quietly halves your craft â€” or a staff that quietly saves it â€” is
...
/** SNG-116: THE single source of truth for an action's substrate CHANCE penalty â€” the exact number the
...
async function onChoice(choice) {
  if (busy) return;
  // SNG-145: while an intent gate stands, the gate IS the choice â€” answer it first.
  if (character._pendingIntent) { renderPlay(character.activeScene?.lastTurn || null, { aside: "Answer the moment of intent first â€” it's waiting on you." }); return; }
  ...
  // power costs less â€” effectiveEnergyCost scales with level and rank)
  ...
  if (energyCost && character.energy < energyCost) { alert("Not enough energy â€” rest first."); return; }
  // SNG-090: the substrate â€” can this CRAFT run here? A weapon/attribute action is unaffected. At the
  // extreme (the lattice is nearly gone for a high-dependency craft), the ability won't answer at all â€”
  ...
  if (substrate?.off) {
    ...
    renderPlay(character.activeScene?.lastTurn || null, { aside: `The lattice is too thin here â€” ${abName} barely stirs (${substrate.percent}% of its strength) and won't answer. Carry charge, or reach denser ground; steel and wit still work.` });
    return;
  }
  const action = {
    ...
    // SNG-140: a wild_current craft carries the tangled current's variance â€” the resolver widens both crit bands
  ...
  if (choice.emergenceId) {
    ...
    if (tpl) {
      ...
      saveCharacter(character);
      if (minted) {
        renderPlay(null, { thinking: "â€¦", playerBeat: { label: choice.label, playerWords: choice.playerWords || null } });
        const result = await runGM({ resolution: null, playerInput: `(Practice has ripened into breakthrough: the character accepts "${minted.name}" â€” ${tpl.tier === "combo" ? tpl.discoveredBlurb || tpl.description : tpl.grants}. Narrate the moment it clicks into something repeatable, then continue the scene.)` });
  ...
  if (choice.encounterId && (CONTENT.encounters?.[choice.encounterId] || character.customEncounters?.[choice.encounterId]) && !character.activeEncounter) {
    ...
    saveCharacter(character);
    renderPlay(null, { thinking: "â€¦", playerBeat: { label: choice.label, playerWords: choice.playerWords || null } });
    const result = await runGM({ resolution: null, playerInput: `(The encounter "${def.name}" begins: ${def.setup} Narrate the opening${isSB ? " of the contest (the mechanics play out in a panel â€” just set the scene and the stakes)" : " and offer round choices"}.)` });
    if (result) renderPlay(result.turn, { playerBeat: { label: choice.label, playerWords: choice.playerWords || null }, degraded: result.degraded });
    if (isSB) renderSkillBattle(); // take over the rounds with the contest panel
    return;
  }
  // trivial actions â€” no real chance of failure, no cost: no dice, no energy
  ...
  // a technique already discovered isn't novel anymore â€” it's earned skill
  ...
  // SNG-145: INTENT GATES fire HERE â€” before the dice roll, before energy is spent, before the GM
  // is called â€” so a decline commits nothing and the moveTo always-emit contract is never touched.
  ...
  if (!choice.intentRung && !choice.departureConfirmed) {
    ...
    // CCODE-01: the rung is per-RANK (147c moved it into ab.tree[]) â€” gate on what this character
  ...
  if (encD) {
    ...
    // SNG-106: name the opposed term so the roll breakdown reads "the raider (threat N) âˆ’11", not anonymous.
  ...
  // SNG-169 Â§2c: keep the ITEM helpers separately. `equipHelpers` mixes items and companions, and
  // the roll receipt makes item names tappable â€” a companion name looked up as an item would
  ...
  // SNG-090: surface the substrate when it bit (starved/crowded) â€” a receipt line + a note the GM
  ...
  if (substrate && substrate.side !== "full" && substrate.side !== "neutral") {
    ...
    pendingSubstrateNote = substrate.side === "starved"
      ? `The lattice is THIN here â€” the character's craft is running at ~${substrate.percent}% (starved of substrate). Let the effort show the strain; a weapon or mundane means is unaffected.`
      : `The lattice is DENSE and CROWDS the character's craft here â€” it runs at ~${substrate.percent}% (too much signal is noise). Let it read as interference, not weakness.`;
  }
  ...
  // novel use: breakthrough or backlash â€” the engine decides, the GM narrates
  if (action.novel) {
  ...
  // SNG-015: a Surge that slips can bite â€” pay by the ability's Tier
  ...
  // SNG-010: practice ledger â€” the single counting site
  {
    ...
    // SNG-011: precursor peril â€” using these changes you (extra alignment drift along the ability's axes)
  ...
  // meditation: engine-owned recovery, scaled by attunement â€” the centered refill faster
  ...
  renderPlay(null, { thinking: "â€¦", playerBeat: { label: choice.label, resolution } });
...
async function onFreeform(text) {
  ...
  if (character._pendingIntent) { renderPlay(character.activeScene?.lastTurn || null, { aside: "Answer the moment of intent first â€” it's waiting on you." }); return; } // SNG-145
  lastPlayerText = text;
  renderPlay(null, { thinking: "Reading your intentâ€¦", playerBeat: { label: text } });
  const location = CONTENT.locations[character.currentLocationId];
...
/** Historical name for the encounter test panel gate â€” resolves to the single SNG-074 model. */
...
async function fireEncounter(entryOrFlavor, { dev = false, news = [] } = {}) {
  ...
  if (offer.routing === "narrative" || offer.routing === "opposed") {
    await startScene(offer.prompt, news, dev ? `ðŸ”§ test: ${entry.flavor} (${entry.id})` : null);
    return true;
  }
  // deterministic offer beat â€” decline/flee is on the table BEFORE engagement (SNG-002b)
  ...
  renderPlay(offerTurn, { newsFlash: news, playerBeat: dev ? { label: `ðŸ”§ test encounter: ${entry.flavor} (${entry.id})` } : null });
  return true;
}

/** SNG-138: the def catalog a prestige-recurrence arc is resolved from (authored arcs + a generated
 *  personal arc â€” a backstory arc can BE a recurrence arc). */
 ...
 *  of a generic encounter â€” the offer carries the guaranteed decline path, and engaging routes into the
...
/** SNG-075: after a narrative GM turn, MAYBE turn something up â€” bound to how long the fiction
...
function maybeNarrativeEncounter(turn, resolution) {
  ...
  const table = CONTENT.randomEncounters;
  if (!table) return;
  // SNG-127: the player's chosen pacing (Calmâ†’Relentless) resolves a rate multiplier + a cooldown; a
  ...
  // SUPPRESSION CONDITIONS (kept): a live encounter Â· a gambit in play Â· a scene that just ended Â· an
  // explicitly intense/intimate beat Â· during combat. SNG-127: the once-per-scene gate is now SOFT â€”
  ...
  // SNG-127 (Q2): an UNDECLARED beat (GM omits timeOps â†’ 0h, yet the clock ticked +1h) is floored to
  // minHoursPerBeat so it still counts as "time passing" â€” otherwise classify returns "none" and the
  ...
  if (!entry) return;
  // WEAVE, DO NOT INTERRUPT: no card, no scene change â€” stash the seed for the next GM turn.
 ...
 *  world already acted this beat (an encounter, a quest change, a scene end), the streak resets â€”
...
function maybeWorldPressure(turn, resolution) {
  const woveEncounter = !!pendingWeave; // SNG-075 just set one â†’ the world already acted this beat
  ...
  quietTurns = 0;     // space them out â€” another threshold of quiet before the next push
}

/** SNG-117: mark a place KNOWN by name (GM-named destination, en-route, minted transit place) so the map
 *  surfaces its name + it becomes a travel target â€” even before it's ever entered. Idempotent. */
...
// SNG-122: a clear travel expression in the player's own words â€” "head to / go to / travel to / set out
 ...
 *   (1) TRUSTED â€” the LLM intent parser named a destination (`travelTo`) for a GO/HEAD/JOURNEY action;
 *       honored even for an unmapped place (minted on arrival, the SNG-117 path).
 *   (2) GUESSED â€” a "travel" tag or a travel-phrase in the label/words. A guess must name a REAL/known
...
function travelIntentOf(action) {
  ...
  // SNG-188 Â§4: DISCUSSING travel is not DOING it. A label led by a speech verb (announce, confide,
  // tell, discuss, planâ€¦) is a conversation about a journey, never a departure â€” however many
  // place-names it holds. The code belt behind the parser prompt: a travelTo the model set on a speech
  // act is stopped HERE, before buildTravelDirective can force a move. Erik's exact turn â€” "confide in
  // Veth â€¦ and announce travel plans to Cairnhold" â€” leaves him in the alcove because of this line.
  ...
  // (2) a GUESS from tag/phrase â€” must resolve to a real place to be trusted as a destination
  ...
  if ((!cand || NOT_A_PLACE.test(cand.replace(/^the\s+/i, "").trim())) && tags.includes("travel")) {
    cand = null; // tagged travel, no clean phrase â†’ find a KNOWN place named in the words
...
function buildTravelDirective(ti) {
  ...
  return `The player INTENDS to travel to ${dest}. If the fiction actually DEPARTS this beat â€” they set out, are led, or the trip is a montage with time passing on the road â€” emit "moveTo": {"location": "${ti.destId || ti.ref}", "why": "â€¦"} so they arrive, and narrate the journey (with timeOps if far). But if this beat is still PLANNING or discussing the trip and they have NOT left yet, do NOT move them â€” keep the scene where it is and offer the road as the next step. Never relocate a character who only spoke about going (SNG-188). ${adj.length ? `Places reachable from ${here?.name || "here"}: ${adj.join(", ")}.` : ""}`;
}

/** SNG-122: the one-tap safety net â€” the travel beat didn't move the player, so arrive now via the SAME
 ...
 *  resume the ESCROWED CHOICE â€” annotated and re-entered through onChoice, so the
 ...
 *  beat. Declining a departure simplyâ€¦ stays: nothing was committed to undo. */
async function answerIntent(optionId) {
  ...
  if (g.resume?.choice) {
    // mark the ask so this encounter/scene never gates twice (spec Â§2: rare or it's noise)
    if (g.askedKey) character._intentAsked = { ...(character._intentAsked || {}), [g.askedKey]: true };
    if (g.kind === "departure" && optionId !== "go") {
      saveCharacter(character);
      renderPlay(character.activeScene?.lastTurn || null, { aside: "You hold at the boundary â€” the road will keep." });
      return;
    }
  ...
  // GM-raised gate: no escrowed act â€” the confirmed intent returns as a synthetic beat.
  saveCharacter(character);
  renderPlay(null, { thinking: "â€¦", playerBeat: { label } });
...
/** SNG-145 trigger 3: confirm or hold a world-scale consequence. Held = dropped â€”
...
async function answerLedger(send) {
  ...
  renderPlay(character.activeScene?.lastTurn || null, { aside: send ? "Word of it travels â€” the world will hear." : "It stays local â€” a story this valley keeps." });
}
 ...
 *  what a GM-emitted sceneEnded does. If the GM can't answer, the scene still closes â€” a scene the
...
async function endSceneNow() {
  ...
  if (!sceneTurns.length) { renderPlay(character.activeScene?.lastTurn || null, { aside: "There's no scene open yet â€” take an action first." }); return; }
  const beats = sceneTurns.length;
  renderPlay(null, { thinking: "Drawing the scene to a closeâ€¦" });
  const result = await runGM({ resolution: null, playerInput:
    `(The player is CLOSING this scene. Write one short closing beat that lets the current moment finish naturally â€” nobody teleports, nothing unresolved is forced shut â€” then set "sceneEnded": true and write "sceneSummary" as a summing-up of the WHOLE scene (${beats} beats), because that summary becomes the chronicle entry. Offer no choices.)` });
  if (result?.turn?.sceneEnded) { renderPlay(result.turn, { aside: `Scene closed â€” ${beats} beats written into your chronicle.` }); return; }
  // The GM didn't set the flag (or the call failed) â€” close it engine-side anyway.
  ...
  saveCharacter(character);
  renderPlay(result?.turn || null, { aside: `Scene closed â€” ${beats} beats written into your chronicle.` });
}

// ---------- SNG-155: read the narration aloud (Web Speech, tier 1) ----------
// Tier 0 is always available: no speechSynthesis, no voices, no key â†’ silence, no error (Law 5).
...
/** Speak a turn's SPEAKABLE PROJECTION â€” narration only; choices, costs and receipts are never
 *  spoken (spec Â§3b). A new turn cancels the old utterance cleanly. */
 ...
 *  speak the DISPLAYED turn â€” falling back to lastTurn only when it was invoked without one. */
...
/** SNG-117: turn a named-but-unrecorded destination ("the pass") into a REAL, travelable place â€” a
 *  lightweight generated location adjacent to where you left, with a stable id + map coord. Idempotent
 *  (keyed off the normalized name, so "the pass" mints exactly once â€” Q1). Deterministic (no model call);
...
function mintTransitLocation(moveRef) {
  ...
  if (CONTENT.locations[id]) { addKnownPlace(id); return id; } // already a real place â€” reuse, never dup
  const here = CONTENT.locations[character.currentLocationId];
  const existing = {}; for (const l of Object.values(CONTENT.locations)) if (l.map) existing[l.id] = l.map;
  const name = String(moveRef).replace(/[-_]+/g, " ").replace(/\b\w/g, c => c.toUpperCase()).slice(0, 60);
  // SNG-154: if this name was already a sub-place somewhere, promotion PRESERVES that containment â€”
  ...
  const rec = {
    ...
    descriptionSeed: `A place the road led to â€” ${name}. The fiction brought you here before the map knew its name.`,
...
async function travelTo(locId) {
  if (busy) return;
  noteGeneratedAttention(locId, "revisit", readClock(character.clock).day); // Â§2: returning to a grown place keeps it alive
...
async function rest(kind = "sleep") {
  ...
  sceneTurns = [];
  sceneState = null; // hours pass â€” the old scene has dissolved
  advanceClock(character.clock, r.hours);
  saveCharacter(character);
  const news = await maybeTick();
  if (await maybeRandomEncounter("onRest", news)) return; // the night was not quiet
  startScene(`(The character takes a real night's rest here â€” camp, inn, or quiet corner. Narrate the rest briefly, then present what's happening when they get up. ${r.hours} hours have passed; do not grant additional energy â€” the engine already restored them.)`, news);
}
...
async function onAsk(text) {
  ...
  renderPlay(lastTurn, { playerBeat: { label: "[to the GM] " + text }, thinking: "The GM leans backâ€¦" });
  // BATCH-11 Â§23: assembled from the registry's "ask" view (same rows, no turn ephemera).
...
// SNG-080: danger, made legible on the map â€” so the player can SEE where fighting lives.
...
// 800Ã—440 canvas is unreadable â€” every label collides and the thing you're looking for is a dot
// among ninety-four others. Each tier answers one question at the scale that question lives at:
//   WORLD    which Reach am I in â€” regions as territories, and the gates that cross them
//   REGION   where can I go from here â€” the settlements around me (today's map, finally scoped)
//   LOCATION what is INSIDE this place â€” Millbrook â†’ the Edge District â†’ the Inn â†’ the booth
...
function mapTierBar() {
  ...
  return `<div class="map-tiers" style="margin-bottom:8px;display:flex;gap:6px;align-items:center;flex-wrap:wrap">
    ${crumb("ðŸŒ World", "world", "", mapTier === "world")}
    <span class="hint">â€º</span>
    ${crumb(rName || "Region", "region", rid, mapTier === "region")}
    ${locName ? `<span class="hint">â€º</span>${crumb("âŒ‚ " + locName, "location", mapFocus, true)}` : ""}
    <span class="hint" style="margin-left:auto">${mapTier === "world" ? "Click a region to enter it" : mapTier === "region" ? "Click a place, then â€œLook insideâ€" : "What's inside this place"}</span>
  </div>`;
}

/** WORLD tier â€” regions as territories. Individual settlements are noise at this scale; the
...
function renderMapWorld() {
  ...
  const cells = nodes.map((n, i) => {
    ...
    return `<g class="map-region-cell ${n.here ? "here" : ""} ${n.known ? "known" : "unknown"}" data-mapregion="${esc(n.regionId)}">
      <title>${esc(n.name)} â€” ${n.count} place${n.count === 1 ? "" : "s"}${n.gates.length ? ` Â· ${n.gates.length} waygate${n.gates.length === 1 ? "" : "s"}` : " Â· no waygate"}${n.here ? " Â· you are here" : ""}</title>
      ...
      ${n.gates.length ? `<text x="${cx}" y="${cy + 26}" text-anchor="middle" class="map-visits">â—ˆ ${hub ? "hub" : n.gates.length + " gate" + (n.gates.length === 1 ? "" : "s")}</text>` : ""}
  ...
  chrome(`<div class="screen" style="max-width:900px">
    <h2>World Map</h2>
    <p class="hint" style="margin-bottom:8px">${nodes.length} regions${nodes.reduce((n, r) => n + r.gates.length, 0) ? ` Â· ${nodes.reduce((n, r) => n + r.gates.length, 0)} waygates` : ""}. The scale where the question is <em>which Reach am I in</em>.</p>
    ${mapTierBar()}
...
/** LOCATION tier â€” the interior. THE TIER THAT DID NOT EXIST. Sub-places and any place promoted
...
function renderMapLocation(locationId) {
  ...
  const nodes = laid.map((c, i) => `<g class="map-node ${c.visited ? "" : "unvisited"} ${c.promoted ? "reachable" : ""}" data-mapinner="${esc(c.id)}" data-innerkind="${esc(c.kind)}">
      <title>${esc(c.name)}${c.promoted ? " â€” a place of its own now, still inside " + esc(name) : ""}${c.note ? " â€” " + esc(c.note) : ""}${c.visited ? "" : " (heard of, not seen)"}</title>
      ...
      ${/* CCODE-11: stagger the label's distance by index so two adjacent ring nodes can never put
            their labels in the same horizontal band â€” measured 3 colliding pairs in the Edge
            District (long authored names) before this. The full name stays in the <title>. */""}
      <text x="${c.x.toFixed(1)}" y="${(c.y + (c.y > 230 ? 26 + (i % 2) * 14 : -14 - (i % 2) * 14)).toFixed(1)}" text-anchor="middle" class="map-label">${esc(c.name.length > 20 ? c.name.slice(0, 19) + "â€¦" : c.name)}</text>
    </g>`).join("");
  chrome(`<div class="screen" style="max-width:900px">
    <h2>${esc(name)} â€” inside</h2>
    <p class="hint" style="margin-bottom:8px">${children.length ? `${children.length} place${children.length === 1 ? "" : "s"} within.` : "Nothing recorded inside here yet â€” the places you visit and the GM names will appear here."} A ringed node is somewhere that grew into a place of its own.</p>
    ${mapTierBar()}
  ...
  // a promoted interior is a real location â€” you can step into ITS interior too (nesting)
...
function renderMap(selectedId = null) {
  ...
  // the region-boundary edge filtering the real one does â€” so an edge leaving the region drew as if
  ...
  // connection whose target merely existed in CONTENT.locations â€” including targets in a DIFFERENT
  ...
  // SNG-082: real terrain â€” each region drawn as a palette-filled hull of its locations. Data-driven
  ...
  const terrain = (CONTENT.regions || []).map(rg => {
    ...
    const body = shape
      ? `<polygon points="${shape.map(p => `${p.x.toFixed(1)},${p.y.toFixed(1)}`).join(" ")}" class="${cls}" fill="${esc(pal.base || "#2a2f28")}" stroke="${esc(pal.edge || pal.base || "#333")}"><title>${esc(rg.name + " â€” " + (rg.terrain || ""))}</title></polygon>`
  ...
  const svg = `<svg id="skill-svg" viewBox="0 0 800 440" class="world-map" preserveAspectRatio="xMidYMid meet"><g class="graph-vp">
    ...
    <text x="20" y="50" class="map-sub">Day ${readClock(character.clock).day} Â· Water Crisis stage ${stage}${mapShowKG ? " Â· showing what you know" : ""}</text>
    ...
    ${locs.map((l, li) => {
      ...
      const tip = visited
        ? `${l.name}${l.id === here ? " â€” you are here" : ""}${pm?.visits ? ` Â· ${pm.visits} visit${pm.visits > 1 ? "s" : ""}` : ""}${dl >= 1 ? ` Â· ${dangerLabel(dl)}` : ""}${reachable ? " Â· one travel away" : ""}`
        : known
          ? `${l.name} â€” you know of it, not yet been${reachable ? " Â· one travel away" : ""}`
          : `Unknown place â€” you've only heard of it${reachable ? " Â· one travel away" : ""}`;
      return `<g class="${cls}" data-mapsel="${esc(l.id)}">
        ...
        ${mapShowSub ? (() => { const sps = Object.values(character.placeMemory?.[l.id]?.subPlaces || {}); return sps.slice(0, 6).map((sp, si) => {
          ...
          return `<circle cx="${sx}" cy="${sy}" r="4" class="map-satellite ${sp.visited ? "visited" : "heard"}"><title>${esc(sp.name)}${sp.note ? " â€” " + esc(sp.note) : ""}${sp.visited ? "" : " (heard of)"}</title></circle>`;
        }).join(""); })() : ""}
        <text x="${P.x}" y="${P.y + (P.y > 300 ? 32 : -20)}" text-anchor="middle" class="map-label">${esc(known ? l.name : "?")}</text>
        ${visited && pm?.visits > 1 ? `<text x="${P.x}" y="${P.y + (P.y > 300 ? 46 : -6)}" text-anchor="middle" class="map-visits">Ã—${pm.visits}</text>` : ""}
      </g>`; }).join("")}
    ${kg.map(e => `<g class="map-kg ${e.kind} ${e.discovered ? "met" : "heard"}" ${e.topicId ? `data-kgtopic="${esc(e.topicId)}"` : ""}>
        <title>${esc(e.label)}${e.kind === "rumour" ? " â€” a thread you've only heard of" + (e.note ? ": " + esc(e.note) : "") : e.discovered ? " â€” you've met them" : " â€” you've only heard of them"}</title>
        ${e.kind === "rumour"
  ...
  // details panel for the selected node â€” travel is an explicit button, never a stray click
  let details = "";
  if (selectedId && CONTENT.locations[selectedId]) {
    ...
    details = `<div class="map-details">
      <div class="map-details-head">
        <h3>${esc(known ? l.name : "An unknown place")}${!visited && known ? ` <span class="hint">â€” known of, not yet been</span>` : ""}</h3>
        ...
        ${(() => { const s = l.communityId ? standingWith(character, l.communityId, CONTENT.rules) : null; return s?.score ? `<span class="rep-band ${s.band}" title="Your standing here â€” ${s.band} (${s.score})">${esc(s.band)}</span>` : ""; })()}
      </div>
      ...
      ${visited
        ? `<p class="map-details-desc">${esc(l.descriptionSeed)}</p>${/* SNG-076: authored descriptionSeed renders IN FULL */""}
           ...
           ${pm?.visits ? `<div class="hint">${pm.visits} visit${pm.visits > 1 ? "s" : ""}${pm.lastVisit != null ? ` Â· last on day ${pm.lastVisit}` : ""}</div>` : ""}
      ...
      ${l.id !== here ? (reachable
        ? `<button class="btn" id="map-travel" data-dest="${esc(l.id)}" style="margin-top:8px">Travel here (+${ADVANCE.travel}h)</button>${(() => {
            ...
            // scale makes the number mean something â€” a neighbouring Reach is weeks and your
            ...
            return days == null ? "" : `<div class="hint" style="margin-top:4px">${days < 1 ? "less than a day" : `about ${Math.round(days)} day${Math.round(days) === 1 ? "" : "s"}`} on foot â€” ${days > 40 ? "a waygate is the difference between a journey and a life" : "walkable, if you have the season for it"}.</div>`;
          })()}`
        + `<button class="opt" id="map-lookinside" data-inside="${esc(l.id)}" style="margin:8px 0 0 6px" title="What's within this place">âŒ‚ Look inside</button>`
        : `<div class="hint" style="margin-top:6px">Not directly reachable from ${esc(CONTENT.locations[here]?.name || "here")} â€” travel via a connected place.</div>`) : ""}
      ${(() => { // SNG-148: standing at a gate, aiming at another gate â€” routing decides (named/hub); never a failure
        ...
        const note = r.routed === "hub" && r.destId !== l.id
          ? ` â€” the gate carries you to ${esc(destName)}, the hub (${!r.known ? "an undiscovered gate can't be aimed at" : "beyond your wayfaring to aim true"})` : ` to ${esc(destName)}`;
        return `<button class="btn" id="map-waygate" data-wgdest="${esc(r.destId)}" style="margin-top:6px">â—ˆ Step through the waygate${note} (+${ADVANCE.travel}h)</button>`;
      })()}
    </div>`;
  }
  chrome(`<div class="screen" style="max-width:900px">
    <h2>${esc((CONTENT.regions || []).find(r => r.regionId === focusRegion)?.name || "Region")}</h2>
    ${/* SNG-154 stage 6: this count is now the REGION's, not the world's â€” and it is derived, so it
    ...
    ${mapTierBar()}
    <div style="margin-bottom:8px"><button class="opt ${mapShowKG ? "selected" : ""}" id="map-kg-toggle" title="People you've met (solid) and threads you've only heard of (dimmed diamonds) â€” where they live">${mapShowKG ? "âœ“ " : ""}Show what you know</button>
      <button class="opt ${mapShowSub ? "selected" : ""}" id="map-sub-toggle" title="The places WITHIN each location (satellites around each node)" style="margin-left:6px">${mapShowSub ? "âœ“ " : ""}Show sub-places</button>
      ${mapShowKG && !kg.length ? `<span class="hint" style="margin-left:8px">You haven't met anyone or heard a rumour yet â€” the world is still a rumour. Play on.</span>` : mapShowKG ? `<span class="hint" style="margin-left:8px">â—† dimmed = heard of Â· â— solid = met</span>` : ""}</div>
    <div class="graph-wrap" id="graph-wrap">
      <div class="graph-zoom-ctl">
        <button id="gz-in" title="Zoom in">ï¼‹</button>
        <button id="gz-out" title="Zoom out">ï¼</button>
        <button id="gz-fit" title="Fit to view">â¤¢</button>
        <button id="gz-me" title="Centre on me">â—Ž</button>
      </div>
      ${svg}
    </div>
    ${details}
  ...
  for (const b of app.querySelectorAll("[data-subgo]")) b.onclick = () => {
    ...
    if (!sp) return;
    if (b.dataset.subloc !== character.currentLocationId) { alert("Travel to " + (CONTENT.locations[b.dataset.subloc]?.name || "that place") + " first â€” then head to the " + sp.name + "."); return; }
    renderPlay(character.activeScene?.lastTurn || null, {});
...
// ---------- skill KG graph (SNG-011 Phase 3a â€” rendered like the world map) ----------

// ---------- SNG-073: THE SKILL WHEEL â€” the skill tree IS the great circle ----------
// Polar layout, radius = tier. 24 tradition nodes on the ring (read from traditions.json, never
// hardcoded); each people's abilities radiate INWARD (V at the node, I near the centre â€” depth is
// mastery). Folk crafts at the CENTRE (open to all). Precursor OUTSIDE the ring (the substrate).
// Known braids drawn as connections â€” cross-pole braids as the DIAMETER through the centre. The
...
function buildWheelModel() {
  ...
  const nodes = [];
  // group abilities: pole-tradition spokes Â· folk centre Â· precursor outer ring
  ...
  for (const ab of Object.values(cat)) {
    ...
    if (!trad || ab.powerSystem === "learned") { folk.push(ab); continue; } // ungoverned/learned â†’ centre
  }
  const mk = (ab, x, y, ang) => {
    const trad = traditionOf(ab, idx);
...
// upgrade grants before spending. Shared by both views â€” pure over character + CONTENT, so the two
...
function skillSelectionActions(ab) {
  if (!ab) return ""; // virtual/emergence node â€” no real ability to act on
  const rules = CONTENT.rules;
  const sp = character.skillPoints || 0;
  const owned = character.abilities.find(a => a.abilityId === ab.id);
  const maxRank = rules.leveling?.maxAbilityRank ?? 3;
  // the tier ladder â€” what every rank grants (owned = filled, the one you'd buy next = highlighted).
  ...
  const ladder = (ab.tree || []).map(t => {
    ...
    return `<div class="skill-rung ${on ? "on" : ""} ${isNext ? "next" : ""}"><span class="skill-rung-dot">${on ? "â—" : "â—‹"}</span> <strong>r${t.rank} ${esc(t.name)}</strong>: ${esc(t.grants || "")}${t.cannot ? ` <span class="skill-rung-cannot">â€” cannot: ${esc(t.cannot)}</span>` : ""}</div>`;
  }).join("");
  const ladderBlock = ladder ? `<div class="skill-ladder">${ladder}</div>` : "";

  if (owned) {
    // ability-arch v2: depth is earned through use, never bought â€” show progress, not a buy button.
  ...
  // not owned â†’ the learn path (same gate as the Level-Up modal's learnRow)
  const req = learnLevelReq(ab);
  const dv = domainVerdict(ab);
  if (req === null || !dv.allowed) return `<div class="skill-actions">${ladderBlock}<span class="hint">ðŸ”’ not open to your domains</span></div>`;
  ...
  return `<div class="skill-actions">${ladderBlock}
    ${blocked
      ? `<span class="hint">ðŸ”’ ${!levelOk ? `opens at level ${req}` : !gate.ok ? esc(gate.why) : capBlock ? "at capacity â€” deepen a craft you know instead" : `need ${cost} point${cost > 1 ? "s" : ""} (you have ${sp})`}</span>`
      : `<button class="btn" data-skilllearn="${esc(ab.id)}">Learn${ripe ? " (free â€” practiced)" : ` (${cost} pt${cost > 1 ? "s" : ""})`}</button>`}
  </div>`;
}

// Wire the learn/deepen buttons a details panel rendered. `rerender(id, status)` re-opens the same
// view with the node still selected and a status line â€” used by both wheel and graph.
function wireSkillSelectionActions(rerender) {
  for (const b of app.querySelectorAll("[data-skilllearn]")) b.onclick = () => {
    ...
    if (r.ok) { if (free) dropAspiration(character, id); saveCharacter(character); rerender(id, `Learned ${fullCatalog()[id]?.name}${free ? " â€” no point spent" : ""}.`); }
    else rerender(id, r.why);
  };
  // ability-arch v2: no data-skillrank handler â€” ranks are no longer bought here; depth is earned
  // through use (rank 2 auto) and a GM-marked defining moment (rank 3).
}

// SNG-129: the node SILHOUETTE for a function family (shape carries the primary family; color retained â†’
...
function renderSkillWheel(selectedId = null, status = "") {
  const idx = CONTENT.traditionIndex;
  if (!idx) { renderSkillGraph(selectedId); return; } // no ring loaded â†’ fall back to the list graph
  ...
  const svg = `<svg id="skill-svg" viewBox="0 0 ${S.size} ${S.size}" class="world-map skill-wheel" preserveAspectRatio="xMidYMid meet"><g class="graph-vp">
    ...
    <text x="${S.cx}" y="${S.cy - S.rPrecursor - 8}" text-anchor="middle" class="wheel-precursor-label">Â· PRECURSOR â€” the substrate, outside the poles Â·</text>
    ${/* faint spokes */""}
    ...
    ${/* braids â€” known discoveries only */""}
    ${braids.map(b => { const A = nodeFor(b.t1), B = nodeFor(b.t2); if (!A) return "";
      if (b.kind === "cross-pole" && B) return `<line x1="${A.x}" y1="${A.y}" x2="${B.x}" y2="${B.y}" class="wheel-braid cross-pole"><title>${esc(b.name)} â€” a cross-pole braid: holding an axis whole</title></line>`;
      if (b.kind === "kin" && B) return `<path d="M ${A.x} ${A.y} Q ${S.cx + (A.x + B.x - 2 * S.cx) * 0.7} ${S.cy + (A.y + B.y - 2 * S.cy) * 0.7} ${B.x} ${B.y}" class="wheel-braid kin" fill="none"><title>${esc(b.name)} â€” a kin braid</title></path>`;
      if (B) return `<line x1="${A.x}" y1="${A.y}" x2="${B.x}" y2="${B.y}" class="wheel-braid cross-axis"><title>${esc(b.name)} â€” a cross-axis braid</title></line>`;
      return ""; }).join("")}
    ...
    ${/* ability nodes â€” SNG-129: function SHAPES (silhouette = primary family) + precursor SEALED state +
    ...
    ${(() => {
      ...
      return m.nodes.map(nd => {
        ...
        const title = nd.name + " â€” " + traditionLabel(nd.cls) + " Â· Tier " + nd.tier + ((nd.functions || []).length ? " Â· " + nd.functions.join(", ") : "") + (nd.effCost != null ? " Â· âš¡" + nd.effCost : "") + (sealed ? " Â· SEALED (precursor â€” earned through play, never bought)" : nd.owned ? " (owned)" : nd.closed ? " Â· CLOSED (your antipode)" : nd.barred ? " Â· barred" : nd.dim ? " Â· costs more" : "");
        return `<g class="${cls}" data-wheelnode="${esc(nd.id)}"><title>${esc(title)}</title>
          ...
          ${sealed ? `<text x="${nd.x}" y="${(nd.y + 3).toFixed(1)}" text-anchor="middle" class="precursor-mark">âœ¦</text>` : opened ? `<text x="${(nd.x + r + 2).toFixed(1)}" y="${(nd.y - r).toFixed(1)}" class="precursor-mark open">âœ¦</text>` : ""}
          ...
          ${lbl ? `<text x="${lbl.x.toFixed(1)}" y="${lbl.y.toFixed(1)}" text-anchor="middle" class="wheel-node-label ${nd.owned ? "owned" : ""}">${esc(String(nd.name).slice(0, 16))}${nd.effCost != null && (nd.owned || nd.reachable) ? ` âš¡${nd.effCost}` : ""}</text>` : ""}
        </g>`;
      }).join("");
    })()}
  </g></svg>`;
  ...
  const gateLine = sel ? (sel.closed ? "ðŸš« CLOSED â€” your antipode; only a cross-pole braid crosses it"
    : sel.band === "primary" ? "âœ¦ your primary domain â€” all tiers, no penalty"
    : sel.band === "adjacent" ? (sel.allowed ? "free â€” kin of your primary (no capstones)" : "ðŸ”’ near a people is not being of them â€” no capstones")
    : sel.band === "secondary" ? (sel.allowed ? "your secondary â€” up to Tier III" : "ðŸ”’ your secondary tops out at Tier III")
    : sel.band === "tertiary" ? (sel.allowed ? "your tertiary â€” up to Tier II" : "ðŸ”’ your tertiary tops out at Tier II")
    : sel.band === "folk" ? "open â€” a folk craft of the Valley"
    : sel.band === "far" ? `costs more â€” ${sel.penalty}Ã— skill points, ${Math.max(2, sel.penalty)} steps out`
    : "learnable") : "";
  ...
  const details = sel ? `<div class="map-details">
    <div class="map-details-head"><h3>${esc(sel.name)}</h3>
      <span class="rep-band" style="border-color:${traditionColor(sel.cls)};color:${traditionColor(sel.cls)}">${esc(traditionLabel(sel.cls))} Â· Tier ${sel.tier}</span>
      ${sel.owned ? `<span class="rep-band trusted">owned</span>` : sealedSel ? `<span class="rep-band" style="border-color:var(--accent);color:var(--accent)">âœ¦ sealed</span>` : ""}</div>
    <div class="hint">${sealedSel ? "âœ¦ a precursor craft of the substrate â€” outside the poles" : esc(gateLine)}${!sealedSel && sel.effCost != null ? ` Â· âš¡${sel.effCost} energy${selAb?.energyCost && sel.effCost !== selAb.energyCost ? ` (base ${selAb.energyCost})` : ""}` : ""}</div>
    ...
    ${selAb?.minted?.kind === "braid" && sel.owned ? `<div class="hint" style="margin-top:4px"><button class="link-btn" id="braid-card-rename" data-braid="${esc(selAb.id)}">âœŽ Rename this braid</button></div>` : ""}
    ${sealedSel
      ? `<div class="precursor-seal-note">Precursor crafts aren't taught or bought. <strong>A door opens only when the fiction earns it</strong> â€” walking the Old Roads, a precursor-touched teacher, a discovery in play. There's no button; keep playing toward it, and one day it may open.</div>`
      : sel.precursorUnlocked ? `<div class="hint" style="color:var(--accent)">âœ¦ a door has opened â€” this precursor craft was earned, and can be learned.</div>${skillSelectionActions(selAb)}`
  ...
  chrome(`<div class="screen" style="max-width:980px">
    ...
    ${/* SNG-124 Phase B: function-family filter â€” tap a family to light up every craft that does it, across all traditions (both axes at once). */""}
    <div class="fn-filter-row">
      <span class="hint" style="margin-right:2px">By function:</span>
      ${FUNCTION_FAMILIES.map(f => `<button class="fn-filter ${familyClass(f)} ${wheelFnFilter.has(f) ? "on" : ""}" data-fnfilter="${f}" title="${f} â€” highlight every craft that can ${f.toLowerCase()}" style="${wheelFnFilter.has(f) ? `background:${FAMILY_COLOR[f]};color:var(--bg);border-color:${FAMILY_COLOR[f]}` : `color:${FAMILY_COLOR[f]};border-color:${FAMILY_COLOR[f]}`}">${FAMILY_GLYPH[f]} ${f}</button>`).join("")}
      ${wheelFnFilter.size ? `<button class="fn-filter" id="fn-filter-clear" title="Clear the function filter">âœ• clear</button>` : ""}
    </div>
    <p class="hint" style="margin-bottom:8px"><span class="grow-badge">${character.skillPoints || 0} skill point${(character.skillPoints || 0) === 1 ? "" : "s"}</span> Â· ${breadthUsed(character)} of ${breadthCap(character, CONTENT.skillCapacity)} crafts${atCapacity(character, CONTENT.skillCapacity) ? " â€” at capacity" : ""}</p>
...
function renderSkillGraph(selectedId = null, status = "") {
  ...
  // layout: class column, tierâ†’name order within column
  const pos = {};
  ...
  const svg = `<svg id="skill-svg" viewBox="0 0 ${width} ${height}" class="world-map skill-graph" preserveAspectRatio="xMidYMid meet"><g class="graph-vp">
    ...
    ${model.nodes.map(n => { const p = pos[n.id]; const r = 6 + (n.levelReq - 1) * 1.5;
      ...
      return `<g class="${cls}" data-skillnode="${esc(n.id)}"><title>${esc(n.name + " â€” Tier " + n.tier + " (L" + n.levelReq + ")" + (n.owned ? ", rank " + n.rank : "") + (n.locked ? " ðŸ”’ " + n.lockText : ""))}</title>
        ...
        ${n.locked ? `<text x="${p.x}" y="${p.y - r - 3}" text-anchor="middle" class="graph-lock">ðŸ”’</text>` : ""}
        ${n.forks ? `<text x="${p.x - r - 5}" y="${p.y + 4}" text-anchor="middle" class="graph-fork ${n.forkChosen ? "chosen" : ""}">â‘‚</text>` : ""}
  ...
  const details = sel ? `<div class="map-details">
    <div class="map-details-head"><h3>${esc(sel.name)}</h3>
      <span class="rep-band" style="border-color:${traditionColor(sel.cls)};color:${traditionColor(sel.cls)}">${esc(traditionLabel(sel.cls))} Â· Tier ${sel.tier}</span>
      ${sel.owned ? `<span class="rep-band trusted">owned Â· rank ${sel.rank}</span>` : ""}</div>
    <div class="hint">Level requirement: ${sel.levelReq}${sel.gated ? ` Â· ${(() => { const g = gateFor(sel.id, CONTENT.attributeGates); return `needs ${g.subAttribute} ${g.learnMin} (rank 3: ${g.rank3Min})`; })()}` : ""}${sel.locked ? ` Â· ðŸ”’ ${esc(sel.lockText)}` : ""}</div>
    <p class="map-details-desc">${esc(selAb?.description || "")}</p>
    ${sel.forks ? `<div class="codex-fact fork-note"><strong>â‘‚ Fork at rank ${sel.forkAt}:</strong> ${sel.forkChosen ? `specialized as <em>${esc(sel.forkChosen)}</em> â€” <span class="fp-cannot">${esc(sel.forkLocked)} locked forever</span>` : "a permanent A-or-B specialization when you rank into it â€” the path you don't take locks."}</div>` : ""}
    ${skillSelectionActions(selAb)}
  </div>` : "";

  chrome(`<div class="screen" style="max-width:960px">
    <h2>Skill Graph</h2>
    <p class="hint" style="margin-bottom:8px">Every ability by class (color) and Tier Iâ€“V (size). Filled = owned. Gold ring = aspired Â· teal ring = ripe to claim Â· ðŸ”’ = gated. Diamonds are emergence techniques; lines link their components. <strong>Tap a node to learn or deepen it here.</strong> Scroll to zoom, drag to pan.</p>
    <p class="hint" style="margin-bottom:8px"><span class="grow-badge">${character.skillPoints || 0} skill point${(character.skillPoints || 0) === 1 ? "" : "s"}</span> Â· ${breadthUsed(character)} of ${breadthCap(character, CONTENT.skillCapacity)} crafts${atCapacity(character, CONTENT.skillCapacity) ? " â€” at capacity" : ""}</p>
    ${status ? `<div class="cs-block" style="border-left:3px solid var(--accent); margin-bottom:8px">${esc(status)}</div>` : ""}
    <div class="graph-wrap" id="graph-wrap">
      <div class="graph-zoom-ctl">
    ...
    <div style="display:flex; gap:8px; margin-top:12px">
      ...
      <button class="btn secondary" id="graph-wheel">âœ¦ Wheel view</button>
    </div>
  </div>`);
...
// desktop). Pure viewport work â€” a transform on the <g class="graph-vp"> group; state persists
...
// skill wheel and the skill graph, reset only on explicit back/fit buttons â€” so zooming the map and
...
function wireSkillGraphViewport() {
  ...
  // SNG-168: PINCH. Zoom was wheel-only and a phone has no wheel â€” `touches[1]` appeared zero times
  ...
  // unguarded is why the wiring could not simply be called on the world and location tiers â€” it
...
/** SNG-094: the Level-Up window â€” one place to spend skill points. DEEPEN a craft you know (rank up)
 *  or LEARN a new one (domain-gated, explained), with the SNG-084 helper text woven in. Stays open so
 *  a player with several points spends them in a row. Replaces hunting the sidebar's scattered â–² + the
...
function renderLevelUp(status = "") {
  ...
  const learnRow = ab => {
    ...
    const band = dv.band === "far" ? " Â· far" : dv.band === "adjacent" ? " Â· kin" : dv.band === "accord" ? " Â· open" : "";
    return `<div class="cs-ability ${blocked ? "locked" : ""}">
      <div><span class="tier-badge">${tierOf(ab.levelReq)}</span> <strong>${esc(ab.name)}</strong> <span class="hint">L${ab.levelReq || 1}${band}${cost > 1 ? ` Â· ${cost} pts` : ""}${ripe ? " Â· practiced (free)" : ""}</span></div>
      <div class="hint">${esc((r1 ? r1.grants : ab.description) || "").slice(0, 130)}</div>
      ${blocked
        ? `<span class="hint">ðŸ”’ ${!gate.ok ? esc(gate.why) : capBlock ? "at capacity â€” deepen instead" : "need " + cost + " point" + (cost > 1 ? "s" : "")}</span>`
  ...
  chrome(`<div class="screen" style="max-width:720px">
    <h2>â¬† Level Up ${infoDot("ability.ranks")}</h2>
    <p class="hint" style="margin-bottom:10px">You have <strong>${sp} skill point${sp === 1 ? "" : "s"}</strong> â€” points <strong>learn new crafts</strong> (breadth). <strong>Depth is earned through use</strong>, not bought. <span class="cap-line">${breadthUsed(character)} of ${breadthCap(character, CONTENT.skillCapacity)} crafts${cap ? " â€” at capacity" : ""}</span> ${infoDot("lock.capacity")}</p>
    ${status ? `<div class="cs-block" style="border-left:3px solid var(--accent)">${esc(status)}</div>` : ""}

    ${(() => { // SNG-124: "Suggested for you" â€” recommend skills that fill FUNCTION-COVERAGE gaps / suit your
      ...
      const gap = cov.missing.length ? `Your kit has no <strong>${cov.missing.join(", ")}</strong> yet â€” gaps worth filling.` : `Your kit already touches all ${cov.covered.length} function families.`;
      return `<div class="cs-block sug-block"><h3 class="codex-title" style="font-size:15px">Suggested for you <span class="hint" style="text-transform:none">â€” what would round out your kit</span></h3>
        ...
        ${suggestions.map(s => { const ab = fullCatalog()[s.abilityId]; return `<div class="cs-ability sug-row">
          <div><strong>${esc(s.name)}</strong> ${functionChips(ab)}${s.cost != null ? ` <span class="hint" title="energy to use (effective)">âš¡${s.cost}</span>` : ""}</div>
          <div class="hint">âœ¦ ${esc(s.why)}</div>
          <button class="btn" data-lvllearn="${esc(s.abilityId)}">Learn</button>
        </div>`; }).join("")}
      </div>`;
    })()}

    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Your crafts ${infoDot("ability.ranks")} <span class="hint" style="text-transform:none">â€” depth is earned through use, not points</span></h3>
      ${rankRows.map(r => { const p = rankProgress(character, r.a.abilityId); return `<div class="cs-ability">
        <div><strong>${esc(r.ab.name)}</strong> <span class="cs-ranks">${[1, 2, 3].map(n => `<span class="${n <= r.a.level ? "cs-rank-on" : "cs-rank-off"}">${n <= r.a.level ? "â—" : "â—‹"}</span>`).join("")}</span> <span class="hint">${r.now ? esc(r.now.name) : ""}</span></div>
        ${r.next ? `<div class="hint">â†’ ${esc(r.next.name)}: ${esc(r.next.grants || "")}</div>` : ""}
        <div class="hint ${p.ripe ? "practiced" : ""}">${esc(p.text)}</div>
      </div>`; }).join("") || "<div class='insight'>no crafts yet â€” learn one below</div>"}
    </div>

    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Learn a new craft ${infoDot("circle.domains")} <span class="hint" style="text-transform:none">â€” broaden into your domains</span></h3>
      ${cap ? `<div class="hint" style="margin-bottom:6px">You're at capacity â€” new points now deepen what you know. ${infoDot("lock.capacity")}</div>` : ""}
      ${Object.keys(byTrad).length
        ...
        : "<div class='insight'>nothing new to learn at this level â€” deepen a craft, or play on to reach the next tier</div>"}
    </div>

    <button class="btn secondary" id="lvl-back">Done</button>
  </div>`);

  // ability-arch v2: no data-lvlrank handler â€” depth is earned through use, not bought.
  for (const b of app.querySelectorAll("[data-lvllearn]")) b.onclick = () => {
    ...
    if (r.ok) { if (free) dropAspiration(character, id); saveCharacter(character); renderLevelUp(`Learned ${fullCatalog()[id]?.name}${free ? " â€” no point spent" : ""}.`); }
 ...
 *  lineage grant. Tap â†’ the one popover surface shows its effect + description + mods. Scales with the roster. */
function aptitudeChips() {
  const held = (CONTENT.rules.playerAptitudes || []).filter(a => character.aptitudes?.includes(a.id));
  if (!held.length) return `<span class="insight">No marked tendencies yet â€” play a while and who you are will show.</span>`;
  ...
  return `<div class="aptitude-chips">${held.map(a => {
    ...
    const detail = `${name}${isLineage ? " Â· lineage" : ""}${isFading ? " Â· fading" : ""}\n${a.description || ""}${modLine ? `\n\n${modLine}` : ""}`;
    return `<button class="aptitude-chip axis-${esc(a.axis || "earned")}${isFading ? " fading" : ""}${isLineage ? " lineage" : ""}" data-aptchip="${esc(detail)}" title="tap for detail">${esc(name)}${isFading ? " âŒ" : ""}</button>`;
  }).join("")}</div>`;
}
...
function renderCharacterScreen() {
  ...
  chrome(`<div class="screen" style="max-width:760px">
    <div class="cs-header">
      ...
      <div class="cs-header-text">
        ...
        <div class="hint">${esc(character.origin)} Â· ${esc(character.background)} Â· level ${character.level} â€” ${character.xp}/${xpNeed} xp ${infoDot("growth.level")}${character.pendingSubPoints ? ` Â· <span class="grow-badge">+${character.pendingSubPoints} attribute</span>` : ""}${character.skillPoints ? ` Â· <span class="grow-badge">${character.skillPoints} skill</span>` : ""}</div>
        ${character.form ? `<div class="hint" style="margin-top:4px"><em>Form:</em> ${esc(character.form)}</div>` : ""}
        <div style="margin-top:6px; display:flex; gap:8px; flex-wrap:wrap">
          <button class="opt" id="cs-form" title="Describe this character's physical form / species so the portrait renders it (e.g. an Ent, a construct)">âœŽ Appearance</button>
          ${imagesEnabled() && !character.portrait ? `<button class="opt" id="cs-gen-portrait">âœ¦ Generate portrait</button>` : ""}
          ${imagesEnabled() && character.portrait ? `<button class="opt" id="cs-regen-portrait" title="Re-mint the portrait">â†» New portrait</button>` : ""}
          <button class="opt" id="cs-gallery">ðŸ–¼ Gallery${galleryCount ? ` (${galleryCount})` : ""}</button>
        </div>
      </div>
    </div>
    ${Object.values(b).some(v => v) ? `<div class="cs-block"><h3 class="codex-title" style="font-size:15px">Story</h3>
      ...
      ${(() => { // SNG-134 P1: the story EVOLVES â€” show the lived "story so far" (SNG-109 paragraph) once it
        ...
        return lived ? `<p class="map-details-desc" style="margin-top:8px">${esc(lived)}</p><div class="hint">â€” your story so far, as it has grown</div>`
    ...
    ${character.domains?.primary ? (() => {
      // SNG-101/102: the great-circle domains you hold â€” station, ceiling, promotions, foreclosures.
      ...
      const rows = [["primary", 5], ["secondary", 3], ["tertiary", 2]].filter(([k]) => character.domains?.[k]).map(([k, def]) => {
        ...
        return `<div class="codex-fact"><strong>${esc(traditionLabel(t))}</strong> â€” ${k}${promoted ? " Â· <em>promoted</em>" : ""}, to Tier ${ROM[c]}</div>`;
      }).join("");
      const acq = (character.domainsAcquired || []).map(t => `<div class="codex-fact"><strong>${esc(traditionLabel(t))}</strong> â€” acquired, to Tier ${ROM[ceilOf(t, 1)]}</div>`).join("");
      const fore = (character.foreclosed || []).length ? `<div class="hint">Foreclosed â€” reachable only as a braid: ${character.foreclosed.map(traditionLabel).join(", ")}</div>` : "";
      const promos = ["tertiary", "secondary"].map(k => {
        ...
        if (!e.eligible) return ""; return `<div class="cs-ability" style="border-left:3px solid var(--accent)"><strong>âœ¦ You may rise:</strong> become ${esc(traditionLabel(e.trad))} as your ${e.to} <button class="grow-btn practiced" data-promote="${k}">Promoteâ€¦</button></div>`;
      }).join("");
      // SNG-102: a new people you may JOIN â€” any tradition you have a willing teacher for that's acquirable
      ...
      const acqs = acqTargets.filter(t => acquirable(character, t, CONTENT.rules, { traditionIndex: CONTENT.traditionIndex }).ok)
        .map(t => `<div class="cs-ability" style="border-left:3px solid var(--accent)"><strong>âœ¦ You may join:</strong> become of the ${esc(traditionLabel(t))} (enters at Tier I) <button class="grow-btn practiced" data-acquire="${esc(t)}">Joinâ€¦</button></div>`).join("");
    ...
    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Abilities ${infoDot("ability.tiers")} <span class="cap-line" style="text-transform:none">${breadthUsed(character)} of ${breadthCap(character, CONTENT.skillCapacity)} skills${atCapacity(character, CONTENT.skillCapacity) ? " â€” at capacity; new points learn other crafts" : ""}</span>${atCapacity(character, CONTENT.skillCapacity) ? infoDot("lock.capacity") : ""}</h3>
      ...
      ${character.abilities.map(a => { const ab = fullCatalog()[a.abilityId]; if (!ab) return ""; const cost = effectiveEnergyCost(ab, character, rules);
        ...
        return `<div class="cs-ability"><span class="tier-badge">${tierOf(ab.levelReq)}</span> <strong>${esc(ab.name)}</strong> <span class="hint">(${ab.powerSystem === "learned" ? "learned" : ab.powerSystem}) Â· ${cost} energy${cost < ab.energyCost ? ` (was ${ab.energyCost})` : ""}</span>
          <span class="cs-ranks">${[1, 2, 3].map(r => `<span class="${r <= a.level ? "cs-rank-on" : "cs-rank-off"}" title="${esc(ab.tree?.[r - 1]?.name || "")}">${r <= a.level ? "â—" : "â—‹"}</span>`).join("")}</span>
      ...
      ${(character.discoveries || []).map(d => `<div class="discovery" title="${esc(d.description)}">âœ¦ ${esc(d.name)} (discovered technique)</div>`).join("")}</div>
    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Aspirations <span class="hint" style="text-transform:none">(declare what you're working toward â€” practice makes it free)</span></h3>
      ${(character.practice?.aspirations || []).map(a => { const ab = fullCatalog()[a.abilityId]; const ripe = aspirationRipe(character, a.abilityId, rules); const need = rules.practice?.aspirationRipe ?? 10;
        return `<div class="cs-attr"><span class="cs-attr-name" style="width:140px">${esc(ab?.name || a.abilityId)}</span>
          ...
          ${ripe && character.level >= (learnLevelReq(ab) ?? 99) ? `<button class="grow-btn practiced" data-asplearn="${esc(a.abilityId)}" title="Fully practiced â€” learn free">âœ“</button>` : ""}
          <button class="grow-btn" data-aspdrop="${esc(a.abilityId)}" title="Drop aspiration" style="background:var(--panel2); color:var(--ink-dim)">Ã—</button>
      ...
      ${(character.practice?.aspirations || []).length < (rules.practice?.maxAspirations ?? 2) ? `
        <select id="asp-pick" style="margin-top:6px; max-width:280px">
          <option value="">Aspire towardâ€¦</option>
    ...
    ${(() => {
      ...
      return `<div class="cs-block"><h3 class="codex-title" style="font-size:15px">Ripe to claim <span class="hint" style="text-transform:none">(practice has ripened these â€” claim now or let the GM offer them in play)</span></h3>
    ...
    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Active quests ${infoDot("quest.routes")}</h3>
      ${(character.quests || []).filter(q => q.status === "active").map(q => `<div class="codex-fact"><strong>${esc(q.title)}</strong> â€” ${esc(q.progress?.slice(-1)[0] || q.summary)}</div>`).join("") || "<div class='insight'>none</div>"}</div>
    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Companions ${infoDot("companion.bond")}</h3>
      ${activeCompanions(character, CONTENT.companions).map(c => `<div class="codex-fact"><strong>${esc(c.name)}</strong> â€” assists: ${(c.assistTags || []).join(", ")}</div>`).join("") || "<div class='insight'>traveling alone</div>"}</div>
    ${canLevelUp(character) ? `<button class="btn" id="cs-levelup" style="margin-top:10px; margin-right:8px">â¬† Level Up${character.skillPoints ? ` (${character.skillPoints})` : ""}</button>` : ""}
    <button class="btn secondary" id="cs-skillgraph" style="margin-top:10px; margin-right:8px">âœ¦ Skill Wheel</button>
    <button class="btn secondary" id="cs-chronicle" style="margin-top:10px; margin-right:8px" title="The story so far â€” the deeds, bonds, and standing you've accreted, read back to you">ðŸ“œ The Chronicle</button>
    <button class="btn secondary" id="cs-repair" style="margin-top:10px; margin-right:8px" title="Fix what the game got wrong at creation â€” domains, background, form, or an ability you never chose. No arguing with the GM.">ðŸ”§ Repair character</button>
  ...
  // ability-arch v2: mastery of a craft that forks at rank 3 â€” the GM marked the defining moment; the
  ...
  for (const btn of app.querySelectorAll("[data-masteryfork]")) btn.onclick = () => {
    ...
    renderForkModal(id, (key) => {
      ...
      autoVerifyLeg("b5-fork", "chose a branch fork â€” the other path locks");
  ...
  // SNG-101: promotion â€” the player commits (Law 9), the modal names the foreclosure cost.
  for (const btn of app.querySelectorAll("[data-promote]")) btn.onclick = () => renderPromotionModal(btn.dataset.promote);
  // SNG-102: acquisition â€” join a new people (Tier I), forecloses their antipode. Player commits.
  ...
  if (formB) formB.onclick = () => {
    ...
    const next = prompt("Describe this character's physical form / species â€” the portrait leads with this.\n(e.g. \"a towering treefolk of bark and heartwood, moss-bearded, eyes like knots of amber\")", cur);
    if (next === null) return;
    character.form = next.trim() || undefined;
    // a new form means a new portrait â€” re-mint so the picture matches
...
/** SNG-110: re-mint the portrait with optional per-generation intent â€” a one-off scene description
...
function regeneratePortraitFlow() {
  const override = prompt("New portrait â€” describe this specific scene or look, or leave blank to re-mint from the character as they are.\n(This is used for THIS image only â€” it doesn't change your saved appearance.)", "");
...
/** SNG-035: the Saga gallery â€” every image this character has accrued (portraits, born-with-image
 *  people/places, moment art), newest first. */
function renderGallery() {
  const gallery = character.gallery || [];
  chrome(`<div class="screen" style="max-width:860px">
    <h2>${esc(character.name)} â€” Gallery</h2>
    <p class="hint" style="margin-bottom:12px">${imagesEnabled()
      ...
      : "Image generation is off. Turn on <strong>Settings â†’ Scene &amp; item art â†’ Generate</strong> to start seeing the world."}</p>
    ${gallery.length ? `<div class="gallery-grid">${gallery.map((g, gi) => `
      <figure class="gallery-item">
        ...
        <button class="gallery-del" data-galdel="${esc(g.url)}" title="Remove this image">âœ•</button>
        ${character.portrait === g.url ? "" : `<button class="gallery-pick" data-galpick="${esc(g.url)}" title="Make this the character's portrait">â˜… Set as portrait</button>`}
        <figcaption>${esc(g.caption || g.kind)}${character.portrait === g.url ? ` <span class="rep-band trusted">portrait${character.portraitPinned ? " Â· pinned" : ""}</span>` : ""}${g.worldDay ? ` <span class="hint">Â· world-day ${g.worldDay}</span>` : ""}</figcaption>
      </figure>`).join("")}</div>`
      : "<div class='insight'>No images yet â€” a portrait is minted at creation (with art on), and the world fills in as you play.</div>"}
  ...
  for (const b of app.querySelectorAll("[data-galdel]")) b.onclick = () => {
    ...
    if (wasPortrait) { character.portraitPinned = false; if (imagesEnabled()) ensureCharacterPortrait(character, { force: true, milestone: "reforged" }); } // the pinned face is gone â†’ unpin + never leave the character imageless
    saveCharacter(character);
    renderGallery();
  };
  // SNG-139: the player curates their own face â€” pick any gallery image as the portrait. A user pick PINS
...
/** SNG-085: the Repair panel â€” expose engine/corrections.js DIRECTLY to the player. A bug the game
 ...
 *  GM's in-fiction repair (`applyStateOps`) â€” it just can't argue. REPAIR, NOT WISH: the engine refuses
 *  anything that ADVANCES (xp/levels/abilities-out-of-domain); power still comes from play. */
function repairLogLine(e) {
  const day = e.worldDay != null ? ` <span class="hint">Â· world-day ${e.worldDay}</span>` : "";
  let txt;
  switch (e.kind) {
    case "field": txt = `${esc(e.field)}: <s class="hint">${esc(e.from || "â€”")}</s> â†’ <strong>${esc(e.to || "â€”")}</strong>`; break;
    case "domain": txt = `${esc(e.field)} domain: <s class="hint">${esc(e.from ? traditionLabel(e.from) : "â€”")}</s> â†’ <strong>${esc(e.to ? traditionLabel(e.to) : "â€”")}</strong>`; break;
    case "remove": txt = `removed ${esc(e.entity)} <strong>${esc(e.id)}</strong>`; break;
    case "quest": txt = `quest ${esc(e.id)} â†’ <strong>${esc(e.to?.status || "?")}</strong>`; break;
    case "location": txt = `re-anchored â†’ <strong>${esc(e.to)}</strong>`; break;
    case "codexFact": txt = `codex fact ${esc(e.id)} corrected`; break;
    case "abilityRank": txt = `${esc(e.id)} rank: <s class="hint">${esc(String(e.from))}</s> â†’ <strong>${esc(String(e.to))}</strong>`; break;
    case "bond": txt = `relationship with ${esc(e.id)} set right`; break;
    case "vital": txt = `${esc(e.field)}: <s class="hint">${esc(String(e.from))}</s> â†’ <strong>${esc(String(e.to))}</strong>`; break;
    case "attribute": txt = `${esc(e.field)}: <s class="hint">${esc(String(e.from))}</s> â†’ <strong>${esc(String(e.to))}</strong>`; break;
...
function renderRepairScreen(note = "") {
  ...
  const tradOptions = (sel) => `<option value="">â€” none â€”</option>` +
  ...
  chrome(`<div class="screen" style="max-width:720px">
    <h2>ðŸ”§ Repair ${esc(character.name)}</h2>
    <p class="hint" style="margin-bottom:6px">Fix what the game got wrong when this character was made â€” no arguing with the GM. Every change is validated, logged, and reversible.</p>
    <p class="hint" style="margin-bottom:14px"><strong>Repair, not wish:</strong> you can correct data that is <em>wrong</em>, but nothing here grants power â€” xp, levels, and abilities still come from play. Stripping an ability you never chose is a repair, not a loss; it frees your breadth so you can re-pick with your own skill points.</p>
    ...
    ${anomalies.length ? `<div class="cs-block" style="border-left:3px solid #c0392b">
      <h3 class="codex-title" style="font-size:15px">âš  Issues the game spotted <span class="hint" style="text-transform:none">â€” ${anomalies.length}</span></h3>
      <p class="hint" style="margin-bottom:8px">These look like data that drifted. Tick to fix, then Apply â€” each is a <strong>repair</strong> (two records merged into one, a vital re-synced to its max, or a rank set back to what practice earned), never a loss of anything you earned.</p>
      ${anomalies.map((an, i) => `<label class="rating-check"><input type="checkbox" data-fix="${i}" checked> ${esc(an.note)}${an.kind === "rankOverPractice" ? ` <span class="hint">(â†’ rank ${an.suggestRank})</span>` : ""}</label>`).join("")}
    </div>` : ""}

    <div class="field"><label>Why this repair (optional â€” recorded in the log)</label>
    ...
    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Your people &amp; craft</h3>
      ...
      <div class="field"><label>Form (physical description â€” leads the portrait)</label>
        <input id="rp-form" value="${esc(character.form || "")}" placeholder="e.g. a lean woman with ash-grey braids and a mourner's shawl"></div>
    </div>

    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Domains <span class="hint" style="text-transform:none">â€” the poles you draw from</span></h3>
      <p class="hint" style="margin-bottom:8px">Changing a domain re-sets which crafts you can <em>learn next</em>. Abilities you already hold are kept (grandfathered) â€” use the strip list below to remove ones you should never have had.</p>
    ...
    <div class="cs-block"><h3 class="codex-title" style="font-size:15px">Strip an ability you never chose <span class="cap-line" style="text-transform:none">${used} of ${cap} breadth used</span></h3>
      <p class="hint" style="margin-bottom:8px">Tick any ability the game gave you that belongs to the wrong pole. Stripping it frees breadth so you can pick your own â€” you do <em>not</em> lose earned power, and your skill points are untouched.</p>
    ...
    ${Object.keys(character.npcRegistry || {}).length ? `<div class="cs-block"><h3 class="codex-title" style="font-size:15px">Set a person's gender <span class="hint" style="text-transform:none">â€” the game rendered someone wrong? fix it here</span></h3>
      <p class="hint" style="margin-bottom:8px">Gender is explicit data now â€” the portrait and narration read it. Type a value to set or correct it; leave blank to keep. Free and inclusive (woman, man, nonbinary, â€¦). A corrected portrait re-mints with the right gender.</p>
      ${Object.values(character.npcRegistry).slice(0, 30).map(n => `
        <div class="cs-attr"><span class="cs-attr-name" style="width:auto; flex:1">${esc(n.name || n.id)} <span class="hint">${esc(n.gender || "â€” unset â€”")}</span></span>
          <input data-npcgender="${esc(n.id)}" placeholder="woman / man / â€¦" value="" style="max-width:160px"></div>`).join("")}
    </div>` : ""}
  ...
  document.getElementById("rp-apply").onclick = () => {
    ...
    // SNG-143: set/correct a known person's gender â€” derive obvious pronouns, leave the rest free
    ...
    // SNG-137: one-click fixes for the anomalies the game spotted â€” each maps to a repair op
    ...
    if (!ops.length) { renderRepairScreen("Nothing changed â€” adjust a field, then Apply."); return; }
    // applyStateOps caps at 6 ops per call by design â€” chunk so a big repair applies fully.
    ...
    // a new form means a new portrait â€” re-mint so the picture matches (mirrors the sheet's form editor)
    ...
    let out = applied.length ? "âœ“ " + applied.map(describeCorrection).join("; ") + "." : "No changes applied.";
    if (stripped) out += ` Your breadth is freed${character.skillPoints > 0 ? ` â€” spend your ${character.skillPoints} skill point${character.skillPoints === 1 ? "" : "s"} on your own people's crafts in the Character screen` : ""}.`;
...
function renderInventoryScreen(openName = null) {
  ...
  chrome(`<div class="screen" style="max-width:680px">
    <h2>Inventory â€” ${esc(character.name)}</h2>
...
function renderQuestDetail(questId, guidance = null, loading = false) {
  ...
  chrome(`<div class="screen" style="max-width:680px">
    ...
    <div class="hint">${q.giver ? `From ${esc(q.giver)} Â· ` : ""}started ${q.startedAt ? "day " + esc(String(q.startedAtDay ?? "").trim() || new Date(q.startedAt).toLocaleDateString()) : "a while back"}</div>
    ${q.giverEntityId && character.codex?.topics?.[q.giverEntityId] ? `<button class="codex-link" data-questgiver="${esc(q.giverEntityId)}" style="margin-top:6px">â—ˆ ${esc(character.codex.topics[q.giverEntityId].label)} in Codex</button>` : ""}
    ...
    <div style="margin-top:14px">
      ...
      ${loading ? `<div class="thinking">The GM considers your situationâ€¦</div>` : `<button class="btn secondary" id="quest-guidance">Ask the GM for guidance</button>`}
      <div class="hint" style="margin-top:6px">Spoiler-safe: possible next steps, who might help, how hard it looks â€” never the hidden truth.</div>
    </div>
  ...
  if (rBtn) rBtn.onclick = () => {
    ...
    applyQuestUpdates(character, [{ op: "complete", questId: q.id, xpReward: 25, note: "(resolved by player â€” tracker repair)" }]);
    saveCharacter(character);
  ...
  if (gBtn) gBtn.onclick = async () => {
    ...
    // BATCH-11 Â§23: the "quest" view â€” focused window (4 turns), no scene context
...
/** SNG-065: a structured quest â€” stakes, engine-testable stages, the routes this character's
...
function renderStructuredQuestDetail(q) {
  ...
  const stageRow = (s, i) => { const done = (q.completedStages || []).includes(s.id); const current = !done && i === (q.stageIndex || 0);
    return `<div class="quest-stage ${done ? "done" : current ? "current" : ""}">
      <div class="quest-stage-obj">${done ? "âœ“ " : current ? "â–¶ " : "â—‹ "}${esc(s.objective)}</div>
  ...
  chrome(`<div class="screen" style="max-width:720px">
    <div class="codex-kind">${esc(q.status)}${q.tier ? " Â· " + esc(q.tier) : ""}${q.axis ? " Â· " + esc(String(q.axis).replace(/_/g, " â†” ")) : ""}</div>
    <h2 style="margin-top:4px">${esc(q.title)}</h2>
    ...
    ${routes.map(r => `<div class="quest-route ${r.open ? "open" : ""}"><span class="quest-route-trad">${esc(traditionLabel(r.trad))}${r.open ? " âœ¦" : ""}</span> ${esc(r.text)}</div>`).join("")}
    ${/* SNG-162 Â§2: the outcome menu appears ONLY at the decision point. Showing every ending
          throughout is what invited clicking through a quest to see how it could go â€” and made
    ...
    ${!resolved && q.awaitingResolution ? `<h3 class="codex-title" style="font-size:15px;margin-top:16px">Resolve â€” decide what the truth is for</h3>
      <div class="hint" style="margin-bottom:8px">Every stage is behind you. Every ending is a real ending â€” what you choose changes the world durably, and you'll be able to go back and see it.</div>
    ...
    : !resolved ? `<div class="hint" style="margin-top:16px">This isn't finished yet. Play it â€” the stages close as you actually do them, and the endings appear when you reach the decision.</div>`
  ...
  for (const b of app.querySelectorAll("[data-outcome]")) b.onclick = () => {
    ...
    if (!confirm(`Resolve "${q.title}" as â€œ${o.name}â€? This is permanent and changes the world.`)) return;
    ...
    if (r.ok) {
      ...
      renderPlay(character.activeScene?.lastTurn || null, { aside: `${q.title} â€” ${o.name} (+${r.xp} xp).${changes.length ? "\n\nWhat changed: " + changes.join("; ") + "." : ""}` });
    } else alert(r.why || "Couldn't resolve.");
  };
}

/** SNG-BATCH-7 Phase 3 + SNG-065: the full quest log â€” available (startable, structured) /
...
function renderQuestLog() {
  ...
  const row = x => `<button class="quest quest-click" data-quest="${esc(x.id)}" style="display:block;width:100%;text-align:left;margin:3px 0">
      <span class="quest-title">${esc(x.title)}</span>${x.structured ? ` <span class="cost">structured${x.axis ? " Â· " + esc(String(x.axis).replace(/_/g, "â†”")) : ""}</span>` : x.giver ? ` <span class="cost">from ${esc(x.giver)}${x.giverEntityId && character.codex?.topics?.[x.giverEntityId] ? " â—ˆ" : ""}</span>` : ""}
  ...
  // SNG-065/SNG-112: authored quests startable HERE â€” giver present, at/adjacent to the quest's place, or thread already touched (not bare region).
  ...
  const availSection = avail.length ? `<div class="codex-group"><div class="codex-group-title">Available here (${avail.length})</div>${avail.map(def => `
      <div class="quest" style="margin:3px 0">
        <span class="quest-title">${esc(def.name)}</span> <span class="cost">${esc(String(def.axis || "").replace(/_/g, "â†”"))}</span>
        <div class="quest-note"><strong>Stakes:</strong> ${esc(String(def.stakes || ""))}</div>${/* SNG-076: authored stakes render IN FULL â€” the prose IS the game */""}
  ...
  chrome(`<div class="screen" style="max-width:680px">
    ...
    ${q.length ? section("Active", group("active")) + section("Resolved", group("resolved")) + section("Completed", group("completed")) + section("Failed", group("failed"))
      : (availSection ? "" : "<div class='insight'>No undertakings yet â€” the valley will provide.</div>")}
  ...
  for (const b of app.querySelectorAll("[data-startquest]")) b.onclick = () => {
    ...
    // uses â€” the generated personalArc is not in CONTENT.quests, so looking only there
...
// ---------- SNG-109: THE CHRONICLE â€” the accreted self, read back to the player ----------
...
// bonds), standing, and the arc. Read-only â€” the Chronicle displays state, never mutates it. The
...
function chronicleViewCtx(character) {
  ...
  const bonds = Object.values(reg)
    ...
    .slice(0, 8)
    .map(n => ({ id: n.id, name: n.name, label: relationshipLabel(n) })); // SNG-134: id â†’ the shared name-hover
  const traditions = [character.domains?.primary, character.domains?.secondary, character.domains?.tertiary].filter(Boolean);
  // BATCH-12 Â§3: this used to iterate ONLY the character's own three domains, so standing with any
  // OTHER people could never appear no matter what the character earned â€” a second, quieter reason
  // Erik's standing panel read empty. standingRoster returns every holder with a non-zero score,
  // people and settlements in one shape (Â§3e), strongest first.
  ...
  const dshape = traditions.map(traditionLabel).join(" Â· ");
  ...
  const arc = `${aim ? aim + " â€” " : ""}${dshape}${fore}`.trim();
  return { bonds, standing, arc, ratingLine: ratingLineForGM() };
}

/** Write (or rewrite) the cached "story so far" paragraph. Cached by major-state hash â€” only regenerates
...
async function ensureChronicleParagraph(force = false) {
  ...
  try {
    ...
    character._chronicleError = e?.message === "NO_API_KEY" ? "Add your API key in Settings to write the chronicle." : "Couldn't write the chronicle just now â€” try again.";
  } finally {
...
function renderChronicle() {
  ...
  // player opted in (privacy â€” a chronicle is private until shared). Only shown when 2+ are visible.
  let family = [];
  try {
  ...
  const paraHtml = character._chronicleBusy
    ? `<div class="insight">writing your storyâ€¦</div>`
    : cache?.text
      ? `<p class="chronicle-para">${esc(cache.text)}</p>${stale ? `<div class="hint">the story has moved on since this was written â€” regenerate to catch it up</div>` : ""}`
      : `<div class="insight">${getApiKey() ? "Your story is just beginning â€” generate the chronicle to read it." : "Add your API key in Settings to write the chronicle."}</div>`;
  chrome(`<div class="screen" style="max-width:680px">
    <h2>The Chronicle</h2>
    ${character.portrait ? `<img class="cs-portrait" src="${esc(character.portrait)}" alt="${esc(character.name)}" data-lightbox="portrait" onerror="this.style.display='none'">` : ""}
    <h3 style="margin-top:6px">${esc(character.name)}${character.level ? ` Â· level ${character.level}` : ""}</h3>
    ${character._chronicleError ? `<div class="hint">${esc(character._chronicleError)}</div>` : ""}
    <section><h3>The story so far</h3>
      ${paraHtml}
      ${getApiKey() ? `<button class="btn secondary" id="chr-regen" style="margin-top:6px" ${character._chronicleBusy ? "disabled" : ""}>â†» ${cache?.text ? "Regenerate" : "Write it"}</button>` : ""}
    </section>
    <section><h3>Major deeds${deeds.length ? ` (${deeds.length})` : ""}</h3>
      ${deeds.length ? deeds.map(d => `<div class="chronicle-deed"><span class="rep-band ${(d.weight | 0) >= 0 ? "trusted" : "wary"}">${(d.weight | 0) > 0 ? "+" : ""}${d.weight | 0}</span> ${esc(d.description)}${d.worldDay != null ? ` <span class="cost">day ${d.worldDay}</span>` : ""}</div>`).join("") : "<div class='insight'>no deeds yet â€” the valley is waiting</div>"}
    </section>
    ...
    <section><h3>The arc</h3>
      ${(() => { // SNG-134 P1: show the SNG-133 personal arc (premise + current stage), not the frozen aim.
        ...
        if (pa && pa.premise) {
          ...
          return `<div class="chronicle-arc"><strong>${esc(pa.name)}</strong><p class="map-details-desc" style="margin-top:2px">${esc(pa.premise)}</p><div class="hint">${stage ? "Now: " + esc(stage.objective) : "a thread waiting to be pulled â€” it will find you in play"}</div></div>`;
        }
    ...
    <section><h3>World authorship <span class="cost">what you've made real</span></h3>
      <div class="authorship-grid">
        ...
        <div class="auth-stat"><span class="auth-num accent">${auth.worldEffect}</span><span class="auth-label">world-effect â€” your fingerprint on the shared world</span></div>
      </div>
      <div class="hint" style="margin-top:6px">Of the world you've touched, <strong>${auth.authoredCount}</strong> ${auth.authoredCount === 1 ? "place/person was" : "were"} written before you â€” and you <strong>called ${auth.novelCount} new ${auth.novelCount === 1 ? "one" : "ones"} into being</strong>.${auth.novelCount ? ` Of those, ${auth.persisted.shared} ${auth.persisted.shared === 1 ? "is" : "are"} now canon the whole valley reads${auth.persisted.rumor ? `, ${auth.persisted.rumor} live${auth.persisted.rumor === 1 ? "s" : ""} on as rumor others may yet confirm` : ""}; ${auth.persisted.personal} real to you, not yet shared.` : ""}</div>
      ${/* CCODE-05: show the number that ACTUALLY gates promotion. This badge used to read
            "weight N" (birthWeight + score/2) â€” so the Low Lamp Inn showed "weight 13" while
      ...
      ${auth.topAttention.length ? `<div style="margin-top:8px"><div class="sys-label">What you're making real right now</div>${auth.topAttention.map(t => {
        ...
        // CCODE-06: the â­ lives HERE too, not only buried on the codex topic page. This row is
        // where the player learns something "needs 4" â€” showing a shortfall with no way to act on
        // it is the readout half-built. â­ Keep is +4, so one tap is usually the whole gap.
        return `<div class="known-npc"><span class="npc-name">${esc(t.name)}</span> <span class="cost">${esc(t.type || "")} Â· ${esc(t.tier)}</span> <span class="rep-band ${ready ? "trusted" : ""}" title="Shared-world canon needs ${NOMINATE_AT} engagement. Revisit +2 Â· interact +2 Â· tie to a quest +2 Â· â­ Keep +4. (realness weight ${t.weight})">${ready ? "ready for canon" : `${t.score}/${NOMINATE_AT} to canon Â· needs ${need}`}</span>${t.id && !ready ? ` <button class="npc-ctl" data-keepfrom="${esc(t.id)}" title="â­ Keep â€” mark this as one that matters to you (+4 toward canon)">â­</button>` : ""}</div>`;
      }).join("")}</div>` : ""}
    ...
    <section><h3>Sessions${sessions.length ? ` (${sessions.length})` : ""}</h3>
      ${sessions.length ? sessions.map(s => {
        ...
        const span = s.startDay != null && s.endDay != null ? (s.startDay === s.endDay ? `day ${s.startDay}` : `days ${s.startDay}â€“${s.endDay}`) : "";
        const title = `${s.ended ? "â–ª" : "â–¸"} ${span || s.id} Â· ${s.beats} beat${s.beats === 1 ? "" : "s"}${s.deeds.length ? ` Â· ${s.deeds.length} deed${s.deeds.length === 1 ? "" : "s"}` : ""}`;
        const recap = raw._recapBusy ? `<div class="insight">writing the recapâ€¦</div>` : raw.recap ? `<p class="chronicle-para">${esc(raw.recap)}</p>` : "";
        return `<details class="session-entry"><summary>${esc(title)}</summary><div class="sec-body">
          ${recap}
          ${getApiKey() && !raw.recap ? `<button class="btn secondary session-recap" data-sess="${esc(s.id)}" style="margin:2px 0 6px" ${raw._recapBusy ? "disabled" : ""}>âœ Write session recap</button>` : ""}
          ${s.deeds.length ? `${s.deeds.map(d => `<div class="chronicle-deed"><span class="rep-band ${d.weight >= 0 ? "trusted" : "wary"}">${d.weight > 0 ? "+" : ""}${d.weight}</span> ${esc(d.description)}</div>`).join("")}` : "<div class='insight'>a quiet span â€” no deeds of note</div>"}
          ${s.placesMinted.length ? `<div class="hint">âœ¦ Places you made: ${s.placesMinted.map(esc).join(", ")}</div>` : ""}
          ${s.peopleMet.length ? `<div class="hint">âœ¦ People first met: ${s.peopleMet.map(esc).join(", ")}</div>` : ""}
          ${s.canonPromoted.length ? `<div class="hint">â˜… Became shared-world canon: ${s.canonPromoted.map(esc).join(", ")}</div>` : ""}
          ${s.canonRumored?.length ? `<div class="hint">â—¦ Live on as rumor others may yet confirm: ${s.canonRumored.map(esc).join(", ")}</div>` : ""}
        </div></details>`;
      }).join("") : "<div class='insight'>your first session is being written now â€” play on</div>"}
  ...
  // CCODE-06: â­ Keep straight from the progress row (+4 â€” usually the whole remaining gap).
  for (const b of app.querySelectorAll("[data-keepfrom]")) b.onclick = () => {
    ...
    renderChronicle();
    // say what it bought â€” the tier language is otherwise invisible from this screen
    const note = s >= NOMINATE_AT ? `${rec.name} is ready for shared canon â€” it promotes on your next sync.` : `${rec.name} â€” ${s}/${NOMINATE_AT} toward canon.`;
...
async function ensureSessionRecap(sessionId, force = false) {
  ...
  try {
    ...
    raw._recapError = "Couldn't write the recap â€” try again.";
  }
...
// SNG-133: enrich the seeded personal arc via the model (best-effort) â€” a richer premise/stakes/stages/
// routes + a catalyst legend NPC, drawn from the bio (the SNG-132 arc as the quality bar). No API key â†’
...
async function enrichPersonalArc(char) {
  if (!char || !getApiKey()) return; // no key â†’ the light fallback arc stands
  try {
  ...
  } catch { /* the fallback arc stands â€” a hiccup never blocks creation */ }
}

// ---------- SNG-061: THE LIBRARY (the world's guide â€” open, readable, no discovery gate) ----------
...
// every render â€” the Library is the player's book, never the GM's. Distinct from the discovered
...
/** Generic lore â†’ readable HTML. Walks objects/arrays into headings + prose; filters GM fields. */
...
/** Minimal markdown â†’ HTML for the .md lore (headings, lists, paragraphs, inline emphasis). */
...
function libGreatCircle() {
  ...
  const circle = domainCircleSVG(idx, { selectable: () => false, centerTop: "the great circle", centerSub: "twelve axes Â· twenty-four peoples" });
  const rows = ringOrder(idx).map(t => { const tr = idx.byId?.[t]; const st = (idx.stations || []).find(s => s.traditionId === t);
    return tr ? `<div class="lore-entry"><h4 class="lore-h">${esc(tr.name || t)}${tr.craft ? ` <span class="hint">â€” ${esc(tr.craft)}</span>` : ""}</h4>
      <div class="lore-field"><span class="lore-key">Pole:</span> ${esc(st?.pole || tr.pole || "?")} Â· <span class="lore-key">Across the ring:</span> ${esc(traditionLabel(antipodeOf(t, idx)))}</div>
  ...
  return `${circle}
    <p class="lore-p">Twelve axes of the world, each a tension between two peoples â€” and every craft sits directly across the ring from its antithesis. Kin stand beside kin; the far pole of what you are is closed to you, reachable only by the great braids.</p>
...
async function renderLibrary(catIdx = 0, entryId = null) {
  ...
  chrome(`<div class="screen" style="max-width:820px">
    <h2>ðŸ“– The Library <span class="hint" style="text-transform:none">â€” the world's guide, open to read</span></h2>
...
// by the userâ€¦ the game should auto merge â€” it's really smart AI, just do it." Gate 1 (structural)
...
async function maybeAdjudicateMerges(query = "") {
  ...
  try {
    ...
    if (r.merged.length || r.rejected) {
      ...
      if (digest) character._codexDigest = `${digest}${r.rejected ? ` ${r.rejected} looked alike but aren't the same thing â€” I won't ask again.` : ""}`;
      else if (r.rejected) character._codexDigest = `${r.rejected} codex entries looked alike but aren't the same thing â€” I won't ask again.`;
...
function renderCodexScreen(query = "", openTopicId = null, mergeMode = false) {
  ...
  chrome(`<div class="screen" style="max-width:720px">
    <h2>Codex â€” what ${esc(character.name)} knows</h2>
    <div class="field"><input id="codex-search" value="${esc(query)}" placeholder="Search topics, facts, factions, mysteriesâ€¦"></div>
    ${open ? `
      <div class="codex-topic-page">
        <div class="codex-kind">${esc(open.kind)}${open.entityId ? ` <span class="codex-anchor" title="anchored to a known entity">â—ˆ ${esc(open.entityId)}</span>` : ""}</div>
        <h3 class="codex-title">${esc(open.label)}</h3>
        ${(open.aliases || []).length ? `<div class="codex-aliases">also called: ${open.aliases.map(esc).join(" Â· ")}</div>` : ""}
        ${(() => {
          // SNG-BATCH-9 Phase 2: a GROWN entity carries a canon-tier badge + a one-tap â­ Keep
          // (the explicit engagement boost â€” available, never nagged). Nominated = a promotion
          ...
          const badge = tier === "nominated" ? "â˜… notable â€” nomination candidate" : tier === "established" ? "â—† established in your world" : "â—‡ freshly grown";
          return `<div class="gen-canon ${tier}">
            <span class="gen-tier" title="grown through play Â· weight ${effectiveWeight(g)}${g._gen.rating ? ` Â· ${g._gen.rating}` : ""}">${badge}</span>
            <button class="opt gen-keep" data-keep="${esc(open.entityId)}" title="Keep this â€” mark it as one that matters to you. Raises it toward notable. Never nags.">â­ Keep</button>
          </div>`;
        })()}
        ${mergeMode ? (() => {
          ...
          return `<div class="codex-merge-pick"><div class="hint">Everything here (facts, links) moves onto the entry you pick; "${esc(open.label)}" becomes one of its names. Permanent.</div>
            ${sorted.map(t => `<button class="opt codex-merge-target ${sug.includes(t.id) ? "suggested" : ""}" data-mergetarget="${esc(t.id)}">
              ${sug.includes(t.id) ? "â—ˆ " : ""}${esc(t.label)} <span class="cost">${esc(t.kind)} Â· ${t.facts.length} fact${t.facts.length === 1 ? "" : "s"}</span></button>`).join("")}
        ...
        <button class="opt" id="codex-merge-mode" style="margin-top:10px">â‡† This is the same as another entryâ€¦</button>`}
        <button class="btn secondary" id="codex-close-topic" style="margin-top:12px">All topics</button>
      </div>` : `
      ${(() => {
        // SNG-BATCH-9 Phase 2 Â§3: entities that grew into 'notable' (nominated) surface as
        // promotion candidates toward shared canon. Surfacing only â€” tap to open + â­/keep.
        const noms = nominationsFor(character);
        if (!noms.length) return "";
        return `<div class="codex-nominations"><div class="codex-group-title">â˜… Notable â€” grown into your world's canon</div>
          ${noms.slice(0, 6).map(n => `<button class="opt codex-nom" data-topic="${esc(n.id)}" title="weight ${n.weight}${n.rating ? ` Â· ${n.rating}` : ""} â€” a candidate to become shared-world canon">â˜… ${esc(n.name)} <span class="cost">${esc(n.type)}</span></button>`).join("")}</div>`;
      })()}
      ${/* SNG-153: a RECEIPT of what the codex did on its own â€” with a one-tap undo, because
      ...
      ${suggestions.length ? `<div class="codex-suggestions"><div class="codex-group-title">These look like the same thing â€” your call</div>
        ${suggestions.map((s, i) => `<div class="codex-sug-row"><span class="codex-sug-pair">Â«${esc(s.a)}Â» â†” Â«${esc(s.b)}Â»</span>
      ...
      <div class="codex-list">
        ${results.length ? (() => {
          // SNG-019: group by kind â€” primary entities first, facts nested under their node
            ...
            ${byKind[k].sort((a, b) => (b.entityId ? 1 : 0) - (a.entityId ? 1 : 0) || b.facts.length - a.facts.length).map(t => `
              <div class="codex-node">
                <button class="codex-item" data-topic="${esc(t.id)}">
                  <strong>${esc(t.label)}</strong>${t.entityId ? ` <span class="codex-anchor" title="known entity">â—ˆ</span>` : ""}
                  ${(t.aliases || []).length ? `<span class="codex-aliases-inline">Â· also: ${t.aliases.slice(0, 2).map(esc).join(", ")}${t.aliases.length > 2 ? "â€¦" : ""}</span>` : ""}
        ...
        })() : "<div class='insight'>Nothing cataloged yet â€” knowledge accumulates as you learn things that matter.</div>"}
      </div>`}
  ...
  // SNG-BATCH-9 Phase 2: â­ keep â€” the explicit engagement boost (raises toward notable)
  for (const b of app.querySelectorAll("[data-keep]")) b.onclick = () => {
    const rec = findGenerated(character, b.dataset.keep);
    if (!rec) return;
    recordAttention(rec, "keep", readClock(character.clock).day);
    autoVerifyLeg("b9p2-keep", `â­ Kept a grown entity â†’ ${rec._gen?.tier}`); // SNG-051 auto-verify
    saveCharacter(character);
  ...
  if (undoBtn) undoBtn.onclick = () => {
    ...
    if (!r) return;
    character._codexDigest = `Put back â€” "${r.restored}" is its own entry again.`;
    saveCharacter(character);
    renderCodexScreen(query);
  };
  ...
  for (const b of app.querySelectorAll("[data-sugnot]")) b.onclick = () => {
    ...
    if (!s) return;
    markNotSame(character, s.aId, s.bId); // remembered â€” never suggested or auto-merged again
    saveCharacter(character);
    renderCodexScreen(query, null);
  };
...
/** SNG-043 Part B: the energy a planned step will cost â€” the ability's effective cost when the
 ...
 *  loading state. Rejects after `ms` so a hang or timeout is caught like any other failure â€” the
...
function renderGambitBuilder(status = "") {
  ...
  chrome(`<div class="screen" style="max-width:720px">
    ...
    ${g.steps.map((s, i) => `
      <div class="field gambit-step">
        <label>Step ${i + 1}${g.assessed?.steps[i] ? ` â€” <span class="g-chance">${g.assessed.steps[i].sense.text ? esc(g.assessed.steps[i].sense.text) : "no read"}</span>${g.assessed.weakIndex === i ? ` <span class="g-weak">âš  weakest link</span>` : ""}` : ""}${stepEnergy ? ` <span class="g-energy">Â· ${stepEnergy[i]} energy</span>` : ""}</label>
        ...
        <div style="margin-top:4px; display:flex; gap:6px; flex-wrap:wrap">
          ${i > 0 ? `<button class="opt g-up" data-i="${i}" title="Move this step earlier">â–² up</button>` : ""}
          ${i < g.steps.length - 1 ? `<button class="opt g-down" data-i="${i}" title="Move this step later">â–¼ down</button>` : ""}
          ${g.steps.length > 1 ? `<button class="opt g-remove" data-i="${i}">remove</button>` : ""}
        </div>
      </div>`).join("")}
    ${totalEnergy != null ? `<div class="gambit-total ${overBudget ? "over" : ""}">Total plan cost: <strong>${totalEnergy} energy</strong> of ${character.energy ?? 0} available${overBudget ? " â€” âš  this plan costs more energy than you have; expect to fall short partway" : ""}</div>` : ""}
    ${g.gmAdvice ? `<div class="gambit-advice">âœ¦ GM: ${esc(g.gmAdviceExpanded ? (g.gmAdviceFull || g.gmAdvice) : g.gmAdvice)}${(g.gmAdviceFull && g.gmAdviceFull.length > g.gmAdvice.length) ? ` <button class="link-btn" id="g-advice-more">${g.gmAdviceExpanded ? "less" : "more"}</button>` : ""}</div>` : ""}
    <div style="display:flex; gap:8px; margin-bottom:14px; flex-wrap:wrap;">
      ${g.steps.length < max ? `<button class="btn secondary" id="g-add">+ step</button>` : ""}
      ${hasPlanDiscussion() ? `<button class="btn secondary" id="g-fill" title="Pull the goal and steps from what you just worked out with the GM">âœ¦ Fill from our conversation</button>` : ""}
      <button class="btn secondary" id="g-assess">Assess plan</button>
      <button class="btn secondary" id="g-advise">âœ¦ GM, look at this</button>
      <button class="btn" id="g-run" ${g.assessed ? "" : "disabled"}>Run it</button>
      <button class="btn secondary" id="g-startover" title="Clear this plan and start fresh â€” costs nothing, keeps nothing">â†º Start over</button>
  ...
  if (fill) fill.onclick = () => { read(); renderGambitBuilder("Reading your conversation for the planâ€¦"); autofillGambitFromConversation(false); };
  ...
  // SNG-088C: a gambit must be abandonable and leave NOTHING behind â€” a fresh, blank draft.
  document.getElementById("g-startover").onclick = () => { abandonGambit(); renderGambitBuilder("Fresh plan â€” nothing carried over."); };
  ...
  document.getElementById("g-assess").onclick = async () => {
    ...
    renderGambitBuilder("Reading the planâ€¦");
    // SNG-093: try/catch/finally + timeout â€” a hang or a throw can never strand "Reading the planâ€¦".
    try {
      ...
      renderGambitBuilder("Couldn't read the plan (" + String(err?.message || err).slice(0, 60) + ") â€” try again.");
      return;
    }
    renderGambitBuilder();
  };
  ...
  if (advise) advise.onclick = async () => {
    ...
    renderGambitBuilder("The GM studies your planâ€¦");
    // SNG-043 Part B: GM-collaborative building â€” one concrete suggestion or warning on the DRAFT
    const q = `[Gambit builder] Look at this DRAFT plan I'm assembling and give me ONE concrete, specific suggestion or warning about its sequencing or risk â€” a single sentence of advice (not a narration of any outcome; nothing is attempted yet). Goal: "${g.goal}". Steps so far: ${texts.map((t, i) => `${i + 1}) ${t}`).join("; ")}.`;
    // SNG-093 (Design Law 5): the GM call is wrapped in try/catch with a TIMEOUT, and `finally`
    // ALWAYS re-renders â€” an AI throw/hang can never leave the UI stuck on "The GM studies your planâ€¦".
    try {
      // BATCH-11 Â§23: the "gambit" view â€” plan-relevant rows only (abilities, inventory, scene).
      const result = await withTimeout(gmAsk(assembleGMContext("gambit", gmEnv()), q), 30000, "the GM");
      // SNG-088A (SNG-076 miss): the GM's plan-advice is PROSE â€” clamp on a word boundary with a real
      ...
      else { g.gmAdvice = g.gmAdviceFull = "The GM couldn't weigh in (" + String(result.error).slice(0, 50) + ") â€” plan on."; }
    } catch (err) {
      g.gmAdvice = g.gmAdviceFull = "The GM didn't answer (" + String(err?.message || err).slice(0, 50) + ") â€” try again.";
    } finally {
      renderGambitBuilder();
    }
  };
...
/** SNG-088C: abandon/reset â€” a gambit must leave NOTHING behind. A fresh, blank draft; no carried
...
async function autofillGambitFromConversation(announce = true) {
  ...
  try {
    ...
    if (!steps.length && !goal) { renderGambitBuilder(announce ? "Nothing to pull from the conversation yet â€” write the plan here." : ""); return; }
    if (goal) g.goal = goal;
    if (steps.length) g.steps = steps;
    g.assessed = null; g.actions = null;
    renderGambitBuilder("âœ¦ Pulled from your conversation â€” edit anything, reorder as you like, then Assess and Run.");
  } catch (e) {
    renderGambitBuilder(announce ? "Couldn't read the conversation (" + String(e.message || e).slice(0, 50) + ") â€” write the plan here." : "");
  }
}
...
function openGambitBuilder() {
  ...
  renderGambitBuilder("Reading your conversation for the planâ€¦");
...
async function runGambit() {
  ...
  const renderComplication = (failed) => {
    ...
    chrome(`<div class="screen" style="max-width:640px">
      ...
      <div class="roll-receipt" style="margin:10px 0">Step ${failed.index + 1}: ${esc(a.label)} â€” d100: ${failed.roll} vs ${failed.breakdown ? `<span class="roll-chance" data-breakdown="${esc(JSON.stringify({ ...failed.breakdown, roll: failed.roll, degree: failed.degree }))}" tabindex="0" role="button" title="Why this number?">${failed.chance}</span>` : failed.chance} â€” <span class="${failed.degree}">${failed.degree.replace("_", " ")}</span></div>
      <div style="display:flex; flex-direction:column; gap:8px; margin-top:14px;">
        ...
        ${run.adaptLeft > 0 ? `<button class="choice" id="c-adapt">Adapt â€” force another try (${run.adaptLeft} adaptation point${run.adaptLeft > 1 ? "s" : ""} left)</button>` : ""}
        <button class="choice" id="c-presson">Accept the failure and press on with the rest</button>
        <button class="choice" id="c-abandon">Abandon the plan â€” get out of it</button>
      </div>
    </div>`);
...
async function finishGambit(run) {
  ...
  // SNG-030 remainder: a carried-out plan PAYS â€” award the completion bonus so planning
  ...
  renderPlay(null, { thinking: "The run unfoldsâ€¦", playerBeat: { label: gLabel } });
...
/** SNG-114: THE shared item card â€” a superset (image when open + effects + bonusTags + use/name/drop),
...
function itemCard(it, { open = false, toggleAttr = "data-item-toggle", showPin = false } = {}) {
  ...
  return `<div class="item-card ${open ? "open" : ""}">
    <button class="item-name" ${toggleAttr}="${esc(it.name)}">${esc(displayName(it))}${it.customName ? ` <span class="cost">(${esc(it.name)})</span>` : ""}${it.qty > 1 ? ` Ã—${it.qty}` : ""}</button>${showPin ? `<button class="item-pin ${it.pinned ? "on" : ""}" data-item-pin="${esc(it.name)}" title="${it.pinned ? "Pinned to the sidebar â€” tap to unpin" : "Pin to the sidebar for quick access"}">${it.pinned ? "ðŸ“Œ" : "ðŸ“"}</button>` : ""}
...
/** SNG-114: bind the shared card's use/name/drop ONCE â€” both surfaces call this, so "Use in scene" does
...
/** SNG-114: the intent step for using a non-consumable â€” offer the item's meaningful uses (authored or
...
function openUseIntent(item) {
  ...
  chrome(`<div class="screen" style="max-width:520px">
    ...
    <div class="field" style="margin-top:10px"><label>â€¦or say how you use it</label>
  ...
  document.getElementById("use-go").onclick = () => { const h = how.value.trim(); submit(h ? `I use my ${nm} â€” ${h}` : `I use my ${nm} here`); };
...
let sbLastPlayerFn = null;        // the last function the player showed â€” the opponent may READ it next round
...
function renderSkillBattle(lastRound = null) {
  ...
  chrome(`<div class="screen sb-screen" style="max-width:640px">
    <h2>âš” ${esc(def.name || "The contest")}</h2>
    <div class="sb-opponent">${esc(def.opponent?.name || "your opponent")}${fog?.label ? ` â€” <span class="hint">${esc(fog.label)}</span>` : ""}${oppTired ? ` <span class="cost">(${oppTired})</span>` : ""}</div>
    <div class="sb-meter" title="Momentum â€” fill it to prevail; empty it and you are overcome.">
      <div class="sb-meter-fill" style="width:${Math.max(0, Math.min(100, momPct))}%"></div><div class="sb-meter-mid"></div>
    </div>
    <div class="sb-vitals">You: ${character.health}/${character.maxHealth} hp Â· ${character.energy} energy</div>
    <div class="sb-fog">${fog ? `
      <div class="sb-fog-line">${esc(fog.revealed.outcome || "They move.")}${fog.revealed.intent ? ` â€” gathering to <strong>${esc(fog.revealed.intent)}</strong>` : ""}${fog.revealed.band ? ` <span class="hint">(${esc(fog.revealed.band)})</span>` : ""}</div>
      ${fog.revealed.skill ? `<div class="sb-fog-line">${esc(fog.revealed.skill)}${fog.revealed.intensity ? ` Â· ${esc(fog.revealed.intensity)}` : ""}${fog.revealed.breakdown ? ` <button class="data-link" data-breakdown='${esc(JSON.stringify(fog.revealed.breakdown))}'>see their math</button>` : ""}</div>` : ""}` :
      `<div class="hint">You size each other up. Strike â€” or read them first.</div>`}</div>
    ...
    <div class="sb-skills">${skills.map((s, i) => `<button class="btn secondary sb-skill" data-sbskill="${i}" style="display:block; width:100%; text-align:left; margin:4px 0">${esc(s.name)} <span class="cost">${esc(s.function)} Â· T${s.tier}${s.energyCost ? ` Â· ${s.energyCost}e` : ""}</span></button>`).join("")}</div>
    <div class="sb-actions" style="display:flex; gap:8px; margin-top:10px; flex-wrap:wrap">
      <button class="btn secondary" id="sb-read" title="Spend the round reading them â€” no attack, but you see their next move more clearly">ðŸ‘ Read them</button>
...
/** Try to break away â€” a quick physical roll against the flee difficulty, resolved through the same lifecycle. */
function sbFlee() {
  const enc = activeEnc(); if (!enc) return;
...
async function sbEnd(rr) {
  ...
  // SNG-138: a resolved PRESTIGE-CHALLENGE duel feeds renown â€” band-scaled (beating a renowned duelist
  ...
  if (def?._challengeBand) {
    ...
    if (w !== 0) {
      recordDeed(character, { description: `${won ? "bested" : "lost to"} ${nm} â€” a ${def._challengeBand} duel`, weight: w, communityId: hereNow()?.communityId, tags: ["duel", "prestige"] }, aptitudeMods(character, CONTENT.rules.playerAptitudes));
      saveCharacter(character);
    }
  }
  const outLine = {
    opponent_fell: `You have beaten ${nm} â€” they go down.`,
    ...
    stalemate: `Neither of you can finish it â€” you both break, spent.`,
    incapacitated: `You fall, overcome.`
  }[rr.outcome] || "The contest ends.";
  renderPlay(null, { thinking: "â€¦" });
  const result = await runGM({ resolution: null, playerInput: `(The skill-battle with ${nm} has resolved â€” outcome: ${rr.outcome}. ${outLine} Narrate the aftermath in one beat and return to the scene.)` });
...
function renderPlay(turn, opts = {}) {
  // SNG-088B / SNG-091: the GM sketched a plan this turn â€” open the builder pre-filled instead of the
  ...
  if (pendingGambitProposal) {
    ...
    renderGambitBuilder(`âœ¦ The GM sketched this plan for you â€” edit any of it, Assess to read your odds, then Run it. Nothing has happened yet.${hadDraft ? " (Your previous unsaved draft was replaced â€” â†º Start over for a blank plan.)" : ""}`);
    return;
  }
  ...
  // CCODE-07: the beat survived but some of its bookkeeping didn't â€” say so plainly rather than
  // letting the player discover a quest/NPC update silently missing. The GM restates next turn.
  if (turn?._applyFailed) { opts = { ...opts, aside: [opts.aside, "*(The scene stands, but part of this turn's bookkeeping didn't land â€” the GM will restate it next beat.)*"].filter(Boolean).join("\n\n") }; }
  const location = hereNow();
  const rules = CONTENT.rules;
  ...
  const sheet = `<div class="sheet">
    ...
    <div class="meta">${esc(character.origin)} Â· ${esc(character.background)} Â· level ${character.level} (${character.xp} xp)</div>
    ...
    <details class="sidebar-sec" data-sec="attributes"${sectionOpen("attributes", false) ? " open" : ""}><summary><span class="sec-title">Attributes</span>${character.pendingSubPoints > 0 ? ` <span class="grow-badge">+${character.pendingSubPoints} to place</span>` : ""}</summary><div class="sec-body"><div class="attr-grid">
      ${SUBS.map(s => `<div style="text-transform:capitalize" title="${esc(SUB_DESC[s])} (${SUB_OF[s]})">${s}</div><div>${(character.subAttributes?.[s] ?? 0) > 6 ? (character.subAttributes[s] + " â—â—â—â—â—â—âº") : "â—".repeat(character.subAttributes?.[s] ?? 0) + "â—‹".repeat(Math.max(0, 4 - (character.subAttributes?.[s] ?? 0)))}${character.pendingSubPoints > 0 && (character.subAttributes?.[s] ?? 0) < (CONTENT.rules.leveling?.subAttributeCap ?? 6) ? ` <button class="grow-btn" data-grow="${s}">+</button>` : ""}</div>`).join("")}
    </div></div></details>
    <details class="sidebar-sec" data-sec="abilities"${sectionOpen("abilities", true) ? " open" : ""}><summary><span class="sec-title">Abilities</span>${character.skillPoints > 0 ? ` <span class="grow-badge">${character.skillPoints} skill pt</span>` : ""}</summary><div class="sec-body">
      ${canLevelUp(character) ? `<button class="opt" id="sidebar-levelup" title="Spend your skill points" style="padding:2px 8px; margin-bottom:6px; display:block">â¬† Level Up</button>` : ""}
      ${(() => {
        ...
        const row = ({ a, ab }) => {
          ...
          return `<div class="ability" title="${esc(rank ? "CAN: " + rank.grants + " | CANNOT: " + rank.cannot : ab?.description || "")}">
            <span class="name entity-hover" data-entity="skill:${esc(a.abilityId)}">${esc(ab?.name || a.abilityId)}</span> <span class="tier-badge" title="Tier ${tierOf(ab.levelReq)}">${tierOf(ab.levelReq)}</span> rank ${a.level}${rank ? ` â€” <em>${esc(rank.name)}${rank.forked ? " â‘‚" : ""}</em>` : ""}
      ...
      ${(() => {
        ...
        const learnable = Object.values(CONTENT.abilities).filter(ab => {
          ...
          if (req === null || character.level < req) return false;
          // SNG-055: the domain gate decides what's OFFERED â€” the antipode of a chosen pole and
          // over-tier picks (secondary>III, tertiary>II, kin-capstones) simply aren't shown.
          return domainVerdict(ab).allowed;
        });
        const capLine = `<div class="cap-line">${breadthUsed(character)} of ${breadthCap(character, CONTENT.skillCapacity)} skills${cap ? " â€” at capacity; points now deepen owned skills" : ""}${cap ? " " + infoDot("lock.capacity") : ""}</div>`;
        ...
        const groups = Object.keys(byClass).sort((a, b) => traditionLabel(a).localeCompare(traditionLabel(b))).map(cls => `<details class="learn-group"><summary>Learn ${esc(traditionLabel(cls))} <span class="cost">(${byClass[cls].length})</span></summary>${
          byClass[cls].sort((a,b)=>(a.levelReq||1)-(b.levelReq||1)).map(ab => {
            ...
            return `<button class="opt ${ripe ? "practiced" : ""} ${blocked ? "locked" : ""}" ${blocked ? "disabled" : `data-learn="${esc(ab.id)}"`} title="${esc(ab.description + " â€” " + dv.reason + (gate.ok ? "" : " Â· " + gate.why))}" style="margin:2px 0; display:block; width:100%"><span class="tier-badge">${tierOf(ab.levelReq)}</span> ${esc(ab.name)} <span class="cost">L${ab.levelReq || 1}${bandTag}${learnCost > 1 ? ` Â· ${learnCost} pts` : ""}${ripe ? " â€” FREE" : ""}${!gate.ok ? " ðŸ”’ " + esc(gate.why) : capBlock ? " ðŸ”’ at capacity" : tooExpensive ? " ðŸ”’ need " + learnCost + " pts" : ""}</span></button>`;
          }).join("")}</details>`).join("");
      ...
      ${(character.discoveries || []).length ? `<details class="skill-group discoveries" open><summary>Discoveries &amp; Combinations <span class="cost">(${character.discoveries.length})</span></summary>${character.discoveries.map(d => {
        // SNG-047: adopt the orphan combo â€” show the source abilities it braids (recipe parts)
        const parts = (d.abilityIds || []).map(id => fullCatalog()[id]?.name || id.replace(/-/g, " "));
        return `<div class="ability discovery-row" title="${esc(d.description || "")}">âœ¦ <span class="name">${esc(d.name)}</span>${parts.length ? ` <span class="combo-parts">= ${parts.map(esc).join(" + ")}</span>` : ""}</div>`;
      }).join("")}</details>` : ""}
    </div></details>
    ${/* SNG-120: the old sidebar "People you know" (a duplicate of the SNG-119 "who's here" list + the full
          Character-screen roster) is GONE â€” its people show once, in the place-scoped section below. */""}
    <details class="sidebar-sec" data-sec="quests"${sectionOpen("quests", (character.quests || []).some(q => q.status === "active")) ? " open" : ""}><summary><span class="sec-title">Quests</span> ${infoDot("quest.routes")}${(character.quests || []).filter(q => q.status === "active").length ? ` <span class="sec-sum">(${(character.quests || []).filter(q => q.status === "active").length})</span>` : ""}</summary><div class="sec-body">
      ${(character.quests || []).filter(q => q.status === "active").map(q => { const stage = q.structured ? (q.stages?.[q.stageIndex] || q.stages?.[q.stages.length - 1]) : null;
        return `<button class="quest quest-click" data-quest="${esc(q.id)}"><span class="quest-title">${esc(q.title)}</span>${q.structured ? ` <span class="cost">âœ¦</span>` : ""}
        ...
        </button>`; }).join("") || "<div class='insight'>no undertakings yet â€” the valley will provide</div>"}
      ${(() => { const avail = availableStructuredQuests(character, [...(CONTENT.quests || []), ...(character.personalArc ? [character.personalArc] : [])], questOfferContext(character, sceneState)); return avail.length ? `<div class="hint" style="margin-top:4px">âœ¦ ${avail.length} quest${avail.length === 1 ? "" : "s"} to take up here</div>` : ""; })()}
      <button class="opt" id="open-questlog" style="display:block;width:100%;margin-top:4px">ðŸ“œ Quest Log${(character.quests || []).some(q => q.status !== "active") ? ` â€” ${(character.quests || []).filter(q => q.status === "completed" || q.status === "resolved").length} done Â· ${(character.quests || []).filter(q => q.status === "failed").length} failed` : ""}</button>
    </div></details>
    ${(() => { // SNG-120/126: Company â€” Party (players) + Companions (catalog) + Allies (NPC party members, roles)
      const comps = (character.companions || []).filter(id => CONTENT.companions[id]);
      const partyOn = syncEnabled();
      // SNG-126: the unified NPC-people half â€” recruited members (with roles) + partner-adjacent NPCs,
      ...
      if (!partyOn && !comps.length && !roster.length && !recruitable.length) return ""; // truly solo â†’ the section disappears
      const count = (sharedScene?.party?.length || 0) + comps.length + roster.length;
      const partyBody = partyOn ? `<div class="company-group"><div class="sys-label">Party</div>${sharedScene ? `${sharedScene.party.map(m => `<div class="known-npc"><span class="npc-name">${m.characterId === character.id ? "you" : esc(m.name)}</span>${sharedScene.turn === m.characterId ? `<span class="rep-band trusted">turn</span>` : ""}</div>`).join("")}<div class="hint">${isMyTurn(sharedScene, character.id) ? "Your turn â€” act." : "Waiting for " + esc(sharedScene.party.find(m => m.characterId === sharedScene.turn)?.name || "â€¦")}</div><button class="opt" id="party-leave" style="display:block; width:100%; margin-top:4px">Leave shared scene</button>` : `<button class="opt" id="party-find" style="display:block; width:100%">Find or start a party here</button>`}</div>` : "";
      // SNG-135: ONE tight flex row per member â€” name Â· inline badge Â· compact action. Roster/roles/recruit
      // gating (SNG-126) unchanged; the companion description moves to the row title (hover) instead of a
      // printed line. data-rename/part/partally/recruit hooks are identical â€” layout only.
      const compBody = comps.length ? `<div class="company-group"><div class="sys-label">Companions</div>${comps.map(id => { const c = CONTENT.companions[id]; const dn = character.companionNames?.[id] || c.name; const b = bondOf(character, c.id, CONTENT.rules); return `<div class="company-row" title="${esc(c.role)}${c.appearance ? " â€” " + c.appearance : ""}"><span class="company-name">${esc(dn)}${dn !== c.name ? ` <span class="hint">(${esc(c.name)})</span>` : ""}</span><span class="company-badge ${b.bond >= 3 ? "on" : ""}" title="bond grows through shared deeds, assists, and encounters">bond ${b.bond}${b.stage === 2 ? " Â· s2" : ""}</span><span class="company-actions"><button class="company-action companion-rename" data-rename="${esc(id)}" title="Name them">âœŽ</button><button class="company-action companion-part" data-part="${esc(id)}" title="Part ways">âœ•</button></span></div>`; }).join("")}</div>` : "";
      const allyBody = (roster.length || recruitable.length) ? `<div class="company-group"><div class="sys-label">Allies</div>${
        roster.map(r => `<div class="company-row" title="${esc(roleBadges(r.roles))}${r.teaches ? " Â· teaches " + traditionLabel(r.teaches) : ""}${r.liaisonFor ? " Â· liaison" : ""}"><span class="company-name">${esc(r.name)}</span><span class="company-badge" title="roles they hold in your company">${esc(roleBadges(r.roles))}</span>${r.teaches ? `<span class="company-badge on" title="a trainer â€” their presence lets you learn this people's capstones">âš”</span>` : ""}${r.liaisonFor ? `<span class="company-badge" title="a liaison â€” faster standing with their people">ðŸ¤</span>` : ""}<span class="company-actions">${r.recruited ? `<button class="company-action ally-part" data-partally="${esc(r.npcId)}" title="Part ways">âœ•</button>` : ""}</span></div>`).join("")
      }${recruitable.map(p => `<div class="company-row" title="${esc(p.label || "at your side")}"><span class="company-name">${esc(p.name)}</span><span class="company-badge hint">${esc(p.label || "at your side")}</span><span class="company-actions"><button class="company-action recruit" data-recruit="${esc(p.id)}" title="Ask them to travel with you">ï¼‹</button></span></div>`).join("")}</div>` : "";
    ...
    ${(() => { // SNG-121: Items â€” the PINNED quick-access set; the rest is one tap away in the full Inventory
      ensurePins(character);
      const pins = pinnedItems(character), total = (character.inventory || []).length, more = total - pins.length;
      const body = `${pins.length ? pins.map(it => itemCard(it, { open: examinedItem === it.name, toggleAttr: "data-examine" })).join("") : `<div class="insight">${total ? "Nothing pinned â€” pin items from Inventory for quick access." : "empty-handed"}</div>`}${total ? `<button class="opt" id="open-inventory-more" style="display:block; width:100%; margin-top:4px">${more > 0 ? `ï¼‹ ${more} more in Inventory` : "Open Inventory"}</button>` : ""}`;
      return `<details class="sidebar-sec" data-sec="items"${sectionOpen("items", true) ? " open" : ""}><summary><span class="sec-title">Items</span> <span class="sec-sum">(${pins.length} pinned Â· ${total} total)</span></summary><div class="sec-body">${body}</div></details>`;
    })()}
    ${(() => { // SNG-119/120: standing + who's HERE (place-scoped) + the committed-partner banner, collapsible
      ...
      // them, so Pell appeared twice â€” the same person, listed as two people.
      ...
      // learns a name, and it intermittently doesn't â€” leaving "Broad Opportunist" beside the same
      // person's real name. Only the player knows they are the same, so give the player the verbs.
      const peopleCtl = (p) => p.id ? ` <button class="npc-ctl" data-setname="${esc(p.id)}" title="Set or extend this person's name (e.g. Pell â†’ Pell Ran Marsh)">âœŽ</button><button class="npc-ctl" data-mergenpc="${esc(p.id)}" title="This is the same person as someone else you know â€” merge them">â‡Š</button>` : "";
      ...
      const body = `${partners.length ? `<div class="partner-adjacent" style="margin-bottom:6px">${partners.map(p => `<div class="known-npc partner"><span class="npc-name entity-hover" ${p.id ? `data-entity="npc:${esc(p.id)}"` : ""}>â¤ ${esc(p.name)}</span> <span class="rep-band trusted" title="A committed partner â€” with you in all but the mechanics">${esc(p.label)} Â· with you</span>${peopleCtl(p)}</div>`).join("")}</div>` : ""}${rep ? `<div style="margin-bottom:4px"><span class="rep-band ${rep.band}">${rep.band} (${rep.score})</span></div>` : ""}${hereRest.length ? hereRest.map(p => row(p)).join("") : (partners.length ? "" : `<span class="insight">no one you know is here right now</span>`)}`;
      return `<details class="sidebar-sec" data-sec="whoshere"${sectionOpen("whoshere", true) ? " open" : ""}><summary><span class="sec-title">${esc(location.name)} â€” who's here</span>${rep ? ` <span class="sec-sum">Â· ${rep.band}</span>` : ""}</summary><div class="sec-body">${body}</div></details>`;
    })()}
    ...
    <details class="sidebar-sec" data-sec="maprest"${sectionOpen("maprest", false) ? " open" : ""}><summary><span class="sec-title">Map &amp; Rest</span></summary><div class="sec-body">
      <button class="opt map-open" id="open-map" style="margin:2px 0 6px; display:block; width:100%">ðŸ—º Open Map â€” travel & places</button>
      ...
      ${/* CCODE-03: the player can always close a scene themselves â€” the GM should do it, but a
            scene that won't end is the player's to end. Writes the chronicle entry either way. */""}
      <button class="opt" id="do-endscene" style="margin-top:8px; display:block; width:100%" title="Draw this scene to a close and write it into your chronicle. A new scene opens on your next action.">â¹ End this scene${sceneTurns.length ? ` (${sceneTurns.length} beats)` : ""}</button>
      ${/* SNG-155: read the narration aloud at the table. Narration only â€” choices, costs and
    ...
    <details class="sidebar-sec" data-sec="codex"${sectionOpen("codex", false) ? " open" : ""}><summary><span class="sec-title">Codex &amp; Library</span></summary><div class="sec-body">
      <button class="opt" id="open-codex" style="display:block; width:100%">${Object.keys(character.codex?.topics || {}).length} topic${Object.keys(character.codex?.topics || {}).length === 1 ? "" : "s"} discovered â€” open Codex</button>
      <button class="opt" id="open-library" style="display:block; width:100%; margin-top:6px">ðŸ“– The Library â€” read the world</button>
    </div></details>
  </div>`;
  ...
  let main = `<div class="play">
    ...
    <div class="location-tag" ${sceneState?.setting ? `title="${esc(sceneState.setting)}"` : ""}>${esc(location.name)}${rep ? ` <span class="rep-band loc-standing ${rep.band}" title="Your standing with ${esc(CONTENT.locations[character.currentLocationId]?.name || "the people here")} â€” ${rep.band} (${rep.score})">Â· ${esc(rep.band)}</span>` : ""}<span class="time-tag" title="Your own clock â€” days, season, time of day (SNG-191). The world's count is a separate shared tally, not a date.">${esc(time.label)} <span class="world-day-tag" title="The Kept Count â€” the shared world tally; it only ever climbs and is not a date">Â· â§— ${worldCount()}</span></span></div>
    ${(() => { const e = activeEnc(); if (!e) return ""; const st = e.state, d = e.def;
      let status = "";
      if (d.type === "duel") status = `${esc(d.opponent.name)}: ${"â–®".repeat(Math.max(0, st.opponentHealth))}${"â–¯".repeat(Math.max(0, d.opponent.health - st.opponentHealth))} Â· you: ${character.health}/${character.maxHealth}${st.tactic ? ` Â· tactic: ${esc(st.tactic)}` : ""}`;
      ...
      if (d.type === "puzzle") {
        ...
        status = `attempts: ${st.attempts} Â· understanding: ${revealed.length}/${(d.hintTiers || []).length}` +
          (revealed.length ? `<div class="enc-hints">${revealed.map(h => `<div>â—ˆ ${esc(h)}</div>`).join("")}</div>` : "");
      }
      return `<div class="encounter-bar"><span class="enc-name">âš” ${esc(d.name)}</span> <span class="enc-round">round ${st.round}</span><div class="enc-status">${status}</div></div>`;
    })()}<div class="transcript">`;
  if (opts.newsFlash?.length) {
    main += `<div class="news-flash"><div class="news-title">While you were awayâ€¦</div>${opts.newsFlash.map(n => `<div class="news-item">â—ˆ ${esc(n.text)}</div>`).join("")}</div>`;
  }
  if (opts.playerBeat) {
    // SNG-181: render what the player TYPED, in full â€” not the compact action label. `label` is a
    ...
    main += `<div class="beat player-action">â–¸ ${esc(said)}</div>`;
    if (opts.playerBeat.resolution) {
      const r = opts.playerBeat.resolution;
      // SNG-169 Â§2c: the item that aided the roll is now tappable. `entityHover`'s item branch and
      // `itemDetail` were fully written and reached by nothing â€” a 12th built-never-reached. The
      ...
      const helpers = r.equipHelpers?.length
        ? ` Â· aided by ${r.equipHelpers.map(h => itemSet.has(h)
          ? `<span class="entity-hover" data-entity="item:${esc(h)}">${esc(h)}</span>`
          : esc(h)).join(", ")}`
        : "";
      const locBits = r.locationAffinity?.length ? `<div class="roll-affinity">${r.locationAffinity.map(esc).join(" Â· ")} ${infoDot("roll.spectral_fit")}</div>` : "";
      const intBit = r.intensity && r.intensity !== "standard" ? ` Â· <span class="intensity-${esc(r.intensity)}">${esc(r.intensity)}</span>${r.energySpent != null ? ` (${r.energySpent} energy)` : ""}` : "";
      const blBit = r.surgeBacklash ? `<div class="roll-backlash">âš¡ surge backlash: ${r.backlash.health} health, ${r.backlash.energy} energy</div>` : "";
      // SNG-090: the substrate receipt â€” the lattice thin (starved) or crowding (interference) here.
      ...
      const subBit = r.substrate ? `<div class="roll-affinity">${r.substrate.side === "starved" ? "the lattice is thin â€” your craft ran at" : "the lattice crowds your signal â€” your craft ran at"} ${r.substrate.percent}%${carriedBit} ${infoDot("roll.spectral_fit")}</div>` : "";
      // SNG-084 Ph2: one contextual â“˜ on the roll â€” why it was hard (novel), suddenly easy (discovery), or the d100-vs-chance basics.
      ...
      // SNG-106: the chance is tappable â†’ the full component breakdown (only when this turn retained one).
      ...
      main += `<div class="roll-receipt">d100: ${r.roll} vs ${chanceCell} â€” <span class="${r.degree}">${r.degree.replace("_", " ")}</span> ${rollHelp}${helpers}${intBit}</div>${locBits}${subBit}${blBit}`;
    }
  }
  if (opts.itemsAdvanced?.length) main += opts.itemsAdvanced.map(a => `<div class="beat item-woke">âœ¦ ${esc(a.itemName)} stirs â€” <em>${esc(a.stageName)}</em></div>`).join("");
  ...
  // SNG-122: the safety net â€” a travel intent that didn't move the player gets a one-tap "arrive" so
  // narrative travel and map travel converge on one path and the player is never stranded.
  // SNG-145: a pending INTENT GATE renders as an inline choice card â€” the act is in escrow and
  ...
  if (character?._pendingIntent) {
    ...
    main += `<div class="intent-card" style="border:1px solid var(--accent,#c9a227);border-radius:8px;padding:12px;margin:8px 0">
      <div style="font-weight:600;margin-bottom:4px">âš– A moment of intent</div>
      ...
      <div style="margin-top:8px;display:flex;gap:8px;flex-wrap:wrap">${g.options.map(o =>
        `<button class="btn${o.id === g.default ? " secondary" : ""}" data-intentopt="${esc(o.id)}">${esc(o.label)}${o.id === g.default ? " Â·" : ""}</button>`).join("")}
      </div></div>`;
  }
  ...
  if (character?._pendingLedger?.length) {
    ...
    main += `<div class="intent-card" style="border:1px dashed var(--accent,#c9a227);border-radius:8px;padding:10px;margin:8px 0">
      <div style="font-weight:600">ðŸŒ This would reach beyond you</div>
      <div class="hint">${esc(ev.what)} â€” this consequence would surface in another player's world.</div>
      <div style="margin-top:6px;display:flex;gap:8px"><button class="btn" id="ledger-send">Let it travel</button>
      <button class="btn secondary" id="ledger-hold">Keep it local Â·</button></div></div>`;
  }
  if (character?._pendingArrival) {
    const p = character._pendingArrival;
    main += `<div class="arrive-banner">You're on your way to <strong>${esc(p.name)}</strong>. <button class="btn arrive-btn" id="do-arrive">â†’ Arrive at ${esc(p.name)}</button></div>`;
  }
  ...
  if (turn) {
    // SNG-155: the speak control belongs ON THE BEAT â€” it reads THIS narration, so it sits with it.
    // It was in the Map & Rest sidebar, nowhere near the prose it speaks: the same mistake as the
    // â­ in CCODE-06, putting the control next to the plumbing instead of next to the thing it acts on.
    const speakCtl = ttsAvailable()
      ? `<button class="beat-speak" id="do-speak" title="Read this beat aloud. Narration only â€” choices and costs are not spoken.">${_speaking ? "â¸ Stop" : "â–¶ Read aloud"}</button>`
      : "";
    ...
    if (opts.degraded) main += `<div class="degraded-note">(${esc(turn._opNote || "The GM's structured reply failed â€” plain narration mode this turn.")})</div>`;
    ...
    main += `<div class="choices">${(turn.choices || []).map((c, i) => {
      ...
      if (c.abilityId && character.abilities.some(a => a.abilityId === c.abilityId)) {
        ...
        abilityHtml = `<span class="ability-tag"> âœ¦ ${esc(fullCatalog()[c.abilityId]?.name || c.abilityId)}</span>`;
      }
      if (c.trivial && !c.abilityId) {
        senseHtml = `<span class="sense trivial-tag">no roll â€” just do it</span>`;
      } else {
        ...
        const aff = affinityFor(action, location);
        // SNG-116: the preview must include the SAME substrate penalty the resolve path applies â€” else "how
      ...
      // SNG-015 Part B: fast tap = auto intensity + default ability; âš™ expand-to-tune
      const canTune = !(c.trivial && !c.abilityId) && !c.encounterId && !c.emergenceId;
      const gear = canTune ? `<button class="tune-toggle" data-tune="${i}" title="Tune which ability + how hard (Conserve / Standard / Surge)">âš™</button>` : "";
      let panel = "";
      if (canTune && tuneOpen === i) {
        ...
        const intBtns = intensityOptions(baseCost, CONTENT.intensity).map(o =>
          `<button class="tune-int intensity-${o.key} ${tuneSel.intensity === o.key ? "sel" : ""}" data-tunint="${o.key}">${o.label}${sel ? ` Â· ${o.energy}e` : ""}${o.warn ? " âš " : ""}</button>`).join("");
        panel = `<div class="tune-panel"><div class="tune-row"><span class="tune-lbl">Apply</span>${abChips}</div>` +
          ...
          (tuneSel.intensity === "surge" ? `<div class="tune-warn">âš¡ Surge amplifies the effect but can backlash â€” the cost scales with the ability's Tier.</div>` : "") +
          `<button class="tune-commit" data-tunecommit="${i}">Commit â€” ${esc(tuneSel.intensity)}${sel ? " " + esc(fullCatalog()[sel]?.name || sel) : ""}</button></div>`;
      }
      ...
      let btns = "";
      if (d.type === "duel") btns = mk("âš‘ Try to flee", "flee") + mk("Yield", "yield");
      ...
      if (d.type === "puzzle") {
        ...
        for (const [ui, u] of puzzleUnlocks(d, character).entries()) btns += mk(`â—ˆ ${esc(u.note.slice(0, 80))}`, "unlock", ` data-unlock="${ui}"`);
  ...
  if (turn || opts.error || opts.gmAside || opts.aside || (!busy && !opts.thinking)) {
    ...
    if (apt) {
      main += `<div class="gambit-hint"><span>This looks like a job for a plan.</span>
        <button class="opt" id="gambit-hint-go">âš™ Plan a Gambit</button>
        <button class="gambit-hint-x" id="gambit-hint-x" title="Not now">Ã—</button></div>`;
    }
    main += `<div class="freeform">
      <div class="mode-chips">
        <button id="mode-act" class="mode-chip ${askMode ? "" : "active"}" title="Act in the scene">Act</button>
        <button id="mode-ask" class="mode-chip ${askMode ? "active" : ""}" title="Ask the GM out-of-character â€” context, rules, what your character would know. Never advances the story.">Ask GM</button>
      </div>
      <input id="freeform-input" placeholder="${askMode ? "Ask the GM anything â€” context, rules, what you'd knowâ€¦" : "Or do something else â€” describe itâ€¦"}" ${busy ? "disabled" : ""}>
      <button id="freeform-go" ${busy ? "disabled" : ""}>${askMode ? "Ask" : "Act"}</button>
      <button id="gambit-open" class="mode-toggle ${apt ? "apt" : ""}" title="Plan a multi-step gambit" ${busy ? "disabled" : ""}>âš™ Plan</button></div>`;
  }
  if (isDev()) {
    const flavors = ["beneficial", "benign", "beautiful", "dangerous", "theft", "chase", "fight"];
    main += `<div class="dev-panel"><div class="dev-title">ðŸ”§ Test encounters (dev only)</div><div class="dev-btns">${
  ...
  if (turn) {
    ...
    // SNG-015 Part B: tune panel (ability + intensity) â€” re-renders to reflect selection
  ...
  for (const btn of app.querySelectorAll("[data-join]")) btn.onclick = async () => {
    ...
    saveCharacter(character);
    renderPlay(null, { thinking: `${c.name} arrivesâ€¦` });
    const result = await runGM({ resolution: null, playerInput: `(${c.name} has just joined the character as a traveling companion. Introduce them into the current scene naturally â€” their arrival, how they read the situation â€” then continue the scene.)` });
  ...
  for (const btn of app.querySelectorAll("[data-part]")) btn.onclick = () => {
    ...
    if (character.companionBonds) delete character.companionBonds[c.id]; // SNG-126: clean part-ways â€” no orphan bond/name state
    if (character.companionNames) delete character.companionNames[c.id];
    saveCharacter(character);
    renderPlay(character.activeScene?.lastTurn || null, { aside: `${character.companionNames?.[c.id] || c.name} drifts on â€” for now.` });
  };
  ...
  for (const btn of app.querySelectorAll("[data-recruit]")) btn.onclick = () => {
    ...
    saveCharacter(character);
    renderPlay(character.activeScene?.lastTurn || null, { aside: `${nm} joins your company${cat.teaches ? ` â€” they can teach you the ${traditionLabel(cat.teaches)} craft` : ""}${cat.liaisonFor ? `, and speak for you among their people` : ""}.` });
  };
  for (const btn of app.querySelectorAll("[data-partally]")) btn.onclick = () => {
    ...
    saveCharacter(character);
    renderPlay(character.activeScene?.lastTurn || null, { aside: `${nm} parts from your company â€” the road may cross again.` });
  };
  ...
  for (const b of app.querySelectorAll("[data-setname]")) b.onclick = (e) => {
    ...
    // ("Pell" â†’ "Pell Ran Marsh"), not just names an unknown. The old empty prompt read as
    ...
    const nn = prompt(`Their full name?\n\n(You can extend it â€” "Pell" â†’ "Pell Ran Marsh". The old name is kept as an alias so the GM still recognises it.)`, cur);
  ...
  // CCODE-05: "this is the same person asâ€¦" â€” the merge only the PLAYER can make. Auto-merge matches
  ...
  for (const b of app.querySelectorAll("[data-mergenpc]")) b.onclick = (e) => {
    ...
    const r = applyStateOps(character, [{ op: "mergeEntity", fromId, intoId, why: `player: same person (${from.name} â†’ ${others[idx][1].name})` }], {
      backgrounds: CONTENT.backgrounds || [], traditionIndex: CONTENT.traditionIndex, locations: CONTENT.locations,
      resolveLocationId, worldDay: absoluteWorldDay(), nowISO: new Date().toISOString()
    });
    saveCharacter(character);
    const ok = r.applied.length > 0;
    renderPlay(character.activeScene?.lastTurn || null, { aside: ok ? `Merged â€” ${from.name} and ${others[idx][1].name} are one person now.` : (r.refused?.[0]?.reason || "Couldn't merge those two.") });
  };
  ...
  // (SNG-114: item name/drop now bound via bindItemCardHandlers above â€” the duplicated data-nameit/data-drop wiring is gone.)
  ...
  // ability-arch v2: no rank-up handlers in the play ability panel â€” depth is earned through use
  ...
  for (const b of app.querySelectorAll("[data-learn]")) b.onclick = () => {
    ...
    if (r.ok) {
      ...
      saveCharacter(character);
      renderPlay(turn || character.activeScene?.lastTurn || null, { aside: `You ${free ? "have practiced your way into" : "begin learning"} ${fullCatalog()[b.dataset.learn]?.name}${free ? " â€” no point spent" : ""}.` });
    } else alert(r.why);
  };
  const findBtn = document.getElementById("party-find");
  if (findBtn) findBtn.onclick = async () => {
    findBtn.textContent = "Lookingâ€¦";
  ...
  // turn gate: not your turn â†’ choices and Act wait (Ask GM stays open)
  ...
  if (retry) retry.onclick = () => startScene("(Retry the previous beat â€” pick up smoothly from where the story last stood.)");
}
...
/** SNG-190 Â§5 / SNG-189 Â§1: coerce a scene summary to a safe string before it can reach the permanent
 *  chronicle. The model is asked for a string; when it returns an object that still parses, `String(it)`
 *  is "[object Object]" â€” so we look inside for a text field first, and only then fall back to a clamp of
 *  the narration (the one durable half we still hold). Returns null when nothing usable survives â€” the
...
function renderForkModal(abilityId, onPick) {
  ...
  el.innerHTML = `<div class="fork-card"><div class="fork-title">${esc(ab?.name || abilityId)} â€” Rank ${f.atRank}: choose a path</div>` +
    `<div class="fork-prompt">${esc(f.prompt)}</div><div class="fork-paths">${paths.map(p =>
      `<button class="fork-path" data-forkpick="${esc(p.key)}"><div class="fp-name">${esc(p.name)}</div><div class="fp-grants">${esc(p.grants)}</div><div class="fp-cannot">â–³ ${esc(p.cannot)}</div></button>`).join("")}</div>` +
    `<div class="fork-warn">Permanent â€” the path you don't take locks forever for this ability.</div>` +
...
/** SNG-101: the promotion commit modal â€” the ONE place a promotion is applied. Names the foreclosure
...
function renderPromotionModal(domainKey) {
  ...
  const closesNew = domainKey === "tertiary" && anti && !(character.foreclosed || []).includes(anti); // secondaryâ†’primary antipode is already closed
  ...
  el.innerHTML = `<div class="fork-card"><div class="fork-title">Become ${esc(traditionLabel(e.trad))} as your ${e.to}?</div>` +
    ...
    (closesNew ? ` It also <strong>closes ${esc(traditionLabel(anti))}</strong> to you by ordinary means â€” anything you've already learned of it you keep, and the braid road remains. This is a choice about who you're becoming.` : ` The far pole of that axis was already closed to you.`) + `</div>` +
...
/** SNG-102: the acquisition commit modal â€” the ONE place a domain is acquired. Names the antipode-
...
function renderAcquisitionModal(traditionId) {
  ...
  el.innerHTML = `<div class="fork-card"><div class="fork-title">Become of the ${esc(traditionLabel(traditionId))}?</div>` +
    `<div class="fork-prompt">You join them as a <strong>novice â€” Tier I</strong>, however great you are elsewhere; their craft grows by the same standing as any other.` +
    (anti ? ` This <strong>closes ${esc(traditionLabel(anti))}</strong> to you by ordinary means â€” the far pole of their axis; the braid road remains. Reach is earned one people at a time.` : "") + `</div>` +
