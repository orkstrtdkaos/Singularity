...
**CCode · started 2026-08-30 · v1.9.459**
...
| `craftSubAttributes` | `resolution` | **on**, ruled 2026-09-11 · power / finesse per parent; harm by `operativeAxis` (v1.9.453) | ✅ Erik 2026-09-11, *"keep the eight stats on"* while the ~10-point tilt toward the player is diagnosed (`po/STOCKTAKE_20260911.md` §2) — *"there are 8 stats, not the 4 we started with"*: each craft row rolls one of the eight; an authored `subAttribute` wins; bodies build toward the subs they roll. `§166` |
...
| `holdFeatures.kinds.*.build` / `.upkeep` | `economy` | Aevi's table (SPEC_hold_costs §2): a keep 40 cut stone · 60 days · 4/pass; a watch 3 raw · 4 days · **8/pass**; a waygate cannot be built · 12/pass | every feature costs to build (store first, purse second, else it stalls as a project) and to keep; inherited and granted are free to raise and still cost their keep |
| `holdStore.growth.wagePerHand` / `improveEnergyMult` / `improveSeasonDays` | `economy` | **3** · **2** · **12** | §5 a hand asked to come and work is paid; §4 a craft on a place costs its energy × 2, and one that holds ground or defends lasts a season (warns one pass before) |
