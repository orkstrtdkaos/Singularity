...
|---|---|---|---|---|
| `tree[].gainAxes` | rank node | **1213 nodes / 976 values** | ⛔ **the KINDS-OF-IMPROVEMENT list (9)** | ✅ `capabilities.js` — **for PRESENCE only** |
...
- ⛔ **`gainAxes` DECIDES WHICH RANKS APPEAR IN THE PLAYER'S CAPABILITY MENU.** The chain is
  ...
  switches on *which* kind of improvement it names. **So the field is load-bearing and the 914 individual values are
...
⛔ **THREE MAGNITUDE LADDERS. THE OTHER TWO NEVER WERE.** ⚠️ **Two rows above are not magnitudes at all,
so a percentage curve applied to them is a CATEGORY ERROR, not a tuning miss** — `stage` is an ordinal where
"+100%" is meaningless, and `reachesDepth` starts at **0**, which a multiplicative curve cannot express at any
rate. **`reachesDepth` is `reachOf(rank)` from `death.js` and must never be computed a second way.**

⛔ **2026-09-05 — THE COUNT WAS FIVE AND THE BASIS WAS FOUR, AND THE TRIPWIRE COULD NOT TELL.** Aevi authored
the remaining eight bond grants; each carries `tree.stage: [1,2,3]` — a **bond BAND**, an identity — and the
ladder count went **5 → 13** while nothing about the curve moved. ⚠️ **Measured back at `34f42789`: the
original five were these three magnitudes, `reachesDepth`, and `the_attended_end`'s `stage` — the FIRST bond
grant.** So `stage` was miscounted from the day the first companion was granted a craft, and the "entire
empirical basis" has been **three magnitudes and one zero-based index** the whole time.

⚑ **The gate now trips on MAGNITUDES and reports ordinals separately.** ⚠️ **This table is a per-KIND census,
not a per-craft one** — the `stage` row stands for every bond grant (**9 as of 2026-09-05, one per companion**),
and listing nine identical ordinals would say the basis grew when it did not. ⛔ **The lesson is the one this
section already taught and its own guard did not learn: a KIND column that the checker never reads is prose.**
...
| a **bearer** (a registry entry) | `inventory[]` + `practice` (`npcs.ensureBearer`), an item carrying `lentBy`/`lentDay`; `giveItemTo`/`takeItemFrom` move the object; `bearersOf` lists who holds anything; `carriedForGM` tells the narrator. `evolution.js` takes the bearer and reads `bonds` separately (the bond is the player's) | an NPC who could not hold a thing at all |
...
| `power_cosmology` | 36.7 KB | | `the_veil` | 7.6 KB |
| `nexuses` | 25.3 KB | | `healing_intent` | 4.8 KB |
...
### ⛔ `requiresSelf` — THE DISCRIMINATOR, BECAUSE I GOT TWO OF SEVEN WRONG

**Erik, 2026-08-30: *"grief strike would work against creatures without self BECAUSE IT'S GENERATED BY ONE
THAT HAS A SELF."***

⛔ **`requiresSelf` ASKS WHETHER THE **TARGET'S** INTERIORITY IS NEEDED. It does not ask whether the craft
is emotional, mental or perceptual.**

| ✅ requires a self | ⛔ does NOT |
|---|---|
| `the_known_name` — **they must KNOW you** | `grief_strike` — ⚠️ **the grief is the WIELDER'S, delivered AT a thing** |
| `break_the_line` — they must **believe in the man beside them** | `known_in_the_dark` — reads **breath and weight**; a construct is invisible because it **does not breathe** |
| `unbearable_word` · `felt_wall` — reach a knowing, an intent | `who_falls_first` — reads **capability**, and ⚠️ **a crew of constructs has capabilities too** |
| ⛔ `psychic_lance` — **removed 08-30**: *"no vector, no contact, nothing to block"* means it ARRIVES; see below |

⚠️ **I ORIGINALLY DECLARED SEVEN AND TWO WERE WRONG** — `known_in_the_dark` and `who_falls_first`. ⛔ **I
had been reading the field as *"is this craft about minds"* when it means *"does this need the target to
have one"*.** ✅ **Erik's grief-strike ruling is what made the error visible, and it is the test to apply to
any new craft: could this land on a mechanism? If yes, it does not require a self.**

⛔ **AND A LIVING-BODY REQUIREMENT IS A DIFFERENT THING.** `known_in_the_dark` fails on constructs and would
read **a raised crew perfectly** — a body with no self. ⬜ **If that ever needs enforcing it wants its own
field, not this one.**


### ⚠️ AND A THIRD CORRECTION: `psychic_lance` — SOME IMPACT IS NOT A BINARY

**Erik, 2026-08-30: *"check psychic lance — it seems like that one might still have SOME impact even if the
thing doesn't have a mind."***

⛔ **I HAD DECLARED `requiresSelf` TO MATCH THE CRAFT'S OWN `notFor` SENTENCE — *"anything without a
mind"* — INSTEAD OF TESTING THE SENTENCE.** ⚠️ **Authored prose is not evidence for a mechanical claim; it
is the thing the claim should be checked against.**

**By Erik's own grief-strike rule the thought is generated by a mind that HAS one**, and it is delivered
with *"no vector, no contact, nothing to block."* ⛔ **It arrives at a construct.** And r3 proves it: *"the
struck mind cannot hold anything for a round — a sustained craft drops, a held action is lost."* **A feral
construct running a broken directive has something it is holding.**

✅ **THE FIX IS `psychic: resist` ON THE CREATURES, NOT A BLOCK ON THE CRAFT.** ⛔ **`requiresSelf` is
BINARY and cannot say "less".** The affinity verdicts can — `immune` · `resist` · `vulnerable` · `absorb` —
and **`resist` is exactly "some impact, not full".** Nine selfless creatures now carry it.

⚠️ **AUTHORED PER-CREATURE, NOT DERIVED FROM THE CLASS**, because a class-level affinity default is the
...
|---|---|---|---|---|
| `rank` | 1724 | `tree`×1213 `rankDeltas`×511 | ✅ READ | `battle_turn.js`, `braids.js`, `capabilities.js` |
| `name` | 1644 | `root`×431 `tree`×1213 | ✅ READ | `affiliation.js`, `arceffects.js`, `art.js` |
...
| `axes` | 431 | `root`×431 | ✅ READ | `affinities.js`, `craftmechanics.js`, `encounters.js` |
| `attribute` | 431 | `root`×431 | ✅ READ | `affinities.js`, `battle_turn.js`, `braids.js` |
| `narrationHints` | 431 | `root`×431 | ✅ READ | `battleprompt.js`, `braids.js`, `companions.js` |
| `description` | 431 | `root`×431 | ✅ READ | `affiliation.js`, `art.js`, `authormode.js` |
| `notFor` | 431 | `root`×431 | ✅ READ | `braids.js`, `entityDetail.js`, `generate.js` |
| `tradition` | 431 | `root`×431 | ✅ READ | `affiliation.js`, `arceffects.js`, `art.js` |
| `operativeAxis` | 431 | `root`×431 | ✅ READ | `craftmechanics.js`, `progression.js` |
| `intensity` | 431 | `root`×431 | ✅ READ | `battle_turn.js`, `canon.js`, `craftmechanics.js` |
| `plainly` | 431 | `root`×431 | ✅ READ | `gm.js`, `narration_voice.js`, `recovery_snapshots.js` |
...
| `rankDeltas` | 283 | `root`×283 | ✅ READ | `craftmechanics.js` |
| `note` | 239 | `mechanic`×239 | ✅ READ | `art.js`, `assignments.js`, `authormode.js` |
...
| `targets` | 148 | `mechanic`×148 | ✅ READ | `capabilities.js`, `craftmechanics.js`, `reconcile.js` |
| `gated` | 112 | `root`×112 | ✅ READ | `borncontract.js`, `generate.js`, `gm_registry.js` |
| `damageType` | 108 | `mechanic`×108 | ✅ READ | `craftmechanics.js`, `damagetypes.js`, `gm.js` |
...
| `schemaVersion` | 65 | `root`×65 | ✅ READ | `canon.js`, `codex.js`, `devreport.js` |
| `wardTypes` | 55 | `mechanic`×55 | ✅ READ | `damagetypes.js`, `gm.js`, `skill_battle.js` |
...
| `reachesDepth` | 2 | `mechanic`×2 | ⛔ DARK | — |
...
| `mechanic.ongoingHarm` | the tier-marker work | ⛔ **never** | ⚠️ read by `capabilities.js` as a TIER MARKER; first craft to author one was rejected |
