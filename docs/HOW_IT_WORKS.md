...
| 08-29 | ⛔ **A LIGHT CREATES SHADOWS — the tradition had light backwards** | Erik: *"just because they are Umbral doesn't mean they hate all light… they do have some lights down there too, biolume."* ⚠️ **The Depths are canon-authored as *"sunless underworlds LIT BY LIVING GLOW"*** | — corrected in `shadowstep`, `shadow_work`, `long_dark` and the culture | ⛔ **I had authored TWELVE crafts where light CLOSES the working and NONE where it ENABLES one.** A pitch-black room is useless to an Umbral: it casts nothing. **What they want is CONTRAST** |
