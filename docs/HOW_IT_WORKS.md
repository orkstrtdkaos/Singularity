...
| 09-12 | ✅ **AND NOW IT IS CASTABLE TOO — the third and last step** | Erik: *"It's both learnable and castable."* The 08-30 row above is history and stays: the direction of travel is the setting's own thesis — **CLOSED → learnable-not-castable → learnable-and-castable-to-an-earned-ceiling.** A model that foreclosed by identity, replaced in three steps by one that prices by relationship | ⛔ **`antipodeCeiling` against `skill_capacity.antipodeCeilingByLean` was ALREADY BUILT AND LIVE** — the engine had grown past the stub and only the prose was still describing a rule nobody enforced | ⚠️ **BUT THREE READERS WERE STILL ENFORCING IT.** `app.js` REWROTE `foreclosed` on every load (so it was never even legacy data), the skill tree rendered those crafts FORECLOSED — *"you chose otherwise"* — and `practicedTraditions` struck the people from the set a teacher could reach. ⛑ And `traditions.js` carried a comment asserting the field was *"READ NOWHERE ELSE"*, written by the module that retired the rule without checking — which is CCode's own 09-01 note, quoted back: **a ruling that updates one reader leaves the others speaking the old law** |
...
| 09-11 | ⛑ **MEASURE WHAT THE CONSUMER READS, NOT WHAT THE FORMULA COMPUTES** | Aevi reported six findings from a value taken partway through a pipeline. Each was arithmetically right and described something no consumer sees | the dice: `perTier 0.5` measured four times and a die ladder specced that **already existed** — the dice path is the FIRST branch, and `mechanicFor` returns `{n:5,d:6}` for a T5. The Great Engine: reported at **1.20 fitting no band**; `resolveSubstrateField` ends in `clamp01` and it rolled on **1.00**, which is IN BAND for precursor and nanite. The arc sign: the shift applied to each source's READING rather than the ground, making `What Wakes Beneath` strengthen the veil | ⚠️ **when a value passes a clamp, a fallback or an overlay, call the reader** · `§154` · `§164` |
| 09-11 | ⛔ **AUTHORING MARKUP REACHED THE PLAYER — 386 of 429 rank lines** | Aevi's PO-document emphasis convention (glyphs, shouting caps) is in `description`, `notFor`, `plainly` and every `tree[].grants`/`cannot`. The skill card renders all four | measured across 429 crafts: `tree[].grants` 386 · `tree[].cannot` 233 · `notFor` 152 · `description` 95. `dread_mantle`'s description and rank-1 grants are CLEAN — the discipline held on the first field of every craft and lapsed on the rest | ⚑ **the convention is right for PO docs; content has a different reader** · `BUG_authoring_markup_in_player_text` · ⬜ a gate: no player-facing craft field carries an authoring glyph |
| 09-11 | ✅ **THE PELL–VETH CENSUS, RE-RUN UNDER R43 — AND IT REVERSED COMPLETELY** | CCode marked the 09-04 census history when he built R43; Aevi re-ran it before it could be quoted again. **Veth won 99.9% then; PELL WINS 95.8% NOW.** Her killers `hastened_grey` and the two T3s were never lethal by design — **they were lethal by a missing gate** | `duel_pell_vs_veth.mjs`, 2000 seeded duels through `engine/battle_turn.js`: death-save kills **Veth 0 · Pell 0**; Pell dies in **0 of 2000**; every win by pressure (1915, health 0); mean 18.7 rounds; finishing blows Keystone Blow ×1480, Necrotic Strike ×277 | `§154` · R43 · ⚠️ **a ruling of Erik's sat unbuilt for seven days and the census measured the gap, not the game** |
...
| 09-08 | ✅ **THE WRONG GLOSSES — twelve crafts described to the GM as something they are not** | ⛔ **`SPEC_tradition_narrative_npc_pass` Tier 1: a gloss naming a verb family the craft does not have is a CORRECTNESS BUG, not polish.** ⚠️ **Measured fresh: 42 crafts carry a stock gloss and TWELVE named the wrong family — and ALL TWELVE SAID THE SAME SENTENCE, *"makes or animates something."*** ⛑ **One stale default, not twelve errors:** a concealment craft, a bargain, a truce and an hour held open were all being told to the GM as though they BUILT something. ⚑ **And every description was already right** — `never_there` is *"erasure of your passage"*, `held_hour` is *"not stopping time, refusing to let ONE hour finish"* — ⛔ **only the gloss was wrong.** Rewritten to name what the GM narrates CHANGING (T7: *"a dice roll is not an answer"*) | ✅ **wrong-family glosses 12 → 0, verified corpus-wide; `craft_lint` 68 unchanged, both suites unchanged** | body — none needed, this is content correctness. ⚠️ **AND THE SPEC'S OWN ESTIMATE WAS ~48: the real number is 12 because the corpus has been repaired since 08-31.** ⛔ **An eight-day-old count is a claim about eight days ago** — measure before authoring. ⬜ **Tier 2 (30 thin-but-correct glosses) remains** |
| 09-08 | ✅ **R50 — seven crafts were gated as harm and offered no harm verb** | ⛑ **CCode's finding, and the first half was his own gate's fault: `abilitiesCombatClaimedNotTaught` GREPPED PROSE for an offensive word, so four crafts went red when Aevi's sentences got BETTER.** ⚠️ *"Wrong in both directions — it reddens when an author improves a sentence, and stays green for a craft that is genuinely toothless but well-described."* ✅ He moved the reader to the authored `functions`/`harmRung` fields, which cover 429 of 429. ⛔ **READ THE OTHER WAY IT FOUND SEVEN THAT WERE AEVI'S**, and the split is the ruling: **does it kill by DOING something to a person, or by GOING WRONG?** ⚑ `case_closed` gains `break` — nothing is struck, a person's reasoning is. ⚑ `calling_back` and `open_threshold` soften to `incapacitating` — retrieval crafts whose danger is the caster's, ⛔ **and `lethal` was firing the intent question at a warden trying to save someone** | ✅ **all seven verified; `how_it_works` 0, `content_ci` 9 unchanged.** ⛑ **AND SHE GOT TWO WRONG.** ⛔ **Erik: *"doublecheck the skills you looked at — case_closed was INTENTIONALLY made to be able to kill."*** ✅ It was, and the audit note says so — `_rungFix_20260830`: *"a GM would have been told the Syllogists' darkest craft is harmless."* ⚠️ **The rung was right and the fix was HALF-DONE: she added the verb to the RECORD and not to any RANK, which is `abilitiesCombatClaimedNotTaught` — the exact violation CCode's gate is named for.** ⛔ **She fixed one direction of the mismatch by creating the other**, on three crafts. ✅ Corrected corpus-wide: zero records claim a harm verb no rank teaches. ⛑ **AND SHE GOT ONE WRONG BEFORE THAT, CAUGHT BY HER OWN CHECK** — she ruled `graftlife` already-correct because `transform` was on the record, ⚠️ **and `transform` is not in the harm family**. Corrected to `none`: *"the graft is not the original, and it knows it"* is a cost to the healed, not damage to a target | body §7c⅘. ⬜ **CCode deliberately did NOT ratchet this** — *"choosing between those two is authoring, and a ratchet would be me making the call for you"* |
| 09-08 | ✅ **R41b — the Sovereign arrivals, four open items ruled** | ⚑ **STAGE 3 IS THE ARRIVAL**, diminished at 45, and arriving is already doing something: *"push the arc back their way and work to eliminate the threats."* ⛔ **AND ERIK REPLACED AEVI'S SECOND QUESTION WITH A BETTER ONE.** She asked *"does the anti-Sovereign KNOW from the start?"*; he answered ⚠️ **KNOWING IS NOT THE VARIABLE — URGENCY IS, AND THE ARC DRIVES IT, INVERSELY FOR BOTH SIDES.** *"If the Sovereign is fat and happy it's business as usual for them and their agents. The opposite is true for the anti-Sovereigns… the middle zone everyone's trying to push… in the end the Sovereign is the most upset, but the anti-Sovereign is also the most motivated to end them."* ⛑ **Which makes the end-state DANGEROUS rather than safe: pushing an arc against a Sovereign produces the angriest version of the thing AND its most committed enemy.** ⬜ They TRY to know each other — both investigating, both partially right | ⬜ **content ruling, no gate.** ⚠️ CCode measured the stage→form mapping already: stage 1 at the mythic floor, stage 3 at 45, stage 4 at 85 — **the machinery was built and waiting for which stage** | body §7c¾. ⬜ **Item 4 — what a recrossing leaves — DELIBERATELY UNAUTHORED.** Erik: *"this might be narrative."* ⚑ A thing declared open is not a thing forgotten |
| 09-06 | ✅ **THE GEOGRAPHY REDS CLOSED — six of them, and two places changed region without moving** | ⛔ **SNG-404: sixteen drifts between Aevi's hand-authored `_measured` blocks and the engine reading the same world** — the cluster moved three times (SNG-427, SNG-434, the 09-06 gate move) and the readings never followed. ⚑ **All 18 layouts re-derived with the call the gate itself uses, so they cannot disagree by construction**; `_wasAuthored` keeps what she measured, because a reading replaced is not a reading that never happened. ⛔ **GREYWATER STILTS MOVED 0.67°** — it measured relief 0.020 against a 0.018 cut, and *"a town on stilts over the southern marsh, dead flat"* cannot sit on a usable slope. ⚑ **`anti-uphill` had a branch, a name in `SITE_BASES` and NO CONTENT** — Millbrook's Long Fields authored, because Erik named the fields in the original seed and every other noun in it had been placed. ⛔ **AND TWO PLACES WERE IN THE WRONG REGION, WHICH IS NOT A POSITION PROBLEM**: Kestrel's Roost had three of its four roads outside the valley, and the Sunken Choir is *"below the Heights"* — and the Heights are the Echo Vale's | ✅ **content_ci 14 → 8; `how_it_works` held at 0.** ⚠️ **Each move was MEASURED BOTH WAYS before it was made** — Kestrel's Roost 5.1× stranded in the valley against 2.2× in the Vale; the Choir 3.7× against 1.5× | body §7e. ⚑ **The valley is now a TIGHT cluster (r 5.6°) and the Echo Vale is the WIDE one (r 27.8°)** — which is the right way round: *"the ground between two civilisations that do not fight and interfere anyway"* should be wide, and a farming valley on a river should not |
| 09-06 | ✅ **A PERSON CAN START AN ARC — 7 NPCs carry `questSeeds`** | ⛔ **MEASURED: ZERO of 48 carried one, and the ratchet went red at 48 because Aevi authored a 48th person without one.** ⚠️ SNG-167 §2 states the design and the corpus had never answered it: *"a LOCATION can start an arc and a PERSON cannot… **that is backwards: the memorable arcs start with someone, not somewhere.**"* ⚑ **A seed is an errand THE PERSON WOULD ASK FOR, drawn from their own `wants` and `fears`** — Calvar tests whether you are competent before he asks; Ama has never been asked in the right way, *"and the right way is asking about HIM rather than about the stone."* ⛔ **R49's shape one layer down: a seed that names no objective is a nebulous hook** | ✅ **the ratchet came DOWN, 48 → 41, rather than being re-baselined** — ⚠️ **which is what the check asks for: *"author seeds, or re-baseline deliberately."*** ⛑ **`questSeeds` declared in `npc.schema.json` — caught by checklist step 4, BEFORE the gate** | body §7c½. ⬜ 41 of 48 still rely on the derived want |
...
| 09-16 | ✅ **WHAT IS TRUE OF A PERSON IS THE WORLD'S; WHAT A PLAYER KNOWS STAYS THEIRS (CCODE-385)** | Erik: people *"grow, have careers, do deeds, and die"*. ⚑ The 4 people with a shared identity in two or more saves (Mara Wells, Edvar Crane, Deni Cors, Pell) had a different role in every save — Edvar a water-reader to Adelheid, a filtration engineer for Stillwater's Trouble to Silas | `§270`: a changed role or status is stamped and published; a death is written onto every registry that knows them, with news; a new role is held beside what the character knows and shown to the GM as IN THE WORLD NOW, and goes when what they know catches up; only between the same full name | ⚠️ nothing is backfilled: a role written before today stays that save's own, because across the saves most of the difference is each GM's wording, not a change in the person |
| 09-16 | ✅ **A SHARED FILE'S MERGE NEVER READS BLIND (CCODE-386)** | `pushMergedFile` and `appendLedger` were the last readers on the shape SNG-549 retired from the saves: the envelope, base64, `catch { remote = null }`. Over a megabyte the envelope is empty, and a body that would not parse became "no remote" — a merge against nothing, written over every player's file | `§271` against the fake GitHub: forty fates kept over the line, every merge read revalidated, an unparseable file refused with nothing written, a big ledger month appended to rather than replaced | ⚠️ a merged write over ~950KB is refused out loud: the git-data fallback compares the branch, not the file, so it cannot guard a merge — the canon store is the largest shared file today, at 110KB |
| 09-16 | ✅ **A JOURNEY IS AGREED, THEN READIED, THEN WALKED (CCODE-387)** | Erik: *"agree to go on a journey and it logs it as a task... like a one step quest... that gives you time to stock up, collect your people, organize the trip, then begin the journey"* — and *"hunger damage is a good idea.... there are travel and gathering skills that would be more useful with it."* ⚑ Every trip cost three hours whatever the distance; a map line read "about 74 days on foot" over "Travel here (+3h)" | `§272`: a day's walk or more logs a plan and a one-step quest and nobody moves — through the GM's move, the departure question (plan / set out now / stay), the one-tap arrival and the map; setting out walks the road's hours, eats rations, and hunger costs energy and health; greenlore, Hard Mile and staunch measurably carry a 10.7-day road (forage 35% → 65%, hungry days 7 → 3) | ⬜ a leg as a gambit (SNG-333), and a journey interrupted and resumed on the road (Aevi's `theQuestShape`), are the next stage — the road's one encounter check still happens on arrival |
| 09-16 | ✅ **TRADING WITH ANOTHER PLAYER'S HOLD (CCODE-388)** | Erik's yes: the owner opens a hold; goods sell at its Reach's price; the visitor pays and carries them away; the owner's next tick fills the order from the store and pays back what had sold out. ⛔ Not a shop — the keeper sells in conversation and `holdTrades` records it | `§273`: closed by default; the card's goods and prices; six refusals; the purchase (goods a trader elsewhere can price); the owner's fill, the short order's refund, each once; the money conserved; forward-only statuses; and the whole round through the fake GitHub | ⚠️ the Fell Pell, opened, would sell its 7 working parts and 7 raw materials at 4 crystal each — Adelheid has no purse yet, so her first attempt is refused for want of crystal |
| 09-16 | ✅ **THREE ARC-SCOPING GATES REPORTED NOTHING SINCE v2.0.24 (CCODE-389)** | v2.0.24 made `leader` the tier ladder's rung "only on the tier ladder" — the engine and content kept to it, but in the tests it reached arc `scale` and the map's zoom: the scoped-arc fixture looked for an arc scaled `leader`, there is none, and the three checks inside its `if` stopped running without a red. Erik's own words changed inside his quotes ("switch to the regional map", "a regional 2D map", "I want regional and local arcs"). ⛑ `regional` restored in the 25 places it meant a scale or a map; every content and engine change of that commit checked and left alone (all of them are rungs). And SNG-600 renamed the census gate to 16 sites without the ledger's two claims, and doubled a numeral in EXESA.md — two more reds on main | smoke `272/273`: the three checks run and pass, and a fourth fails if the fixture ever finds no regional arc again; the ledger claims all four (781 gates, 1 problem: SNG-391, the known red) | ⚠️ the ledger saw the three checks missing; the suite did not — a check inside an `if` is a check that can vanish |
| 09-17 | ✅ **NOTHING FORCES A RELOAD (CCODE-399)** | Erik, mid-scene: *"2.0.58 landed and it reloaded the game… I was in the middle of reading. I thought I'd have a button… if it was due to a save, why was a save being sent up when I hadn't taken an action worth saving?"* ⛔ He is right and my reasoning was wrong: CCODE-396 kept the forced reload for a REFUSED WRITE, on the argument that the player's own action had just failed — but almost no write is a player's action. `saveCharacter` queues a push sixty seconds later, and the world tick that runs when a tab OPENS is enough to write, so a man reading was reloaded by a background backup. ⛑ The refusal offers now, exactly as the quiet check does, and the roster carries the banner too — the screen where the refusal already explains itself (*"this tab runs 2.0.57 and 2.0.58 is deployed — it must reload before it writes"*). Nothing is lost while he waits: the save is written locally, the guard protects the copy in the cloud, and the push happens after the reload | `§277`: nothing calls the forced path but the button, and the banner is on both screens | ⚠️ Two builds to get one guard right, and the lesson is the same as the message one: a refusal a player did not cause must not act like one |
| 09-19 | ✅ **THE STARTING PURSE, AND WHAT WAS OWED (CCODE-443)** | Aevi's `SPEC_aevi_starting_purse` and Erik's ruling: *"backfill the saves to the floor and credit them like we did for silas for their deeds."* ⛑ `startingPurseFloor`: LIQUIDITY by background — how much of a prior life converted to portable value — on the `worthBands` rungs (0 · 4 · 15 · 50), never approval (DIRECTIVE_SNG-280); the place sets which money. A new character is paid it at birth (born current, so no step runs). ⛔ Step 71 for the rest: the floor as `max(worth, floor)` — a backfill — and 8 a deed, flat, as arrears; both through `earnAt` by where the character stands; two lines. Silas untouched (R48). A background the table does not name (Adelheid's `medic`, Usnea's `community-organizer`) is asked, never assigned — the deeds pay, the floor waits | `§322`, 4 checks: the table; floor and settlement by place; Silas, the asked, once; at birth | ⚠️ Aevi's §4 table pays the foothills in coin; `money.js` pays them in crystal and this pays through it — flagged to her |
| 09-19 | ✅ **SOLDIERS ON A JOB ARE PAID (CCODE-442)** | Erik: *"If soldiers are working or active on a job they get paid. If they're eating then part of that pay is in food. But the hold stores are meant to be for battles and journeys... so it might make sense to have the everyday food be something that happens narratively and explained through the pay they already get."* ⛑ `jobWages`: a band's hands and a hold's guards sent on a job cost `wagePerHand` (authored, 3) a head for every pass (three days) away; people who are not soldiers are not paid here. The wage joins the job's cost on the Jobs tab — shown before the send ("12 soldiers × 3 crystal × 3 passes away — 108 crystal, and their food comes out of it"), refused whole if the purse cannot carry both, paid in the money of the job's place. ⛔ No ration is kept and no store is drawn — the GM is told soldiers out are paid and eat from it | `§321`, 3 checks: the wage; the cost; the GM | — |
| 09-19 | ✅ **THE GM'S MONEY DOOR (CCODE-441)** | ⚑ Found while building money by place: the GM was never shown the purse and never given `exchangeOps` — `git log -S` finds it in no contract, ever — so nothing bought in the story cost anything and no payment ever arrived; the applier and the purse functions sat behind a door the producer was never told about. ⛑ A `moneyDetail` block shows the GM the purse and the money of the place, on the turn and the ask; `exchangeOps` joins the reply contract with its rule, the salvage list, and the ask channel's forbidden list. ⛔ `market.applyMoneyOps` settles each op BY PLACE: a value with no money named is paid here local first (`payAt`) or paid out in the place's money (`earnAt`); a named money is that money; `buy` moves goods only if the price was paid, `sell` takes the goods and pays in the local money, `change` is the place's rates; coin is only found or traded. A refusal moves nothing and is said under the beat — "The purse did not move: …" — so a purchase the purse could not cover never stands in the story | `§320`, 5 checks: by place and all-or-nothing; sale, reward, coin, change; the GM is shown; the GM is given; the turn applies and says | — |
| 09-19 | ✅ **A HOLD IS A LOCAL MARKET (CCODE-440)** | Erik: *"I want to be able to trade/sell random items and cruft from my pack in at my holding for money... keep our packs tidy"* — and *"A hold should transact and act as a local exchange as well… The Crossing is obvious and likely the most universal and relatively inexpensive. You can find better rates in the reaches but only for the money they want vs the ones they don't."* ⛑ `engine/market.js`: a thing fetches `priceOf` (worth × the place's need × scarcity, the whole stack) and is paid through `earnAt` in the place's own money; most pack items carry no worth, so a stand-in by KIND prices them and says so; a thing the story carries is never sold, the irreplaceable has no price. ⛔ **Changing money, by Erik's words** (superseding the authored "a Reach mostly will not convert"): the Crossing anything for anything at its 15%; the foothills what they take, never scrip; a Reach its own scrip against the money it WANTS — in at 5%, better than the Crossing, out at 35% — and nothing it does not want. ⚑ On Silas's pack: 19 things sellable, 89 shards in the valley and 161 in the Palelands; 10 the story carries. The hold card's two doors stand only where you stand; the purse offers the change at the Crossing or a hold of yours | `§319`, 5 checks: the price; the sale; Erik's rates; the quote and the piece; the doors | ⚠️ The kind prices, each Reach's wants and the two Reach bites are Aevi's to author |
| 09-19 | ✅ **THE NEWS, KEPT (CCODE-439)** | Erik: *"I'd like the news to be something that is saved and I can navigate to review... like a log. in case I missed something or wanted to look back"* — and of the update notes, *"maybe a popup from a news item"*. ⚑ The news lived in two twenty-line buffers: `ws.news`, the GM's rumours, and `ws.unseenNews`, the flash, emptied when shown — so nothing was kept, and a week away dropped the oldest lines before they were ever shown. ⛑ `takeUnseenNews` now keeps what it takes in `worldState.newsLog` (the last 250, never the same line twice), begun on first use from the recent news the world already held; what waits to be shown has its own cap of 60. A 📰 **News** tab on the character shows it newest first by world-day, filtered by whose it is (the world, your work, elsewhere, updates), rendered by the SAME function the flash uses — lifted whole out of `renderPlay` — so a fight still opens from the log. ⛑ **An update is news**: the first time a character comes into play on a newer build, a line says so with the notes one tap away, and so it is kept; a character made on the build is current | `§318`, 5 checks: kept and begun; never twice; the caps; the tab and the shared renderer; the update line. §255 moved to the shared renderer, still near-first in the flash | — |
| 09-19 | ✅ **THE VERSION FOLLOWS A RULE, AND EVERY UPDATE SAYS WHAT IT DID (CCODE-438)** | Erik: *"so far we just keep racking up the smallest increment. Set something that will tell us when we go to 2.1.0 and 3.0.0 etc as the rule that we can follow… a summary for players to see… keep it tidy and options to click on for more detail."* ⚑ §25.7's rule was prose no script could read — 1.8.x ran to ~180 releases and 2.0.x to 100. ⛔ **The rule is code now** (`scripts/version_rule.mjs`, read by the bump and by this suite): a patch every update; the MINOR on the update that brings the fifth new feature since it last moved (the x.y.0 release's own features do not count again); a MAJOR only by Erik's call, for a change that makes the game a different game for someone already playing, refused without the reason in words. ⛑ **Every update says what it did**: `release_notes.json` → `next` holds its lines (feature, fix, change, or internal — recorded, never shown), in plain words with no ticket number, and the bump moves them under the version it cuts and refuses an update with nothing to say. ⛑ **What a player reads**: after an update a banner on the title page and in play says how much arrived since that device last looked; the popup groups New · Fixed · Changed, each line opening for its detail; the version stamp opens the notes at any time, back through the whole 2.x line and the summary of what 2.0 brought before the notes began. A device that played before the notes is told; a new one is not. ⚑ 2.1.0 is cut by hand as the line where the notes begin | `§317`, 7 checks: the rule; a major's reason; the script reads the rule; every update has fit notes; the 2.0 summary; the banner and popup; the stamp and the first-look default | ⚠️ `minorAt` is 5 — Erik's to move |
| 09-19 | ✅ **MONEY BY PLACE — LOCAL FIRST, ELSE A WORSE RATE (CCODE-437)** | Erik: *"The world has several kinds and so far we've just been using crystal... let's fix that"* — and his pick of three, local first, else a worse rate. ⚑ Five currencies were authored with an acceptance table and a conversion formula, and nothing read the table: the keep, a keeper's sale, runner fees, alms, a build, a store sold, quartering, a caravan, a job, a call and a trade between holds were all crystal, and in 16 saves no other money ever held a value. ⛑ `engine/money.js`: a place pays in its own money and takes the others at its rate — the Crossing's region takes every money at its 15% bite; the valley, the Echo Vale and the foothills take foothill money at 25%, paper at half; a Reach pays in ITS scrip and takes crystal at 70% (a stand-in for "crystal (some)"). `payAt` pays a value (shards, the world's own unit) local money first, all or nothing, in each money's authored piece; `earnAt` pays out in the place's money, rounded down. Every flow goes through them, every line names the money that moved, and a refund comes back in the money it was paid in. ⚑ On Silas's save the valley holds are unchanged; Stillwater's Trouble, in the Palelands, now earns Palelands scrip and pays its keep from it — the same worth, a different money | `§316`, 8 checks: the classes; local first; all or nothing in pieces; paid in the place's money; no flow hardcodes crystal; a trade and its refund; the tick; the purse line. §69, §100 and §103 moved from the crystal count they pinned to the worth they guard | ⚠️ The classes and the Reach rate are stand-ins for Aevi. Nothing CHANGES money yet, and no line says it does: Erik ruled mid-build that the Crossing is the universal changer and a Reach changes at better rates for the money it wants — built next, with the GM's money door |
| 09-19 | ✅ **THE PHANTOM AXIS — A CHARACTER'S FINGERPRINT IS THE TWELVE (CCODE-436)** | Aevi's SNG-632 handoff found the intent parser handed the schema's placeholder back as a key (`"axes": {"spectrumId": …}`); CCODE-433 fixed that prompt. ⚑ **The GM's own schema showed the same placeholder** on its choices and on a craft it mints, and nothing checked either: **13 of 16 saves carried `alignment.spectrumId`** (and three `spectrum` or `value`), drifted into by every choice the player took. ⛔ **Every reader read it**: a roll's self-fit is a cosine over the union of keys, so a choice carrying only the placeholder swung the roll by up to 15 points on a sign the GM made up (Splarf 15.0, Brynjar 14.9, Saehara 13.1, Cellaceron 8.3, Loki 6.4, Silas 0.4), and the GM was shown the whole fingerprint each beat, which taught it the placeholder was an axis. ⛑ **One door** (`engine/spectrum.js`): `cleanAxes` (the twelve, and a number by `typeof`) at every intake — a GM turn's choices and minted craft where the turn arrives, a plan's steps in the gambit reader, drops counted for the dev report — and `driftAlignment` for both writers, which wrote any key they were handed. The three prompts that showed the placeholder show a real id and name the twelve. Step 70 sets the off-atlas values ASIDE in `_retiredAxes` (19 keys on 13 saves), as SNG-633 kept content's | `§315`, 8 checks: the clean; the door; both writers; the intake; the plan; the prompts' lists ARE the content's; step 70 with and without content | ⚠️ A phantom-only choice now rolls with no spectral fit at all, until the GM keys real axes |
| 09-19 | ✅ **A BAND WITH NO BARRACKS IS QUARTERED, AND THAT COSTS (CCODE-435)** | SNG-627 `housing`. Erik: *"Barracks house troops."* Aevi, on the barracks record: *"a band without barracks should be a band that has to be quartered somewhere, and that should cost."* ⛑ **Who needs a bed**: hands raised at a hold of yours live there, up to what it can feed (`handsCap` less its crew — the muster's own bound); recruits a job brought in, a band the fiction raised (the GM's `raise` names no place) and a hold's overflow need your barracks; never the named, a slot out on a job, or a unit called into the field. Beyond the beds they are QUARTERED at `quarterPerHead` a head a pass (one crystal stands in), paid through the purse on the hold pass's own three-day counter; a pass nobody can pay is said, and nothing is invented about what unpaid soldiers do. ⛔ **The tag cannot say which housing houses a band**: `housing` + `residents: true` is carried by a longhouse, a keeper's hut, deck space and the barracks — reading it gave Silas eighty band beds and he has no barracks. So a record authoring `bandBeds` does, and until one does the barracks and its variants stand in at `barracksBeds` (twenty). ⚑ Two bugs on the way: a barracks counted as a home for one worker (`Number(true)`), and a band raised empty from the button carried one nobody, whom the first person to join made real beside them | `§314`, 8 checks: what houses a band; beds are not homes; who needs a bed; a standing barracks; the pass; the tick; the empty band; the tab | ⚠️ No save holds a barracks or an unnamed head today — dormant until a job recruits or the fiction raises a band; `bandBeds`, `barracksBeds` and `quarterPerHead` are Aevi's to author |
| 09-19 | ✅ **AN INFIRMARY HEALS THE HURT AND TENDS THE DEAD (CCODE-434)** | SNG-627 `healing`. Aevi: *"somebody hurt can be brought here and get better"* — and *"an infirmary should matter to a death-ladder retrieval. SNG-566's depths are built and an infirmary touches none of them."* `healingAt` asks `propertyAt`. ⛑ **Three effects, each a stand-in that says so**: a night slept where you keep an infirmary heals `recovery.infirmaryHealth` more (a night's own health again); a band standing there — where it was called, else its seat — counts it as `infirmaryMenders` of its own (two), so a band with no menders is no longer beyond help; and a body lying there is TENDED and sinks `tendedSinkFactor` times slower (twice) — ⛔ on ONE divisor read by the depth and the seal alike, stamped by the tick each pass from where the body lies (the person last seen there; the player where they fell) and cleared when the infirmary is gone. ⚑ At forty days the untended are in the deep dark and the tended still in the near dark; the untended seal at 130 days, the tended at 240. The GM is told where a body is tended | `§313`, 7 checks: where you keep one; the depth; the seal on the same divisor; the stamp; the GM; the band; the night and the seat | ⚠️ Silas keeps no infirmary yet; the three numbers are Aevi's to author |
| 09-19 | ✅ **THE AXES ARE KEYED BY THEIR IDS (CCODE-433)** | Aevi, SNG-632: *"the five 'not a number' drops are a prompt defect and they are CCode's one-line fix… The model fills it as a field because it is written as one."* The intent parser's template read `"axes": {"spectrumId": -1..1}` — a word that looks exactly like a field name, while every other closed vocabulary on the line is enumerated in place — so the model wrote a field called `spectrumId` holding an axis name (Erik's trace on 09-18; five pushes on 09-19). ⛑ The template shows `"<spectrum id>": <number -1..1>` with a worked example, and the ids line is built from the list `sanitizeIntent` validates against (it was typed by hand, so Aevi's order_chaos → chaos_order rename had to be made twice) | `§312`, 3 checks: no `spectrumId` field; the ids offered are the ids accepted; the asked-for form kept and the old one still dropped audibly | ⚠️ Aevi's flag for Erik stands: the world carries 15 axes and the GM is offered 12 — individual_collective and tradition_innovation are authored and unread |
| 09-19 | ✅ **WHERE THE MOUNTS ARE KEPT, YOU RIDE (CCODE-432)** | SNG-627 `mounts`. Erik: *"Stables can shorten journeys but can also house the mounts for cavalry."* ⛑ `propertyAt` is the one question every property hook asks — a hold of yours at this place with a STANDING feature of that property (`trainingAt` is it now, and `mountsAt`). ⛔ **A journey set out from where you keep mounts is RIDDEN** (`mountedFrom`): the walked days shrink by `rules.journey.mountShare` — ⚠️ UNAUTHORED, a third stands in — and a gate's own hours do not; it is the better of a mount and a craft's march, never both, and the plan says it is riding ("riding from Stillwater's Trouble (the long stable)"). ⚑ Silas, Stillwater's Trouble to Millbrook with a stable granted on a copy: the gate road 10.2 → 7.8 days, the long way on foot 205 → 137.4. A job team setting out from that hold rides too. ⛑ **Hands raised where the mounts are kept can be raised mounted** — cavalry, doing MOVE as well as fighting (and a band of nothing else still bleeds for its gaps) | `§311`, 6 checks: where the mounts are kept; the walked days; the gate's hours; the better of mount and march; the job team; the muster | ⚠️ Silas keeps no stable yet; a boat-house and a corvid loft are named in Aevi's property list but not authored as kinds |
| 09-19 | ✅ **TROOPS ARE NOT FURNITURE (CCODE-431)** | SNG-627. Erik: *"troops get fed and paid, but they can also be put to work and do jobs and missions."* A garrison was only a bill. ⛑ **The Jobs tab can send them**: a band's hands as ONE member of a team (`jobUnitFor` — `n` of them, at the level their quality stands for, rolling the families the contingent does, working as `n` pairs of hands) and a hold's GUARDS as people, from the hold. ⛔ **Sent, they LEAVE** (`detachForJob`) — the heads come out of their contingent (the slot stays, empty, so an id never names anyone else) and the guard off the garrison — so a fight, a call, a raid, the watch and a hull's crew all see them gone without one of those readers learning about jobs; and a hold's capacity still counts them (`detachedFrom`: a head out on a job eats its bread). ⛔ **Whoever comes back comes back** (`returnFromJob`): a job that carries harm costs heads by the band's own blood formula at the degree's tide — a critical failure bleeds them as a rout would, a success barely — and the band's losses and condition carry it; a guard goes back on the watch; a band that no longer stands or a hold no longer yours is SAID. The GM's directive says who came back. ⚑ On Silas's save, his guards Siol and Bryn Calowell can be sent; his one band is six named people and no hands. ⚠️ And the muster picker counted only the band being mustered into, so two bands could each raise a hold's full count — it counts every band now | `§310`, 10 checks: the pool; what a unit rolls and works; they leave; the capacity still counts them; the blood a harmful job costs; a harmless one returns all; the guard back and said; nothing silently absorbed; the app; the forecast's words · `§305`'s Veth check rewound — step 68 has run in Erik's own tab | ⚠️ keepers and crew stay at their work; a band sent on a MISSION (an errand kind) is not built yet |
| 09-18 | ✅ **A YARD TRAINS (CCODE-430)** | SNG-627's first hook. Erik: *"the muster yard enable training and a place to raise troops to, perhaps at less cost."* Aevi tagged every feature kind with a `property` (defence · sense · housing · training · mounts · production · craft · meaning · healing · trade · transit) and nothing read one. ⛑ `featureProperty` reads it; `trainingAt` finds a hold of yours where you stand with a STANDING `training` feature (a yard still being built trains nobody); `callCostOf` takes it and a head costs `trainedCallMult` of the price — ⚠️ UNAUTHORED, so three-quarters stands in and says so (4 a head becomes 3). Both places a price is shown — the call and the Bands tab — ask where you stand, and say the yard. ⚑ On Silas's save: Stillwater's Trouble has The Stand, a muster yard; ten heads called there cost 30, not 40 | `§309`, 4 checks: the property read; found where you stand, standing, and nowhere else; the price and its dial; both prices ask | ⚠️ the legion planner's forecast still prices at the plain rate — it plans no place |
| 09-18 | ✅ **A HOLD HAS ROOM (CCODE-429)** | SNG-628 / SNG-630, Erik GO 2026-09-18: *"sometimes there's just not the room."* Today a hold took any number of features. ⛑ **`roomOf`**: a ROOTED hold's room is its RUNG on Aevi's ladder (post 2 · steading 4 · hamlet 7 · village 11 · town 16 · keep 20 · fortress 25 · stronghold 32) — the one it was named, or the SMALLEST that fits what it already has; a MOVING hold's is the lesser of its rung and its FRAME (hull 3 · legs 2 · lift 4 · grown 5 · borne 6). ⚑ **Measured on every hold in play: nothing is over its room** — Stillwater's Trouble, a `post` carrying nineteen features, is a keep; Threshold Post and Whistling Woman are full; the Standing Annex is a post on legs. ⛔ **A BUILD past the room is refused, naming both ways out** — what would have to come down, and what the hold would have to become (or, when the frame binds — an equal frame binds too — what would raise it); the GM's build op is bound and the refusal is SAID. What the fiction ESTABLISHED is recorded and raises the rung (or the frame) to fit. ⛑ **The next rung is OFFERED, never taken**: full, thriving, and thriving for a season (`conditionSince`, stamped from now on — unstamped is "we do not know", not "never"); `rung` is written in one place, the player's answer. The GM is told each hold's room; the popup shows it as dots, offers the rung, says the refusal; and the Build verb asks the hull rule only the GM's build asked | `§308`, 12 checks: every hold in play fits; the largest reads as its rung; the refusal both ways; the fiction's feature; the frame and its rise; the offer's four conditions; the answer; one writer of `rung`; the stamp; the GM told and bound; the popup | ⚠️ "earned" reads thriving-for-a-season only — Aevi's other roads to a rung (people living there, a Mason's craft on the ground) and raising a frame are hers and Erik's to rule |
| 09-18 | ✅ **ERRANDS BY THE JOB'S ROLL (CCODE-428)** | The first item of the order of battle Erik approved: delegated work decided by the job's roll instead of `aiAssignmentAdvancement`'s model call. ⛑ A charge left in someone's hands is a ONE-NEED JOB for the one person carrying it (`errandOdds` → `planJob`'s dice): a MISSION rolls on its kind's family (or its second), a STANDING charge — most of them — on the family its own words name, read by the stems that tell what a person is good for. ⚑ With a "their best family" rule every one of Silas's four charges rolled HARM, the Raven's Home rebuild included; now the rebuild rolls SHAPE with Thingcraft and Edvar's committee seat INFLUENCE. Someone with no craft for it works by plain effort — a heuristic's miss must not doom a charge. The level is 10 for ordinary work and five more per stage of a crisis it is set against (`rules.jobs.errand`, Aevi's to author). ⛔ `rollErrands`: ONE roll per three days since the charge last moved (at most ten a tick), each degree an outcome (headway ×3, stall, trouble); a mission is done on the step that reaches its kind's steps, a standing charge never is (the fiction ends those), and trouble ends the rolls. The tick applies every roll in order and gives each person ONE status line, the most telling (Edvar's second charge had overwritten his first); the model pass is gone, so a tick makes no call for it. ⛑ The ⚒ Jobs tab shows **Charges in your name**: a face, the charge, its steps, and one roll's odds as headway · stall · trouble | `§307`, 11 checks: the family a charge rolls on, on Silas's save; the shares; plain effort and a mission's kind; the levels; one roll per interval, never done when standing, ten at most, done at a mission's steps, stopped by trouble; no model and no prompt; nine days on Silas's copy; a stub pass as before; the tab | ⚠️ the stem "war" prefix-matches "warden", so Fendt's watch rolls HARM — the stems are Aevi's, and they read fights too |
| 09-18 | ✅ **THE FIRE TESTS, SEALED (CCODE-427)** | CCODE-424 paused the fire tests that run themselves on a new build: on Silas's save a fire test's job offer became a beat of his scene, and a fire test's COPY went up as his save. ⚠️ Swapping `character` had closed one door and `applyTurn` writes through others. ⚑ **Measured, not listed by hand**: `tests/scope_scan.mjs --writes` follows the call graph from `applyTurn` (renderers not followed — from the screen the whole file is reachable) and finds every module variable a turn can write: the scene alias, the notes it leaves for the next GM turn, the party scene, the push queue, the dev report, a preview leg's verification, the live content. ⛑ Every copy's apply now runs SEALED: `_fireTestDry` shuts the one writer (so the push never targets a copy), the profile, the shared-world block and its `backupSaves`, `autoVerifyLeg` and `mintTransitLocation`; and each copy gets its own copy of `turnModuleState()`, with the live one put back whatever the op did. The apply Erik asks for for real stays unsealed. They run themselves again. ⛑ **Reconcile 69** fills a stored last turn that has no narration and no choices from the scene's last narrated beat — Cellaceron's copy under Erik's key has opened on a blank beat since 2026-09-13 | `§306`, 8 checks: every reachable module variable held or shut, with the reason for each shut one; the three that leaked or could are held; the doors shut; the control and every copy sealed and the for-real apply not; the automatic run back; the blank beat filled on the real copy, once, and a narrated one untouched · `§303` now holds the seal instead of the pause | ⚠️ the scan follows direct calls and callbacks passed by name, not a function stored in a variable and called later |
| 09-18 | ✅ **THE PARSER MAY NOT REFUSE A CRAFT YOU HOLD (CCODE-426)** | Erik: *"I don't know that I've seen the GM do this kind of refusal before... and it cut itself off..."* ⛔ **It was not the GM.** The intent parser (Haiku) returned `feasible: false` for Open Material — one of Silas's own crafts, turned on the reclamation system's failure — and a refused intent never reaches the GM: the last beat is redrawn under the parser's reason. Its own prompt forbids that refusal: *"novelUse=true when an ability is being pushed OUTSIDE its normal envelope … this is allowed and interesting, not infeasible."* ⛑ `sanitizeIntent` now keeps the rule the prompt states: an intent that names a craft the character holds (alone or in a braid) comes back novel use — the dice price the push, the GM rules on what it does. It may still refuse an act that names no craft, and that reason is clamped on a word with the cut marked (it was a raw 200-character slice: "…material struct") | `§304`, 4 checks: a held craft pushed past its envelope is novel use, not a refusal; the same in a braid; a craftless act may still be refused, its reason ending on a whole word; the prompt still states the law the code keeps | ⚠️ the parser's reason for such a push is dropped, not handed to the GM, which judges from the player's words and the novelty flag |
| 09-18 | ✅ **EVERY NAME A FILE READS IS A NAME IN SCOPE (CCODE-425)** | The class behind CCODE-424: `node --check` accepts an undeclared name, so a name deleted from its own function reaches the player — the fifth to (`formerCompany`, `pinFact`, `companyPlaces`, `opts`, `disc`). `import_integrity` closed the imported half; ⚠️ a file-wide list of names would not have caught `disc`, which IS declared in app.js — as another function's parameter. ⛑ **A scope scan**: `tests/scope_scan.mjs` parses with the acorn Node ships for its own REPL (`--expose-internals`; nothing downloaded, nothing vendored) and reports every read that no enclosing scope declares. ⚑ **First run, three more, all live**: `num` — every project sabotage threw; `currentDay` — every project the fiction opened threw; `verified` — the Legs copy button. Each fixed where it stood: the sabotage cap reads an absent dial as absent (not `Number(null)` = 0), an opened project takes the story clock as the job board does, the Legs count is counted. All 129 engine modules were already clean | `import_integrity`, 3 checks: the scan ran and every file parsed; it reads SCOPE (the CCODE-424 shape is caught beside a parameter called `disc`, and `typeof` of an absent name is not a read); nothing app.js or the engine reads is unresolved, against the language's globals and a browser list checked on a live `window` | ⚠️ a project the fiction opens is still never ticked — `tickProject` has no caller outside projects.js |
| 09-18 | ✅ **EVERY ROLLED ACTION DIED BEFORE THE GM (CCODE-424)** | Erik: *"The game seems to be having trouble with the GM"* — with the intent parse he pasted, which had succeeded. ⛔ **What followed it threw.** CCODE-415 moved "is this novel?" into `noveltyOf` and deleted the line that declared `disc`; the receipt line reading it stayed, so every rolled action threw `disc is not defined` between the parse and the GM call. ⚑ Reproduced in the browser on Silas's save with the model calls stubbed: the parse returned, the error fired, and no GM request was ever built. ⛑ `disc` is declared again from the same lookup the old block used. ⚠️ **And the fire tests ran on his live save**: a new build runs them once, each on a COPY — but `applyTurn` pushes its beat onto `sceneTurns`, app.js's alias of the LIVE scene, and saves the character it is handed, which points the push at the copy. So a fire test's job offer became a beat of his scene ("⚒ A job is on your board", no job on it), and the save that went up was a fire test's, with no narration and no choices. Paused until the harness is sandboxed; the button still runs them | `§303`, 2 checks: the roll path declares the technique before its receipt reads it; the automatic fire tests are paused and say why | ⚠️ the sandbox, the repair of the stray beat, and a scan for any other name app.js reads and never declares are CCODE-425 |
| 09-18 | ✅ **THE PEOPLE ERIK NAMED (CCODE-423)** | Erik: *"yes join Aevi and Fendt, Sable is not the legend... she is someone pursuing the deep dark for loki's quest. Cassiel Ord is a man and is not lvl 8. he knew Veth for years as a warden. he's lvl 15."* ⛑ **Joined on his word** (reconcile 68, through `rekeyPerson`): Cellaceron's Aevi is `aevi_the_watcher`, in both copies of that save, and Usnea's Fendt is `fendt` — one record each, every reference moved, the old id kept, the join marked as his. ⛔ **Loki's Sable is her own person**: `distinctFrom: ["sable_the_runner"]`, she/her as he names her. ⚑ Until now the sheet MERGED her with the legend (`authoredFor` matched the only authored "Sable"), and a GM op written for the legend would have landed on her by name; both refuse a person marked distinct now, and so does a reveal. ⛔ **One word is not identity**: the automatic join (CCODE-421) needs a full name — "Halvex Coil" still joins, "Sable", "Aevi" and "Fendt" join nobody by themselves. ⛑ **Cassiel**: "he knew Veth for years as a warden" is a fact on both of them in Silas's save, in his words, because a person already met is read from the registry. His rung is Aevi's: `tier: "leader"` reads exactly 15 there today, where an authored `level: 15` would read 18, because play adds growth on top. My three texts that called him "she" are corrected; his record always said he is a man | `§305`, 10 checks: the merge and the refusal; the lookup and the reveal refuse a distinct person; one word joins nobody and a full name still does; a one-word stranger keeps her id; Aevi and Fendt joined on the real saves with their levels unchanged; Loki's Sable apart; the Veth fact once on both; the rung that reads 15 | ⚠️ the sheet still merges a UNIQUE one-word name (Aevi's rule, and right for Aevi and Fendt) — `distinctFrom` is the exception, and `po/CCODE_20260918_people_for_aevi.md` asks her whether the sheet should want a full name too |
| 09-18 | ✅ **A CANON LOOK FOR EVERYTHING THE WORLD ALREADY HAS (CCODE-422)** | Erik: *"the canon look isnt' working a lot of the time"* — The One Called Zeus refused: *"that is not in the shared world yet — a look is a property of something already in it, and joining is earned"*. ⛔ **Measured, two causes.** The store took a look only for a record the game GREW and the world PROMOTED, so every legend, authored place, authored person and craft — most of what anyone looks at — was refused. And the shared world is read at most once a world day by a stamp kept on the SAVE, so a page reloaded the same day never read it, and each tick then adopted an empty view and wiped whatever the page held. ⛑ **An authored subject is already in every player's world**, so its look is filed in `world/canon`'s `looks` by kind and id (`lookKey`: person · place · craft) through the same merge (`applyCanonLook`, pure); a grown record still joins by promotion. The page reads the world once a session and keeps it between reads. **Places and crafts now SHOW the world's look** — `lookFor` gained the missing last rung: what you chose, the world's look, the authored picture, then a picture the game drew, which had stood ahead of the world's look because a kept and a cached image share a field (a kept one is now pinned). The button says a subject cannot carry a look before the click | `§302`, 11 checks: an entity's look as before; an authored one filed by kind and id; a grown one refused; a craft and a place sharing an id kept apart; the order of the four rungs; Zeus, Halvex, Millbrook and the Waygate eligible; the push, the once-a-session read and the kept view, the handlers, the readers, the button | ⚠️ items and holds still take a look only once promoted: an item's picture lives on the instance and a hold's on the hold, and neither reader asks the world yet |
| 09-18 | ✅ **ONE PERSON, ONE ID: HALVEX IS ONE MAN (CCODE-421)** | Erik, on the Halvex at Loki's side: *"I like the one man hiding story... i thought of him as more of a chaos agent - like a broken wright... Either way I want to keep Loki's play as canon AND adapt the authored legend to explain it."* ⛔ **Measured**: Loki's GM met "the Churn-Revel orchestrator" on d3 and revealed him as Halvex Coil on d4 — the world's legend's name — and the reveal took any name, so one man had TWO records: the world's (`halvex_coil`: wounded by The Appetite, his arcs) and Loki's (`churn-revel-orchestrator`: twelve meetings, the bond, the portraits Erik redrew). ⚑ **The same split sits in four more save records** (Aevi in Cellaceron's two copies, Usnea's Fendt, Loki's Sable) — keyed by a slug of the name while the world keys the authored id; left for Erik's word. And content itself holds Mara Wells twice and two different Wrens, so a name TWO authored records answer to joins nobody. ⛔ **`rekeyPerson`** moves every EXACT reference to a person (registry, company, bands, jobs, codex, pictures, portrait keys of any kind, world maps), never prose, and keeps `formerIds`, which the registry lookup now reads. ⛔ **A person met or revealed under an authored person's name IS that person** (`authoredPersonNamed`: the whole name or the name before its epithet — never a first name, never a grown record): a meet keys the authored id; a reveal joins the record to it and marks `linkedByReveal` for the author's review. **Reconcile 67** makes Loki's Halvex one man, on Erik's word. ⚠️ His prose is untouched: the LEGEND is what gets rewritten (Aevi, po/CCODE_20260918_halvex_is_one_man.md) | `§301`, 12 checks: every reference moves and no prose does; a taken id refused; a former id still finds him; strict names; a meet keys the authored id, an ambiguous name joins nobody, and without authored people a meet keys as before; a reveal joins the legend, a plain reveal does not; both turn paths hand the people in; Loki's real save one record at the same level with the world's wound on it, and Silas's untouched | ⚡ on a copy of Loki's save: 13 references moved, the old id left only on `formerIds` and two seed labels; idempotent. ⚠️ `§292` narrowed on the way: Cassiel Ord, authored `notable` by SNG-616, now reads 8 where his role guessed 15 — an author's rung is a ruling, not a lost craft |
| 09-18 | ✅ **JOBS: SENT, TIMED, AND DECIDED BY THE DICE THE PEOPLE SENT WOULD ROLL (CCODE-420)** | Erik: *"Each task needs to come in with a level reference and a difficulty… feed into a roll… the outcome should have concrete effects"*; *"select people instead of only taking the best… giving someone a job needs to take time"*; *"credit the crafts people have"*; and *"take into account the use of images and styling"*. ⛔ **One roll, the fight's**: each of a job's needs is covered by whoever on the team is best at it, and the chance is `successChance` with the job's opposition as a numeric difficulty — the door a fight uses — so the odds shown ARE the dice; the job's five outcomes are enumerated over every combination of its needs. ⛔ **One scale**: a level-L job opposes as a level-L person does (`personThreatForLevel`, now the one source for both). **Work credits the crafts** (×3 a tier, ×rank): thirty hand-days of building are under an hour's work for Maren and Veth. **The road is each person's own** (the journey planner, their wits, the gates the character found) at the slowest's pace; back at a WHOLE hour, because the clock floors its hour. **Effects go through the game's doors** — the purse, the pack (and a full pack is said), one deed at the place (standing is the deeds a community knows), xp only to the character when they go, a completion (growth) to each person who went well, recruits as hands. **Busy is real**: someone out on a job is not present in a fight (`alliesOf`), cannot be sent twice, and the GM is told who is out. **The GM offers** (`jobOps`, rule 14D), clamped to what a level can honestly pay; **never narrates an outcome**; a result is told once, marked only after its beat's call returns. **The Jobs tab** (`engine/jobs.js`, `engine/jobstate.js`): a board of the places the work is at, with their pictures; the chosen job's banner, needs as chips, everyone you lead with their odds for its main need, suggest one/two/three, the job's odds bar, who does what, the work, the road, the return day, what each outcome pays; out and came-back lists; post a job yourself; go yourself and the time passes for you | `§300`, 24 checks: the dice and the scale; exact odds; work, road, roll; effects and their doors (and the full pack); clamps; send, out, due, settle, told once; the fight's ally list; the contract, rule, salvage and write lists; the app's posting, settling and told-after-return timing; the tab; whole-hour returns; and on Silas's save — the pool from the records a fight reads, a keeper at the hold they keep, and a build of hours | ⚡ on screen on Silas's save: posted a job at Threshold Post (a hold's place is now pickable — it was missing from his known places), suggested Maren and Veth, 3 days each way through the Made Gate, back day 24; went myself on another and it came back a partial, +5 xp, the GM to tell it; no overflow at 375px |
| 09-18 | ✅ **A GATE AIMED OPEN (CCODE-418)** | Erik: *"It CAN also be used to travel directly to other gates, and if you're really good, can take you to any location you want (any waygate can do that)."* Gate to gate was SNG-243 §4. ⛔ **Now: at a network gate, a wayfarer good enough folds straight to any place they KNOW** — no gate at the far end, so nothing walked out of one. ⚑ **"Really good" is data** (`waygateRules`, `rules.waygate` overrides): a wayfaring tier past every gate's — content gates run 1–3, so the bar is **4** (wits 8, or wits 6 and five regions walked; Silas is 3) — or a craft whose whole art is the fold (`waygate`, the Numinous capstone). Still knowledge AND skill: the place must be one they know. `resolveWaygateTransit` routes such a place `open`; `routeBetween` offers it as THE gate way when it is quicker than gate-to-gate (priced like any hop on the distance folded), so the journey card walks to the gate and folds the last leg; the GM's gate block says so only for a wayfarer who can; the map's place panel, standing at a gate, offers *Aim the gate straight at X*. ⚑ And a gate route no longer reads *"through the Made Gate gate"* | `§299`, 13 checks: the bar above the hardest gate; one tier short, at the bar, and the craft alone; the named craft real; routed open, and not for the unskilled, an unknown place, or a gate outside the network; the route ending at the place with nothing walked out, one gate way, quicker than a plainer traveller's, and its line; the journey plan folding the last leg; the GM told only when true; the wiring. `§99` now asserts only that the traveller rides along (the call carries the rules too) | measured on Silas's save: at tier 3 he still goes gate to gate, Whistling Woman to Raven's Home 10.1 days; a tier-4 Silas folds straight there in 3.1 |
| 09-18 | ✅ **A TRAINED HAND CAN STILL FAIL BADLY (CCODE-417)** | Erik: *"I notice the crit failure bands are always VERY low… while the crit success can be higher"* — and, shown the proposal, *"I'm good with proposed critical failure rate."* ⛔ **The crit-failure dial was base 5, −2 a rank, −1 a level of practice, floored at 1**: from rank 2 up every hand sat ON the floor, so a trained character's catastrophe was the same 1 in 100 at every rank. `rules.crit` is now **base 12, −1 a rank** (practice unchanged); crit-success untouched. ⚑ The dial is the SECOND roll, taken only on a failure, so the whole-roll rate moves less than the dial: an untrained hand at 50% goes 1.7% → 4.2%, a rank-3 hand with practice at 70% 0.2% → 1.1%, and a master at 90% stays at 0 — the partial band takes every miss | `§298`, 3 checks on the live rules: a rank-5 hand with practice above the floor; the dial still falling rank by rank until the floor; an untrained hand's crit-failure dial at least its crit-success. No gate pinned the old values (craft_crit, roll_sensitivity, smoke, wiring_audit all green) | the dials stay Erik's: the numbers live in `rules.crit`, the gate asserts only the shape he ruled |
| 09-18 | ✅ **A GATE LEADS TO THE HUB THROUGH THE NETWORK, AND A ROAD GOES BOTH WAYS (CCODE-416)** | Erik: *"The made gate is next to the Whistling woman post, and it connects to the hub at the crossing, just like all other gates… fix the travel bugs."* ⛔ **Measured on Silas's save through the load path**: the Whistling Woman Post 34 days from the Crossing and 208 back, Millbrook 203 days from the Threshold Post, Millbrook 160 from a waygate a quarter-day off, five roads running one way. ⚑ **Two causes.** Content wins an id clash at load, and SNG-396 had promoted play-authored places with a ROOM's inherited position over the save that holds Erik's 09-06 ruling (reconcile 45 and 47): the Made Gate at the Crossing's own coordinates with a road there and no `networkCapable`; the Pale March waygate at Cairnhold's and carrying no `waygate` at all, so the gate Erik walked out of was a plain place to every save, his included; the fork and Stillwater's Trouble at Cairnhold too. Content now carries the ruled record — positions, `regionId: valley`, community by Millbrook, both gates in the network leading to the Crossing, no road to it; the Made Gate TOP-LEVEL (`parentId: null`), because the region map draws only places whose parent is not in the region, and under Millbrook it would have hidden inside Millbrook's sub-places while the Whistling Woman stood on the map alone. ⛔ And a road back to a grown place is an edit to an AUTHORED neighbour, which the next load reads fresh — so `twoWayRoads` runs in `hydrateGeneratedIntoContent` right after the grown places land. It matters beyond walking: `routeBetween` reads the gate search BACKWARDS from the destination, which is only sound on a symmetric graph. ⚠️ `content_ci` SNG-387 counts no sites now — SNG-398 puts a site within a day of its parent, so at region scale it is its parent's position again; four places a quarter-day from Millbrook had made three untouched valley places read 4.4–5.7×. ⚑ **And the region's ground map now names its places through `placeLabels`**: placed where Erik put them, seven names within a quarter-day of Millbrook printed as one smear, so a name that would land on another yields (where you stand first, then gates, settlements, sites) and the one that stays says how many it covers — *"Whistling Woman Post +8"*; the ground map also stops drawing a promoted stub its own schematic already drops | `§297`, 15 checks: the road back made and nothing else, idempotent; the load running it after the grown places; every gate in the network, none with a road to the place it leads, none at an inherited position, every place named a waygate one to the engine, every place Erik placed standing where he placed it; and on real content with two grown places in the ruling's shape — one-way before, none after, the Whistling Woman to the Crossing and back a gate of under 3 days, Millbrook to the ridge post a gate of under a week; the label rule, and the ground map using it | ⚡ Silas's save, before → after: Whistling Woman ↔ Crossing 33.9 / 208.3 days → 1.5 both ways; Millbrook → Threshold Post 202.6 → 3.1; Whistling Woman → Millbrook 87.3 → 0.4; one-way roads 5 → 0. On screen, on Silas's save: the valley map draws the Made Gate beside the Whistling Woman, *"about 2 miles on foot"*; the world map counts 28 waygates. `content_ci` back to its one standing red (SNG-391) |
| 09-18 | ✅ **A CHOICE DRAWS THE BAR ONCE THE SENSE READS NUMBERS (CCODE-415)** | Erik: *"yes, the choice buttons should show the bar when the sense is sharp enough."* ⛔ **The bar belongs to the precise tier and says no more than it does**: a choice speaks through the character's sense (SNG-098) — nothing, then "feels doable", then five worded bands, then *"roughly 60 in a hundred"* at the tier the rules name "precise" — and `sensedOdds` draws nothing below that tier and, at it, draws from the SAME rounded chance the words give (`sensedChance`, one rounding for both), with the crit dials the character's own. ⛑ **And the preview now pays what the roll pays**: the roll's crit terms (a wild current's widening, each craft's own critical) and its novelty (a novel use costs 15 and widens critical failure; a technique already found does not) each moved into one helper — `choiceCritTerms`, `noveltyOf` — that the roll, the choice's "how hard" line, the auto-intensity read and the skill list all ask. ⚑ The "how hard" line and the auto-intensity read had never priced novelty at all, so a novel choice read 15 points easier than the dice would roll it | `§296`, 4 checks: no bar below the precise tier; at it, the bar drawn from the rounded chance the words give (a true 64 reads *roughly 60*, and the bar is 60); the precise tier being the one the rules name; and the preview asking the roll's own helpers. `§264` and smoke's SNG-140 wiring check now read the helper rather than the old line | ⚡ verified on screen on Silas's save: all three choices drew the bar under *"roughly 50 / 70 / 70 in a hundred"*, each bar summing to its words |
| 09-18 | ✅ **THE FIVE WAYS A ROLL LANDS, ON EVERY SKILL (CCODE-414)** | Erik, of the band prototype's bar: *"This little bar that visually shows the success and failure chance and degree is fantastic. I think we should use it instead of just showing the % value on each skill. It can show crit success and failures as well as the partial band and it would change based on location."* ⛔ **`outcomeOdds` is the dice's own grading as probabilities**: the first d100 at or under the chance succeeds, within `partialBand` above it is a partial, beyond is a failure; then — for a success or a failure only — the second d100 against the crit dial turns it critical. ⛑ **The preview pays what the roll pays**: `craftChanceHere` already carried the ground, the aptitudes, gear, companions and affinity; it now also puts the crit terms a real cast of the craft carries on the action — the craft's own authored critical (CCODE-76) and a wild current's widening (SNG-140) — and returns the five-way odds. ⛔ **Both places a skill shows its chance draw the bar**: every row of the skill list (*"58% here"* with the bar before it) and the skill's hover card (the bar under the 🎯 line, with the five numbers spelled out beneath), each with its numbers in the label for a screen reader. It moves with the ground because the chance under it always did. ⚠️ The choice buttons are untouched: they speak through the character's SENSE (SNG-098), and what a character can read of their odds stays theirs to earn | `§295`, 4 checks: **`outcomeOdds` equal to what `resolveAction` produces over every one of its 100 × 100 rolls, across five rolls (a middling chance, a hard one, one clamped at the ceiling with a wild current, a novel use, one clamped at the floor with a craft's own critical)**; summing to one with no critical partial; the preview paying the roll's crit terms; and both surfaces drawing it | ⚡ verified on screen on Silas's save: 39 skills drew the bar, and Hunter's Strike's card read *58% base chance here (the ground lends it +3)* over *strong success 10% · success 48% · partial 15% · failure 26% · critical failure 1%* |
| 09-18 | ✅ **EVERYONE YOU KNOW PRACTISES SOMETHING (CCODE-413)** | Erik: *"Aldric and Dara are not lvl 1 riff raff... they're people of note at Millbrook... they should be built up just like any other NPCs - they get skills. We probably want to take a look across the board at NPCs and see who has NOT been skilled up through the NPC sheet build process - find and address the gaps."* ⚑ **The census, every person built the way a fight builds them against what their level holds:** 152 authored people all full; **5 of 37 people the game grew and 52 of 108 people met in play short — 56 of them with no domains at all**, so the kit draw had nothing to draw from and each fought with a plain strike (Dara Holt, level 14, 0 of 7; Aldric, level 8, 0 of 4). ⛔ **THE CAUSE WAS A SECOND COPY OF THE CHAIN.** The mint's affiliation read Aevi's authored region map, the regions and Erik's 2026-09-09 distance rung; the meet path (`affiliateNpc`) and the SNG-185 backfill each asked `regionHomeTradition(region, index)` with neither the map nor the regions, so only a region some tradition names as its own answered — and the valley is not one. The backfill is stamped done on every save and could never run again. ⛑ **One implementation now** (`affiliationAt`): the mint delegates to it unchanged, the meet path runs it — finding a place the game grew as well as an authored one — and reconcile step 66 fills every person the shorter copy missed, in each player's own tab, additive and silent. ⚠️ **Home ground first**: 13 of the 31 people with both places recorded were last seen somewhere else, and Dara, the Ditch-Mother of Millbrook, would have practised the Centre's craft from a visit to the Crossing. ⛑ And a grown person whose `abilities` is a map of prose (Silas's grown Calvar) no longer throws inside the fight's own sheet build. **After, through the runner on every save: 0 grown and 8 met short** — seven level-1 faces with no place recorded at all, who are given nothing rather than a guess, and Aevi at level 62, whose harmonic draw holds 25 crafts against a capacity of 31 | `§294`, 7 checks: the one chain reading the map where the shorter copy found nothing; the stronger rungs still winning and no guess without a place; the mint returning exactly what the chain returns in four places; the meet path's reading; **home ground first, idempotence and the untouched practised, through the runner**; **every person met at a recorded place and every grown person practising something and drawing a kit, on every real save**; and prose that is not a kit and not a crash | ⚡ Dara Holt 7 crafts, Aldric 4, both of the valley's mason |
| 09-18 | ✅ **A BODY LEANS; IT DOES NOT POUR A LEVEL INTO ONE STAT (CCODE-412)** | Erik, of the band prototype's "how good": *"fix the sheets and account for rank of skills."* ⚑ **This corrects his 09-11 ruling as it was built** — *"a character who rolls a SUB builds toward that sub, one at a time, before the parents"* — which filled each focus sub to the cap before anything else rose. Of the 138 bodies the builder makes, **69 had a sub at 20 and 47 had every other sub still at its floor**: Fendt, level 16, reason 20 and 3 in the other seven, which is why the prototype read him as the band's best at five things. A player does not build like that — Silas at 33 runs 4 to 10 across all eight, Loki at 8 runs 3 to 5. ⛑ **The points now lean, in proportion**: each goes to the open sub with the fewest points gained for its weight — a sub the person's crafts roll weighs 3, the other subs of the parents they lean on 2, everything else 1 (`leveling.bodyLean`, a dial; weighting only the focus sub reproduces the old shape). The total is untouched. ⚠️ **It moves fights, measured**: an NPC's rank-1 attack on a normal task, before any opponent, was 90.8% on average with 90 of 115 at the 95% ceiling and ~20 points of opposition to spare; it is now 72.9% with 17 at the ceiling — by level, 77 → 50 (1–10), 95 → 65 (11–20), 95 → 83 (21–40), 95 → 95 (41+). That is where a PLAYER of the same level already stands: Loki at 8 hits 55%, Silas at 33 hits 85%. People with an authored body (Pell, Calvar, Siol, Veth, Mara Wells) are untouched. ⬜ Rank already counts in the engine's roll (+5 a rank to success, and to the crit dials); the prototype's "how good" now reads that roll | `§293`, 6 checks: the same points at five levels; Fendt's shape leaning with nothing at the floor; the 3 · 2 · 1 proportion at 33; a legend at 60; the dial both ways; and **across every body the builder makes for the authored corpus, through the fight's own call, nobody below level 40 at the cap and no one-sub spikes**. `§162` and `§166` stop pinning the fill-first shape and keep their rules (the point count; a person's body not flat; a body leaning toward the sub it rolls) | ⚡ measured before and after on all 138 built bodies |
| 09-18 | ✅ **SOMEONE YOU HAVE MET FIGHTS AS THEMSELVES (CCODE-411)** | Erik, asked whether to fix it now: *"Yes, fix it now."* ⚑ The fight's lookup preferred the registry entry — what the player knows of someone — and that entry carries neither the authored rung nor the kit, so **everyone the player had met came to a fight as a thinner copy of themselves**: Pell Ran Marsh at level 13 with 13 lines against 35 with 64; Halvex Coil, on Loki's save, at 28 with one line against 63 with 59; Mara Wells at 7, 8, 12 or 28 depending on which save had met her. ⛔ **`personRecordFor`** puts the person back together: authored underneath, the registry's non-empty fields on top (the name the player uses, the bond, the status, the id), the rung on SNG-572's rule, and `authoredFor`'s join, so an ambiguous one-word name still joins nobody. Across the saves, **24 of the 26 met authored figures fight differently now**; two come DOWN (Cassiel Ord 15 → 8, one Mara Wells 28 → 17) because a role title had been guessing a rung their authored record does not give them. ⚠️ **An authored refusal to fight now reaches the fight**, which the registry copy never carried. ⛔ **The roster and a hold's keeper had the same fault by a second route**: they found the authored figure by the registry's id alone, and Halvex is filed as `churn-revel-orchestrator`, so the roster read him at 28 while the fight now reads 63. Both now join as the fight does: Fendt on Usnea's save 1 → 12, Halvex 28 → 63; on Silas's holds Pell's two keeperships rise leader → heroic, Fendt's notable → leader, and Cassiel Ord's falls leader → notable | `§292`, 6 checks: the merge rule on synthetic records; the record's own rung, the ambiguity refusal and the untouched stranger; an authored refusal honoured with nothing half-written; **on every real save through `duelFromTarget`, all 24 met authored figures fight at exactly their roster level**; the merge never takes a line away; and every keeper floored at the tier of the roster's level | ⚡ measured through the production call on copies of every save |
| 09-18 | ✅ **AN INVITATION RIDES THE NEXT BEAT WITH ROOM, AND ONE PLAYER'S BEASTS ARE PAUSED (CCODE-410)** | Erik, of Courtney's Adelheid: *"i want to make sure she gets the quest for the monastery and healing house. did we still need to close that loop?"* — then *"ship it and remove the swarm attack for now."* ⚑ **Measured on her save:** Sister Vreni's invitation (CCODE-357) had sat first in her pressure queue since day 2 and had not arrived. It waited on SNG-080's push, which releases only after **three quiet beats in a row** — and her GM filed a quest update on **19 of her 22 beats**, each one a reset, with the count itself unsaved, so a reload started it over. ⛔ **THE INVITATION NOW RIDES THE GM CALL** on the next beat with room, and room is the hard floor only (`roomForAnInvitation`: no fight, nothing being framed, no plan, no intent question, no earned reveal, the world not already pushing, nothing tender, intense or intimate) — **a quest update is not a reason to wait**. ⛔ **IT IS SAID ONCE THE BEAT COMES BACK**, not when it is set: the old line marked it delivered the moment the directive was queued, so a reload, a closed tab or a failed call on the next beat would have lost it for good. ⚠️ And it makes no false claim: `drivenPressureDirective` opens *"The scene has gone quiet"* and the slot's header said the same, so the invitation has its own directive that says only who has come, and the header no longer says why. ⛔ **THE SWARM IS PAUSED, NOT DELETED**: the threat producer re-rolls on every refresh, so removing one glimmerling swarm would have lasted until the next roll. Reconcile step 65 sets `threatsPaused` on **Adelheid alone, by id**, in the player's own tab on the next load — an edit to a save somebody is playing races their tab, and the lower revision loses. The producer makes none while it is set, and a queued one stops applying, so the pull prunes it. Additive and silent; lifting it is removing the field. ⬜ The ordinary encounter roll (what she meets when she explores, at her calm pacing) is unchanged | `§291`, 13 checks: found without being spent; said once and never produced again (a day-0 record is not falsy); one taken up from the log or already said is pruned; the quiet-turn pull leaves invitations alone; room is the hard floor, with no quest-update input; the tender floor read in one place; no claim of quiet; in play, room → find → the call comes back → said, in that order; one rule for the pull; the pause at both ends; the step owed to her committed save; **through the runner on a copy of her real save**, with Loki's left without it; and her swarm pruned by the pull. `§249`'s wiring check moved from the old line to the one door | ⚡ verified on a copy of her real save through the runner: step 65 set the pause, stamped the version and removed nothing |
| 09-18 | ✅ **CAVALRY, ARCHERS, A DRAGON — KINDS AS LABELS OVER THE PARTS (CCODE-409)** | Erik: *"yes i mean cavalry and archers... that type of thing... a dragon. Remember - the legion will have a combination of the things a band is built from. Wards skills etc."* Offered two shapes, he chose the first: *"proceed with 1"* — **a kind is a LABEL over the parts**, and everything mechanical comes from the families, wards and crafts a contingent carries. So *"archers"* harm because they are ticked to harm, never because of the word, and the same families under two names are worth exactly the same. ⛔ **A WARD IS PROTECTION**: a contingent that brings one PROTECTS, written into what it does rather than inferred by every reader, and it lifts the unwarded 1.4× off a formation. ⛔ **A LEGION CARRIES THE UNION** of its parts' kinds and wards through `resolvedUnit`, with no legion-specific code. ⛔ **RELABELLING IS A DESCRIPTION, NEVER CONJURING**: `n`, `quality`, `npcId` and `from` cannot be edited however they are asked for. ⚠️ The wards offered are the ones the engine can RESOLVE — `typesOfFamily` over the runtime `rules.damageFamilies` (**20** types) — not the **18** in `damage_types.json`, which disagrees with it; and the kind suggestions (cavalry → move + harm) only pre-fill boxes the player keeps or changes, because a table that decided anything would be a second rules table nobody authored. ⛔ And no GM op was added to conjure a dragon: the raise op's own comment says *"EARNED, AND THE ENGINE DECIDES. A narrator that could hand out a warband would make the whole command ladder decorative"* | `§290`, 9 checks: a label buying nothing; a ward protecting and lifting the gap; the legion's union of kinds and wards; a dragon expressible as one quality-6 contingent; relabelling unable to resize; a purposeless contingent refused; the normaliser carrying all three fields (it dropped `npcId` once and `from` this morning); the resolvable ward set; and the suggestions deciding nothing | ⚡ verified on screen: typing *cavalry* pre-filled harm + move, twenty wards were offered, and the row then read **"10 cavalry — warded against physical"** with the unit now protecting |
| 09-18 | ✅ **A COMPANION'S LEVEL, AND WHAT A UNIT CARRIES (CCODE-408)** | Erik, from the band screen: *"I saw Marengo at a low level in the band screen"* — and on pricing what a unit holds: *"Let's sum the carried objects for now."* ⛑ **MAREN (MARROW) OSSITIDE**, a legendary Ashwarden warden travelling with a level-33 Silas, priced at **15**; the other **eight** companions priced at **0**. `content.companions` was never a source for a level at all, so a companion had one only if they happened to also be a registry record — and that record carries no `level` or `tier` to derive one from. ⛔ THE RULE IS NOT MINE: `presenceSheet` already states it where companions FIGHT — *"a modest floor that scales with the character they travel with, because a companion of a level-9 character is not a level-1 bystander"* — and this is that rule reaching the roster. ⚠️ Narrow by construction: a companion who does NOT travel with them keeps their own reading, a plain registry person is untouched, and an authored companion level would still win. ⛔ AND WHAT A UNIT CARRIES IS THE SUM OF ITS PEOPLE'S AURAS AND ITS ARTIFACTS — the model already existed and this file's own note said so: all nine companions author a `substrateAura`, fourteen items a `substrateCharge`, and *"that falls out of the band model rather than being bolted on."* Negatives count (a suppressor carried by a column is a weapon), the ±1 clamp is substrate.js's existing rule rather than a ceiling of mine, and it is **itemised** on §9b invariant 5 — a legion is the largest such cause the game can field. ⬜ A person's own kit is NOT counted and cannot be: a contingent records who somebody is, not what they are carrying | `§289`, 11 checks: every travelling companion on every real save reading their character's level; the quoted rule present in both files; the narrowness; an authored level winning; the sum matching item + aura arithmetic; negatives counted; itemised strongest-first; **no float noise in a number a player reads**; a non-companion contributing nothing; a legion priced over its PARTS' contingents; and the screen naming the causes | ⚡ verified on screen: *◈ carries +0.13 to the ground where it stands — Waystaff +0.18, Marrow −0.05*, with Maren reading level 33 |
| 09-18 | ✅ **SOMEBODY ELSE DRAWS UP THE ORDER OF BATTLE (CCODE-407)** | Erik: *"I want this to be easy on the PC so we can have a delegate (GM through a chosen npc) come up with the legion plan."* ⛑ AND THE SPLIT THAT MAKES IT BUILDABLE: **the arithmetic is knowable, so it is not guessed.** Which holds have spare capacity, which people the WORK bar reaches, which gaps a person would close, who is the highest level standing in a band — the engine knows every one exactly. So `engine/legionplan.js` drafts **deterministically** (no rng, no model call, no hidden state) and the delegate's name goes on it; what a delegate adds is a voice and the player's decision to trust somebody with it. ⚠️ That also makes it **testable against the real saves**, which a plan composed by a model would not be — and a plan the player is asked to approve had better be one the suite can check. ⛔ IT PROPOSES AND NEVER WRITES: every step names the existing writer that performs it (`addContingent`, `setUnitLeader`, `formLegion`), so there is one way to put somebody in a unit and a plan cannot drift from what a player does by hand. ⛔ And drafting is FREE on his ruling — the only price named is what CALLING the result would come to, which is a separate press. Somebody who brings KNOW is asked first, because reading the ground is what planning is, and the player can ask somebody else and see **their** arrangement | `§288`, 10 checks: the plan deterministic across every real save; **every step bounded by something real** (no head beyond a hold's authored capacity, no person unmet, no unit or place not theirs); drafting writing nothing and costing nothing; every step naming its writer; KNOW asked first with the reason said; a different drafter giving their own plan; **never spending one person twice**; the two config bags never crossed; the clause agreeing with its pronoun; and the whole plan shown before any of it is done | ⚡ verified on the real save: Fendt drew up seven steps — captain Pell (+2), ask Veth to close the shieldwall gap, raise 26 hands across five holds — and one press carried all of it out: **33 under arms, 7 with a trade and 26 hands**, nothing refused. ⚠️ The screen caught two defects: *"they reads the ground"*, and Fendt reading **level 4** in the plan against **16** on the roster two lines above, because the app handed `meleeCfg()` in where the `npcStanding` bag belongs — the third time today I crossed those two, and `memberRows`'s own comment records that trap collapsing everyone to level 1 |
| 09-18 | ✅ **COMMANDERS AND CAPTAINS, AND WHAT THE POSITION IS WORTH (CCODE-406)** | Erik: *"Legions have commanders and unit captains. Those positions should grant bonuses... like a scaled up band"*, and on which ladder: *"Build level based for now and crafts can add to it later... or decrease it if opposing."* ⛑ A band's leader is its CAPTAIN and a legion's its COMMANDER — his two words, kept as two fields, so the GM narrates the position it actually is. ⛔ THE BONUS IS NOT A NEW LADDER: `contingentsFromPeople` already rates a person at `1 + floor(level/10)`, *"DELIBERATELY FLAT — a level-27 smith is worth three soldiers, not twenty-seven"*, so a leader is that same step without the base, content-dialled. ⚠️ **AND A LEADER CAN AT MOST DOUBLE HIS UNIT**: measured uncapped, a level-35 captain took six at quality 2 from 12 effective to **30** — a 2.5× swing where this file's largest multiplier is 1.4× and bigger than the shield wall's. The ceiling is the unit's OWN quality and not a constant I picked: *a commander makes the most of the troops he has and cannot make raw hands into veterans.* It scales with the unit and states as one sentence. ⛔ THE BONUS RIDES IN THE RESOLVED CONTINGENTS' QUALITY, so strength, threat, composition and `legionClash` all pick it up **without one of them gaining an argument somebody can forget to pass** — four readers taking a new parameter is four chances to ship it half-wired. It stacks at both scales, and a leader must be somebody actually standing in it. ⬜ The craft term he named is a **seam, not a dead parameter** — a parameter nothing fills reads to the next author as a built system; opposition already bites, because both sides' bonuses ride in the strengths a clash compares | `§287`, 12 checks: his two words as two fields; the flat ladder and a level-9 leader worth nothing; a bonus nobody can price reading **0, never a guess**; the doubling ceiling; the bonus reaching every reader while head-count stays put; both scales stacking; a stranger refused; **one person one level across every real save**; the clash and threat reading the resolved unit; and the seam left open | ⚡ Verified on screen — and the screen caught the defect: the picker offered *"Pell Ran Marsh level 9"* while the roster two lines above said **35**, because `derivedLevel` needs the day, the merged `npcStanding` bag and the authored record, and my new lookup passed none of it. `memberRows`'s own comment records that exact trap collapsing everyone to level 1. There is now ONE `levelOfPerson` and all three surfaces ask it |
| 09-18 | ✅ **THE LEGION: FORMED OF BANDS THAT KEEP THEIR IDENTITY, FREE ON PAPER, PAID WHEN CALLED (CCODE-405)** | Erik's ruling: *"They keep their identity. And they gain a legion tag... which legion are they in. Where is the legion located... is it camped or dispersed to forage in the region… Soldiers cost to call together for a campaign or mission... but you should be able to build the legion (identify who and how many from where) without incurring the cost."* ⛑ THE CONTAINER WAS ALREADY DECIDED and deliberately left unbuilt — `fellowship.js`'s header, on his 2026-09-12 ruling: *"a legion is a UNIT WHOSE CONTINGENTS ARE THE CONTINGENTS OF SEVERAL BANDS, with `formedFrom` naming them… I HAVE NOT BUILT THE FORMING"*, because a panel offering to form one before the forming existed is the message-claiming-a-mechanism defect. `unitsOf` has carried `formedFrom` since, marked EMPTY TODAY. ⛔ **A legion owns NO contingents**: its strength, make-up and gaps are resolved from its parts, because two copies of one contingent double-count every head and `bloodBand` is pure — losses stored on a legion would never reach the bands that took them. ⛔ **A unit on paper is nowhere and free**: no place, no posture, nothing paid, and a posture is refused until it is called. ⛔ **Calling costs per head, and the price says whose number it is** — he ruled that it costs and gave no number, so it falls back to the already-authored `wagePerHand` and says so on the screen, because an unauthored dial passing itself off as a decision is the defect this project keeps finding. ⚡ **AND HE CORRECTED ME MID-BUILD**: *"a shield wall unit should be able to protect other units. Just like a band."* My first cut bled each part under its own gaps — so a shield wall protected nobody but itself. The loss rate is now read ONCE from the resolved formation, so one part's PROTECT lifts the 1.4× off every band in it (measured: 3 lost with a wall, 5 without), while each band's CONDITION still comes from its own history | `§286`, 13 checks: the parts keeping their people and gaining the tag; every strength reader answering for a legion unchanged; **its people not listed twice**; two bands plus a legion reading as two commands; the three refusals; free-and-nowhere on paper; the fallback price and an authored one overruling it; the call carrying place and posture; **the shield wall covering the formation**; each part's own condition; never broken on contact by its own emptiness; and disbanding releasing them whole | ⚡ verified on the real save: formed from two bands, **54 crystal actually debited (1696 → 1642)**, dispersed to forage at Whistling Woman Post, toggled to camped, taken apart and both bands came back whole. ⚠️ And the screen caught a regression the tests could not: filtering the parts out of the top level made every person in them render NOWHERE — Pell's row vanished — so a legion's people now nest under their own band inside it |
| 09-18 | ✅ **RECRUITING INTO A UNIT — WORKERS, GUARDS AND GENERAL TROOPS (CCODE-404)** | Erik: *"Recruiting into the band next please. I want to have a source of workers and guards as well as a way to raise geneal troops... on the way to having a legion."* ⛑ MEASURED FIRST, and almost all of it was built: `raiseBand`, `contingentsOf`, `unitComposition`, `bandStrength`, `bandThreat`, `bandGaps`, `legionClash` and `bloodBand` are written and calibrated against the real engine. What did not exist was any way for a **player** to reach them — `raiseBand` had exactly ONE caller, a GM op, and `unitComposition`/`bandGaps` had **no caller outside the tests**. Silas has commanded six people since day 16 with nothing shielding them — 1.4× losses, every clash — and no screen ever said so. ⛔ THE SOURCE IS NOT INVENTED: a person is asked at the **work** bar SPEC_hold_costs §5 ruled (known, here, not hostile — 36 of Silas's 39 people, where the travelling bar reaches 18), what they bring is `contributionsOf` with the evidence read (CCODE-402) and their quality is their derived level; **general troops** are anonymous hands raised at a place he holds, bounded by that place's own authored `handsCap` (`maxHands` plus each people-feature's `hands`) less its crew and the heads already raised there — **soldiers and workers draw on one capacity because they eat the same bread**. ⚠️ A head remembers where it was raised, or the bound could never be checked again — and `contingentsOf`'s normaliser dropped `from` on my first cut, the same defect its own comment records about `npcId` | `§285`, 11 checks: a flat unit's six people surviving the first recruit (the implicit contingent made real first, or they vanish); `count` kept in step; one person never twice; `from` riding through and a named person NOT spending capacity; the bound being `handsCap` and reaching zero; **a gap closing when somebody who covers it joins**; and the three readers now having a caller | ⚡ verified on the real save in the browser: 6 → 7 standing and threat 21 → 25 when Veth joined, the *no protect* warning replaced by *"Nothing it cannot cover"*, then ten hands raised at Stillwater's Trouble — 17 standing, threat 31 — and that hold then correctly **disappeared** from the muster list. Silas's ceiling is 26 heads: a war band, and the honest answer to *"how do I get to a legion"* is that it is his holdings |
| 09-18 | ✅ **A COMPANION'S CRAFT RIDES THE BOND — EIGHT OF NINE DID NOT (CCODE-403)** | Found by working the field atlas's own DARK list, which is what it is for. `companionTaught: true` is authored on **9** crafts and had **no reader anywhere**. The engine's mechanism for exactly this (CCODE-199: the rank rides the bond, and `rankUpAbility` refuses to sell it) keys on `progression: "stage"` — and only ONE of the nine says so. ⛔ So eight crafts whose own descriptions are the companion DOING the thing — *"Coil performs the correct procedure"*, *"Bristle tells you what he can smell"*, *"Aevi spreads thin around your sleep"* — were ordinary purchasable crafts, buyable with a skill point by somebody who does not travel with them. ⚠️ AND THE GATE THAT EXISTED COULD NOT SEE THEM: `content_ci` asks whether a STAGE craft's teacher resolves, a check that only ever looks at crafts already declaring `stage` — the eight were outside the `if`. ⛑ Fixed at ASSEMBLY, not in the resolver: six readers branch on `progression === "stage"`, so teaching one of them about `companionTaught` would leave five disagreeing, which is worse than the bug. The declaration becomes the mechanism once, where every reader sees the same craft, and an authored `progression` still wins. **No save loses anything: Silas's Attended End was already in sync at rank 3, and Loki's Old Procedure rises 1 → 2, the rank his bond with Coil had already earned.** The field atlas now reads DARK 16 → 15 | `§284`, 5 checks, all asked of the **declared** population so a tenth companion craft is covered the day it is authored: every declared craft resolving to a real companion; a skill point refused for **every one of them**, with the reason a player can act on; the pack's own `progression` still winning; the bond's rank held on every real save and **idempotent**; and the sync still following the bond BOTH ways | ⚡ the third defect this week whose gate was inside an `if` — ask about the population the content DECLARES, never the population that already passes |
| 09-17 | ✅ **ONE PERSON, ONE ANSWER, EVERY DOOR (CCODE-402)** | SNG-541c built the reader that turns `role`, `description` and `skillsObserved` into contribution families — *measured then: 128 registry people carried ZERO `assistTags` while 87 of them carried 368 entries of the GM's own prose about what it watched them do* — Erik ruled it in (*"these aren't just bodies that can hit something — they have skills and abilities they can bring to bear"*), and it was wired into the two FIGHT callers by hand. ⛔ `alliesOf` never passed it, so the party, the roster, the champions panel and the skill-battle seat all asked the narrower question: **the same person answered differently depending on which screen asked** — Pell Ran Marsh brought SUSTAIN to a caravan escort and not to the party she is standing in. ⚠️ This is the THIRD time this exact defect has appeared one level up in this function: CCODE-265 was `stageOf` riding through the same options object. ⛑ Measured over the 21 allies on the 16 saves: none was HARM-only (the authored companions carry real tags) and two gain one family each — Pell + SUSTAIN, Veth (Stillwater) Ondra + SHAPE — additive by construction, since the prose reader can only find a family, never take one away. The ruled DEFAULT stays off, because the restraint is that a new caller must decide rather than inherit; this ships a decision at one door | `§283`, 4 checks: the reader still additive and still finding more than the tags; **the door's answer equals the record's own for every ally on every save**, so a surface added later that forgets to ask is caught by the population and not by a fixture; the option riding through; and the default still off with both fight callers still deciding for themselves | ⚡ the two stale items I checked first and did NOT build: `gearWords` singular-only is **1 line in 546** (147 of 150 armed people are armed correctly), and the purse was wired by CCODE-234 — a backlog note is a claim about a mechanism too |
| 09-17 | ✅ **A SURGE THAT LANDS CANNOT BITE (CCODE-401)** | Erik, in play, watching it happen: *"i think the surge and backlash is broken. it's not supposed to fire unless i fail."* ⛑ His screen read `d100: 54 vs 89 — success · surge (5 energy)` and, under it, `⚡ surge backlash: -4 health, -10 energy`. 54 ≤ 89 is a clean `success`, and a clean success carried a **0.3** multiplier — 0.25 × 0.3 = a **7.5% chance of being bitten for doing everything right**, once every thirteen surges. ⚠️ AND EVERY NAME AROUND IT ALREADY SAID SO: the trigger the engine pays is called `surgedSlip`, the call site says *"a Surge that SLIPS can bite"*, and the function's own docstring said *"near-nil on a clean success"*. Three statements of the rule, and the arithmetic was the fourth line and disagreed — a comment is a claim about a mechanism, and when the claim and the number differ it is the number the player feels. ⛔ Now zero, not near-nil, and the dice are not even consulted on a landed surge: a roll asked for is a roll that can go against you. What remains is a slip in three ascending grades — partial 25%, failure 37%, critical failure 50% | `§282`, 5 checks: never on a clean or critical success **and it holds at `backlashChance: 1.0`**, so it is the rule and not a number that rounded down; the rng untouched on a landed surge and still asked on a slip; the three grades ascending and none of them zero; no other intensity biting at any degree; and the trigger's name agreeing with the arithmetic | ⚠️ the `partial` grade is left firing — a partial landed short, which is a slip — and that is the one part of Erik's *"unless i fail"* I have not assumed: asked, not decided |
| 09-17 | ✅ **A FIRST NAME THE CONTEXT SETTLES, AND ONE MAN ONE PAGE (CCODE-400)** | Erik: *"why Loki's save still has 'Halvex' as a mystery person, 'Halvex Coil' as the legend and orchstrater, and a Halfvex name that has no underline at all.. plus Vessin has no underline. When the GM narrates first name only, but the context tells you who it is fully, there should still be an underline."* ⚑ Measured: 1,184 people sit in the click index across the 16 saves and **1,160 are known by a multi-word name**, so a narrator writing *Halvex* or *Vessin* writes a name it does not hold — 334 of them gain their first name now. ⚠️ A LINK IS A CLAIM THAT A WORD IS SOMEBODY, so three classes of first word are refused, and the measurement found all three rather than my guessing: an honorific is stepped OVER (*Sister Alder* is Alder), a name opening with an article is an epithet with no name inside it (*the Starless One*), and a lowercase word is not a name (*Unknown (east bank traveler)* would have handed a link to the word *east*). ⛔ And acquaintance settles a clash, which is Erik's *"the context tells you who it is"*: Loki knows **two** Vessins — Tallow-bark, met fourteen times, and a Long Bough courier he has never met — so they are never merged, but *Vessin* in a sentence he reads means the one he knows; where he has met both, no link at all. ⛑ Halvex was **three records**: a `mystery` page with ten facts anchored to nothing, `halvex_coil` the legend in the content pool, and `churn-revel-orchestrator` the man in the registry with four — most of what he knew sat on the page that could not point at him. The standing load-time sweep now binds a page whose label IS somebody's name (never `namesMatch`'s containment, or *The Long Bough* would file a place under a person), the existing kind repair corrects `mystery` → `person`, and the merger folds the pair: **one page, 14 facts, both names on it** | `§281`, 14 checks: the three refused classes; people only, never a place or a topic; acquaintance resolving a clash and refusing an ambiguous one; the role rule; the binding, the registry preference, the same-person test that keeps Vessin apart, idempotence; and the upstream `mystery` lookup | ⚠️ two of my own gates failed on my first cut and both were right: I had treated an article as an honorific, and the new-mystery path anchored to the legend's id instead of the man he met — the same duplicate-id trap, one layer down |
| 09-17 | ✅ **EVERY KIND OF NEWS CARRIES ITS PLACE (CCODE-398)** | Erik, alongside CCODE-367: *"we might want a revamp pass on the world news soon"* — give every kind of news its place. ⚑ Measured by driving both news passes headlessly over the 16 saves at v2.0.60: `runWorldTick` wrote 94 items and placed **none** of them; `advanceGeneratedOffscreen` placed 82 of 147. 82 of 241, and the place was there to be had every time — the deed-spread model **computes** the community word reached, printed it into prose (*"As far as cairnhold"*) and threw the fact away; a figure murmur names a person who has a home. Now the writers carry what they know and the stamper DERIVES the rest from the record a line already names: a person's home, and — for an arc, which has no place of its own — the hinge person nearest the player. ⛔ And an id is not a place until the content bag agrees: 8 of the roster's 84 `homeLocation` values name rooms nobody authored, and a `locationId` the reader cannot resolve reads as placed while marking nothing. ⛑ The reader gained the two coarse tiers and neither may say *near* — `valley.millbrook` holds Archive Hollow nine days from the square — so a community or region match claims no distance at all, shows no day count, and ranks between a measured walk and nowhere. **After: 173 of 249 placed, and what the reader can speak to rose from 70 to 116** | `§280`, 13 checks: a named person placed at the stamper and from any who-field; the writer's own place winning; the unresolvable id placing nothing; the bag deciding location-vs-region rather than the field name; the arc's nearest hinge, and the branch **alive on all 6 authored arcs** where the `arc.regions` it replaced could never fire; the hop's community surviving `NEWS_FACTS`; one bag at all 7 stamping sites; the three tiers and their order; the chip's own class | ⚠️ the branch I replaced read `arc.regions`, which **no** authored arc has — dead on the day it shipped. And naming a figure in the news wakes them, so the tick's own item count rose 94 → 102: telling the truth about who a line is about is a change to the world, not only to the record |
| 09-17 | ✅ **THE GENERATORS AUTHOR TO THE STANDARD, AND A PLACE EARNS ITS LOOK (CCODE-397)** | SNG-582 (Aevi), on Erik's point: *"All of this authoring will fall to the generators, so they need to be able to faithfully continue what you are doing."* ⚑ Measured on the 16 saves: 24 of 24 grown places carry no look and 20 share one boilerplate sentence — the sentence every picture of them was drawn from — and the mint handed a place its own name as its description. Now: the mint ASKS for `appearance` (the visual only, a concrete noun in the first sentence) and is taught the split — `descriptionSeed` is what a place is LIKE and never the picture; every mint is asked for player register (no glyphs, no shouted clauses), which is the half that keeps Aevi's 1,144 rewritten lines true; the stub and the transit mint carry the fields EMPTY, because an empty field is findable and a filled one that says nothing is not; and ⛔ enrichment — which fired for people alone — now opens for a PLACE the player returns to, additive only, with the register enforced rather than asked | `§279`, 7 checks: the look asked and the split taught; player register on every type; the stub and the boilerplate; what a place is owed vs a person; the enrichment door; the schema's own words | ⚠️ two gates of mine asked about a phrase and meant a field — §204 matched "ALSO REQUIRED" and failed for a location getting its own block |
| 09-17 | ✅ **THE RELOAD IS THE PLAYER'S, EXCEPT WHEN A WRITE WAS REFUSED (CCODE-396)** | Erik, after watching CCODE-394 reload a tab out from under him: *"I do want a 'new build, reload now' button."* ⛑ A quiet check now OFFERS — a banner saying a newer build is out and that nothing this tab writes will go up until it reloads, with **Reload now** and **Not yet** — and the tab keeps playing either way. The forced reload is kept for the one moment it is not a nuisance: a write the guard REFUSED, where the save cannot go up until the tab reloads. ⚠️ The cadence therefore asks `deployedBuild` for itself rather than through `staleBuild`, which is what tells sync's listeners — a quiet check finding a new build must not reload anybody | `§277`: the banner's words and both buttons, the forced path kept for the refusal, and the cadence not going through the listener | ⚑ verified in the browser: the banner appeared instead of a reload, *Not yet* kept playing and said what it costs, *Reload now* went to the fresh URL |
| 09-17 | ✅ **A PLACE, A CREATURE AND A THING CAN BE ASKED ABOUT — AND A FIGHT LINE'S NAMES CAN BE CLICKED (CCODE-395)** | Erik's §3: *"It should all be added to and pulled from a single source — viewable as a codex, and the popups… but the TRUTH needs to be known and documented somewhere."* ⚑ Measured: `knownIndex` answered for people, arcs, codex topics, titles and rungs and had NO source for places, creatures or things — of the place names in a player's own news and codex, 6 of 7 were unclickable on Silas's save, 4 of 6 on Loki's, 7 of 8 on Adelheid's (*Pressureholt*, *Harmonic Heights — Lower Terrace*, *The Service Ways*). All three now answer from the record and nothing else: a place says where it sits, whether they have stood in it and the look Aevi authored — and its danger only once they have been, because that is learned by going; a creature answers from the bestiary, with *"you have faced one"* only where their own encounters say so; a thing answers from what they carry. ⛔ And no name in a FIGHT line could ever be clicked, because the whole sentence rendered inside its `<button>` and the linker refuses text inside a button — correctly. The prose is a sibling now and the cue is the control | `§278`, 6 checks: the three sources and their guards; a place been-in vs only heard of, with danger withheld; a creature faced vs not; a thing carried vs not; the app feeding all three from the character's own records; the fight line's prose outside the button | ⚑ after: every place, creature and object name in all three saves' prose is answerable — 0 unlinkable, from 6, 4 and 7 |
| 09-17 | ✅ **A TAB RUNNING AN OLD BUILD DOES NOT WRITE TO THE WORLD (CCODE-394)** | Erik: *"instead of requiring a manual reload… any browser running the game would have an auto reload on next action? would that minimize the risk of having an old version written?"* ⚑ It would, and the hazard has bitten twice: the shared region file overwritten by a tab still running old code, and SNG-549's day undoing three pushes that carried lower revs onto the good copy. `version.json` is written by the bump and is the one thing a running tab can ask; the single write chokepoint in `sync.js` refuses every write — shared merge and owned save alike — when a newer build is deployed, and the app reloads at a safe moment (never mid-turn, never mid-encounter) through a fresh URL so the old code is not served again. A rollback never triggers it, and an unaskable `version.json` is not a verdict | `§277`, 7 checks: numeric version compare (2.0.10 over 2.0.9); the cache-proof ask and its three non-verdicts; newer vs rollback vs same; the refusal through the real write path with nothing PUT; the current-tab control; `version.json` agreeing with the code; and the app's watch, its wait and its fresh URL | ⚑ verified in the browser: a tab at v2.0.55 reloaded itself to the new build within seconds of `version.json` moving |
| 09-17 | ✅ **A PLACEHOLDER ARC IS A DEBT, AND A NAME IS A NAME ON THE SAVES TOO (CCODE-393)** | Aevi, SNG-588: *"the players who wrote the most got the least arc… a fallback arc is a debt, not a resting state"* — enrichment fired once, at creation, and returned on no API key, so a character made before the key was entered kept the placeholder forever. ⚑ Measured on the saves: Chernak 2,226 written characters and a placeholder, Loki 1,583 and a placeholder, against Silas and Rhinofire with real arcs. The load retries it now, bounded to three attempts, for a placeholder standing on a real backstory — and ⚠️ never for one already TAKEN UP: Loki's and Brynjar's placeholders are resolved quests, and replacing a lived arc would strand the story they played. `contentGenerator` does not gate it (her question) — only the key does. ⛔ And SNG-587 O3: the arc name that was a 188-character paragraph is repaired on the two saves that still carry it (Erik's Loki and Brynjar's, both with the same string as the title of a quest in their log), through the versioned door, silently. A premise stops pasting the paragraph too | `§276`, 6 checks: the fallback's own marker (`personal_fallback`, the word this module has written since SNG-133) and the legacy shape; the debt and its four refusals; the name repair on the arc and its quest, idempotent; the reconcile step; the premise's clause cut; the retry's wiring and bound | ⚠️ I almost shipped a second marker — `source: "fallback"` as a duplicate key in the object that already wrote `personal_fallback`, which the language silently drops |
| 09-17 | ✅ **THE WHOLE OF AN AUTHORED AFTERMATH IS READ (CCODE-392)** | SNG-601 authored all four of the water crisis's aftermaths — `summary`, `aftermath` (the concrete changes: the rationing ended, the guards simply not posted one morning, Edvar's records with a clean stretch in them for the first time in fourteen years) and `toneForGM` (*"relief with an open end"*) — and the engine read only the summary, one day after she wrote them. The GM's ANSWERED line now carries what is TRUE now, as facts it may state plainly, and the register it is said in. ⚠️ And my own §246 gate had pinned the FALLBACK (`_theWorldNow`) rather than the rule: it went red for her content arriving, which is the thirteenth gate to pin one instance of something general. It asserts the precedence now — the outcome's own aftermath where there is one, the quest board's otherwise | `§246`: the precedence; the whole read proved on a fixture (and a bare summary carrying neither line); and every one of the live outcome's facts reaching the GM with its register | ⚑ the four paths leave four different valleys, and the GM could not tell which one it stood in |
| 09-17 | ✅ **A PLACE IS DRAWN FROM WHAT IT LOOKS LIKE (CCODE-391)** | Aevi authored `appearance` on all 141 places — look only, split from manner — and nothing drew from it: the picture came from `descriptionSeed`, which she measured as mixing look with CHARACTER in 25% of them and carrying no concrete visual noun in its first sentence in 59% (Bedrock was handed *"a deep suspicion of anything that cannot be weighed"*). ⚠️ And her one-line fix could not have worked alone: `ensureLocationImage` built its own subset of the record and `appearance` was not in it, so the field could not reach the prompt at all. The look now leads at all three doors — the prompt builder, the first-visit banner and the codex's place art — with the seed kept as the fallback for a place minted before anyone wrote its look | `§275`, 3 checks: the look leads and the seed falls back; every one of the 141 leads its own prompt; the three doors carry the field, and the banner stays a subset so a minted url is never written into `image` | ⚑ all 141 prompts changed. ⚠️ The subset is deliberate: `ensureImage` writes the url onto the record it is handed, and a grown place's record is the character's own save |
| 09-16 | ✅ **THE ROAD, WALKED LEG BY LEG — A PERILOUS LEG IS A PLAN, AND A STOPPED JOURNEY IS TAKEN UP AGAIN (CCODE-390)** | Erik: *"Proceed"* on CCODE-387's open stages — Aevi's SNG-333 (*"you either play out of a failure or pass the way you intended"*) and her theQuestShape (*"an encounter mid-leg does not cancel it … a party that turns back has a record of how far they got"*). ⚑ Measured: a journey is 4.45 legs (median leg 29 days); danger 2+ would make 66% of legs gambits, so the line is 3 (17% of legs, 55% of journeys). The road is walked leg by leg on one hunger model; the authored `onTravel` roll after each leg stops it where an encounter happens; a perilous leg is three steps taken with the character's own crafts by their `challengeTypes` (never read before), at their rank, with real fallbacks and the plain way last; a broken step is played out where it happened; go around, take it up again from where they stand, or end it with the record. And a declared gambit's craft now counts its rank, as the same craft alone does | `§274`, 18 checks: legs on every way; the legs cost exactly what the road does; the line at 3, measured; three steps, crafts, fallbacks, Aevi's authored `crafts` list, the read and Wayfinding's weak link; a broken step; stopped, re-planned from a waypoint with the road kept, set down or ended with its record; going around; the arrival; the doors | ⚠️ perilous is perilous: measured on the saves, all three steps first try — Silas 87%, Cellaceron 41%, Loki 28%, Brynjar 17%, Adelheid 13%; with the fallbacks 100%, 93%, 78%, 58%, 35% (adaptation points on top). ⚠️ And the road is eventful: the authored `onTravel` 0.45 "per travel leg" now rolls per leg — 1.5 encounter stops in an average journey, 81% of journeys stopped at least once (balanced pacing); the dial is that chance and the player's pacing. `the_laid_ground` in CCODE-387's shelter crafts was a renamed id nobody could hold; it reads `laid_ground` |

**Last verified: 2026-09-19 · v2.2.0 · 432 crafts.**
...
✅ **Since v1.9.448 that threshold is capped at `breakAtMax` (10) and EASES with the fight** — one tick less for every
`breakEasesEvery` (2) rounds fought, never below `breakEaseFloor` (1). Erik 2026-09-11: *"I want breaking to be the outcome that
increases likelihood after a long fight."* Break is 21% of wins: 5% of fights won in 1–3 rounds, 56% of those past 12. `§156`.
...
at the parent, two creation points and `subPointPerLevel` a level spent LEANING toward the subs a character builds toward
(CCODE-412: in proportion, 3 · 2 · 1 by `leveling.bodyLean`, never past `subAttributeCap`), each parent the mean of its subs, health 15 + 5 × physical and energy `energy.max`, +5 to both a level.
...
Measured: 11.1% of craft-and-place pairs (3.3% at a core of 0.1, 5.1% at 0.15, 6.5% at 0.2). ⚠️ Standing on one of the 29 pool
...
✅ **A fold stays folded (Erik 2026-09-11: *"I'd like them to stay the way I put them"*).** Every foldable group — the skills
panel's braids, craft families and discoveries, the learn list by tradition (both the sheet's and the wheel screen's plain-list
fallback), and the fight menu's move families — takes its open/closed state from `sectionOpen(key, default)` and writes it back
through `saveSidebarState()`, the profile-backed store SNG-120 already uses for the sidebar sections. So a fold survives a turn,
a view change and a reload. ⛔ Before this, the craft groups wrote the word `open` as a LITERAL (every render reopened them), the
...
✅ **A beat's bookkeeping that did not land is restated, not lost (Erik 2026-09-12, the Maren beat).** An op step that throws is
isolated and the beat stands; the aside says which step. Now the next prompt carries it too (`bookkeepingRestate`, `§172`): the step,
its error, and whom it touched, with the instruction to restate those changes as facts already true. It fires once. Two things the
...
✅ **The Library serves the player, not my notes (SNG-538 §4, Aevi 2026-09-11; Erik GO: *"not only are they out of date, but they are
riddled with all caps notes and other phrasing that is not meant for a player finished product audience"*, `§181`).** SNG-165 cleaned
every craft surface through `playerText` and never looked at the larger one: `renderLibrary` rendered fifteen documents through `esc()`
alone — 788 authoring glyphs reached the player — and `LIB_SKIP` filtered a *named* list while 57 `_`-prefixed build notes, carrying
no name it knew, were served as lore under headings. The Library's index and renderers live in `engine/library.js` now, pure string
builders with no DOM: every string they emit passes `playerText`, and a `_`-prefixed key is private, as it is everywhere else in this
repo. The gate renders every `LIBRARY_INDEX` document through exactly those functions and asserts that no glyph and no private key's
text reaches the page — the craft half of SNG-165 had a gate and stayed fixed; the Library had none and drifted from the day it was
written. The capitals and the staleness are content and Aevi's (her §5).

✅ **A reader with no legal author, gated (B3 — Aevi's ask after §166, `§182`).** `subAttribute` sat unauthorable for a day because
the engine read it off a craft while the schema (`additionalProperties: false`) refused to admit it. The reverse gate now scans every
field the engine reads off a craft-typed variable in `engine/` and `app.js`, subtracts the 63 top-level and 24 rank fields the schema
declares and the three stamps the engine itself writes (`minted`, `subAttributeByFunction`, `image` — each named with its writer), and
fails on anything left. Building it found eight: `unlockCondition` and `combinationAxis` — read by `practice.js`, validated by
content_ci, forbidden by the schema — are declared now; and seven fallbacks that read a field no authored craft has ever carried
(`effect` ×3, `ranks`, `function`, top-level `crit`, top-level `tierGap`) are gone. The forward direction stays with content_ci: the
...
✅ **A gate may not pin the typography of prose it does not own (Aevi 2026-09-12, SNG-538 §5, `§184`).** Her register pass over the
player documents hit three of my gates and she restored my emphasis to get a green run: two in this suite matched her headings'
shouting *literally* — `/RANKS ARE ADDITIVE/` with no flag and a `.includes("PART I½ · WHERE EVERYTHING IS")` — and
`certify_counts` refused to stamp the guide's *Last verified* line unless it kept its `**`. All three are loosened: the two gates
match the claim in any register (as named objects `§184` proves both spellings against), and a certified claim's wrapper is now
optional and *preserved* — `**`, `__`, or nothing, whatever the document wears is what the restamp comes back wearing, through
`scripts/certify_claims.mjs`, the module the tool itself uses. The numbers stay mine; the emphasis is the author's. Six other
caps-run pins in the suite read `docs/HOW_IT_WORKS.md`, which is mine, and they stay — the rule is ownership, not case-sensitivity.
And the sweep that found them had the same shape of blind spot as the gate it was auditing: it read the subject only *after* the
pattern, which is right for `/…/.test(pg)` and wrong for `pgSrc.includes("…")`, so §12's pin escaped the first pass.

✅ **The Library surface the gate could not see (`§184`).** §181 renders every index entry that has a `path` — and the great circle
has none, because it is drawn rather than fetched, so the gate reported coverage of fifteen documents while the sixteenth surface
went to the page through `esc()` alone. Its prose (the peoples, their poles and civilizations, the antipode's name, the Valley's
folk crafts) is a pure reader in `engine/library.js` now, every string through `playerText`, and the gate renders it with glyphs in
...
| `damage_types` | every declared type is admitted by **either canon** — this enum or `damage_families.json` | green — the five the enum refused were already family canon and are now listed (force, spatial, radiance, corrosive, psychic); **their wards are unwritten and named every run**; a type outside both fails |
...
### A PERSON CAN START AN ARC (SNG-167 §2)

A place carries `questSeeds` and rule 10 weaves them. A person carries them too, and
`npcQuestSeedBlock` offers them to the GM under rule 10b when that person is present — because **the
memorable arcs start with someone, not somewhere.**

A seed is **an errand this person would actually ask for**, drawn from their own `wants` and `fears`. It is
not a hook the world hands them: Calvar wants the filtration done correctly and will test whether you are
competent before he asks; Ama has never been asked in the right way, and the right way is asking about her
husband rather than about the stone. Where a person has no seed the `want` is the derived fallback, which
is a premise rather than an errand.

*7 of 48 authored people carry seeds; 41 rely on the fallback.*

### R41b — A SOVEREIGN ARRIVES AT STAGE 3, AND BOTH SIDES ESCALATE INVERSELY

A Sovereign is one record with forms, never two — identity is what R41 is about. The form is chosen by the
arc's live stage: stage 1 at the mythic floor, **stage 3 diminished at level 45**, stage 4 final at 85.

**Stage 3 is the arrival**, and arriving is already doing something. A diminished Sovereign has two jobs
from the moment it lands: push the arc back its own way, and eliminate the people opposing it. It is an
active party on the board, not a boss waiting at the end of one.

**And the arc's position drives both sides in opposite directions.** A Sovereign whose arc is going its way
is fat and happy — business as usual, agents working, nothing escalating — while the anti-Sovereign
opposing it is desperate. Push the arc the other way and it inverts: **A FAT AND HAPPY SOVEREIGN IS BUSINESS
AS USUAL**, and a threatened one is the angriest version of itself facing the most committed enemy it has.
In the middle both push, which is where the game happens.

So pushing an arc against a Sovereign does not make things calmer. Neither side is ever idle: a complacent
Sovereign still has agents working, which is why the world feels pressed before anyone knows why.

Both sides investigate each other. Neither simply knows — each holds a picture that can be wrong, and the
four anti-Sovereigns have a reason to have met that did not exist before: they are researching the same
seven entities.

*What a recrossing leaves behind is deliberately unauthored. It fed on the way out; what that leaves is the
GM's until somebody plays it.*

---

### R50 — A CRAFT THAT HARMS ON PURPOSE DECLARES A HARM VERB

`battleSkillsForCharacter` builds the player's menu from the **record's** `functions`, one row per verb, and
never reads the rank's. `intent.js` reads the **rank's** rung. So a craft gated as harm that offers no harm
verb fires the lethal-intent question and then hands the player `heal`, `bind`, `ward`.

**The test is what the craft is FOR.** Does it kill by doing something to a person, or does it kill by going
wrong?

The first gets the verb. `case_closed` walks someone to the conclusion where the consistent act is a final
one — that is `break`, because nothing is struck and a person's reasoning is. `grief_that_stops` installs a
feeling at a weight no body holds. `bark_and_briar`'s thorns cut whoever tries the wall.

**The second gets the rung softened**, because a craft whose danger is its own failure is not an attack and
must not be offered as one. `calling_back` reaches into the deep dark and *"it works rarely and it is never
free"* — the cost is the warden's and the returned's, and nobody is being attacked. `open_threshold` leaves
a way home standing that other things can use. `given_errand` sends a shade, and **the harm is whatever the
player set it to do** — a shade carrying a message and a shade sent to kill are the same craft at the same
rank.

**And the verb goes on the RANKS that carry the rung, not only on the record.** `case_closed` teaches
`break` at rank 3 and nowhere else: rank 1 settles a matter and rank 2 compels, and being unable to act
against a conclusion is not being broken by it. A record claiming combat no rank teaches is the same defect
pointed the other way.

*Binding is not harm. `strike`, `break` and `hinder` are the family, and a craft that stops someone by
holding them is `incapacitating` and correctly declared.*
...
## 7e · REGIONS — a place belongs where its roads and its name say

A region is not a bounding box drawn round whatever happens to be inside it. **It has a centre where the
region IS**, and a radius that holds its members from that centre.

When a place disagrees with its region there are three answers and they are not interchangeable:

- **the reading is stale** — the land moved under a fixed measurement. Re-derive it with the same call the
  gate uses.
- **the place is in the wrong region** — check its ROADS and its NAME before its coordinates. Kestrel's
  Roost had three of four connections outside the valley; the Sunken Choir is *"below the Heights"*, and the
  Heights belong to the Echo Vale. **Neither of them moved a metre.**
- **the place is genuinely on the wrong ground** — then it moves, and Greywater Stilts did: a stilt-town
  over a marsh cannot sit on a usable slope.

**A bridge location belongs to the region it is NAMED for**, and its roads into the neighbour are what make
it a bridge rather than a mistake.
...
`local_layouts.json` places **84 sites across 18 of 141 locations** — a well at a centre, a river 2.2 miles
...
| ⛔ **`deniesPhase` is inert — the vocabularies do not meet** | the only value any content authors is `"setup"`; the only phases anything asks about are `"sense"` (`app.js`) and `"action"` (`skill_battle.js`), compared with `===`. ⚠️ So no craft can shut a step, and `CCODE-41`'s counterplay is advertised and unreachable. ⛔ **Do not simply rename it**: under Erik's 2026-09-07 ruling a denial is a CONDITION and must be an opposed roll first — see `po/SPEC_encounter_overhaul.md` §6
| ✅ **CLOSED 2026-09-07 — a braid was a bare number for 15 of 28 verbs** | `wovenBonus` reads only the TIER, and a verb with no `persistentEffects` row left nothing — so CCODE-37's *"one turn leaves two things standing"* reached the effect-bearing half only, and `strike ⋈ break` was a +4. ⚑ **Aevi authored all 15**; it reaches 28 of 28 now, with shared `kind`s so a family extends rather than stacks. §145
| ✅ **CLOSED 2026-09-08 — the three hinge NPCs have records** | Aevi authored Seraphine, Corvane and Aevi-the-Watcher as id-bearing legendaries (L60–62, 16 crafts, domains) after the roster found them hinging four of six arcs while resolving to nothing. ⚠ `the_iron_kestrel_buyer` stays absent and that is correct — *"a hidden hand, not a single person."* §10's check now asserts every hinge RESOLVES |
...
whole interface is **`app.js`, 19,956 lines**, and there are **53 `render*` functions** that paint into one
shell function, `chrome()`, called from **55 sites**. ⚠️ **THERE IS NO ROUTER AND NO SCREEN VARIABLE.**
...
**Measured 2026-09-07: of 50 named render functions, 49 have a caller.** ⛔ **One does not:
