...
| 08-29 | ⚠️ **`soakBonus` authored, READ BY NOTHING — flagged not hidden** | I checked before leaving it, having documented that exact failure an hour earlier | ⛔ `openProtection` has `resistBonus` and no soak equivalent | ✅ declared in the schema so it is authorable; **CCode owes the reader, and until then the soak half is prose** |
