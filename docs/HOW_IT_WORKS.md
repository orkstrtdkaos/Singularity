...
| 08-29 | ✅ **3 harm-gloss inversions + 3 repricings** | `quick_hands`, `second_wind`, `perfect_motion` all declared `none` while a rank damages — the gloss told the GM *"never invent a wound from it"* | as above | ⚠️ `shaped_body` was **e2 for 720 duration and standing soak** — the most defensive craft in the tradition priced below a cantrip |
