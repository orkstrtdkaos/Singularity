...
| 09-11 | ✅ **THE PELL–VETH CENSUS, RE-RUN UNDER R43 — AND IT REVERSED COMPLETELY** | CCode marked the 09-04 census history when he built R43; re-run before quoting. **Veth won 99.9% then; PELL WINS 95.8% NOW.** Death-save kills: **Veth 0 · Pell 0** — the save fired on every landed lethal hit and now fires only where a `killCost` is authored (one craft, `the_cut_thread`). Pell dies in **0 of 2000**. Every win is by pressure (1915 of 2000, health 0), mean 18.7 rounds. Her killers `hastened_grey` and the two T3s were **never lethal by design — they were lethal by a missing gate**; finishing blows are now Keystone Blow ×1480, Necrotic Strike ×277. ⚠️ **A ruling of Erik's sat unbuilt for seven days and the census measured the gap, not the game** | `§154` · `duel_pell_vs_veth.mjs` · R43 |
