...
| 08-28 | ✅ **27 narrative `extend` axes RULED PROSE** | `reach` `persistence` `foresight` `timeReach` have no engine field and say something no field says | — marked in-file so nobody "fixes" them | ⚠️ Erik 24 Aug: a named axis is shown to the player, not arithmetic |
