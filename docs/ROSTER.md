...
**GENERATED by `scripts/roster.mjs` · 147 records across 6 files · 131 reachable as opponents · 0 would fight at level 1 · 58 carry only the bare strike · 42 tiers derived from a role · 1 authored but never loaded.** ⚠️ Regenerate with `node scripts/roster.mjs --write`; never hand-edit inside the markers.
...
| ⚠️ reachable with no AUTHORED level (tier floor or derived) | 98 | of 131 reachable |
| ⚠️ reachable with no authored `abilities` (kit drawn from domains) | 98 | of 131 reachable |
...
|---|---|---|
| person | **70** | single authored people — the ones you can MEET |
