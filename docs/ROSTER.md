...
**GENERATED by `scripts/roster.mjs` · 166 records across 6 files · 147 reachable as opponents · 5 would fight at level 1 · 57 carry only the bare strike · 4 tiers derived from a role · 1 authored but never loaded.** ⚠️ Regenerate with `node scripts/roster.mjs --write`; never hand-edit inside the markers.
...
| ⚑ tier DERIVED from the role string, not authored | 4 | heroic · notable — shown as `~name` in the tier column |
