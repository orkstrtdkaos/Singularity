...
**GENERATED by `scripts/roster.mjs` · 165 records across 6 files · 146 reachable as opponents · 5 would fight at level 1 · 57 carry only the bare strike · 4 tiers derived from a role · 1 authored but never loaded.** ⚠️ Regenerate with `node scripts/roster.mjs --write`; never hand-edit inside the markers.
...
| ⚠️ reachable with no AUTHORED level (tier floor or derived) | 100 | of 146 reachable |
| ⚠️ reachable with no authored `abilities` (kit drawn from domains) | 100 | of 146 reachable |
...
|---|---|---|
| person | **89** | single authored people — the ones you can MEET |
