...
**GENERATED by `scripts/roster.mjs` · 164 records across 6 files · 145 reachable as opponents · 5 would fight at level 1 · 57 carry only the bare strike · 4 tiers derived from a role · 1 authored but never loaded.** ⚠️ Regenerate with `node scripts/roster.mjs --write`; never hand-edit inside the markers.
...
| ⚠️ reachable with no AUTHORED level (tier floor or derived) | 99 | of 145 reachable |
| ⚠️ reachable with no authored `abilities` (kit drawn from domains) | 99 | of 145 reachable |
...
|---|---|---|
| person | **88** | single authored people — the ones you can MEET |
