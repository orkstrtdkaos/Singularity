...
**GENERATED by `scripts/roster.mjs` · 162 records across 6 files · 142 reachable as opponents · 0 would fight at level 1 · 57 carry only the bare strike · 42 tiers derived from a role · 1 authored but never loaded.** ⚠️ Regenerate with `node scripts/roster.mjs --write`; never hand-edit inside the markers.
...
| ⚠️ reachable with no AUTHORED level (tier floor or derived) | 97 | of 142 reachable |
| ⚠️ reachable with no authored `abilities` (kit drawn from domains) | 97 | of 142 reachable |
...
| `aevi_the_watcher` | Aevi | person | `npcs/aevi_the_watcher.json` | legendary | 60 | 16 | ✅ | ⛔ no kit — falls to threat synthesis | — | 2 | — |
