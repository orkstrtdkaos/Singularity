...
**GENERATED by `scripts/roster.mjs` · 150 records across 6 files · 134 reachable as opponents · 0 would fight at level 1 · 58 carry only the bare strike · 42 tiers derived from a role · 1 authored but never loaded.** ⚠️ Regenerate with `node scripts/roster.mjs --write`; never hand-edit inside the markers.
...
| ⚠️ reachable with no AUTHORED level (tier floor or derived) | 98 | of 134 reachable |
| ⚠️ reachable with no authored `abilities` (kit drawn from domains) | 98 | of 134 reachable |
| ⛔ **reachable but the ONLY move is the bare strike** (no real craft) | **58** | `neth_the_stayed` · `morvane_the_harvest` · `cinder_vael` · `halcyon_the_finished` · `the_hollow_king` · `vessa_truename` · +52 |
...
| `the_long_remembering` | The Long Remembering | person | `npcs/the_long_remembering.json` | legendary | 63 | 16 | ✅ | ✅ | 32 | 2 | — |
