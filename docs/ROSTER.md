...
**GENERATED by `scripts/roster.mjs` · 144 records across 6 files · 125 reachable as opponents · 0 would fight at level 1 · 59 carry only the bare strike · 41 tiers derived from a role · 4 authored but never loaded.** ⚠️ Regenerate with `node scripts/roster.mjs --write`; never hand-edit inside the markers.
...
| ⛔ **reachable but the ONLY move is the bare strike** (no real craft) | **59** | `the_last_choirmistress` · `neth_the_stayed` · `morvane_the_harvest` · `cinder_vael` · `halcyon_the_finished` · `the_hollow_king` · +53 |
