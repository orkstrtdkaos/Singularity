...
**GENERATED by `scripts/roster.mjs` · 144 records across 6 files · 125 reachable as opponents · 0 would fight at level 1 · 59 carry only the bare strike · 40 tiers derived from a role · 4 authored but never loaded.** ⚠️ Regenerate with `node scripts/roster.mjs --write`; never hand-edit inside the markers.
...
| ⚑ tier DERIVED from the role string, not authored | 40 | heroic · notable · regional — shown as `~name` in the tier column |
