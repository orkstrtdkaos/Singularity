 ...
 *
 * ⛔ SNG-435 §C1 — `narrationHints` LEADS, AND THE ARGUMENT IS INFORMATION, NOT WORD-CHOICE. Erik could not
 * get a good Radiant Lance picture; its `shape` is `"damage"`, so this built "Radiant Lance — damage,
 * attack, precise". My first fix registered the category words and routed those to `narrationHints`. Aevi's
 * answer is better and it retires that mechanism:
 *
 *     25 distinct `shape` values across 376 abilities
 *     376 distinct `narrationHints` — every single one unique
 *
 * ⚠️ `shape` CANNOT DISTINGUISH MORE THAN 25 THINGS. The 33 abilities sharing `damage` include a coherent
 * light beam, a resonant shattering wave, and "they read the body the way a mason reads a wall, and then
 * they go to the seams." Those look nothing alike, and no choice of word for a 25-value enum could ever
 * make them. So the fault was never that the words are categories — it is that the FIELD IS LOW-CARDINALITY
 * BY DESIGN, and a "visual-sounding" shape would still be shared by thirty crafts that look unrelated.
 * A registry encoding that distinction encodes one that cannot matter, so it is gone.
 *
 * ⛔ HER SENTENCE IS THE RULE NOW: `shape` and `effectTags` are what make one craft RESOLVE differently;
 * `narrationHints` is what makes it LOOK different — it is written for a painter, and `description` is the
 * one written for a reader. Authored on 376 of 376, and read here for the first time.
 *
 * ⚠️ The shape/tags path stays as the FALLBACK, for a craft minted at runtime that has no hints yet. */
export function powerPhrase(ability = null, killer = {}) {
  ...
  if (ability) {
    const hint = clause(ability.narrationHints, CAP.power);
