...
19B. MASTERY (the defining moment). Depth is earned through use — rank 2 lands on its own. Rank 3, mastery, is NOT accumulation: it is a single beat where an ability was expressed in its complete form under conditions that demanded it (the umbral who HAD to vanish completely, and did). ⛔ When a craft LISTED in RIPE FOR MASTERY is used in a clearly decisive, scene-carrying way this beat, you MUST emit "markDefiningMoment" with its abilityId — do NOT merely narrate the triumph and move on; a described mastery with no op leaves the rank unearned and the player waiting. At most one per turn; only for a listed ability. ANTI-INFLATION still holds: the beat must be genuinely decisive (the craft carried the scene under real pressure) — "used it again routinely" is NOT a defining moment. The engine confirms the earn; if it isn't ripe, narrate the effort without claiming mastery.
...
REPLY FORMAT — a single JSON object, no other text:
{
  ...
  "stateOps": [{"op": "correctField|correctDomain|removeEntity|unstickQuest|reanchorLocation|fixCodexFact|correctAbilityRank|correctBond|correctVital|correctAttribute|mergeEntity|refuse", "field": "background|origin|nativeTradition|form", "slot": "primary|secondary|tertiary", "to": "corrected value / people / location / rank / number", "kind": "ability|companion|quest|codex|npc", "id": "entity id", "questId": "...", "toStage": "stageId", "topicId": "...", "text": "corrected fact", "vital": "health|energy|maxHealth|maxEnergy", "sub": "strength|agility|reason|insight|presence|rapport|craft|wits", "bondType": "...", "bondStage": "...", "relationship": 0, "fromId": "duplicate id", "intoId": "kept id", "what": "what was refused", "why": "why this is a REPAIR of something wrong"}],
...
export function salvageOps(raw) {
  const out = {};
  const keys = ["questUpdates", "npcUpdates", "placeUpdates", "codexUpdates", "deeds", "ledgerEvents", "encounterOps", "characterDeltas", "scene", "relationshipDeltas", "timeOps", "moveTo", "stateOps", "itemUpdates", "gambitOps", "markDefiningMoment", "markTeacher", "offerPromotion", "offerAcquisition", "generateRequest", "imagePrompt"];
...
Use "travel" (and set travelTo to the destination) whenever the player is trying to GO/HEAD/JOURNEY/SET OUT to another place ("head to the edge district", "travel to Millbrook", "let's go back to the mill") — not for moving around within the current location.
