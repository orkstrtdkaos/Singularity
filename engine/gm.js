...
${npcSeedDetail}`);
  ...
  if (latentArcsDetail) world.push(`## STIRRING IN THE WORLD (SNG-191 §7 — things that were fomenting on their own and have now SURFACED to the character; the world has its own agenda). These are REAL and already underway — the character's first contact is a late event in something that has been building, never its beginning. Develop them as the fiction touches them (a face to meet, a place changed, a rumour to follow); they are not the player's alone and some may resolve without them.\n${latentArcsDetail}`);
