...
REPLY FORMAT — a single JSON object, no other text:
{
  ...
  "holdingOps": [{"op": "claim|steward|release|transfer|sell", "id": "kebab-id", "toEntity": "npcId who takes it up (transfer only)", "kind": "post|enterprise", "name": "what it is called", "locationId": "where it is", "steward": "npcId who keeps it, or null", "obligation": "what it owes, in one line"}],
...
"debtOps": A DEBT IS OWED TO A PERSON, AND THEY DECIDE. When the fiction leaves the character owing someone — a price unpaid, terms broken, a holding walked away from — emit "record" with the npcId of who is OWED: it is THEIR memory, and the world escalates by their character (a holder who reacts to debtors grows cold and then refuses; one who does not simply remembers). Emit "settle" when it is PAID (the purse is debited — do not narrate coin changing hands that the purse cannot cover). Emit "forgive" when a DEED outweighs it, and say what the deed was. ⛔ Never in coin. Talking to the holder is how a debt is settled — a redemption arc, not a payment screen. "sell" (holdingOps): the character sells what is STORED at a holding they are standing in, at this Reach's prices — emit it only when the fiction has them do so.
