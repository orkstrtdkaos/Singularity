...
"gambitOps": THE PLAYER PLANS; YOU OPEN THE BOARD — NEVER RUN A PLAN AS ONE ACTION. When the player says they want to PLAN or SET UP a multi-step approach to a gambit-apt objective ("let me plan this", "I want to set up a gambit", "here's my plan: first… then…"), DO NOT resolve it as a single action and DO NOT narrate any outcome — emit "gambitOps" with a proposed "goal" and 2–5 seeded "steps", each a prose maneuver with an optional "fallback". This OPENS the gambit builder PRE-FILLED; the PLAYER then edits, assesses, and commits it, and the ENGINE runs it step by step (Design Law 1 — you propose the plan, the engine resolves it). ⛔ HOLD THE PLAN-LEVEL VIEW AGAINST THE PUZZLE'S STEPWISE PULL: a gambit is declared as a whole and resolved as a whole; the maneuvers live INSIDE it — never collapse "declare the plan → resolve within" into turn-by-turn "now what?" prompting. Seed the steps from what the player actually described; if they only said "I want to plan this", propose a sensible first sketch they can rewrite. FEWER STEPS ARE HARDER (less slack); a fallback ADDS a step and lets the gambit absorb a bad roll. A brief in-fiction line is fine ("Alright — here's how I'd lay it out") but the plan itself goes in gambitOps, not the prose. Omit gambitOps on every ordinary turn.
...
export function salvageOps(raw) {
  const out = {};
  const keys = ["questUpdates", "npcUpdates", "placeUpdates", "codexUpdates", "deeds", "ledgerEvents", "encounterOps", "characterDeltas", "scene", "relationshipDeltas", "timeOps", "moveTo", "stateOps", "gambitOps", "generateRequest", "imagePrompt"];
