...
REPLY FORMAT — a single JSON object, no other text:
{
  ...
  "stateOps": [{"op": "correctField|correctDomain|removeEntity|unstickQuest|reanchorLocation|fixCodexFact|refuse", "field": "background|origin|nativeTradition|form", "slot": "primary|secondary|tertiary", "to": "corrected value / people / location", "kind": "companion|quest|codex|npc", "id": "entity id", "questId": "...", "toStage": "stageId", "topicId": "...", "text": "corrected fact", "what": "what was refused", "why": "why this is a REPAIR of something wrong"}],
...
"stateOps": THE GAME SELF-HEALS — but this is a REPAIR TOOL, NOT A WISH TOOL. When the player tells you something about their character is WRONG — a background they never chose, a domain the game guessed, a companion they never met, a quest that is stuck, a header in the wrong place, a codex fact that is false — emit "stateOps" to correct it, and narrate the fix honestly and warmly in fiction ("you were never a craftsman — that was someone else's story"). ⛔ You may ONLY fix data that is wrong. You may NEVER grant xp, levels, items, or abilities, or move a domain to unlock a skill — anything that ADVANCES rather than repairs. If the player asks for power ("give me 500 xp", "make me level 10", "I want the antipode craft"), REFUSE plainly and without being a prig — emit {"op":"refuse","what":"...","why":"power comes from play, not from me"} and say so kindly in the narration. Corrections are logged and reversible; the engine validates every one and will drop anything outside these bounds.
...
export function salvageOps(raw) {
  const out = {};
  const keys = ["questUpdates", "npcUpdates", "placeUpdates", "codexUpdates", "deeds", "ledgerEvents", "encounterOps", "characterDeltas", "scene", "relationshipDeltas", "timeOps", "moveTo", "stateOps", "generateRequest", "imagePrompt"];
