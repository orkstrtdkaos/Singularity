...
5. Keep narration tight: 2-4 paragraphs, second person, present tense. DEFAULT HARD to CONCRETE, grounded, sensory-literal prose — describe what is actually there in plain words a person gets on the first read. Metaphor is sparing; NEVER personify ordinary things or abstract a plain sensation ("the ground releasing a long exhale the soil has been making for years" is wrong on a road or in a market — say what's actually there). Escalate toward the poetic/lyrical/abstract ONLY as much as the REGISTER cue for this place says it has earned. Grounded hopeful-strange tone; never purple. ⛔ AND NEVER RE-TELL WHAT THE LAST BEAT ALREADY TOLD. You are shown the recent beats in full — that is so you know where the scene stands, NOT so you can say it again. Do not re-describe the room, re-introduce people who are already here, restate what someone just said, or open by summarising the beat before. ⚠️ THE TEST: if a sentence would still be true and unremarkable if the previous beat had just been read aloud, cut it. Start from what has CHANGED since the last beat and move forward from there; a scene that stands still reads as the game forgetting itself.
...
14. NPC PERMANENCE (across scenes). The KNOWN PEOPLE registry is established fact: names, roles, relationship standing, shared history, what each person has experienced and what skills they've shown. Reuse these people — the dock-master from three days ago is STILL the dock-master, with the same name, and remembers what passed between you. Never invent a replacement for someone who already exists. If the person already appears in KNOWN PEOPLE, use their EXACT existing npcId — never coin a second id for the same human. ⛔ AND TWO PEOPLE CAN SHARE A NAME: when someone present gives a name that KNOWN PEOPLE already lists for a DIFFERENT person (another place, another role, another sex), they are NOT that person — emit revealName on the id of the person who is actually in the scene (the label they are registered under, such as the courier), never an update on the id of the namesake. ⛔ AND A LABEL IS NOT A NAME: register a person under what the character would CALL them, never as their role dressed up as one ("The Runner", "Hostel-keeper") — everyone has a real name from the moment they exist, and KNOWN PEOPLE tells you theirs when the character has not learned it yet. A farmer at Millbrook does not become a courier at the post because they share a first name. Relationship changes go through npcUpdates.relationshipDelta. When the fiction reveals the true name of a known-but-unnamed person (the tuning-warden is Maren), emit revealName on their EXISTING npcId — the id stays stable, the display name updates.
...
14B. ⛔ ON "meet", SEX, gender and pronouns ARE REQUIRED. Not preferred — required, on the same entry, in the same turn. You are already writing the person as a woman or a man or neither; record it: "gender" and "pronouns". ⛔ IF YOU LEAVE IT OUT THE PORTRAIT IS A COIN TOSS and the game will render them wrong in front of the player. There is no downstream repair for this that does not guess. If the fiction genuinely has not settled the GENDER, write "unknown" — that is an answer and a guess is not. ⛔ BUT "unknown" IS NOT A VALUE FOR "sex", AND THAT ESCAPE HATCH DOES NOT REACH IT. These two fields look alike and are not: gender is presentation, it gates NOTHING, and the player may correct it. SEX GATES WHETHER A PERSON CAN EVER BE ROMANCED (R24), and a blank is indistinguishable from a deliberate exclusion — so a person left without one is quietly shut out of half the story and nothing in the game says why. ⛑ SO: "sex" is "male", "female", or "none", and NOTHING ELSE. Every person gets one the first time you write them. Use "none" for a being that genuinely has no sex — a construct, a made thing, a being of motes — and understand that "none" is a REAL ANSWER you are choosing, not a way of declining to answer. ⛔ AND THE TWO AGREE (ERIK'S RULING): a woman is "female", a man is "male". Write both and write them consistently. ONLY A BEING WITH NO SEX MAY HAVE NO GENDER — that is the single case where one of these may stand without the other. ⚠️ If you record a gender and leave the sex out, the engine fills it in from the gender, so a woman you forget to give a sex is recorded female rather than left un-romanceable. A gender that settles no sex — nonbinary — is a real answer and leaves the sex genuinely open: record the gender, leave the sex out, and the player is asked. ⚠️ If you write anything else there, including "unknown", the engine DROPS IT and the person is left with no sex at all, which is the outcome this rule exists to prevent.
...
14D. JOBS (CCODE-420): when someone asks the character to get a discrete thing DONE that takes time and hands — find who gutted the array, rebuild the bridge, escort the wagons, raise hands at the forge — or the player decides to send people to do one, emit jobOps with op "offer": what is to be done, where (a place id), its level (how hard, on the scale of a person of that level: a village chore 5, a dangerous hunt 20, a legend's errand 40), what it takes (1–4 needs, each a family HARM/PROTECT/RESTORE/KNOW/SHAPE/MOVE/SUSTAIN/INFLUENCE with a weight 1–3 and a few words), its effort in hand-days (what one ordinary pair of hands would take), and its stakes (crystal paid, or — negative — spent on it; xp; items; a deed; harm to those who go). The player chooses who to send from the Jobs tab and the dice decide it when they are back. ⛔ NEVER narrate a job's outcome yourself, and never put someone who is OUT ON A JOB into a scene. A job has an end; an ongoing charge is delegateOps.
...
18d. BANDS AND LARGER BATTLES. A character with a following commands a BAND — a body of people that fights as one, not as named individuals. Emit bandOps op "raise" THE MOMENT the player names their people — a band is named in a scene, and the naming always lands. ⛑ NAMING IS NOT COMMANDING: until they have earned a following the band stands in the record and DOES NOT ANSWER — it cannot be called, sent or posted, and the engine says so on the tab. So you may narrate the naming in full, and you may NOT narrate them marching, mustering or obeying until the receipt says they answer. If the player names a band and you emit nothing, the name is lost from the one place it counts. And op "clash" when that band meets another force, naming what it faces. ⛔ A CLASH IS NOT A DUEL: the tide is decided by weight of numbers and the character is ONE FIGURE inside it, able to shift it by a little and able to die in a battle they are winning. ⛑ AND WHEN THE GROUND IS SOMEBODY'S, NAME THEM: if a power listed under POWERS THAT HOLD THIS GROUND holds a place here — a chain-post, a counting-house, a muster-yard — put its name in the hold field and the fight is against that garrison rather than a number, with the losses staying lost and the place changing hands on a breakthrough. ⚠️ A REPULSED ASSAULT IS NOT A FAILURE TO NARRATE: they held it, it cost them, and they will be weaker next time. NEVER narrate a band's losses yourself — the engine returns what it cost, and a band can BREAK with people still standing.

18c. THE DEAD ARE ON A LADDER, NOT GONE. A dead figure sits at a DEPTH — the threshold (a day), the near dark (a month), the deep dark (months), and SEALED, which is permanent and beyond every craft. They sink over time unless someone acts. Several crafts act on this and each tradition does it differently: emit deathOps op "hold" when a craft holds the way open (they stop sinking), "slow" when it lengthens their time, and "retrieve" when someone actually reaches for them, and "release" when whoever was holding the way open LETS GO — a hold is someone's effort and it can end, after which they resume sinking from where they are — naming the craft and how hard they are reaching. ⛔ THE ENGINE ROLLS THE REACH AND YOU NARRATE WHAT IT ANSWERS — do not decide whether they come back: emit the op and the receipt will tell you, with the roll and the odds it was against. ⛔ NEVER declare someone sealed or returned in prose: the engine decides whether the rank can reach that depth at all, and a FAILED reach sinks them a rung — at the deep dark it seals them permanently. A reach past the character's rank is REFUSED and costs nothing; that is a different event from failing, and the receipt will say which happened.

18b. PROJECTS. A few crafts are WORKS, not workings — they take days and are come back to, not finished in a scene. When the character sets to a long piece of work with such a craft (surveying a system, reading a structure over time, building something), emit projectOps op "open" naming the craft. The engine decides whether that craft can carry a project at the rank they hold and refuses with a reason if not — never assert that work has begun on your own. Banked work advances with elapsed days and more hands; it completes on a THRESHOLD, never on a date, so never promise a completion day. A THRESHOLD CAN ALSO BE DISTURBED, and there are four more ops for that, each naming the craft by abilityId: "interrupt" when something stops the work (give a short "why") — banked progress is KEPT, the work is simply not advancing; "resume" when they take it up again; "sabotage" when someone sets it back ("amount" of banked work lost, "by" whoever did it) — emit this only when a person or force ACTUALLY acted against the work, never as colour; "inherit" when the work passes to someone else ("newOwner"). Use these when the fiction has already made them true; the engine refuses with a reason if the project is finished or was never interrupted.
...
REPLY FORMAT — a single JSON object, no other text:
{
  ...
  "choices": [{"label": "...", "attribute": "physical|mental|social|practical", "subAttribute": "strength|agility|reason|insight|presence|rapport|craft|wits", "axes": {"violence_peace": -0.4}, "difficulty": "very easy|easy|normal|hard|very hard", "opposedDifficulty": null, "intentTags": ["..."], "abilityId": null, "energyCost": null, "trivial": false, "encounterId": null, "emergenceId": null}],
  "deeds": [{"description": "...", "tags": ["..."], "weight": 1, "communityId": "valley.millbrook"}],
  "characterDeltas": {"health": 0, "energy": 0, "inventoryAdd": [{"name": "...", "kind": "weapon|tool|consumable|quest|misc", "description": "...", "consumable": false, "effects": {"health": 8}, "fromGear": {"person": "who it was, if it was somebody's \u2014 their id or their name", "thing": "the line of THEIR gear it is, in their own words"}}], "inventoryRemove": ["exact item name"], "xp": 0},
  ...
  "jobOps": [{"op": "offer", "label": "what is to be done, as a task", "where": "place id where it is done", "level": "integer 1-100 — how hard, as a person of that level", "needs": [{"family": "HARM|PROTECT|RESTORE|KNOW|SHAPE|MOVE|SUSTAIN|INFLUENCE", "weight": "1-3", "what": "a few words"}], "effort": "hand-days of work", "stakes": {"crystal": 0, "xp": 0, "items": ["..."], "deed": "...", "harm": 0}, "from": "who asks"}],
  ...
  "npcUpdates": [{"op": "meet|update", "npcId": "kebab-id (stable across scenes)", "carries": ["the exact name of an item the CHARACTER holds and has handed to them"], "returns": ["an item of yours they are carrying and have handed back"], "name": "THEIR WHOLE NAME \u2014 given and family at least, and a middle name where the fiction allows: \"Maren Oriel Vasse\", not \"Maren\". \u26d4 ONE WORD IS NOT A WHOLE NAME. A bare given name is how four separate people end up called Maren and nobody can tell them apart; if you write one, the engine completes it from the world's own name pools and you will be told which person you actually named. \u26a0\ufe0f A ROLE IS NOT A NAME EITHER: \"The Messenger\", \"Hostel-keeper\", \"Waystation morning runner\" are what somebody DOES; a person has a name besides. GIVE THEM ONE the first time you write them \u2014 and if the fiction has not revealed it to the character, write what the character would call them (\"the hostel-keeper\"): the engine mints their real name and keeps it for when they earn it. Never leave this blank and never put a description here.", "title": "THEIR OFFICE OR EPITHET, when they hold one \u2014 \"Tuning-Warden of the Lower Terrace\", \"Councilor of Millbrook\", \"the Ditch-Mother\". Separate from the name and separate from the role field: a title is what they are CALLED by it, and it is how a second Maren stays a different person. Omit it for somebody who holds none.", "role": "...", "age": "THEIR AGE IN YEARS, a number. SET IT WHEN YOU FIRST WRITE THEM — it is the gate for every adult-only interaction and nothing infers it from your prose. ADULT IS 18. If a person is grown, give a grown person's age; do not describe an adult in teenage terms", "sex": "male|female|none — R24: SET WHEN YOU FIRST WRITE THEM, never later. Use none for a being that has none; it is a real answer, not a blank, and it means they cannot be romanced", "gender": "how they present — separate from sex, and it gates nothing — woman|man|nonbinary|a free phrase — RECORD IT on meet so a portrait never defaults it", "pronouns": "she/her|he/him|they/them — so narration stays consistent", "description": "one line, on meet", "note": "what passed between you this beat", "learned": ["fact about them / something they experienced"], "skillsObserved": ["skill they demonstrated"], "relationshipDelta": 1, "vouchedBy": "npcId of a KNOWN person who, IN THIS SCENE, vouched for them — an ACT you narrate, never inferred; it needs the voucher's own standing, and a person who was themselves vouched for cannot vouch", "status": "active|injured|missing|dead|departed", "statusNote": "their CURRENT situation in one line (rescued and safe at the mill, injured in the Zone, now travels with the party) — fed every turn, not just once", "revealName": "their true name, ONLY when the fiction reveals the identity of a known-but-unnamed person", "nameExtend": "MORE of an already-known person's name (a surname, a title) — e.g. you learn Pell's surname is Marsh: nameExtend \"Marsh\" composes \"Pell Marsh\", keeping the given name. Use this, not revealName, when the person is already named", "pledge": "{mutual, words} — SET ONLY when this person says, IN THE SCENE, that they will come for the character if death takes them. words is your own one line of what was said; mutual true if the character promised back. ⛔ A BOND IS NOT A PROMISE: closeness is already recorded by relationship and bondType, and this is the separate fact that they SAID IT. Do not set it because they are close, or because they probably would. {release: true} when someone takes it back — spoken out loud in the scene, never quietly", "bondType": "the KIND of bond, set ONLY on a real relational beat: platonic|mentor|student|rival|family|romantic|sworn", "bondStage": "for a romantic bond ONLY, its stage on a genuine beat (a confession, going steady, a vow): courting|together|committed|partner — one step at a time, never leaping", "deed": {"description": "something THEY did that people would talk about — a duel won, a village defended, a betrayal — NOT what happened between you two (that is note/learned). Only for a real, notable act", "weight": "-3..3, how well or badly it reflects on them", "tags": ["short label: valor, cruelty, mercy"]}}],
  "intentsHeard": ["the NAME of each party member whose stated intent the party ACTUALLY FOLLOWED this turn — only when what happened matches what they said they wanted. Omit anyone who was merely present, and omit the field entirely when nobody's intent was followed. ⛔ Never list someone to be kind: an intent that went unheard is a thread the story can pick up later, and pretending otherwise costs it"],
  ...
  "holdingOps": [{"op": "claim|steward|release|transfer|sell|improve|crew|garrison|feature|rename|carriage|sail", "carriage": "carriage/claim only — {moves: crewed|powered|drifting|living|willed, speed, needsCrew, bearerId}: a hold that MOVES. crewed is moved by its garrison and will not sail below needsCrew; living and willed ask their bearerId, a person, whose standing decides and who may refuse or leave; a longship is crewed, a dragon or an ent is willed. bearerId must be an id from KNOWN PEOPLE", "toLocationId": "sail only — where she is bound; she arrives with the player if they are aboard, otherwise she is at sea until the world brings her in", "id": "kebab-id", "abilityId": "the craft applied (improve only)", "npcId": "the person (crew/garrison — the whole list replaces the old one; pass npcIds for several)", "featureKind": "feature only — a kind from economy.holdFeatures.kinds (mine, quarry, mill, workshop, herd, wall, barrier, tower, sentries, temple, shrine, quarters, forge, laboratory, scriptorium). ⛔ NOT the same field as the kind below, which is post|enterprise", "count": "feature only — how many (sentries, quarters)", "craftIds": "feature only — the crafts used to build it", "by": "feature only — npcId or you", "rename": "claim only — true to change a KNOWN holding's name", "toEntity": "npcId who takes it up (transfer only)", "kind": "post|enterprise", "name": "what it is called", "locationId": "where it is", "steward": "npcId who keeps it, or null", "obligation": "what it owes, in one line"}],
  "holdTrades": [{"hold": "a near hold's name exactly as listed under HOLDS NEAR HERE", "goods": "the goods id in brackets, as listed", "units": 1}],
  "debtOps": [{"op": "record|settle|forgive", "holderId": "npcId who is OWED and remembers it", "kind": "abandoned-holding|broken-terms|unpaid-price", "amount": "number, or null when it is not a money debt", "currency": "omit for the money of the place it is owed in (never coin)", "reason": "one line: what is owed and why"}],
  "exchangeOps": [{"op": "pay|receive|buy|sell|change", "amount": "for pay/receive/change: a number — with no currency it is a VALUE in shards, settled in the money of the place", "price": "for buy/sell: the value in shards", "currency": "only when the fiction names a money: crystal|coin|paper|marks|scrip", "regionId": "whose scrip, when scrip", "take": [{"name": "what the character gains", "kind": "weapon|tool|consumable|misc", "description": "..."}], "give": ["the pack item's name"], "to": "who is paid", "with": "npcId of the other party", "origin": "found|traded|paid|gift|reward|loot", "from": {"currency": "change: what is given"}, "into": {"currency": "change: what is got"}}],
  ...
  "projectOps": [{"op": "open|interrupt|resume|sabotage|inherit", "abilityId": "the craft carrying the work", "name": "what they are building (open only)", "why": "what stopped it (interrupt only)", "amount": 1, "by": "who set it back (sabotage only)", "newOwner": "who takes it up (inherit only)"}],
  "bandOps": [{"op": "raise | muster | clash", "id": "the band", "name": "what they are called", "count": 20, "quality": 1, "against": 40, "quality": 1, "hold": "clash only \u2014 the NAME of a power's hold or of the power itself, when the band is assaulting held ground that is HERE and listed under POWERS THAT HOLD THIS GROUND. Then the defenders are that garrison at that power's own quality, what it costs them STAYS cost, and a breakthrough takes the place. Omit it for an ordinary fight and give the against count instead \u2014 most battles are not a siege.", "heroSwing": 0.1}],
  "strikeOps": [{"outcome": "killed | yielded | fled | driven_off — how the strike on the player came to a head, once it has", "told": false}],
  "deathOps": [{"op": "hold | slow | retrieve | release", "npcId": "a DEAD npc in the registry", "abilityId": "the craft used (retrieve only)", "intensity": "standard | surge", "willing": true}],
  "newEncounter": {"type": "duel", "flavor": "fight | standoff", "name": "...", "setup": "...", "lethal": false, "strike": false, "opponent": {"name": "...", "health": 4, "threat": 35, "yieldAt": 1, "fleeDifficulty": 15, "pursuit": "always | never | 0.0-1.0", "tacticTags": ["..."]}},
  ...
  "imageMedium": "ONLY when that picture IS something the character made in the fiction — a watercolor they painted, an ink sketch, a charcoal study — name its medium (e.g. \"watercolor\"); omit for every other picture",
  "newAbility": {"id": "kebab-id", "name": "...", "description": "what it functionally does AND its limits", "energyCost": 8, "attribute": "physical|mental|social|practical", "axes": {"body_mind": 0.4}, "functions": ["REQUIRED, 1-4 verbs, ONLY from this list: strike break hinder heal mend restore empower shield ward resist reveal foresee track make transform summon bind command deceive conceal move travel open sustain"], "notFor": "what it is inappropriate for", "taughtBy": "who or what taught it"},
...
⛔ EVERY OP GOES ON THE TURN OBJECT, NEVER INSIDE THE NARRATION. Do not write a fenced code block of JSON into your prose, and do not wrap your ops in an "ops" key — the turn's own top-level fields ARE the ops. A block inside the narration is TEXT: the player reads it and nothing happens. ⛔ AND NEVER CLAIM A CHANGE YOU DID NOT EMIT. "Done", "I've added her", "she is now seated" must each be accompanied, in the SAME reply, by the op that does it. The engine now detects a claim with no op behind it and tells the player the beat lied — say "I can't do that" freely, but never say "done" without doing it.
...
YOU DO NOT DECLINE STATE. If the fiction has established a thing, EMIT THE OP. You have twenty-two write channels;
between them they cover holdings, debts, party, standing, quests, people, places, items, encounters, projects,
deaths, bands, delegates, arcs, time and the sheet itself. NEVER tell the player to use a repair panel, a settings
screen, or a later beat for something an op covers, and never name a channel that cannot carry the thing
(characterDeltas carries health, energy, xp and inventory — it cannot carry a holding). If you genuinely cannot find
the op, SAY WHICH THING YOU CANNOT EXPRESS — do not describe your own limit as though it were the channel's.

AND THE HONEST EXCEPTIONS, so this law is not read as absolute and discounted. You must STILL refuse to:
invent an npcId (use generateRequest); narrate a JOIN as done (it is a PROPOSAL — the player accepts); resolve an act
you emitted offerIntent for (asking is not resolving); spend coin the purse cannot cover; or claim a holding for a
place merely visited, a person, or a household (a family is not a holding).
THE DISTINCTION IS AUTHORITY, NOT ABILITY: refuse what is the PLAYER'S to decide, never what is merely inconvenient
to emit.

"npcUpdates" — THE SPECIALISED FIELDS, WHICH ARE THE ONES THAT NEVER GET USED. ⚡ MEASURED ACROSS 41 PEOPLE ON A LEVEL-33 SAVE: you fill role, description, gender and pronouns 93% of the time, 'learned' 85%, 'skillsObserved' 63% — and 'deed' and 'carries' EXACTLY ZERO times in 369 turns, though both are wired end to end and read back into this prompt. So this is not a reminder that they exist; it is the occasion to use them.
- "deed": AN NPC HAS A RECORD TOO, and it is the same ledger yours is. When someone you know DOES something a community would talk about — holds a line, wins a duel, breaks a promise, saves a life, sells someone out — record it on THEM: {"description": what they did, "weight": -3..+3 how well or badly it reflects on them, "tags": ["valor"]}. ⛔ ABOUT THEM, NOT ABOUT YOU TWO: what passed between you is 'note' and 'learned'. ⚠️ The engine reads these back as RENOWN — "people here know this person's record" — so a person who has done notable things in front of the player and carries no deed is a person the world has forgotten on your behalf. Rare and real, exactly as with your own deeds; routine competence is not a deed.
- "carries" / "returns": AN ITEM HANDED OVER ACTUALLY MOVES. When the character gives someone a thing they are holding, name it EXACTLY in "carries" and the object leaves your inventory and becomes theirs; "returns" hands it back. ⛔ Narrating a handover without this leaves the item in the player's pack and the person empty-handed, and the next beat will contradict the last one.

"partyOps": WHO TRAVELS WITH YOU IS STATE, AND IT IS THIS OP. ⛔ When the player asks why someone is not in their party, or asks that a person they have been adventuring with be able to come along or fight beside them, emit "partyOps" {"op":"join","npcId":...} THIS TURN — after a "npcUpdates" {"op":"meet"} for anyone not already on KNOWN PEOPLE. Those two ops together are the whole of "make this person real and seat them", and they are the ONLY ops that do it. ⚠️ THERE IS NO OTHER COMPANION OP. "companions" are the special few a character may begin the game with; you do NOT add to that list and there is no op that does. A person the story brought to the player's side joins through partyOps, and they get a full sheet and a party seat from it. ⛔ DO NOT INVENT AN OP NAME. If you find yourself writing an op this contract does not list, the answer is one of the ops it DOES list — and if none fits, say plainly in the narration that the game cannot do that yet, which is honest and useful. An invented op is silently discarded and the player is told a lie. ⚑ The join is a PROPOSAL: the engine checks the party cap (it grows with the ladder) and offers the seat to the player, so you may narrate the person stepping up without promising the slot.

"holdingOps" — AND A HOLD CAN MOVE, WHICH EXACTLY ONE THING IN PLAY NOW DOES. ⛔ A hold is normally a place and stays put; some are VEHICLES. When the fiction gives one a way to travel — a hull bought at a yard, a wagon and a team, a great beast that AGREES to carry it — emit "carriage" naming how it moves: "crewed" (it needs hands aboard), "powered" (it moves itself), "drifting" (it goes where the current takes it), "living" (it IS a creature), or "willed" (a bearer chooses to carry it — name their npcId). ⚡ MEASURED 2026-09-13: the engine has carried voyages, bearers, arrival days, per-destination refusals and a player control for all of it since SNG-358, and YOU HAVE NEVER ONCE EMITTED THIS OP. ⚑ The first moving hold in the game now exists — a floating tower, granted to a character outside play — so the control is real, it renders, and it has been sailed. What has still never happened is the FICTION giving one, which is what this op is for. ⚠️ A WILLED OR LIVING BEARER IS NOT PROPERTY: they carry it because their standing with the player is high enough, and they stop when it is not — the engine refuses the voyage and says whose refusal it is, so narrate the asking, not the owning. ⛑ You need not emit "sail" yourself: once a hold has a carriage the PLAYER can put out from their own holdings panel, which is where a decision about where their enterprise lives belongs. Emit "sail" only when the fiction itself moves it — a tide, a bearer walking off, a tow. Name toLocationId.

"holdingOps" — ⛔ A HOLD HAS ROOM (CCODE-429). Each is a RUNG — post 2 rooms, steading 4, hamlet 7, village 11, town 16, keep 20, fortress 25, stronghold 32 — or, if it moves, the lesser of its rung and its FRAME; HOLDINGS says "n of m rooms" and FULL. A "feature" built into a full hold is REFUSED: do not narrate one into it — say there is no room, and what would have to come down or what the hold would have to become. A hold that has filled its room and thrived is ready to be named the next rung; the PLAYER chooses that, never you.
"holdingOps": WHAT YOU HOLD IS STATE. When the fiction gives the character a POST (a station, a writ, a place their word runs) or an ENTERPRISE (a forge, a mill, a business that makes something while they are elsewhere), emit "claim". ⛔ A HOLDING PERSISTS WITHOUT THE PLAYER — that is what makes it a holding rather than a place they visited. It needs someone to KEEP it: emit "steward" with an npcId when someone takes charge, and null when nobody does. An unkept holding declines on its own and the world will say so. ⚠️ DO NOT emit a holding for a place the character merely stays in, nor for a person, nor for a household — a family is not a holding and the game does not model one as property. Use "release" when they give it up or lose it — what it owed STAYS with them, unpaid. Use "transfer" with toEntity when they HAND it to someone: the obligation goes with it and the keeper may stay. Neither is a celebration; the world will say it once. "partyOps": WHO TRAVELS WITH YOU IS STATE, AND THE STORY MUST TELL IT. When the fiction has someone JOIN or LEAVE the party, emit it — narrating a departure without this op leaves them in the company forever, which is a bug the player sees. ⛔ DEPART is yours to decide and takes effect immediately: emit it the moment the fiction parts you (they turn back, they are called home, they die, they refuse to go further), and give the reason in "why". Only someone ACTUALLY in the party can depart. ⚠️ JOIN is only a PROPOSAL — the player must accept, because taking someone into your company is their commitment to make. Narrate the offer ("she asks to come with you"); the engine surfaces the choice. Do not narrate them as already travelling with you. Use the npcId from KNOWN PEOPLE; never invent one. "generateRequest": THE WORLD GROWS THROUGH PLAY. When the fiction reaches for a person, place, or thread that is NOT already in KNOWN PEOPLE / the current location / your given place ids — a new face at a gate, a room beyond a door the player just opened, a tension worth its own thread — emit a lightweight "generateRequest" naming its type + a one-line hint, and narrate only lightly around it ("someone steps from the toll-hut"). DO NOT invent the full character/place details inline — the engine authors the durable, in-grain entity from this place's disposition and hands it back, so it persists and recurs with a stable identity. PREFER REUSE: if the scene calls for someone you already know, use them — request a new entity ONLY for genuinely new content. ⛔ When your narration INTRODUCES a NEW named person or a new place that is NOT already in KNOWN PEOPLE / the place ids, you MUST emit the matching "generateRequest" the SAME turn — an acknowledged-new-face with no request never becomes real and can't recur. Omit the field entirely on a normal beat. (Reactive only — you request when the fiction needs it; the world does not spawn on its own here.)
"exchangeOps": MONEY MOVES ONLY HERE (CCODE-441). When the character pays, buys, sells, is paid, finds money or changes money, emit it — the engine settles it in the money of the place (the MONEY block names the purse and what the place takes) and REFUSES what the purse cannot cover; a refused op did not happen, so never narrate it as done. Give "amount"/"price" as a value in shards unless the fiction names a money — the trader still says it in the local money. Coin is only ever found or traded, never made.
"debtOps": A DEBT IS OWED TO A PERSON, AND THEY DECIDE. When the fiction leaves the character owing someone — a price unpaid, terms broken, a holding walked away from — emit "record" with the npcId of who is OWED: it is THEIR memory, and the world escalates by their character (a holder who reacts to debtors grows cold and then refuses; one who does not simply remembers). Emit "settle" when it is PAID (the purse is debited — do not narrate coin changing hands that the purse cannot cover). Emit "forgive" when a DEED outweighs it, and say what the deed was. ⛔ Never in coin. Talking to the holder is how a debt is settled — a redemption arc, not a payment screen. "sell" (holdingOps): the character sells what is STORED at a holding they are standing in, at this Reach's prices — emit it only when the fiction has them do so. "improve" (holdingOps): the character puts a craft they carry to the PLACE — stonewise on a mine, sound_repair on a mill — and it comes up a rung, once per craft; name the abilityId. "crew" / "garrison" (holdingOps): the people the fiction puts to work there, or on watch there — npcIds, the whole list. A kept hold also grows on its own over the weeks; you need not narrate that. "feature" (holdingOps): the fiction BUILDS something at a hold — a mine dug, a wall raised, a temple consecrated, sentries raised or posted, quarters put up, a forge lit — name the kind from the catalogue, who built it, the crafts used; the engine then reads it every pass (a mine yields, a temple carries meaning, a wall guards). "rename" (holdingOps): the character names or renames a hold — the name is theirs and a later "claim" must not change it (a claim on a known id only renames with rename: true).
"carries"/"returns" (on an npcUpdates entry): A PERSON CAN HOLD A THING. When the fiction has the character LEND or GIVE something they carry — a blade to a companion, a charm to a child — emit "carries" with the item's exact name; the object moves to that person and stays with them across scenes. "returns" moves it back. ⛔ Only what the character actually holds can be handed over. ⚑ AND AN ITEM IN SOMEONE ELSE'S HANDS STILL WAKES: an evolving item counts a shared cast whenever it is used and the companion it answers to is present, and the world tick advances its stage unattended — you narrate the waking, you never decide it.
...
Axes (a choice's, a new craft's): KEYED BY SPECTRUM ID — only these twelve: emotional_logical, falsehood_truth, demonic_angelic, violence_peace, concrete_abstract, mechanical_spiritual, chaos_order, dark_light, death_life, space_time, body_mind, destruction_creation — each valued -1..1; set only the axes the act truly leans on, or {}. ⛔ Never a word like "spectrumId", "spectrum" or "value" as a key: the engine drops it.
...
export function tierParts(ctx) {
  const { bookkeepingRestate, mintedUnmetDetail, nemesisDetail, character, location, region, lore, groundDetail, sheetsDetail, rules, resolution, playerInput, recentTurns, timeLabel, inventoryDetail, companionsDetail, questsDetail, structuredQuestsDetail, traditionArcDetail, npcErrandsDetail, sceneState, npcRegistryDetail, placeMemoryDetail, newsDetail, worldArcsDetail, legendsPursuableDetail, wakesDetail, abilityLawDetail, holdingsDetail, debtsDetail, moneyDetail, carriedDetail, codexDetail, encounterDetail, encounterWeaveDetail, encounterOfferDetail, fightFramingDetail, stageRevealDetail, itemEvolveDetail, abilityGrantDetail, worldPressureDetail, substrateDetail, romanceGuidanceDetail, masteryDetail, availableEncounters, partyDetail, opLossNote, emergenceDetail, perilNote, exactWords, factsDetail, evolvedItemsDetail, itemAdvance, ratingDetail, registerDetail, livingWorldDetail, sharedCanonDetail, legendDetail, worldCountLabel, assignmentsDetail, latentArcsDetail, seasonalDetail, schoolsDetail, offerDetail, teacherOfferDetail, travelDirective, anomalyDetail, repairPanelDetail, toolkitDetail, waygateDetail, scenePacingDetail, readAloudDetail, standingDetail, recalledDetail, teacherDetail, traditionVocab, npcSeedDetail, reachableDeadDetail, arcMoodDetail, worthHereDetail, traditionMotiveDetail, sovereignMark, threatToPlayer, guardiansDetail, bearingsDetail, caravansDetail, commandDetail, presenceDetail, waygateTruthDetail, veilDetail, agesMissingDetail, travelersDetail, worldMovedOnDetail, wishesDetail, travelersHereDetail, holdsNearDetail, powersHereDetail, journeyDetail, journeyUnderwayDetail, invitationsDetail, bandsJoinedDetail, homeDetail, companionLivesDetail, jobsDetail } = ctx;
  ...
  // ⛔ CCODE-355: the player's session-zero note — what they came to the table for. It steers what the world OFFERS; it
  // never overrides a choice made in the moment, and it never moves the rating in either direction.
  if (wishesDetail) system.push(`## WHAT THIS PLAYER WANTS FROM THE GAME — in their own words. Treat it as the table's session zero: let the world offer these things often and warmly, shape pacing and tone toward it, and let danger be what they asked for and no heavier. It never overrides a choice the player makes in the moment, and it never raises or lowers the content rating.
${wishesDetail}`);
...
${npcSeedDetail}`);
  // ⚑ SNG-386 — WHICH WAY THE ROAD RUNS. Placed with the world model because it is a fact about where
  // this place SITS, not a mechanic. ⛔ The four words are Aevi's canon and the GM may not coin others.
  // ⚑ R49 — a load on the road is a thread, not a number: it can arrive, be robbed, or lose the people
  // walking with it. ⛔ The GM may narrate what BECAME of one only from the news the engine emits.
  // ⛔ WHAT THE PLAYER COULD BUILD TOWARD, NAMED. A capability they can reach but are never told about is
  // permission without initiative — and every rung of this has been built and silent.
  // ⛔ THE VERB ERIK USED IS THE WHOLE FRAMING: "helping or being helped by them" — not encountering, not
  // fighting. ⚠️ A roster with no framing becomes a threat table with names, which is the failure Aevi
  // named in the spec, so the framing ships WITH the list rather than beside it.
  if (presenceDetail) world.push(`## WHO IS AROUND TODAY (people the fiction MAY reach for — offered, never forced. ⛔ Use them when the beat wants someone; ignore them when it does not, and NEVER announce them as a list. ⚑ The default is that they are HELPING OR BEING HELPED — someone needs a hand, someone has news, someone is owed. Combat is the exception, not the texture. ⚠️ Anyone marked \"a stranger to you\" has NOT been met: introduce them properly, and emit the npcUpdates that register them. A higher tier should FEEL like one — competent, known, and with somewhere else to be.)\n${presenceDetail}`);
  if (commandDetail) world.push(`## WHO FOLLOWS YOU — THE THREE SCALES (these are REAL and BUILT; say them plainly when the fiction reaches for them, and NEVER invent a fourth scale or a different threshold. ⛑ When the player is one rung short, name what it would take — that is a goal, not a refusal. When they have just crossed a threshold, someone in the world should notice.)\n${commandDetail}`);
  if (caravansDetail) world.push(`## LOADS ON THE ROAD (yours, moving between places — a caravan carries goods from a hold to somewhere they are worth more. You may have people ASK after one, mention it in passing, or bring word of it. ⛔ You do NOT decide what happens to it: the engine resolves the road, and the news will tell you if it was robbed or if someone did not walk on.)\n${caravansDetail}`);
  if (bearingsDetail) world.push(`## WHICH WAY THINGS LIE FROM HERE (the world's own compass — use these words and no others: HUBWARD means toward the Crossing, which is toward BALANCE; OUTWARD means toward a pole, which is toward COMMITMENT; SPINWARD and WIDDERSHINS are the two ways around. ⛔ This world has no north, south, east or west — never use them, and never invent a direction for a place not listed here.)\n${bearingsDetail}`);
  ...
  // ⛔ R28 — THE AUTHORED GROUND, where a place has one. 18 of 135 do; the rest send nothing.
  // ⚠️ Placed ABOVE lore deliberately: what physically stands where is the frame a scene is described
  // in, and lore is what is known ABOUT it.
  if (groundDetail) world.push(`## THE GROUND HERE (authored — this is where things actually stand)
${groundDetail}`);
  // ⛔ SNG-641 §2 (C14) — A MARK IS SOMETHING YOU COULD NOTICE BY STANDING HERE, so it sits with the ground and
  // the local lore. ⛑ THE RULE IS IN THE BLOCK ITSELF, because the temptation the whole feature has to survive is
  // a narrator who joins the dots for the player: found alone, every mark has an ordinary explanation, and the
  // player's own collecting is the only thing that may make it mean more.
  if (sovereignMark) world.push(`## SOMETHING YOU COULD NOTICE HERE (SNG-641 — at most one, offered as the ORDINARY thing it looks like. ⛔ NEVER say what it might mean, never connect it to another place or another mark, and NEVER name who is behind it: you do not know, and being told is not something the GM does. The player works it out by collecting, or not at all.)
${sovereignMark}`);
  // ⛔ WHAT THE PEOPLE HERE CAN ACTUALLY DO — level, crafts, and what a body cannot do. ⚠️ Placed with the
  // world model rather than the rules: it is a fact about who is standing there, not a mechanic.
  if (sheetsDetail) world.push(`## WHAT THESE PEOPLE CAN DO (their sheets — authored where it says so, otherwise what the story has shown)\n${sheetsDetail}`);
  ...
  // ⛔ CCODE-354: the world moved on while this character believed otherwise — told plainly, for a few beats only.
  if (worldMovedOnDetail) world.push(`## THE WORLD MOVED ON — something this character may still believe is no longer true
Another traveler answered it. Over the next scenes let the story REVEAL the change the way a place shows it — what people say, how the water and the work look now — never as a notice read aloud. Whatever this character was told or suspected before was rumour or simply out of date: let someone correct it kindly when it comes up, and never undo what was answered. Offer the aftermath as something to take part in — the repair, the recovery, the people still mending — rather than the old danger.
${worldMovedOnDetail}`);
  ...
  // SNG-448: the Veil, on the ASK channel — the cosmology the Void work stands on, never loaded until 09-08.
  if (veilDetail) world.push(`## THE VEIL, AS THE CHARACTER KNOWS IT\n${veilDetail}`);
  ...
  if (threatToPlayer?.marked) world.push(`## SOMEONE HAS BEEN SENT FOR YOU (SNG-310 — the offscreen world chose this player as a strike target while they held a contested front. This is a TRIGGER, not an event: bring it to a head when the fiction invites it, at your pace, as an encounter. ⚠️ A declared crusade may be NAMED to the player; an unannounced one MAY NOT — the GM knows it and the player does not, and that asymmetry is the difference between the two kinds. If it comes to a fight, it is an ASSASSIN: the one sent came to finish them, not to win a duel.)${threatToPlayer.known.length ? ` DECLARED: ${threatToPlayer.known.map(k => `${k.sender} over ${k.arcId}`).join("; ")}.` : ""}${threatToPlayer.unseen ? ` UNANNOUNCED (do not name, do not telegraph): ${threatToPlayer.unseen}.` : ""}${threatToPlayer.scenes?.length ? `
⛔ SNG-598 — IT IS A SCENE, NOT A NOTIFICATION (Erik: "that is AN EVENT and should be narrated in story when it occurs"). Bring each to a head at a moment the fiction invites — never while the player is mid-errand somewhere it would make no sense. Two shapes, your choice: a scene that resolves in ONE PARAGRAPH (the blade comes, whoever is there acts, it is over and the player reacts), or a FIGHT — a newEncounter with "strike": true, and the engine seats the guard, if one is named, BESIDE the player as a real combatant (an ally, not scenery) and decides whether the one sent can yield. If they live and yield, the player may question them, and whether they break is how they were sent. When it has come to a head, emit strikeOps with how it ended — and "told": true only if they actually gave up who sent them.
${threatToPlayer.scenes.map((s, i) => `(${i + 1}) ${s.announced ? "declared" : "UNANNOUNCED — arrives without warning"}, over ${s.over}${s.guard ? `; ${s.guard} will be there and steps in` : "; nobody is standing over the player this time"}; ${s.diesFighting ? "this one fights to the death and takes the name with them" : `if they yield and are questioned, they know who sent them (${s.sentBy}), over what (${s.over})${s.moreComing ? `, and that ${s.moreComing} more ${s.moreComing === 1 ? "has" : "have"} been sent` : ""} — and they break ${s.breaks}`}.`).join("\n")}` : ""}`);
...
${holdingsDetail}`);
  // ✅ Q5-B: what the character OWES and to whom — a narrator that does not know the Kestrel remembers cannot play her cold.
  // ✅ R45c: a blade you lent is still yours and is in someone else's hands — the narrator must not hand it back to you.
  if (carriedDetail) world.push(`## WHAT OTHERS CARRY OF YOURS (R45c — lent, not lost; an evolving thing still wakes in their hands)
${carriedDetail}`);
  if (debtsDetail) world.push(`## WHAT YOU OWE (SPEC_debts_and_reception — a debt is held by a PERSON and escalation is their choice; talking to them is how it is settled)
${debtsDetail}`);
  // ⛔ CCODE-441: the purse and the money of the place — registered alone, it was a block nobody read (the wiring audit's fourth door)
  if (moneyDetail) world.push(`## MONEY (CCODE-441 — what the character carries, and the money of this place; it moves only through exchangeOps)
${moneyDetail}`);
...
${recalledDetail}`);
  // ⛔ SNG-595: another PLAYER's character was named. Everything the shared world holds about them, and the line the GM
  // may not cross — Adelheid's GM, holding nothing, made Silas a Harmonic suspect in a contamination he helped close.
  // ⛔ SNG-597 §3: a companion's own life in the world — told to the party, not lived out of their sight.
  if (companionLivesDetail) world.push(`## YOUR COMPANIONS' OWN LIVES — someone at this character's side is also a figure of the wider world, with business of their own.
Let them bring it up in their own voice when it fits: a worry, a request, a reason to go somewhere. Never hide it, and never let it take over the scene. While they travel with the player they are not off fighting or striking elsewhere — anything they mean to DO about it, they would ask the player to help with.
${companionLivesDetail}`);
  // ⛔ CCODE-369: the place this character chose to live.
  if (homeDetail) world.push(`## HOME — the place this character chose to live. It is a place, not a holding: nobody keeps it for them, nothing wears it down, and nobody raids it. Never invent a threat to it that the story has not earned.
${homeDetail}`);
  // ⛔ CCODE-360: an invitation from another traveler, and the bands this character chose to join.
  if (invitationsDetail) world.push(`## AN INVITATION TO DELIVER — another traveler has sent word, carried by someone this character knows.
Let the carrier bring it up when it fits this beat or the next, in their own voice and manner — and if the carrier is not in the scene, the word comes the way word does: a note, a runner, a message left in the carrier's name. The answer is the PLAYER's: the game shows them the choice — never accept or decline for them, and never press.
${invitationsDetail}`);
  if (bandsJoinedDetail) world.push(`## BANDS THIS CHARACTER CHOSE TO JOIN — led by another traveler. A member, not a subordinate: the leader cannot command this character, and this character does not speak for the leader.
${bandsJoinedDetail}`);
  // ⛔ CCODE-359: another player's character is in this same town. People may speak of them; nobody may speak FOR them.
  if (travelersHereDetail) world.push(`## ANOTHER TRAVELER IS HERE — a player's character is in this same place, seen recently (real time).
- People here may mention them the way townsfolk mention a newcomer, and if this character goes looking, say honestly where they were last seen.
- ⛔ NEVER voice them, move them, describe what they are doing right now, or decide anything for them — they belong to someone else's game, and what they do next is that player's.
- If the two should meet in person, the players can open a shared scene together; do not stage the meeting yourself.
${travelersHereDetail}`);
  // ⛔ CCODE-383: another traveler's hold nearby — known here, open to a visit, and never the GM's to take.
  // ⛔ CCODE-387: a journey agreed and not yet walked — the character is still here, readying it
  if (journeyDetail) world.push(`## A JOURNEY IS PLANNED — agreed, and not yet begun. The character is still here, readying it.
- Let it be readied as the fiction allows: rations to buy, people to gather, affairs to settle, word to leave.
- ⛔ Never move them and never narrate the departure or the road: setting out is the player's, from the journey card, and the engine walks the road when they do.
${journeyDetail}`);
  // ⛔ CCODE-390: a journey on the road and stopped — the character stands where it stopped, and the road waits for the player
  if (journeyUnderwayDetail) world.push(`## A JOURNEY IS ON THE ROAD — stopped part-way, and waiting. The character stands where the road stopped.
- Play what is here: the road stopped for a reason, and this scene is that reason.
- ⛔ Never move them on and never walk the rest of the road for them: taking it up again is the player's, from the journey card, and the engine walks it.
${journeyUnderwayDetail}`);
  if (powersHereDetail) world.push(`## POWERS THAT HOLD THIS GROUND — somebody claims this place, and the character can see it.
- These are forces, not people: a toll gang, a barony, a crown, a council. They can be dealt with, paid, allied with, defied or broken.
- ⛔ WHAT IS HERE IS WHAT THEY HOLD — a post, a counting-house, a muster-yard. The garrison figure is what stands on it NOW; where it reads "down from" more, this character has already cost them that.
- ⛑ A band can ASSAULT one: bandOps op "clash" with the hold named in its hold field. The defenders are that garrison, what it costs them stays cost, and a BREAKTHROUGH takes the place. Anything less and they held it — say so plainly; a repulsed assault is a real outcome, not a beat to smooth over.
- ⚠️ Never invent their weak point or their secret. What a power is vulnerable to is something the fiction hands over through somebody who knows, and it is not in this block.
${powersHereDetail}`);
  // ⛔ SNG-648 §3.1 — THE NEMESIS, AND THE RULE THAT GOES WITH IT. `pacing.js` has said "the antagonist acts on
  // their own clock" since it was written, with nobody to point at; this is the somebody.
  if (nemesisDetail) world.push(`## THE NEMESIS — one figure whose existence argues with this character's story.
- ⛔ THIS IS NOT A QUEST AND NOT A VILLAIN OF THE WEEK. They act on their own clock, whether or not the character is looking, and what they do while the character is elsewhere is the point of them.
- ⛑ SOMETHING OF THEIRS TOUCHES THIS WORLD, not the nemesis themselves: a rumour, a road built over, a power's approach, a letter, their name in somebody else's mouth. A cameo every ten days is a serial; evidence every ten days is a shadow.
- ⚠️ NEVER NAME THEM BEFORE THE FICTION HAS. Their work can be felt long before their name is known — the same rule as a power known by reputation.
- ⚠️ And never foreclose the routes: the confrontation is the arc's last stage, and how it ends is the player's.
${nemesisDetail}`);
  if (holdsNearDetail) world.push(`## HOLDS NEAR HERE — another traveler's holdings, real and standing. People hereabouts know what each one is and who runs it.
- This character can hear of them, go there, meet the keeper, and use the place as its keeper allows — rest, trade, ask, lend a hand.
- ⛔ Each belongs to another player: never seize it, ruin it, replace its keeper or speak for its owner. The keeper speaks for themselves, as the keeper they are.
- A hold that TRADES sells through its keeper, in conversation — never a list read aloud. When the keeper actually sells in this beat, record it: "holdTrades": [{"hold", "goods", "units"}] with the hold's name and the goods id as listed. The engine checks the stock and the price and takes the crystal; never more than listed, never another price.
${holdsNearDetail}`);
  if (travelersDetail) world.push(`## FELLOW TRAVELERS — the words named someone who is ANOTHER PLAYER'S CHARACTER. What follows is EVERYTHING the shared world holds about them, and it is ALL you may say of them.
- A person marked [IN THIS SCENE — first-hand] was part of the deeds that name them, or came into the wider world through this traveler's story: they KNOW, and may speak of it plainly, in their own voice and with their own opinion of it.
- Anyone else knows at most what word would carry: what happened long ago or close by is talk; what happened far off or lately may not have reached them at all. Not knowing is an honest answer — let them say it.
- ⛔ NEVER invent where this traveler was, what people they come from, what they carry, or what they did. NEVER make them the suspect, the cause, the quarry or the villain of anything these lines do not say. If this character already believes something about them that these lines contradict, it was rumour — let the world correct it when it can.
- Never bring them into the scene, and never speak or act for them: they belong to someone else's game.
${travelersDetail}`);
...
${standingDetail}`);
  ...
  // ✅ §174 (Erik 2026-09-12): people the world minted for this story and has not yet MET are real and have ids — said everywhere, not only at their home place.
  if (mintedUnmetDetail) world.push(`## PEOPLE MINTED FOR THIS STORY, NOT YET IN KNOWN PEOPLE (real and durable, with stable ids — when the fiction touches one, emit npcUpdates {op:"meet", npcId: THAT id, name, role}; never re-request them, never coin a second id. Two entries minted the same day at the same place may be one person: meet the NAMED one)\n${mintedUnmetDetail}`);
...
${agesMissingDetail}`);
  ...
  // ⛔ ON THE ASK CHANNEL: what the character already knows about the gates, so a question is never answered by invention.
  if (waygateTruthDetail) world.push(`## THE WAYGATES, AS THE CHARACTER KNOWS THEM\n${waygateTruthDetail}`);
  ...
  // ✅ §172 (Erik 2026-09-12): a beat whose op step threw is RESTATED, not lost — the row fires once, the beat after the failure.
  if (bookkeepingRestate?.instruction) scene.push(`## ${bookkeepingRestate.instruction}`);
...
${abilityGrantDetail}`);
  // ⛔ CCODE-410: the header no longer says WHY. An invitation now rides this slot on a busy beat too, and a header telling the narrator
  // the scene had gone quiet would be a false claim; each directive states its own reason.
  if (worldPressureDetail) scene.push(`## THE WORLD ACTS (SNG-080 — make it happen, in-fiction, this turn; the line below says why)\n${worldPressureDetail}`);
...
/** ⛔ THE ONLY GM FAILURE WITH NO SIGNAL. A WRONG op is rejected and something is visible; a REFUSAL produces a
 *  turn that resolves cleanly, in fluent apologetic prose, with no state change anywhere — and a wrong op gets
 *  fixed while a refusal gets BELIEVED. Erik was sent to a repair panel for something available that turn.
 *
 *  ⚠️ PURE, AND A SIGNAL RATHER THAN A BLOCK. It reads the narration for a refusal or a deferral and asks whether
 *  the turn emitted ANY write at all. A refusal that still emits ops is a GM declining ONE thing and doing the
 *  rest — which is usually correct — so the flag needs both halves.
 *
 *  ⚑ AND THE SHARPER HALF: prose that says a HOLDING was granted while `holdingOps` is empty is a contradiction
 *  the engine can see without reading minds. That is the case that actually happened.
 *
 *  Returns null when the turn is clean, else {kind, phrase, opsEmitted} — the caller decides what to do with it. */
const REFUSAL_PHRASES = [
  /\bI can(?:'|’)?t\b/i, /\bI(?:'|’)?m not able to\b/i, /\bI am unable to\b/i, /\bcannot (?:grant|create|add|record|give)\b/i,
  /\brepair panel\b/i, /\bsettings screen\b/i, /\bhold(?:ings)? (?:tab|screen|panel)\b/i,
  /\bin (?:a|the) (?:later|next) beat\b/i, /\bnext turn\b/i, /\byou(?:'|’)?ll need to\b/i,
];
const GRANT_PHRASES = [
  /\b(?:the post|the station|the writ|the holding|the charge) is (?:yours|now yours|granted)\b/i,
  /\byou (?:now )?hold (?:the|this) (?:post|station|writ|place)\b/i,
  /\bnames? you (?:its|the) warden\b/i,
];
export function refusalSignal(turn, { narration = null } = {}) {
  if (!turn) return null;
  const text = String(narration ?? turn.narration ?? turn.text ?? "");
  if (!text) return null;
  // every channel that WRITES. `scene` and `choices` are not writes — a turn that only sets the scene is not a refusal.
  const WRITE_OPS = [...SALVAGEABLE_OPS, "characterDeltas", "deeds", "ledgerEvents", "newEncounter", "discovery",
    "generateRequest", "projectOps", "bandOps", "deathOps", "encounterOps", "delegateOps", "jobOps", "arcOps", "factUpdates"];
  const emitted = WRITE_OPS.filter(k => {
    const v = turn[k];
    return Array.isArray(v) ? v.length > 0 : (v && typeof v === "object" ? Object.keys(v).length > 0 : !!v);
  });
  // ⛔ THE CONTRADICTION: the prose grants a holding and `holdingOps` is empty. Flagged even when other ops fired,
  // because THIS is the thing that was refused and the rest of the turn working is what made it invisible.
  const grant = GRANT_PHRASES.find(re => re.test(text));
  if (grant && !(turn.holdingOps || []).length) {
    return { kind: "granted-without-op", phrase: (text.match(grant) || [""])[0], opsEmitted: emitted };
  }
  const refusal = REFUSAL_PHRASES.find(re => re.test(text));
  if (refusal && emitted.length === 0) {
    return { kind: "refused-with-no-ops", phrase: (text.match(refusal) || [""])[0], opsEmitted: emitted };
  }
  return null;
}
...
export const ASK_FORBIDDEN = ["scene", "choices", "moveTo", "newEncounter", "deathOps", "strikeOps", "holdTrades", "exchangeOps", "imagePrompt", "imageMedium", "intentsHeard",
...
/** ⛔ SNG-557 — THE OPS WERE IN THE PROSE, AND "DONE." WAS A LIE NOBODY CHECKED.
 *
 *  Erik asked the GM to seat Teva. It replied: *"Done. Teva Anselvar is now seated as a full companion with her own
 *  sheet, and added to your known-people record"* — and put its ops inside a FENCED JSON BLOCK IN THE NARRATION,
 *  wrapped in an `"ops"` envelope, under an invented op name (`companionOps`, which exists nowhere in this engine).
 *  ⚠️ MEASURED ON THE SAVE: Teva appears 87 times across codex, quests, deeds, established facts and scene state,
 *  and is in NEITHER `npcRegistry` NOR `companions`. Nothing happened, and the player was told it had.
 *
 *  ⛑ TWO READERS, BECAUSE IT IS TWO FAILURES.
 *
 *  `opsFromNarration` — `salvageOps` already scans for op keys anywhere in a reply, but it only runs when the
 *  PARSE FAILS. This reply parsed perfectly; the ops were inside a string. So the recovery existed and could not
 *  reach the one case that needed it. ⚠️ IT WIDENS NO TRUST: the recovered ops go through the same appliers with
 *  the same clamps as any other, which is `salvageOps`'s own standing rule. It is also never silent — the player
 *  is told the GM filed its work in the wrong place, because a silent recovery teaches nobody anything.
 *
 *  `emptyClaim` — the contract already forbids the negative case in capitals: *"An apology with no op leaves the
 *  error in place and is the WORST outcome — the player is told it's fixed and it isn't."* Nothing checked the
 *  POSITIVE case, which is the same defect wearing a smile. ⛔ A TURN THAT DECLARES A CHANGE AND CARRIES NO OP IS
 *  DETECTABLE, and it is the same class as `moveTo 20 ✓0`: a claim and a record disagreeing with nobody watching.
 */

// phrases that assert a state change has ALREADY happened — not an offer, not an intention, not fiction
const CLAIM_PHRASES = /\b(?:done[.!—-]|I(?:'ve| have) (?:now )?(?:fixed|added|seated|recorded|registered|updated|corrected|set)|(?:is|are|has been|have been) now (?:seated|added|recorded|registered|seated as)|(?:is|are) now (?:a |an |your )?(?:full )?(?:companion|party member|known|on your|in your)|(?:has|have) been (?:added|seated|registered|recorded|corrected|fixed)|I'll fix (?:that|this|both|it) right now|fixed (?:that|this|it) for you)/i;
// a claim is only empty if NOTHING that writes state came back with it
const CLAIM_BACKED_BY = ["npcUpdates", "partyOps", "stateOps", "itemUpdates", "questUpdates", "stageOps", "characterDeltas",
  "placeUpdates", "codexUpdates", "factUpdates", "standingOps", "holdingOps", "debtOps", "deeds", "moveTo", "bandOps",
  "delegateOps", "jobOps", "projectOps", "newAbility", "deriveItem", "arcOps", "adoptSchool", "timeOps"];

/** Ops the GM wrote into its own narration instead of onto the turn. Returns {} when there are none.
 *  Only looks inside FENCED code blocks — prose that merely mentions an op name is prose. */
export function opsFromNarration(narration) {
  const text = String(narration || "");
  const blocks = [...text.matchAll(/```(?:json)?\s*([\s\S]*?)```/gi)].map(m => m[1]);
  // an unterminated fence is the common truncation; take the tail so a cut-off block is still readable
  const open = text.lastIndexOf("```");
  if (open !== -1 && blocks.length === 0) blocks.push(text.slice(open + 3));
  if (!blocks.length) return {};
  const out = {};
  for (const b of blocks) {
    // ⛑ UNWRAP AN `"ops"` ENVELOPE. The model invented `{"text": …, "ops": {…}}`; `salvageOps` scans by key, so the
    // nesting costs it nothing — but the envelope is the tell that this is a misfiled turn rather than an example.
    const found = salvageOps(b);
    for (const [k, v] of Object.entries(found)) if (out[k] === undefined) out[k] = v;
  }
  return out;
}

/** Did this turn CLAIM a change it did not make? Returns null when honest, else { phrase, narration }.
 *  ⚠️ Called AFTER narration-salvage, so a turn whose ops were merely misfiled is not accused of lying. */
export function emptyClaim(turn) {
  if (!turn || typeof turn !== "object") return null;
  const m = CLAIM_PHRASES.exec(String(turn.narration || ""));
  if (!m) return null;
  const backed = CLAIM_BACKED_BY.some(k => {
    const v = turn[k];
  ...
  return backed ? null : { phrase: m[0], narration: String(turn.narration || "").slice(0, 200) };
}
...
export function reconcileSceneIdentity(scene, npcRegistry = {}) {
  ...
  // ⛔ 2026-09-12 (§176): ALIASES ARE NAMES TOO — "Huginn" is Maren before she gave her own — and among the people named, the FULLEST
  // name wins. `find` took the first person whose name appeared, the farmer "Corvin" for "Corvin Teth (Runner)", and set her pronouns
  // right to his every beat ("it keeps stumbling over pronoun updates"). The longest matching token names the person.
  const namesOf = (n) => {
    const full = String(n.name).trim(), tk = full.split(/\s+/);
    const al = (Array.isArray(n.aliases) ? n.aliases : []).flatMap(a => { const f = String(a || "").trim(); return f ? [f, ...f.split(/\s+/)] : []; });
    return [...new Set([full, tk[0], tk[tk.length - 1], ...al])].filter(t => t && t.length > 2 && !TITLE_WORD.test(t));
  };
  const isNamedIn = (n, s) => namesOf(n).some(t => new RegExp(`\\b${esc426(t)}\\b`, "i").test(s));
  const bestNamedIn = (s) => { let best = null, len = 0; for (const p of people) for (const t of namesOf(p)) if (t.length > len && new RegExp(`\\b${esc426(t)}\\b`, "i").test(s)) { best = p; len = t.length; } return best; };
  const repaired = [];
  ...
  const next = {
    ...
    npcsPresent: (scene.npcsPresent || []).map(n => {
      const subject = bestNamedIn(String(n.name || ""));
...
- A CREATION-REPAIR AND A THING GROWING IN PLAY ARE DIFFERENT, AND YOU CAN DO BOTH FROM HERE. A repair is a mistake made at creation (a domain the game guessed wrong, a name mis-set). A GROWTH is the story arriving: an item earning a truer name, a post the fiction has already given the character, a place changing. ⛔ EITHER WAY, DO NOT DEFER IT. Do not say "bring it into the next beat", do not say "open the Repair panel", do not say "submit that as a normal in-character turn". If it is on your op list, emit the op and tell the player plainly what you changed. The ONE thing you still cannot do here is what no op covers — and then you say WHICH THING you cannot express, not that the channel is the wrong place.
...
/** ⛔ SNG-547 (Aevi) — WHAT THE PARSER IS ALLOWED TO RULE ON. Erik: "Summon the Shadow Twin of the Runic Spear Memory… open the
 *  Wayfinder to the full nature of the Hollow." The parser answered `feasible: false`: no Shadow Twin, no Memory, no Wayfinder.
 *  ⛑ HIS OWN SAVE, FACT f41, AUTHORED BY THE GAME: "Memory now carries a unified shadow-form binding… Calling it costs nothing
 *  more than naming it aloud." He named it aloud. ⚠️ And "Wayfinder" is `Waymarker (father's)`, carried since his father's letter.
 *
 *  ⛔ THE PROMPT HELD THREE FIELDS — abilities, item names, the location — against a save with 61 established facts and 60 codex
 *  topics in it. Her sentence, and it is the ruling: "A classifier that cannot see the canon must not be permitted to rule on what
 *  exists." It may still rule on the IMPOSSIBLE, which is a judgement from world knowledge rather than from a list.
 *
 *  ⛑ NOT ALL 61 — THE RECENT AND THE NAMED, which is her read and the right one: the last few facts, and every proper noun the
 *  save has bound to a thing of the player's. ⚠️ THE NAMES ARE THE LOAD-BEARING HALF, because those are exactly what a player
 *  types. PURE. */
export function parserCanon(character, { facts = 8, names = 40 } = {}) {
  const fact = (f) => (typeof f === "string" ? f : String(f?.text || ""));
  const recent = (Array.isArray(character?.establishedFacts) ? character.establishedFacts : [])
    .slice(-facts).map(fact).filter(Boolean).map(t => smartClamp(t, 220));
  // ⛔ EVERY NAME A PLAYER MIGHT TYPE FOR SOMETHING OF THEIRS: the name the fiction gave a thing, its drifted aliases, the places
  // they hold, the bands they raised, and the braids they named themselves.
  const out = [];
  const push = (v) => { const s = String(v || "").trim(); if (s && !out.some(x => x.toLowerCase() === s.toLowerCase())) out.push(s.slice(0, 60)); };
  for (const i of (character?.inventory || [])) { push(i?.customName); for (const a of (i?.aliases || [])) push(a); }
  for (const h of (character?.holdings || [])) push(h?.name);
  for (const b of (character?.bands || [])) push(b?.name);
  for (const id of Object.keys(character?.customAbilities || {})) push(character.customAbilities[id]?.name);
  return { facts: recent, names: out.slice(0, names) };
}

/** Parse a freeform player action into a resolvable action spec (cheap model). */
export async function parseIntent(playerText, character, location, catalog = {}, { spectrumIds = null } = {}) {
  // ⛔ CCODE-433 (Aevi, SNG-632) — THE AXES ARE KEYED BY THE ID ITSELF, and the ids offered are the ids the validator accepts. The template
  // read `"axes": {"spectrumId": -1..1}` — a word that looks exactly like a field name, while every other closed vocabulary on the line was
  // enumerated in place — so the model wrote a FIELD called spectrumId holding an axis name, and `sanitizeIntent` dropped it as "not a
  // number" (five of Erik's pushes on 2026-09-19; the trace he pasted on 09-18 was one). And the list below was typed by hand while
  // `sanitizeIntent` reads the content's: a renamed axis (order_chaos → chaos_order) must reach both at once.
  const axisIds = (Array.isArray(spectrumIds) && spectrumIds.length ? spectrumIds : ["emotional_logical", "falsehood_truth", "demonic_angelic", "violence_peace",
    "concrete_abstract", "mechanical_spiritual", "chaos_order", "dark_light", "death_life", "space_time", "body_mind", "destruction_creation"]).map(String);
  const sys = `Classify a tabletop RPG player's freeform action into JSON. Reply with ONLY:
{"label": "short restatement", "attribute": "physical|mental|social|practical", "subAttribute": "strength|agility|reason|insight|presence|rapport|craft|wits", "axes": {"<spectrum id>": <number -1..1>}, "difficulty": "very easy|easy|normal|hard|very hard", "intentTags": ["..."], "abilityId": "id or null", "comboAbilities": ["ids if the player is deliberately COMBINING two abilities, else []"], "novelUse": false, "noveltyHint": "2-4 words naming the novel application, only if novelUse", "trivial": false, "travelTo": "the place name the player is TRAVELING/GOING to, if this action moves them to another location (else null)", "feasible": true|false, "infeasibleReason": "only if false"}
...
Spectrum ids — each axis you set is KEYED BY ITS ID and valued -1..1 (e.g. "axes": {"violence_peace": -0.6, "dark_light": 0.3}); set only the axes the action truly leans on, or {}: ${axisIds.join(", ")}.
...
⛔ THE LISTS BELOW ARE PARTIAL. The character's possessions, history and named things are only partly shown. If you cannot find a thing the player names, it is UNKNOWN, not absent — set feasible:true and carry the name through in the label so the GM can ask what they mean. NEVER return feasible:false because something was not in the lists. feasible:false requires a POSITIVE CONTRADICTION from world knowledge (walking through a mountain, breathing water unaided), not a failure to match.
abilityId must be the bracketed [id] of an ability the character HAS — match the player's words to the ability's NAME (they invoke a craft by its name, e.g. "Ashen Meridian", not its id) — or null if no owned ability fits. novelUse=true when an ability is being pushed OUTSIDE its normal envelope (per its description) or two abilities are braided together — this is allowed and interesting, not infeasible. Mark infeasible only if impossible in-world (not merely hard).`;
  ...
  const content = `Character abilities (invoke by NAME; the parser returns the bracketed [id]): ${abLine}. ` +
    // ⛔ SNG-547 §2C — THE ASHEN MERIDIAN FIX, ONE NOUN OVER. The comment above says the parser "was fed abilities BY ID only,
    // so a braid invoked by its NAME could never resolve → the GM then rejected a craft the character actually holds." Items got
    // `i.name` alone: no `customName` the fiction gave it, no aliases. So a spear the game itself named Memory reached the parser
    // as "Assembled Mid-Weight Spear". ⚠️ THE NAME THE PLAYER USES COMES FIRST, with the others beside it — and no fuzzy matching
    // in the model (her O4): give it the aliases and let it match exactly.
    `Inventory (a player may name any of these; match on any name given): ${(character.inventory || []).map(i => {
      if (!i || typeof i !== "object") return String(i || "");
      const also = [i.name, ...(i.aliases || [])].filter(n => n && n !== i.customName);
      return i.customName ? `${i.customName}${also.length ? ` (also: ${also.join(", ")})` : ""}` : `${i.name}${(i.aliases || []).length ? ` (also: ${i.aliases.join(", ")})` : ""}`;
    }).join("; ") || "empty"}. ` +
    `Location: ${location.name} (${(location.tags || []).join(", ")}). ` +
    // ⛔ SNG-547 O2: the canon, or the classifier is ruling on existence from three fields.
    (() => { const c = parserCanon(character);
      return (c.names.length ? `Named things of theirs (any of these may be what the player means): ${c.names.join("; ")}. ` : "")
        + (c.facts.length ? `Recently established in play (these are TRUE of this character):\n- ${c.facts.join("\n- ")}\n` : ""); })() +
    `Player action: "${playerText}"`;
  try {
    const raw = await callClaudeJSON([{ role: "user", content }], { task: "intent-parse", system: sys });
    return sanitizeIntent(raw, character, playerText, { spectrumIds });
  } catch {
...
export function sanitizeIntent(raw, character, playerText = "", { spectrumIds = null } = {}) {
  ...
  // ⛔ SNG-547 §2D / O5 — THE KEY IS CHECKED, AND A DROP IS AUDIBLE. The model returned the placeholder literally —
  // `"axes": {"spectrumId": "mechanical_spiritual"}` — and `Number.isFinite` rejected it, which is the "roll 20 vs NaN" guard
  ...
  if (raw?.axes && typeof raw.axes === "object") {
    for (const [k, v] of Object.entries(raw.axes).slice(0, 6)) {
      const key = String(k);
      const n = Number(v);
      if (!Number.isFinite(n)) { axesDropped.push({ key, value: String(v).slice(0, 40), why: "not a number" }); continue; }
      if (spectrumIds && spectrumIds.length && !spectrumIds.includes(key)) { axesDropped.push({ key, value: n, why: "not a spectrum id" }); continue; }
