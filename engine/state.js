...
export async function loadContent() {
  ...
  const [rules, emergence, attributeGates, skillCapacity, locationAffinities, intensity, branchForks,
    ...
    // ⛔ SNG-634 C4 — AND THIS ONE IS LAST FOR THE REASON THE TWO NOTES ABOVE GIVE. `powersRule` is the last
    // name in the destructuring, so this is the last call in the array. ⚠️ APPENDING IS THE ONLY SAFE
    // INSERTION into a positional list: anywhere else and every rule after it shifts by one and receives the
  ...
  // ⛔ SNG-634 C4 — the powers' own dials: `growth` (what a verb earns them), `tempers`, `density`, `eligibility`.
  // ⚠️ DISTINCT FROM `rules.powerBands`, which is the seven-rung standing ladder, and from `powerSources`,
  ...
  // ⛔ SNG-634 C9 — WHO HOLDS THE GROUND. `jSettled` rather than `jAll` deliberately: a pack with no
  // `powers.json` is the normal state of every pack but the valley, and a missing optional collection must
  // not take the whole load down with it. ⚠️ The readers (`engine/powers.js`, C1 and C2) shipped in the same
  ...
  // ⛑ FLATTENED TO ONE LIST, because every reader asks "who reaches this place" and none of them cares
  // which file a power was authored in. ⚠️ A file whose `powers` key is missing contributes nothing rather
  // than an `undefined` in the array — the shape a reader would then have to guard for on every pass.
  const powers = [];
  for (const r of await powersP) {
  ...
  const [region, substrate, greaterArcs, sovereignMarksDoc, genNpc, genLoc, genArc, genCreature, originsDoc, backgroundsDoc, regionsDoc,
    ...
    // ⛔ SNG-641 §2 (C14) — the marks a supply line leaves. ⛑ THE WHOLE DOC, not just `marks`: the thresholds and
    // the GM's lines are the reader's dials and its words, and flattening to the array would have left both in code.
    fetchJSON("content/packs/valley/lore/sovereign_marks.json").catch(() => null),   // no marks → nothing to find, exactly as before
    ...
    // ⛔ SPEC_generation_reaches_every_tier §2.1 — the target pyramid, so RARITY does the work the retired
    // `ceiling` dial was doing. Rides in `npcStanding` for the same reason `tier_signals` does: every
    // caller already threads that block as `cfg`, and a dial nobody passes is a dial nobody reads.
  ...
  // ⛔ AND THE DOC ITSELF, because this line THROWS THE SIBLING KEYS AWAY. `regions.json` carries dials
  // beside its array — `nearestTraditionWithinDeg` is the first — and flattening to `.regions` meant a dial
  // could be authored, registered, loaded and then silently dropped one character before it was read.
  // ⚠️ The fourth door, in one line of destructuring.
