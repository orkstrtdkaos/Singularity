...
export async function loadContent() {
  ...
  const [rules, emergence, attributeGates, skillCapacity, locationAffinities, intensity, branchForks,
         romanceGuidance, functionVocabulary, nativeGrants, skillBattle, traditionsRaw, worldClock, schools, classArchetypes, repairPanelManifest, craftMechanics, titlesRule, arcResponseRule, encountersRule, coliseumGrid, economyRule, chargesRule, threatRule, incapRule] = await Promise.all([
    fetchJSON(resPath),
    ...
    loadRule("incapacitation", null)   // SNG-323: aggressor weights, tunable instead of a const   // SNG-322/CCODE-52: the threat ladder — bands decide the lethal flag and the warn offer   // SNG-316: what satisfies each prep condition, so the tick can bank a charge without reading prose
  ]);
  ...
  if (incapRule) rules.incapacitation = incapRule;   // SNG-323   // SNG-322: the threat ladder   // SNG-316: the charge-condition vocabulary
