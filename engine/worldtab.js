...
export function worldTabHtml({ arcs = [], foot = {}, effects = [], name = "", tabBar = () => "", esc = (s) => String(s ?? "") } = {}) {
  ...
  const arcCards = arcs.map(a => `<div class="wt-arc">
    ...
    ${a.strikes.map(s => `<div class="wt-cost">† ${s.outcome === "guarded" ? `a strike at ${person(s.target)} was turned aside${s.guard ? ` by ${person(s.guard)}` : ""}` : `${person(s.target)} was struck at by ${person(s.sender)}’s people${s.unseen ? ` <span class="hint">(escaped undetected)</span>` : ""}`}</div>`).join("")}
