...
## SNG-103 — GM effective energy cost (bug, small, live-play-found)

`po/SPEC_SNG-103_gm_effective_energy_cost.md`. `abilitiesForGM` (progression.js) prints each ability's **base** `energyCost` into the GM's ABILITY LAW block instead of the **effective** cost after level+rank discounts. The GM is handed 6 for an ability the panel correctly charges 3, sees a mismatch, and repeatedly flags correct data as a Repair-tool error — fires on every leveled character using any discounted ability. **The formula and panel are correct; only the GM's input is wrong.** Danger: the false flag invites a player to "repair" the base down to the floor, which IS real data corruption. Fix: thread `CONTENT.rules` into `abilitiesForGM` (3 call sites in app.js, all already pass `CONTENT.branchForks`) and interpolate `effectiveEnergyCost(ab, character, rules)` instead of `ab.energyCost`; recommended polish shows both numbers so the GM never re-flags. No formula edit, no data edit. Found in live play (Silas Weir, Palework rank 2, Millbrook). Awaiting CCode ROUND 2.

---

## SNG-106 — Roll-breakdown popup (transparency, resolver + UI)

`po/SPEC_SNG-106_roll_breakdown_popup.md`. The roll receipt shows `d100: 43 vs 62 — success` but never WHY 62. Tappable/hoverable breakdown on the chance: attribute (competence+mastery split), skill, ability rank, spectral fit (self+place), aptitude mods, equipment, novel/discovery, substrate, exhaustion, and difficulty — with the OPPOSED contribution NAMED ('the raider (threat 35) −11') rather than anonymous, since encounters already fold opponent threat into difficulty (threat×0.3). **Spine is a data-retention change:** `successChance` computes every component and discards them at return; it must return `{total, components}` so the popup renders the resolver's real math and never recomputes (behavior-preserving refactor, guarded by a sum-equals-old-chance test). Reuses the info-dot popover for phone parity. Companion to SNG-104 (vitals) and SNG-084 (the existing contextual roll ⓘ). Awaiting CCode ROUND 2.

---

## SNG-098 — Skill Battles (two-sided contest, fog-gated) — SPEC AUTHORED

`po/SPEC_SNG-098_skill_battles.md`. **Elevated from "wire skill_battle_system.json" to its true shape by Erik's insight:** a duel/contest is a turn-based exchange where BOTH sides are rolling, DECIDING agents — and how much of the opponent's move/roll/breakdown you see is GATED BY YOUR INSIGHT (`senseTier`), fog of war as the mechanic. Tier 0 blind (outcome only) → tier 3 sees their full roll breakdown (SNG-106 popup on THEIR roll). Reveal skills buy a tier (tempo vs information). Composes three existing foundations: the 2026-07-07 skill_battle seed (Erik already wrote 'both sides declare'), sense.js senseTier (the fog primitive, now pointed at the adversary), successChance+SNG-106 (each side's roll). At HEAD the opponent does NOT roll — it's a stat block whose threat is difficulty — so this is a combat-MODEL change. PvP falls out (symmetric agents, each read through their own insight). Folds in the withdrawn §7b salvage (three-type taxonomy, conditions, Coliseum). Depends on SNG-106. Awaiting CCode ROUND 2.
