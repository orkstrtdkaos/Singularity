...
### Build inputs (all present at origin — VERIFIED 2026-07-10, build-ready; Aevi owes no new content)
- **Grammar source (authoritative):** `content/packs/valley/lore/generative_substrate.json` — type→template→grammar map for all 13 types + generationGrammar (inputs / process / in-grain guarantee / asymmetry / function-awareness) + persistence model. `generate()` assembles the prompt-grammar from THIS; do not re-derive it.
- **Type templates / few-shot taste:** NPCs = `content/packs/valley/npcs/*.json` (31); locations = `content/packs/*/locations/*.json` + `location_affinities.json` (27) + `universal_roles.json`; **arcs = `content/packs/valley/lore/greater_arcs.json` — STRUCTURAL (tension between two dispositions/factions + hinge NPCs + stages), NOT prose few-shot.**
- **BUILD STEP 1 — schemas (do first):** formal JSON schemas exist only for character / ledger_event / player_profile. CCode DERIVES npc/location/arc validation schemas from the authored corpus + greater_arcs + generative_substrate, writes them to `schemas/`, THEN `generate()` validates against them (reject → repair → minimal stub). **Never mint unvalidated into the permanent codex.**
- **Context inputs (engine already emits):** poleIntensity (SNG-022), location tags/seedFiction, active universal role, arc-pressure, season (worldtime). No new GM plumbing.
