...
import { derivedLevel, tierFromRole } from "../engine/npcsheet.js";
...
const rows = records.map(r => {
  ...
  let reach = false, kit = 0, why = "", fallbackOnly = false;
  if (r.kind === "companion") { why = "companion — fights beside you from stages, not as a foe"; }
    ...
    try {
      ...
      // ⛔ REAL CRAFTS ONLY. An underscore id is engine furniture — R47's bare strike is given to nearly everyone,
      // and a kit of one fallback swing is not a kit. Counting it made this column say yes for 59 records that
      // cannot do a single authored thing.
      const real = o ? [...new Set((o.skills || []).map(k => String(k.id)))].filter(id => !id.startsWith("_")) : [];
      reach = !!o; kit = real.length; fallbackOnly = !!o && real.length === 0;
      if (!o) why = notOpp ? "declared notAnOpponent" : "no kit — falls to threat synthesis";
      else if (fallbackOnly) why = "only the bare strike — no real craft";
    } catch (e) { why = "ERR " + String(e.message).slice(0, 40); }
  }
  const derived = loaded ? derivedLevel(loaded, { day: 100, cfg }) : null;
  // ⚑ derived-vs-authored, so a rung the engine guessed can be argued with rather than assumed (Aevi's ask)
  const tierGuess = (!rec.tier && rec.level == null && loaded) ? tierFromRole(loaded, { cfg }) : null;
  const hooks = (rec.questSeeds || rec.hooks || []).length;
  const portrait = !!(rec.image || rec.portrait || rec.imageSeedKey || loaded?.image);
  return { ...r, tier, level, derived, abilities, domains, primArray, notOpp, loaded: !!loaded, reach, kit, why, hooks, portrait, fallbackOnly, tierGuess,
...
const fallbackKit = rows.filter(r => r.fallbackOnly);                                 // ⛔ reachable, but the only move is R47's bare strike
const guessed = rows.filter(r => r.tierGuess);                                        // ⚑ rung derived from the role string, not authored
...
out.push(`**GENERATED by \`scripts/roster.mjs\` · ${rows.length} records across 6 files · ${reachable.length} reachable as opponents · ${levelOne.length} would fight at level 1 · ${fallbackKit.length} carry only the bare strike · ${guessed.length} tiers derived from a role · ${dropped.length} authored but never loaded.** ⚠️ Regenerate with \`node scripts/roster.mjs --write\`; never hand-edit inside the markers.`);
out.push("");
...
out.push(`| ⛔ **reachable but the ONLY move is the bare strike** (no real craft) | **${fallbackKit.length}** | ${fallbackKit.slice(0, 6).map(r => `\`${r.id}\``).join(" · ")}${fallbackKit.length > 6 ? ` · +${fallbackKit.length - 6}` : ""} |`);
out.push(`| ⚑ tier DERIVED from the role string, not authored | ${guessed.length} | ${[...new Set(guessed.map(r => r.tierGuess.tier))].sort().join(" · ")} — shown as \`~name\` in the tier column |`);
...
for (const r of rows) {
  ...
  const tierCell = r.tier ? String(r.tier) : (r.tierGuess ? `~${r.tierGuess.tier}` : no);   // `~` = derived, not authored
  out.push(`| \`${r.id}\` | ${r.name} | ${r.kind} | \`${r.file}\` | ${tierCell} | ${lvl} | ${r.abilities || no} | ${r.domains ? (r.primArray ? "⛔ array" : yes) : no} | ${reach} | ${r.kit || no} | ${r.hooks || no} | ${r.portrait ? yes : no} |`);
}
...
const summary = `${rows.length} records · ${reachable.length} reachable · ${levelOne.length} at level 1 · ${fallbackKit.length} bare-strike only · ${guessed.length} tier derived · ${dropped.length} never loaded · ${hingeMissing.length} hinge ids unresolved`;
if (CHECK) {
