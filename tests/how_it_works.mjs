...
console.log("\n── §70 · per-rank source · the revised kill cost · ongoing damage · a spec reaches built ──");
{
  ...
  const BASELINE_DERIVED = 22;   // 2026-09-10 (21→22): SPEC_damage_make_it_vary.md names `mechanicFor`/`skillBattleRound`. ⛔ AND THIS ONE IS THE PROXY BEING RIGHT: Aevi wrote a spec proposing a die ladder that ALREADY EXISTS, after measuring the flat FALLBACK branch four times without checking which branch executes. ⚠ `mechanicFor` is exactly the export that would have told her. The superseding spec is CCode's; hers is marked WITHDRAWN and her duplicate retraction was deleted rather than kept.
  // 2026-09-10 (20→21): SPEC_what_a_support_character_does.md — Aevi, spec_ready, naming `contributionsOf`/`kitFor`. ⚑ AND THE RATCHET EARNED ITS KEEP THIS TIME: its own output listed SPEC_party_contributions.md citing `contributionsOf`, Aevi opened it, and found the SAME FINDING already measured and already RULED by Erik. She linked the two instead of filing a duplicate. ⚠ The proxy still fires on good specs — but a list of who-else-cites-this-export is exactly how you notice you are about to repeat someone.
  // 2026-09-08 (19→20): SPEC_BUILD_six_fields.md — Aevi, spec_ready, naming `resolveSubstrateField`/`applySubstrateField`/`naniteAt` in a §0 table whose WHOLE PURPOSE is 'what is already built, so nobody rebuilds it'. ⚠ Which is this proxy at its most inverted: the section written to PREVENT duplicate work is the section that trips the gate for suspected duplicate work. ⛔ Fifth move on good specs; `declaredStale` still 0.
  // 2026-09-08 (18→19): SPEC_arcs_move_the_substrate.md — Aevi, spec_ready, naming `resolveSubstrateField` as the export its whole proposal extends. ⚠ CCode flagged this half as a PROXY THAT MEASURES THE WRONG THING (2026-09-08): it counts specs naming engine exports as a stand-in for "probably already built", and a GOOD round-2 reply or a well-measured spec names exports as CONTEXT — so answering well pushes it up every time. Aevi agrees and it has now moved on replies four times. ⛔ The hard half (`declaredStale`) is still 0 and that is the one that means what it says.
  // 2026-09-09 (17→18): SPEC_mobile_holdings.md — raised by MY OWN ROUND 2 REPLY, which cites `reconcile` because the finding IS that reconcile's "a hold that is nowhere" repair would teleport a ship modelled as `locationId: null`. ⚠ WORTH NOTING FOR WHOEVER READS THIS NEXT: this half of §70 is a PROXY for "the spec is probably already built", and a good round-2 reply names exports as CONTEXT rather than as evidence of building — so answering a spec well pushes this number up every time. The hard half (`declaredStale`, specs that DECLARE `builds:` whose every export exists) is still 0, and that is the one that means what it says.   // 2026-09-08 (16→17): SPEC_generative_arcs_and_bestiary.md — Aevi, spec_ready, naming `growthFor` as the export whose beast-growth question §6.3 turns on. ⚠ Moved deliberately with the reason and the name, per the ratchet's own rule: a landed spec legitimately raises this and the forbidden thing is nudging it silently. `declaredStale` is still 0.
  // 2026-09-09 (15→16): SPEC_ground_card_everywhere.md — Aevi, spec_ready, landed today naming `substrateVerdict`/`groundCardFor`/`bandFactor` as the exports it reasons about. ⚠ Her spec, her landing, nothing stale: `declaredStale` is still 0 and it comes down when she marks it built. Moved with the reason and the name, per this ratchet's own rule.
  // 2026-09-08 (14→15): SPEC_generation_reaches_every_tier.md — Aevi, spec_ready, naming `tierOf`/`derivedLevel`/`sheetFor`/`tierFromRole` as the exports my ROUND 2 reasons about. ⚠ Same case as SPEC_npc_sheet_generation below: a round-2 ask, not a landing. ⛑ It is NOT fully built — Q4 is wired (`tierNow`) and Q1's pyramid is still Erik's number — so the spec stays `spec_ready` and the status is Aevi's to flip, not mine. `declaredStale` is still 0.
