...
const LEDGER = [
  ...
  },
  {
    ...
    how: "scripts/world/reanchor.mjs — pair-matched polar signatures with write-back; the town anchors kept as the one-time bootstrap; Erik's SNG-389 rulings applied in the same change",
    gates: ["SNG-393: every name binds by signature, or sits in the KNOWN census with its diagnosis",
            ...
            "SNG-393: after the bootstrap, no name still leans on the town fallback",
  ...
  {
    id: "SNG-394", ask: "(Erik) do the milestone effects — the harm-rung drops and the novel-penalty removal I flagged and never built",
    ...
    gates: ["394: every ⚑ milestone is transcribed into a structured effect — the marker has a reader now",
            "394: …and an effect the engine cannot honour carries a WRITTEN reason, never silence",
            "394: no ⚑ milestone is transcribed as `unclassified` — that value exists to fail loudly",
            "394: milestones are ABSOLUTE, not cumulative — rank 14 grants two rungs, not three",
            "394: …and a rank below the milestone grants nothing",
            "394: a blocked effect never appears in the LIVE map — a caller cannot act on an unbuilt promise",
            "394: agility 7 softens the blow — an assassin's table stops producing `slain`",
            "394: …but softening stops at `spared` when you are alone — nobody revives you but somebody",
            "394: the roll and the softening are BOTH reported — a mercy the player is not told about cannot be learned",
            "394: rapport names the places at your side — 1, then 2 at rank 4, 3 at 7, 4 at 10",
            "394: a join past capacity is REFUSED, and refused where joins really happen (recruit, not the dead op)",
            "394: an over-capacity save loses NOBODY — the cap refuses a new join, it never ejects",
            "394: …and someone who left can come back even at capacity",
            "394: with no ladder passed the cap does not apply — every existing caller keeps working"],
