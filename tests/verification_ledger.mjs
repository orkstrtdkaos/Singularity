...
const LEDGER = [
  ...
  },
  {
    ...
    gates: ["SNG-392/398/396: the hierarchy matches the RATIFIED census — 25 regions, 96 settlements, 22 authored-in-play sites",
  ...
  },
  {
    ...
    gates: ["385: every location gets a nanite reading — the field is authored per region and now resolves",
            ...
            "385: the location strip reads the nanite as its OWN source on its OWN field, never merged into the ground word"],
  ...
  },
  {
    ...
    gates: ["SNG-384: a substrateSource is an OBJECT, not a bare string",
            ...
            "SNG-384: the engine's map→world constant IS the ratio the content uses — one number, not two",
            "SNG-384: ⛔ a source authored WITHOUT radiusWorld resolves the SAME field as one with it",
            "SNG-384: …and that comparison is not two empty fields",
            "SNG-384: …and the source visibly BENDS the field it sits in — otherwise both sides are inert",
  ...
  },
  {
    ...
    gates: ["382: the source mixes are LOADED, not merely registered — a loadRule call, not a CI-only read",
            "382/CCODE-221: every row names a ratified source, or is explicitly DEFERRED",
            "382/CCODE-221: every null mix is MARKED unauthored — null never means pure",
  ...
  },
  {
    ...
    gates: ["370: the gender repair list is not capped — a cap on a repair tool is a repair you cannot perform",
            "370: …and the people MISSING SOMETHING are listed first, because they are who the control is for",
  ...
  },
  {
    ...
    gates: ["358: a holding's condition moves BOTH ways — it recovers, it does not complete",
            ...
            "358: a holding keeps its keeper when they merely stopped travelling with you; dead or departed still loses it",   // CCODE-514: renamed with the rule — a keeper who left your side has not left your service
  ...
  },
  {
    ...
    gates: ["345: martial_paths REACHES the rules — the file is loaded, not merely registered",
            ...
            "345: the baseline kit is RETIRED — a character with nothing is granted none of it",
            "345: …and it is retired on a DIAL, reversible without a code change",
            ...
            "345: a baseline craft is still RECOGNISED as baseline though it is no longer granted",
            ...
            "345: an Ent still gets its FORM kit — Erik's branch-club — though the generic floor is retired",
            "345: an EXISTING save gains the floor on login, through the ctx app.js really passes",
            "345: …and an existing save is CLEANED of the retired four on login"],
  ...
  },
  {
    ...
    gates: ["272/273: every authored stage effect is REACHABLE by the reader",
            ...
            "272/273: …and that comparison runs on real effects across real regions",
            "272/273: ⛔ a REGIONAL arc scoped to a region is in force THERE",
            "272/273: …and is NOT in force in a region it does not name",
            "272/273: ⛑ …while a WORLD arc listing a region is still everywhere — scale outranks reach",
            "272/273: ⚠️ …and the fixture FOUND a regional arc with stages to scope — or the three checks above never ran",
            "272/273: ⚠️ `crossesRegions` PROSE is never read as the reach — only `regions` ids are",
            "272/273: …and an arc asked about NO place answers as the world, not as nowhere",
            "272/273: …but asked about a real elsewhere, it says no — the guard above is not blanket-true",
            "272/273: ⛔ a GENERATED local arc is born with the region it was minted in",
            "272/273: …and a WORLD-scale wake inherits no reach, because that is what it is",
            "272/273: …and that born reach actually binds through the reader",
            "272/273: ⛑ a generated arc on the effects path is INERT — a stub borns no stages",
            "272/273: ⛔ …but a generated arc WITH a stage binds through the same reader as an authored one",
            "272/273: …and it is scoped to where it was born — a local thread does not tax the far side",
            "272/273: the app passes the PLAYER'S region to the one effects reader",
